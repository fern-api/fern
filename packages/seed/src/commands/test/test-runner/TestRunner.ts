import { FernWorkspace, type Spec } from "@fern-api/api-workspace-commons";
...
export declare namespace TestRunner {
    ...
    interface RunArgs {
        ...
        lenient?: boolean;
        /** If true, skip autogenerated examples when manual examples exist (matches CLI remote generation behavior) */
    ...
    interface DoRunArgs {
        ...
        /** Absolute path to fern.config.json (used for license path resolution) **/
        absolutePathToFernConfig?: AbsoluteFilePath;
        /** If true, skip autogenerated examples when manual examples exist */
        skipAutogenerationIfManualExamplesExist?: boolean;
        /** Raw API spec files to pre-process and mount into the generator container */
        rawApiSpecs?: Spec[];
    }
    ...
    public async run({
        ...
        try {
            ...
            if (this.workspaceCache != null && generatorInvocation == null) {
                ...
                if (wantsSpecs) {
                    const cachedApiWorkspace = await this.workspaceCache.getOrLoadApiWorkspace({
                        fixture,
                        absolutePathToAPIDefinition: absolutePathToApiDefinition,
                        taskContext
                    });
                    if (cachedApiWorkspace instanceof OSSWorkspace) {
                        rawApiSpecs = cachedApiWorkspace.allSpecs;
                    }
                }
            } else {
                ...
                // Extract raw spec file paths before conversion to FernWorkspace
                // (OSSWorkspace has allSpecs; after toFernWorkspace() the paths are lost)
                if (wantsSpecs && apiWorkspace instanceof OSSWorkspace) {
                    rawApiSpecs = apiWorkspace.allSpecs;
                }
