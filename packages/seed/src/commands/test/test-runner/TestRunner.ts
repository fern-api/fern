...
export declare namespace TestRunner {
    ...
    interface RunArgs {
        ...
        /** Whether this is the implicit baseline run (customConfig=null, output to baseline/) */
        isBaseline?: boolean;
        /** Path to the baseline dir (needed by non-baseline variants to compute diffs) */
...
export abstract class TestRunner {
    ...
    public async run({
        ...
        // Track temp dir for non-baseline variants so we can clean up
        let tempOutputDir: AbsoluteFilePath | undefined;
        try {
            ...
            const id = isBaseline
                ? `${fixture}:baseline`
                : configuration != null
                  ? `${fixture}:${configuration.outputFolder}`
                  : `${fixture}`;
            ...
            if (!outputDir) {
                if (isBaseline) {
                    ...
                    tempOutputDir = AbsoluteFilePath.of(tmpDir.path);
                    outputDir = tempOutputDir;
                } else if (configuration == null) {
                    outputDir = join(this.generator.absolutePathToWorkspace, RelativeFilePath.of(fixture));
                } else {
                    outputDir = join(
            ...
            // Clean up temp directory for non-baseline variants
