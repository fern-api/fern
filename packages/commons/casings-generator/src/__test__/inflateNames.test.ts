...
describe("inflateIrNames", () => {
    ...
    it("inflates name in example endpoint calls (NameOrString field)", () => {
        ...
        // biome-ignore lint/suspicious/noExplicitAny: test helper
        const exampleName = (result as any).services.svc1.endpoints[0].autogeneratedExamples[0].example.name;
        expect(exampleName.originalName).toBe("my-example");
        expect(exampleName.camelCase).toBeDefined();
    });

    it("does not inflate plain string fields inside the dynamic sub-IR", () => {
        const ir = {
            apiName: "TestApi",
            smartCasing: false,
            generationLanguage: undefined,
        ...
        // biome-ignore lint/suspicious/noExplicitAny: test helper
        const result = inflateIrNames(ir as any);
        // dynamic.generatorConfig.apiName is type: string (NOT NameOrString) — should NOT be inflated
        // biome-ignore lint/suspicious/noExplicitAny: test helper
