...
describe("formatNavigationConfigError", () => {
    it("adds tab and section names for nested navigation errors", () => {
        ...
        expect(message).toBe(
            "Invalid object at path $.navigation[0].layout[1]: does not match any allowed schema (/functions/Regular functions/)"
        );
    });
    ...
    it("does not add breadcrumbs for non-navigation errors", () => {
        ...
        expect(message).toBe("Incorrect type at path $.colors.accent");
    });
    ...
    it("adds nested page names for deeply nested navigation errors", () => {
        ...
        expect(message).toBe(
            "Invalid object at path $.navigation[0].layout[0].contents[0]: does not match any allowed schema (/functions/Regular functions/Create user/)"
        );
    });
    ...
    it("does not include the root docs title in navigation breadcrumbs", () => {
        ...
        expect(message).toBe(
            "Invalid object at path $.navigation[0].layout[0]: does not match any allowed schema (/functions/Overview/)"
        );
    });

    it("uses a consistent fallback message", () => {
        const message = formatNavigationConfigError({
            value: {
                navigation: []
            },
