...
export function injectAutogeneratedExamples({
    ...
    const genOptions: Required<ExampleGenerationArgs> = {
        disabled: exampleGeneration.disabled,
        includeOptionalRequestPropertyExamples: exampleGeneration.includeOptionalRequestPropertyExamples ?? false,
        skipAutogenerationIfManualExamplesExist: exampleGeneration.skipAutogenerationIfManualExamplesExist ?? false,
        skipErrorAutogenerationIfManualErrorExamplesExist:
            exampleGeneration.skipErrorAutogenerationIfManualErrorExamplesExist ?? false
    };
    for (const [_, service] of Object.entries(ir.services)) {
        for (const endpoint of service.endpoints) {
            ...
            if (
                ...
                genOptions.skipAutogenerationIfManualExamplesExist === false
            ) {
                const generatedExample = generateEndpointV1Example({
                    ...
                    skipOptionalRequestProperties: genOptions.includeOptionalRequestPropertyExamples ? false : true,
                    generationResponse: { type: "success" }
                });
                if (generatedExample.type === "success") {
                    autogeneratedExamples.push({ example: generatedExample.example });
                }
            }
            if (
                endpoint.userSpecifiedExamples.filter((e) => e.example?.response.type === "error").length === 0 ||
                genOptions.skipErrorAutogenerationIfManualErrorExamplesExist === false
            ) {
                for (const error of endpoint.errors) {
                    const errorDeclaration = ir.errors[error.error.errorId];
                    if (!errorDeclaration) {
                        continue;
                    }
                    const generatedErrorExample = generateEndpointV1Example({
                        ir,
                        service,
                        endpoint,
                        typeDeclarations: ir.types,
                    ...
                    if (generatedErrorExample.type === "success") {
                        autogeneratedExamples.push({
                            example: generatedErrorExample.example
                        });
                    }
                }
            }
            endpoint.autogeneratedExamples = autogeneratedExamples;
        }
    }
    for (const [_, channel] of Object.entries(ir.websocketChannels ?? {})) {
        if (channel.examples.length > 0 && genOptions.skipAutogenerationIfManualExamplesExist === true) {
            continue;
        }
