...
describe("v1 cycle detection in generateTypeReferenceExample", () => {
    ...
    it("should include baseProperties in discriminated union examples (Webflow audit log scenario)", () => {
        // Simulates: WorkspaceAuditLogItem is a discriminated union on "eventType"
        // with base properties: timestamp (datetime), actor (object), workspace (object)
        // and variants like "user_access" with property "eventSubType"
        const datetimeRef = (): TypeReference =>
        ...
        // Union type with base properties (like Webflow's WorkspaceAuditLogItem)
        const AuditLogItemDecl: TypeDeclaration = {
            name: {
                typeId: "AuditLogItem",
                fernFilepath: DUMMY_FILEPATH,
                name: makeName("AuditLogItem"),
                displayName: undefined
            },
            shape: Type.union({
                discriminant: makeNameAndWireValue("eventType"),
                extends: [],
                types: [
                    {
                        discriminantValue: makeNameAndWireValue("user_access"),
                        shape: SingleUnionTypeProperties.samePropertiesAsObject({
                            typeId: "UserAccessProperties",
                            name: makeName("UserAccessProperties"),
                            fernFilepath: DUMMY_FILEPATH,
                            displayName: undefined
                        }),
                        displayName: undefined,
                ...
                baseProperties: [
                    {
                        docs: undefined,
                        availability: undefined,
                        name: makeNameAndWireValue("timestamp"),
                        valueType: datetimeRef(),
                        propertyAccess: undefined,
                        v2Examples: undefined
                    },
                    {
                        docs: undefined,
                        availability: undefined,
                        name: makeNameAndWireValue("actor"),
                        valueType: namedRef("Actor"),
                        propertyAccess: undefined,
                        v2Examples: undefined
                    },
                    {
                        docs: undefined,
                        availability: undefined,
                        name: makeNameAndWireValue("workspace"),
                        valueType: namedRef("Workspace"),
                        propertyAccess: undefined,
                        v2Examples: undefined
                    }
                ],
        ...
        const UserAccessPropertiesDecl: TypeDeclaration = makeObjectTypeDeclaration(
        ...
        const typeDeclarations: Record<TypeId, TypeDeclaration> = {
            AuditLogItem: AuditLogItemDecl,
            Actor: ActorDecl,
            Workspace: WorkspaceDecl,
            UserAccessProperties: UserAccessPropertiesDecl
        };

        const result = generateTypeReferenceExample({
            fieldName: undefined,
            typeReference: namedRef("AuditLogItem"),
            typeDeclarations,
            maxDepth: 10,
            currentDepth: 0,
            skipOptionalProperties: false
        ...
        // The jsonExample MUST include the base properties
        expect(json).toHaveProperty("eventType", "user_access");
        expect(json).toHaveProperty("timestamp");
        expect(json).toHaveProperty("actor");
        expect(json).toHaveProperty("workspace");
        expect(json).toHaveProperty("eventSubType");

        // Also verify the structured example includes baseProperties
        const shape = result.example.shape;
        expect(shape.type).toBe("named");
        if (shape.type === "named") {
