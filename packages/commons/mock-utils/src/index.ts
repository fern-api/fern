...
/**
 * Returns the best example for an endpoint. For SSE endpoints, prefers
 * autogenerated examples when they have richer multi-variant event coverage
 * than user-specified examples (which often only contain a single variant
 * from OpenAPI imports). For non-SSE endpoints, prefers user-specified.
 */
export function getBestEndpointExample(endpoint: FernIr.HttpEndpoint): FernIr.ExampleEndpointCall | undefined {
    const userExample = endpoint.userSpecifiedExamples[0]?.example;
    const autoExample = endpoint.autogeneratedExamples[0]?.example;
    if (userExample != null && autoExample != null) {
        const userSseCount = getSseEventCount(userExample);
        const autoSseCount = getSseEventCount(autoExample);
        if (autoSseCount > userSseCount) {
            return autoExample;
        }
    }
    return userExample ?? autoExample;
}

/**
 * Extracts the SSE events from an endpoint example, or undefined if not SSE.
 */
export function getSseExampleEvents(example: FernIr.ExampleEndpointCall): FernIr.ExampleServerSideEvent[] | undefined {
    if (example.response.type !== "ok" || example.response.value.type !== "sse") {
        return undefined;
    }
    return example.response.value.value;
}

function getSseEventCount(example: FernIr.ExampleEndpointCall): number {
    return getSseExampleEvents(example)?.length ?? 0;
}
...
export class WireMock {
    public convertToWireMock(ir: FernIr.IntermediateRepresentation): WireMockStubMapping {
        ...
        for (const service of Object.values(ir.services)) {
            for (const endpoint of service.endpoints) {
                ...
                const bestExample = getBestEndpointExample(endpoint);
        ...
        for (const service of Object.values(ir.services)) {
            for (const endpoint of service.endpoints) {
                ...
                // Use the best available example — prefer autogenerated when it has
                // richer SSE event coverage (more union variants).
                const bestExample = getBestEndpointExample(endpoint);
                ...
                if (bestExample != null) {
                    ...
                    const mapping = this.convertExampleToMapping(ir, service, endpoint, bestExample, needsBodyPattern);
                    if (mapping) {
                ...
                body = sseEvents
                    .map((sseEvent) => {
                        ...
                        if (isProtocolContext) {
                            ...
                            // so we extract just the "data" field for the SSE data: line.
                            const envelopeData =
                                typeof eventData === "object" && eventData !== null && "data" in eventData
                                    ? (eventData as Record<string, unknown>).data
                                    : undefined;
                            if (envelopeData !== undefined) {
                                const dataJson = JSON.stringify(envelopeData);
    ...
    private isSseProtocolContext(ir: FernIr.IntermediateRepresentation, endpoint: FernIr.HttpEndpoint): boolean {
        ...
        if (responseBody.type === "streaming" && responseBody.value.type === "sse") {
            ssePayload = responseBody.value.payload;
        } else if (responseBody.type === "streamParameter") {
            const streamResponse = responseBody.streamResponse;
            if (streamResponse.type === "sse") {
                ssePayload = streamResponse.payload;
            }
        }
