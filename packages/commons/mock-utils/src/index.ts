...
export class WireMock {
    public convertToWireMock(ir: FernIr.IntermediateRepresentation): WireMockStubMapping {
        ...
        for (const service of Object.values(ir.services)) {
            for (const endpoint of service.endpoints) {
                ...
                const selected = this.selectExampleForMapping(endpoint);
                ...
                if (selected) {
                    ...
                    const mapping = this.convertExampleToMapping(
                        ...
                        selected.serveBinaryFileDownload
                    );
                    if (mapping) {
    ...
    /**
     * Picks the example that drives an endpoint's mapping (user-specified examples win over autogenerated ones).
     *
     * File download endpoints are special: autogenerated example generation cannot produce a success
     * example for them ("File download unsupported"), so on specs that declare typed error responses
     * (e.g. OpenAPI 4xx/5xx responses) the only autogenerated examples are error examples. Those must not
     * turn the download stub into a JSON error body, so the mapping prefers a success example and, failing
     * that, still serves the binary fixture unless the user explicitly authored an error example.
     */
    private selectExampleForMapping(
        endpoint: FernIr.HttpEndpoint
    ): { example: FernIr.ExampleEndpointCall; serveBinaryFileDownload: boolean } | undefined {
        const userSpecified = endpoint.userSpecifiedExamples[0]?.example;
        const autogenerated = endpoint.autogeneratedExamples[0]?.example;
        if (endpoint.response?.body?.type !== "fileDownload") {
            const example = userSpecified ?? autogenerated;
            return example != null ? { example, serveBinaryFileDownload: false } : undefined;
        }

        const isSuccess = (example: FernIr.ExampleEndpointCall | undefined): example is FernIr.ExampleEndpointCall =>
            example != null && example.response?.type !== "error";
        const successExample =
            endpoint.userSpecifiedExamples.map((wrapper) => wrapper.example).find(isSuccess) ??
            endpoint.autogeneratedExamples.map((wrapper) => wrapper.example).find(isSuccess);
        if (successExample != null) {
            return { example: successExample, serveBinaryFileDownload: true };
        }
        if (userSpecified != null) {
            return { example: userSpecified, serveBinaryFileDownload: false };
        }
        return autogenerated != null ? { example: autogenerated, serveBinaryFileDownload: true } : undefined;
    }
    ...
    private convertExampleToMapping(
        ...
        serveBinaryFileDownload: boolean
        ...
        // Serve real binary bytes for file download responses. File download examples carry no body, so
        // the JSON path above would otherwise stub the endpoint with the JSON string `""`. User-authored
        // error examples keep their declared status and JSON error body.
        const isBinaryFileDownload = serveBinaryFileDownload && endpoint.response?.body?.type === "fileDownload";
