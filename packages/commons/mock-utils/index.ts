...
export class WireMock {
    public convertToWireMock(ir: FernIr.IntermediateRepresentation): WireMockStubMapping {
        ...
        for (const service of Object.values(ir.services)) {
            for (const endpoint of service.endpoints) {
                ...
                const exampleWrapper = this.getBestExample(endpoint);
    ...
    /**
     * Returns the best example for an endpoint. Prefers autogenerated examples
     * when they have richer SSE event coverage (more union variants) than
     * user-specified examples from OpenAPI imports.
     */
    private getBestExample(
        endpoint: FernIr.HttpEndpoint
    ): { example: FernIr.ExampleEndpointCall | undefined } | undefined {
        const userWrapper = endpoint.userSpecifiedExamples[0];
        const autoWrapper = endpoint.autogeneratedExamples[0];
        if (userWrapper?.example != null && autoWrapper?.example != null) {
            const userSseCount = this.getSseEventCount(userWrapper.example);
            const autoSseCount = this.getSseEventCount(autoWrapper.example);
            if (autoSseCount > userSseCount) {
                return autoWrapper;
            }
        }
        return userWrapper ?? autoWrapper;
    }

    private getSseEventCount(example: FernIr.ExampleEndpointCall): number {
        if (example.response.type !== "ok" || example.response.value.type !== "sse") {
            return 0;
        }
        return example.response.value.value.length;
    }
