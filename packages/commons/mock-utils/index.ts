...
export class WireMock {
    public convertToWireMock(ir: FernIr.IntermediateRepresentation): WireMockStubMapping {
        ...
        for (const service of Object.values(ir.services)) {
            for (const endpoint of service.endpoints) {
                ...
                const exampleWrapper = this.findBestExample(endpoint);
        ...
        for (const service of Object.values(ir.services)) {
            for (const endpoint of service.endpoints) {
                ...
                // Use the best available example: prefer SSE success examples for streaming
                // endpoints, then fall back to the first user-specified or autogenerated example
                const exampleWrapper = this.findBestExample(endpoint);
    ...
    /**
     * Finds the best example to use for WireMock mapping generation.
     * For SSE endpoints, prefers success examples with SSE response data over error examples,
     * since the SSE happy-path example may not be the first in the list (e.g., if an error
     * example was defined first in the spec, or if the SSE example failed conversion and was
     * silently dropped, leaving only the error example).
     */
    private findBestExample(
        endpoint: FernIr.HttpEndpoint
    ): { example: FernIr.ExampleEndpointCall | undefined } | undefined {
        const allExamples: { example: FernIr.ExampleEndpointCall | undefined }[] = [
        ...
        // Check if this is an SSE endpoint based on response type
        const isStreamingEndpoint =
            endpoint.response?.body?.type === "streaming" && endpoint.response.body.value.type === "sse";

        if (isStreamingEndpoint) {
            // For SSE endpoints, prefer an example with an SSE success response
            const sseExample = allExamples.find(
                (e) => e.example?.response?.type === "ok" && e.example?.response?.value?.type === "sse"
            );
            if (sseExample) {
                return sseExample;
            }
        ...
        // Fall back to the first available example
