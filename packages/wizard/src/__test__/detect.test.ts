...
import { walkFiles } from "../detect/walk";
...
describe("repository detection", () => {
    ...
    it("detects Mintlify and coding agents", async () => {
        ...
        await mkdir(path.join(dir, ".readme"), { recursive: true });
        await writeFile(path.join(dir, "CLAUDE.md"), "# Instructions\n");
        const files = await walkFiles(dir);
        await expect(detectDocsTools(dir, files)).resolves.toEqual([
            { name: "readme", path: ".readme" },
            { name: "mintlify", path: "mint.json" }
        ]);
        await expect(detectAgents(dir, files)).resolves.toEqual(["cursor", "claude-code"]);
        await rm(dir, { recursive: true, force: true });
    });
    ...
    it("reports the containing directory for a root Fern marker", async () => {
        const dir = await fixture();
        await writeFile(path.join(dir, "fern.yml"), "organization: acme\n");
        await expect(detectFernProject(dir)).resolves.toEqual({ exists: true, path: "./", docsConfigExists: false });
        await rm(dir, { recursive: true, force: true });
    });
});
