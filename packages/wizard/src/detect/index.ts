...
import { detectPackageManager, isFernCliInstalled } from "./package-manager";
...
export async function detectRepository(dir: string, checkCli = true): Promise<Detection> {
    const [fernProject, apiSpecs, frameworks, docsTools, agents, packageManager, fernCliVersion] = await Promise.all([
    ...
    return { dir, fernProject, apiSpecs, frameworks, docsTools, agents, packageManager, fernCliVersion };
}
