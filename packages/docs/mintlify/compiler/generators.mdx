...
The TypeScript SDK generator outputs a fully functional TypeScript/JavaScript
SDK. It can publish the SDK to npmjs.org (or any other npm repository).
...
The Express starter allows you to generate a complete skeleton for your Express server. It lets you create
strictly-typed request handlers, which allows you to safely access well-typed parameters, and prevents you
from sending incorrect responses to the client.

⚠️ Make sure to enable `allowSyntheticDefaultImports` in your `tsconfig.json` before using this starter
