...
If you run `fern check` then fern will lint the OpenAPI. As long as `fern check` does not surface any errors, you can use all of Fern's generators with OpenAPI. 

## Convert OpenAPI to a Fern definition

If you'd rather start writing a Fern Definition, then you can import your OpenAPI by running 
```bash
