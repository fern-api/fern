...
describe("formatConflictReason", () => {
    it("maps same-line-edit", () => {
        expect(formatConflictReason("same-line-edit")).toBe(
            "The new generation changed the same lines you edited"
        );
    });
    ...
    it("returns fallback for undefined", () => {
        expect(formatConflictReason(undefined)).toBe(
...
describe("parseCommitMessageForPR", () => {
    ...
    it("with versionBumpReason -> prepends to body", () => {
        const { prTitle, prBody } = parseCommitMessageForPR("Update SDK", undefined, undefined, "Minor dependency bump");
        expect(prTitle).toBe("Update SDK");
...
describe("fernignore", () => {
    it("REPLAY_FERNIGNORE_ENTRIES contains expected values", () => {
        ...
        expect(REPLAY_FERNIGNORE_ENTRIES).toHaveLength(2);
    });
    ...
    it("appends missing entries to existing .fernignore, returns true", async () => {
        ...
        try {
            ...
            // Write a .fernignore that already has one entry but not the other
