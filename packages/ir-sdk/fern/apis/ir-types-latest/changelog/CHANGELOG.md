...
- Add `Pagination.itemCursor` (`ItemCursorPagination`), for cursor pagination where the cursor for
  the next page lives on an element of the current page's results rather than on a top-level
  response property. It carries the request `page` property, the `results` property, the
  `itemCursor` property (resolved relative to a result element) and the `element` (`first` or
  `last`) the cursor is read from. Produced when `next_cursor` in `x-fern-pagination` or a Fern
  Definition indexes into the results, e.g. `$response.data[-1].token`.
...
- Feature: Add optional `subtitle` field to `HttpEndpoint`. Populated from the
  `x-fern-subtitle` OpenAPI extension, rendered below the endpoint title in the
  docs UI. Defaults to `undefined` for backward compatibility.
