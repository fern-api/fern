...
- Feature: Add optional `unwrapPath` field to `InlinedRequestBody`. When set (e.g.
  `["data", "attributes"]`), generators flatten the properties of the nested object at
  this path into the SDK method signature, while still serializing to the full nested
  wire format. Populated from the new `x-fern-request-body-unwrap` OpenAPI extension.
