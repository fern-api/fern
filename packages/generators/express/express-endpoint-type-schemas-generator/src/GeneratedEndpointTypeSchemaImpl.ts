...
export class GeneratedEndpointTypeSchemaImpl extends AbstractGeneratedEndpointTypeSchema {
    ...
    protected generateRawTypeDeclaration(context: ExpressEndpointTypeSchemasContext, module: ModuleDeclaration): void {
        module.addTypeAlias({
    ...
    protected getReferenceToParsedShape(context: ExpressEndpointTypeSchemasContext): ts.TypeNode {
        return context.type.getReferenceToType(this.type).typeNode;
    }

    protected buildSchema(context: ExpressEndpointTypeSchemasContext): Zurg.Schema {
