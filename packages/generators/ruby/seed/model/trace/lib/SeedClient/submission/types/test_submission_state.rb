...
module SeedClient
  ...
  Commons::TestCase.from_json(json_object: v)
        ...
        new(problem_id: problem_id, default_test_cases: default_test_cases, custom_test_cases: custom_test_cases, status: status, additional_properties: struct)
      end
      # Serialize an instance of TestSubmissionState to a JSON object
      #
      # @return [JSON]
      def to_json
        { problemId: @problem_id, defaultTestCases: @default_test_cases, customTestCases: @custom_test_cases, status: @status }.to_json()
      end
