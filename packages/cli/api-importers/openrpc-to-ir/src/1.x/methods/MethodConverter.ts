...
export class MethodConverter extends AbstractConverter<OpenRPCConverterContext3_1, MethodConverter.Output> {
    ...
    public convert(): MethodConverter.Output | undefined {
        ...
        const endpoint: HttpEndpoint = {
            ...
            v2Examples: {
                autogeneratedExamples: {},
                userSpecifiedExamples: this.convertExamples(pathString)
            },
            transport: undefined,
    ...
    private convertExamples(path: string): Record<string, FernIr.V2HttpEndpointExample> {
        const examples: Record<string, FernIr.V2HttpEndpointExample> = {};
        const pathParameters: FernIr.V2ValueExamples = {};
        for (const parameter of this.pathParameters) {
            const userExample = Object.values(parameter.v2Examples?.userSpecifiedExamples ?? {})[0];
            const autogeneratedExample = Object.values(parameter.v2Examples?.autogeneratedExamples ?? {})[0];
            const example = userExample !== undefined ? userExample : autogeneratedExample;
            if (example !== undefined) {
                pathParameters[getOriginalName(parameter.name)] = example;
            }
        }
