...
import { GraphQLNonNull, GraphQLObjectType, GraphQLOutputType, GraphQLSchema } from "graphql";
...
export class GraphQLToIRConverter {
    ...
    public async convert(): Promise<IntermediateRepresentation> {
        ...
        this.schema = buildGraphQLSchemaFromString({ content, filePath: this.filePath });

        const { types, graphqlFieldArguments, unmappedCustomScalars } = convertGraphQLTypes({
            schema: this.schema,
            namespace: this.namespace
        });

        if (unmappedCustomScalars.length > 0) {
            this.context.logger.warn(
                `No built-in mapping for custom GraphQL scalar(s): ${unmappedCustomScalars.join(", ")}. ` +
        ...
        return this.assembleIR({ types, services, graphqlFieldArguments });
    }
    ...
    private assembleIR({
        ...
        return {
            ...
            idempotencyHeaders: [],
            types,
            graphqlFieldArguments: Object.keys(graphqlFieldArguments).length > 0 ? graphqlFieldArguments : undefined,
            services,
            errors: {},
            webhookGroups: {},
