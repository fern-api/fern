...
import {
    buildArgumentsPassthrough,
    buildVariableDefinitions,
    generateSelectionQuery,
    QueryGenerationConfig
} from "../query-generation/generateSelectionQuery.js";
import {
    convertDeprecation,
    convertInputTypeToTypeReference,
    convertOutputTypeToTypeReference
} from "./convertGraphQLTypes.js";
...
export function convertRootFieldToEndpoint({
    ...
    const requestBodyProperties: FernIr.InlinedRequestBodyProperty[] = field.args.map((arg) => ({
        ...
        availability: convertDeprecation(arg.deprecationReason)
    }));
    ...
    return {
        ...
        transport: FernIr.Transport.graphql({
            ...
            // Carry the query scaffolding so the SDK can rebuild the envelope around a
            // caller-provided field selection (the `select` parameter).
            variableDefinitions: buildVariableDefinitions(field),
            arguments: buildArgumentsPassthrough(field)
        }),
        source: undefined,
        audiences: undefined,
        retries: undefined,
