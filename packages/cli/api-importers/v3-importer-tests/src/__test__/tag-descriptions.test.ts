...
describe("tag descriptions", () => {
    it("carries top-level tag descriptions onto subpackage docs", async () => {
        const ir = await getIRForFixture("tag-descriptions");
        expect(ir.subpackages["subpackage_plants"]?.docs).toBe("Everything about plants");
        expect(ir.subpackages["subpackage_seeds"]?.docs).toBeUndefined();
        expect(ir.rootPackage.docs).toBeUndefined();
    }, 90_000);
});
