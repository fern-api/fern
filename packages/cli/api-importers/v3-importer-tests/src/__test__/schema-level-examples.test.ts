...
describe("schema-level examples", () => {
    it("uses only the first schema-level example by default", async () => {
        const v2Examples = await getEndpointExamplesForFixture("schema-level-examples-default");

        expect(Object.keys(v2Examples?.userSpecifiedExamples ?? {})).toHaveLength(0);
        const autoExamples = Object.values(v2Examples?.autogeneratedExamples ?? {});
        expect(autoExamples).toHaveLength(1);
        expect(getResponseBodyValue(autoExamples[0])).toMatchObject({
            tag: "async_job_id",
            async_job_id: "34g93hh34h04y384084"
        });
    }, 90_000);

    it("surfaces every schema-level example when use-all-schema-examples is enabled", async () => {
        const v2Examples = await getEndpointExamplesForFixture("schema-level-examples-opt-in");
