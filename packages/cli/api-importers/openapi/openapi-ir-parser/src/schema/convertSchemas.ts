...
export function convertSchemaObject(
    ...
    if (
        ...
        if (!isListOfStrings(schema.enum) && !isListOfNumbers(schema.enum) && !isListOfBooleans(schema.enum)) {
        ...
        return convertEnum({
            ...
            enumValues: isListOfStrings(schema.enum) ? schema.enum : schema.enum.map(String),
...
function isListOfBooleans(x: unknown): x is boolean[] {
