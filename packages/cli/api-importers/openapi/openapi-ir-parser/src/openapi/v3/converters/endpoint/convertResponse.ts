...
import {
    ...
    getApplicationJsonSchemaMediaObjectFromContentEnhanced,
    getSchemaMediaObject,
    getSchemaMediaObjectEnhanced,
    getTextEventStreamObject
} from "./getApplicationJsonSchema";

/**
 * Enhanced response conversion configuration
 */
export interface ResponseConversionConfig {
    useEnhancedExamples?: boolean;
    exampleConfig?: Partial<ExampleGenerationConfig>;
}
...
export function convertResponse({
    ...
    source: Source;
    config?: ResponseConversionConfig;
}): ConvertedResponse {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (responses == null) {
        return { value: undefined, errors: {} };
    }
    const errors = markErrorSchemas({ responses, context, source, namespace: context.namespace, config });
...
function convertResolvedResponse({
    ...
    const jsonMediaObject = context.options.experimental?.examplesV2
        ? getApplicationJsonSchemaMediaObjectFromContentEnhanced({
              content: resolvedResponse.content ?? {},
              context,
              config: {
                  limits: { maxAutoGeneratedExamples: 3, respectUserExamples: true },
                  sampler: {
...
function markErrorSchemas({
    ...
    for (const [statusCode, response] of Object.entries(responses)) {
        ...
        const resolvedResponse = isReferenceObject(response) ? context.resolveResponseReference(response) : response;
        const mediaObject = context.options.experimental?.examplesV2
            ? getSchemaMediaObjectEnhanced(resolvedResponse.content ?? {}, context, {
                  limits: { maxAutoGeneratedExamples: 3, respectUserExamples: true },
                  sampler: {
                      skipNonRequired: false, // Error responses should show comprehensive examples
                      skipReadOnly: false
                  }
              })
            : getSchemaMediaObject(resolvedResponse.content ?? {}, context);
