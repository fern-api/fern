...
import {
    ...
    getApplicationJsonSchemaMediaObjectEnhanced,
    getEnhancedExamples,
    getExamples
} from "./getApplicationJsonSchema";

/**
 * Enhanced request conversion configuration
 */
export interface RequestConversionConfig {
    useEnhancedExamples?: boolean;
    exampleConfig?: Partial<ExampleGenerationConfig>;
}

/**
 * Helper function to choose between old and new example extraction
 */
function getRequestExamples(
    mediaTypeObject: OpenAPIV3.MediaTypeObject,
    context: AbstractOpenAPIV3ParserContext,
    config?: RequestConversionConfig
): NamedFullExample[] {
    if (context.options.experimental?.examplesV2) {
        // Use enhanced examples with request-specific configuration (minimal examples)
        console.log("🔧 Enhanced examples activated (examples-v2) for request processing");
        const requestConfig: Partial<ExampleGenerationConfig> = {
            limits: { maxAutoGeneratedExamples: 3, respectUserExamples: true },
            sampler: {
                skipNonRequired: true // Requests should use minimal examples (required fields only)
            }
        };
        return getEnhancedExamples(mediaTypeObject, context, requestConfig).map((ex) => ({
            name: ex.name,
            value: ex.value,
    ...
    // Use original example extraction
    return getExamples(mediaTypeObject, context);
}
...
function findApplicationUrlFormEncodedRequest({
    ...
    for (const [mediaType, mediaTypeObject] of Object.entries(content)) {
        const result = getApplicationUrlFormEncodedRequest({ mediaType, mediaTypeObject, context, config });
        if (result) {
...
function getApplicationUrlFormEncodedRequest({
    ...
    if (MediaType.parse(mediaType)?.isURLEncoded()) {
        return {
            ...
            examples: getRequestExamples(mediaTypeObject, context, config)
        };
    }
    return undefined;
}
...
function findMultipartFormDataRequest({
    ...
    for (const [mediaType, mediaTypeObject] of Object.entries(content)) {
        const result = getMultipartFormDataRequest({ mediaType, mediaTypeObject, context, config });
        if (result) {
            return [mediaType, mediaTypeObject];
        }
    }
...
export function convertToSingleRequest({
    ...
    const multipartFormDataRequest = findMultipartFormDataRequest({ content, context, config });
    ...
    const urlEncodedRequest = findApplicationUrlFormEncodedRequest({ content, context, config });
...
export function convertRequest({
    ...
    const multipartFormData = getMultipartFormDataRequest({ mediaType, mediaTypeObject, context, config });
    ...
    // convert as application/json
    const jsonMediaObject = config?.useEnhancedExamples
        ? getApplicationJsonSchemaMediaObjectEnhanced({
              mediaType,
              mediaTypeObject,
              context,
              config: {
                  ...
                  sampler: {
                      skipNonRequired: true, // Requests should use minimal examples
                      ...config.exampleConfig?.sampler
                  }
              }
          })
        : getApplicationJsonSchemaMediaObject({ mediaType, mediaTypeObject, context });
    ...
    const urlEncodedRequest = getApplicationUrlFormEncodedRequest({ mediaType, mediaTypeObject, context, config });
    // convert as application/x-www-form-urlencoded
