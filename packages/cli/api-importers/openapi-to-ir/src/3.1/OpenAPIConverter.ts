...
import { constructHttpPath, convertApiAuth, convertEnvironments, injectAutogeneratedExamples, injectAutogeneratedV2Examples } from "@fern-api/ir-utils";
...
export class OpenAPIConverter extends AbstractConverter<OpenAPIConverterContext3_1, IntermediateRepresentation> {
    ...
    private async convertSecuritySchemes(): Promise<Record<string, AuthScheme>> {
        if (this.context.authOverrides) {
            this.ir.auth = convertApiAuth({ rawApiFileSchema: this.context.authOverrides, casingsGenerator: this.context.casingsGenerator });
            return {};
        }
