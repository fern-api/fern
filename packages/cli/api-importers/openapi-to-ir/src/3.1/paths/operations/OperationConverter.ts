...
export class OperationConverter extends AbstractOperationConverter {
     ...
     * Each request body example is paired with the first response example (if available).
     */
    private convertNativeOpenApiExamples({
        ...
        // Get the first response example to pair with request examples
        const responseExample = this.getFirstResponseExample(convertedResponseBody);
    ...
    /**
     * Gets the first response example from the converted response body.
     * This is used to pair with request body examples.
     */
    private getFirstResponseExample(
        convertedResponseBody: OperationConverter.ConvertedResponseBody | undefined
    ): FernIr.V2HttpEndpointResponse | undefined {
        if (convertedResponseBody == null) {
            return undefined;
        }

        // Try to get response example from v2Responses
        const v2Responses = convertedResponseBody.v2Responses;
        if (v2Responses != null && v2Responses.length > 0) {
            const firstResponse = v2Responses[0];
            if (firstResponse?.body != null) {
                // Get the first example from the response body's v2Examples
                const responseBody = firstResponse.body;
                const v2Examples = this.getV2ExamplesFromResponseBody(responseBody);
                if (v2Examples != null) {
                    // Prefer user-specified examples, then autogenerated
                    ...
                    const firstExampleValue = userExamples[0] ?? autoExamples[0];
                    if (firstExampleValue != null) {
                        return {
                            docs: undefined,
                            statusCode: firstResponse.statusCode,
                            body: FernIr.V2HttpEndpointResponseBody.json(firstExampleValue)
                        };
                    }
                }
            }
    ...
    /**
     * Extracts v2Examples from a response body type.
     */
    private getV2ExamplesFromResponseBody(responseBody: FernIr.HttpResponseBody): FernIr.V2SchemaExamples | undefined {
        return responseBody._visit<FernIr.V2SchemaExamples | undefined>({
            fileDownload: () => undefined,
            json: (json) => json.v2Examples,
            streaming: () => undefined,
            streamParameter: (sp) => this.getV2ExamplesFromResponseBody(sp.nonStreamResponse),
            text: () => undefined,
            bytes: () => undefined,
            _other: () => undefined
        });
