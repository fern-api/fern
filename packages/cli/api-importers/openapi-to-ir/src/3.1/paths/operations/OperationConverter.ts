...
export class OperationConverter extends AbstractOperationConverter {
    ...
    public convert(): OperationConverter.Output | undefined {
        ...
        let endpointUserSpecifiedExamples = fernExamples.examples;
        let endpointStreamExamples = fernExamples.streamExamples;
        if (Object.keys(endpointUserSpecifiedExamples).length === 0) {
            const responseBodySchemaExamples = this.getResponseBodyV2SchemaExamples(convertedResponseBody);
            const requestBodySchemaExamples = this.getRequestBodyV2SchemaExamples(requestBody);
            if (
                (responseBodySchemaExamples != null &&
                    Object.keys(responseBodySchemaExamples.userSpecifiedExamples).length > 0) ||
                (requestBodySchemaExamples != null &&
                    Object.keys(requestBodySchemaExamples.userSpecifiedExamples).length > 0)
            ) {
                endpointUserSpecifiedExamples = this.createEndpointExamplesFromSchemaExamples({
                    responseV2Examples: responseBodySchemaExamples,
                    requestV2Examples: requestBodySchemaExamples,
                    httpPath: path,
                    httpMethod,
                    baseUrl,
                    statusCode: convertedResponseBody?.response?.statusCode ?? 200
                });
            }
        }
        ...
        return {
            ...
            endpoint: {
                ...
                v2Examples: {
                    autogeneratedExamples: {},
                    userSpecifiedExamples: endpointUserSpecifiedExamples
                },
                v2Responses: {
            ...
            streamEndpoint:
                streamResponse != null && streamResponse.body != null
                    ? {
                          ...
                          v2Examples: {
                              autogeneratedExamples: {},
                              userSpecifiedExamples: endpointStreamExamples
                          },
                          v2Responses: {
                              responses: [
                                  {
    ...
    private getResponseBodyV2SchemaExamples(
        convertedResponseBody: OperationConverter.ConvertedResponseBody | undefined
        ...
        if (body.type === "json") {
            return body.value.v2Examples;
        }
        if (body.type === "streaming") {
            return body.value.v2Examples;
        }
        if (body.type === "fileDownload" || body.type === "text" || body.type === "bytes") {
            return body.v2Examples;
        }
        return undefined;
    }
    ...
    private getRequestBodyV2SchemaExamples(requestBody: HttpRequestBody | undefined): V2SchemaExamples | undefined {
        ...
        if (
            ...
            requestBody.type === "fileUpload" ||
            requestBody.type === "bytes"
        ) {
            return requestBody.v2Examples;
        }
        return undefined;
    }

    private createEndpointExamplesFromSchemaExamples({
        responseV2Examples,
        requestV2Examples,
        httpPath,
        httpMethod,
        ...
        responseV2Examples: V2SchemaExamples | undefined;
        requestV2Examples: V2SchemaExamples | undefined;
        httpPath: HttpPath;
        httpMethod: FernIr.HttpMethod;
        baseUrl: string | undefined;
        statusCode: number;
        ...
        const responseExamples = responseV2Examples?.userSpecifiedExamples ?? {};
        const requestExamples = requestV2Examples?.userSpecifiedExamples ?? {};

        if (Object.keys(responseExamples).length > 0) {
            for (const [name, responseValue] of Object.entries(responseExamples)) {
                const requestValue = requestExamples[name];
                examples[name] = {
                    displayName: name,
                    request: {
                        docs: undefined,
                        ...
                        queryParameters: {},
                        headers: {},
                        requestBody: requestValue ?? undefined
                    },
                    response: {
                        docs: undefined,
                        statusCode,
                        body: FernIr.V2HttpEndpointResponseBody.json(responseValue)
                    },
                    codeSamples: undefined
                };
            }
        } else if (Object.keys(requestExamples).length > 0) {
            for (const [name, requestValue] of Object.entries(requestExamples)) {
                examples[name] = {
                    displayName: name,
                    request: {
                        docs: undefined,
