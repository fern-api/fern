...
export class RequestBodyConverter extends Converters.AbstractConverters.AbstractMediaTypeObjectConverter {
    ...
    private handleJsonOrFormContent({ contentType }: { contentType: string }): RequestBodyConverter.Output | undefined {
        ...
        if (convertedSchema == null) {
            // No schema defined. Allow the request body through only when there are
            // native OpenAPI examples so that docs and code snippets can render them.
            // Without examples (e.g. google.protobuf.Empty → empty JSON body) we
            // correctly treat the endpoint as having no request body.
            return this.handleSchemalessJsonContent({ contentType, mediaTypeObject });
        }
    ...
    /**
     * Handles JSON request bodies that have no schema but DO have native OpenAPI
     * examples. Creates an unknown-typed reference request body so that examples
     * are preserved for docs rendering and code snippet generation.
     */
    private handleSchemalessJsonContent({
        contentType,
        mediaTypeObject
    }: {
        contentType: string;
        ...
        const hasExamples =
            ...
            (mediaTypeObject.examples != null && Object.keys(mediaTypeObject.examples).length > 0);
        if (!hasExamples) {
            return undefined;
        }
