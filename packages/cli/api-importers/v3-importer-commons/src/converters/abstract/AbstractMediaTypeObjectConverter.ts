...
> {
    ...
    protected convertMediaTypeObjectExamples({
        ...
        if (
            this.context.settings.useAllSchemaExamples &&
            schema != null &&
            Object.keys(v2Examples.userSpecifiedExamples).length === 0
        ) {
            const schemaExamples = this.getExamplesFromSchemaOrReference(schema);
            // A single schema example already flows through the autogenerated path below.
        ...
        if (Object.keys(v2Examples.userSpecifiedExamples).length === 0 && schema != null) {
            const autoExampleName = camelCase(`${[...this.group, this.method].join("_")}_example`);
