    ...
    for (const [propertyName, propertySchema] of Object.entries(properties ?? {})) {
        ...
        if (convertedProperty != null) {
            convertedProperties.push({
                ...
                v2Examples: (() => {
                    const baseV2Examples = convertedProperty.schema?.typeDeclaration?.v2Examples ?? {
                        userSpecifiedExamples: {},
                        autogeneratedExamples: {}
                    };

                    // Add propertySchema.examples to userSpecifiedExamples if they exist
                    if (propertySchema.examples != null && Array.isArray(propertySchema.examples)) {
                        const propertyExamples = { ...baseV2Examples.userSpecifiedExamples };

                        for (const [, example] of propertySchema.examples.entries()) {
                            const resolvedExample = context.resolveExample(example);
                            Object.assign(propertyExamples, resolvedExample);
                        }

                        return {...baseV2Examples, userSpecifiedExamples: propertyExamples};
                    }

                    return baseV2Examples;
                })()
            });
