...
> extends AbstractConverter<Context, Output> {
    ...
    protected finalizeIr(): IntermediateRepresentation {
        ...
        // Normalization of user-written examples, so it must run whether or not V1 examples are generated.
        backfillMissingPathParameterExamples({ ir });

        const exampleGeneration = this.context.exampleGenerationArgs;
        const irWithV2Examples = injectAutogeneratedV2Examples({ ir, exampleGeneration, logger: this.context.logger });
    ...
    /** Delegates to the shared IR audience filter so V3 importers share Fern Definition semantics. */
    private filterIrForAudiences(ir: IntermediateRepresentation): IntermediateRepresentation {
        const filtered = filterIntermediateRepresentationForAudiences(ir, this.irGraph.build());
        return {
            ...filtered,
    ...
    protected addEndpointToIr({
        ...
        if (service != null) {
            ...
            if (
                inlinedRequestPropertiesByAudience != null &&
                Object.keys(inlinedRequestPropertiesByAudience).length > 0
            ) {
                this.irGraph.markInlinedRequestPropertiesForAudiences(endpoint.id, inlinedRequestPropertiesByAudience);
            }
            if (queryParametersByAudience != null && Object.keys(queryParametersByAudience).length > 0) {
                this.irGraph.markQueryParametersForAudiences(endpoint.id, queryParametersByAudience);
            }
        }
    }
    ...
    protected addWebhookToIr({
        ...
        if (
            webhook.id != null &&
            inlinedPayloadPropertiesByAudience != null &&
            Object.keys(inlinedPayloadPropertiesByAudience).length > 0
        ) {
            this.irGraph.markInlinedWebhookPayloadPropertiesForAudiences(
                webhook.id,
                inlinedPayloadPropertiesByAudience
            );
        }
