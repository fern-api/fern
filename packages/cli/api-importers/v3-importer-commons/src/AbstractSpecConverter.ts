...
> extends AbstractConverter<Context, Output> {
    ...
    private filterIrForAudiences(ir: IntermediateRepresentation): IntermediateRepresentation {
        // Delegate to the shared audience filter so the OpenAPI/AsyncAPI/OpenRPC paths
        // share the same filtering semantics as the Fern Definition IR pipeline, including
        // property-level filtering for object types, inline request bodies, query parameters,
        // and inline webhook payloads.
        const { sdkConfig, subpackages, rootPackage, ...rest } = ir;
        const filteredIr = this.irGraph.build();
        const filtered = filterIntermediateRepresentationForAudiences(rest, filteredIr);
        return {
            ...filtered,
            sdkConfig,
            subpackages,
