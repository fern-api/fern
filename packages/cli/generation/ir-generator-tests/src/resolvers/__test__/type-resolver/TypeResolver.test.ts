...
import { CliError, createMockTaskContext, shouldReportToSentry } from "@fern-api/task-context";
...
describe("TypeResolver", () => {
    ...
    it("treats missing named type declarations as non-reportable user reference errors", async () => {
        const context = createMockTaskContext();
        const parseResult = await loadAPIWorkspace({
            absolutePathToWorkspace: join(
                AbsoluteFilePath.of(__dirname),
        ...
        let thrownError: CliError | undefined;
        try {
            typeResolver.getDeclarationOfNamedTypeOrThrow({
                referenceToNamedType: "MissingType",
                file: fernFileContext
            });
        } catch (error) {
        ...
        if (thrownError == null) {
            throw new Error("Expected missing type declaration to throw");
        }
        expect(thrownError.code).toBe(CliError.Code.ReferenceError);
        expect(shouldReportToSentry(thrownError.code)).toBe(false);
    });
});
