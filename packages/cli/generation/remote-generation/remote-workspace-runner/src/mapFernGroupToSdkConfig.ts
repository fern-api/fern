...
/**
 * Fields of the SDK Config `api` block that the source specs already carry, so the mapper
 * must not also derive them from the Fern definition. Declared here (not imported from the
 * CLI package) because the runner cannot depend on `@fern-api/cli`; `fern sdk migrate`
 * passes its structurally identical object.
 */
