...
describe("publishDocs S3 path sanitization", () => {
    describe("sanitizeRelativePathForS3", () => {
        ...
        it("replaces multiple ../ segments with _up_/", () => {
            const path = RelativeFilePath.of("../../assets/images/favicon.ico");
            const result = sanitizeRelativePathForS3(path);
            expect(result).toBe("_up_/_up_/assets/images/favicon.ico");
            expect(result).not.toContain("../");
        });
        ...
        it("handles mixed ../ segments throughout path", () => {
            const path = RelativeFilePath.of("../docs/../assets/logo.svg");
            const result = sanitizeRelativePathForS3(path);
            expect(result).toBe("_up_/docs/_up_/assets/logo.svg");
            expect(result).not.toContain("../");
        });
        ...
        it("handles deeply nested ../ segments", () => {
            ...
            const result = sanitizeRelativePathForS3(path);
            expect(result).toBe("_up_/_up_/_up_/_up_/other-project/assets/logo.svg");
            expect(result).not.toContain("../");
        });

        it("preserves path that already contains _up_/", () => {
            // Edge case: if somehow a path already contains _up_/, it should be preserved
            const path = RelativeFilePath.of("docs/_up_/assets/logo.svg");
            const result = sanitizeRelativePathForS3(path);
            expect(result).toBe("docs/_up_/assets/logo.svg");
        });
        ...
        it("handles root-relative paths starting with ../", () => {
            ...
            const result = sanitizeRelativePathForS3(path);
            expect(result).toBe("_up_/favicon.ico");
            expect(result).not.toContain("../");
        });
        ...
        it("prevents S3 signature mismatch by avoiding HTTP normalization", () => {
            ...
            // against the original path. By replacing ../ with _up_/, we prevent
