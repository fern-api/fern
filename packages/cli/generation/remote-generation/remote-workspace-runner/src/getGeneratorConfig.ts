...
export declare namespace getGeneratorConfig {
    export interface Args {
        ...
        /** Preserve publish mode metadata while omitting credentials from serialized configs. */
        omitPublishCredentials?: boolean;
        paths: {
    ...
    return githubPublishInfo != null
        ? FernFiddle.GithubPublishInfo._visit<FernGeneratorExec.GithubPublishInfo | undefined>(githubPublishInfo, {
              npm: (value) => {
                  ...
                  const oidcMarker = getTrustedOidcMarker(token);
                  ...
                  return FernGeneratorExec.GithubPublishInfo.npm({
                      ...
                      shouldGeneratePublishWorkflow: oidcMarker != null || hasToken
                  });
              },
              maven: (value) =>
                  FernGeneratorExec.GithubPublishInfo.maven({
                      ...
                      usernameEnvironmentVariable: EnvironmentVariable(
                          omitPublishCredentials ? "" : (value.credentials?.username ?? "")
                      ),
                      passwordEnvironmentVariable: EnvironmentVariable(
                          omitPublishCredentials ? "" : (value.credentials?.password ?? "")
                      ),
                      signature:
                          value.signature != null && !omitPublishCredentials
                              ? {
              ...
              pypi: (value) => {
                  ...
                  const oidcMarker = getTrustedOidcMarker(password);
                  const hasCredentials = value.credentials != null;
                  return FernGeneratorExec.GithubPublishInfo.pypi({
                      registryUrl: value.registryUrl,
                      packageName: value.packageName,
                      usernameEnvironmentVariable: EnvironmentVariable(omitPublishCredentials ? "" : "PYPI_USERNAME"),
                      passwordEnvironmentVariable: EnvironmentVariable(
                          omitPublishCredentials
                              ? oidcMarker != null
                                  ? "OIDC"
                      ...
                      shouldGeneratePublishWorkflow: oidcMarker != null || hasCredentials,
              ...
              rubygems: (value) =>
                  FernGeneratorExec.GithubPublishInfo.rubygems({
                      registryUrl: value.registryUrl,
                      packageName: value.packageName,
                      apiKeyEnvironmentVariable: EnvironmentVariable(omitPublishCredentials ? "" : (value.apiKey ?? ""))
                  }),
              postman: (value) =>
                  FernGeneratorExec.GithubPublishInfo.postman({
                      apiKeyEnvironmentVariable: EnvironmentVariable(
                          omitPublishCredentials ? "" : (value.apiKey ?? "")
                      ),
                      workspaceIdEnvironmentVariable: EnvironmentVariable(
                          omitPublishCredentials ? "" : (value.workspaceId ?? "")
                      )
                  }),
              nuget: (value) => {
                  const apiKey = (value.apiKey ?? "").trim();
                  const oidcMarker = getTrustedOidcMarker(apiKey);
                  return FernGeneratorExec.GithubPublishInfo.nuget({
                      ...
                      shouldGeneratePublishWorkflow: oidcMarker != null ? true : undefined
                  });
              },
              crates: (value) =>
                  FernGeneratorExec.GithubPublishInfo.crates({
                      registryUrl: value.registryUrl,
                      packageName: value.packageName,
                      tokenEnvironmentVariable: EnvironmentVariable(omitPublishCredentials ? "" : (value.token ?? ""))
                  }),
              _other: () => undefined
          })
        : undefined;
...
function getTrustedOidcMarker(value: string): "OIDC" | "<USE_OIDC>" | undefined {
    ...
    const output = generatorInvocation.outputMode._visit<FernGeneratorExec.GeneratorOutputConfig>({
        publish: (value) => {
            if (publishToRegistry === true) {
                const publishTarget = getPublishTargetFromPublishMode(value, omitPublishCredentials === true);
                return {
        ...
        publishV2: (value) => {
            if (publishToRegistry === true) {
                const publishTarget = getPublishTargetFromPublishModeV2(value, omitPublishCredentials === true);
                return {
                    ...newRealPublishOutputConfig(outputVersion, publishTarget, paths),
                    snippetFilepath: snippetPath,
                    publishingMetadata: generatorInvocation.publishMetadata
    ...
    if ("registryOverrides" in mode) {
        ...
        if (overrides.npm != null) {
            return FernGeneratorExec.GeneratorPublishTarget.npm({
                ...
                token: omitPublishCredentials ? "" : overrides.npm.token,
        ...
        if (overrides.maven != null) {
            return FernGeneratorExec.GeneratorPublishTarget.maven({
                ...
                username: omitPublishCredentials ? "" : (overrides.maven.username ?? ""),
                password: omitPublishCredentials ? "" : (overrides.maven.password ?? ""),
    ...
    return mode._visit<FernGeneratorExec.GeneratorPublishTarget | undefined>({
        npmOverride: (value) =>
            value != null
                ? FernGeneratorExec.GeneratorPublishTarget.npm({
                      registryUrl: value.registryUrl,
                      token: omitPublishCredentials ? "" : value.token,
                      packageName: value.packageName
                  })
                : undefined,
        mavenOverride: (value) =>
            value != null
                ? FernGeneratorExec.GeneratorPublishTarget.maven({
                      registryUrl: value.registryUrl,
                      username: omitPublishCredentials ? "" : value.username,
                      password: omitPublishCredentials ? "" : value.password,
                      coordinate: value.coordinate,
                      signature: omitPublishCredentials ? undefined : (value.signature ?? undefined)
                  })
                : undefined,
        pypiOverride: (value) =>
            value != null
        ...
        rubyGemsOverride: (value) =>
            value != null
                ? FernGeneratorExec.GeneratorPublishTarget.rubygems({
                      registryUrl: value.registryUrl,
                      apiKey: omitPublishCredentials ? "" : value.apiKey,
                      packageName: value.packageName
                  })
                : undefined,
        nugetOverride: (value) =>
        ...
        postman: (value) =>
            FernGeneratorExec.GeneratorPublishTarget.postman({
                apiKey: omitPublishCredentials ? "" : value.apiKey,
                workspaceId: omitPublishCredentials ? "" : value.workspaceId
            }),
        _other: () => undefined
    });
}
