...
export function generateIntermediateRepresentation({
    ...
    const intermediateRepresentation: Omit<IntermediateRepresentation, "sdkConfig" | "subpackages" | "rootPackage"> = {
        selfHosted: false,
        // Populated only for GraphQL specs, which arrive as `additionalIrs` and are merged in below.
    ...
    // Merge any pre-built IRs carried on the workspace (e.g. generated from GraphQL schemas,
    // which convert directly to IR rather than to a Fern definition). This is what lets GraphQL
    // SDKs generate through the standard `fern generate` pipeline.
    //
    // These IRs are merged after the fern-definition example injection above, so inject
    // autogenerated examples into each one here — otherwise GraphQL operations would have no
    // examples, leaving the SDK with no README usage snippets, no reference.md, and no tests.
    const mergedIRBeforeGraphqlAuth = (workspace.additionalIrs ?? []).reduce<IntermediateRepresentation>(
        (acc, additionalIr) => {
            const additionalIrWithExamples: IntermediateRepresentation = exampleGeneration.disabled
                ? additionalIr
                : { ...additionalIr, ...injectAutogeneratedExamples({ ir: additionalIr, exampleGeneration }) };
            return mergeIntermediateRepresentation(acc, additionalIrWithExamples, casingsGenerator);
        },
        finalIR
    );

    // GraphQL operations come in with `auth: false` (a GraphQL schema cannot express auth). Now that the
    // IR is fully merged, let them participate in any auth the API actually declares (combined OpenAPI
    // spec or generators.yml). Remove this call (and the helper) to revert GraphQL auth support.
    const mergedIR = applyAuthToGraphqlEndpoints(mergedIRBeforeGraphqlAuth);
