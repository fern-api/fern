...
/**
 * The slice of the IR this pass reads: the `types` map (for both views) plus the API-surface
 * structures that can reference a type outside a union-variant position. Guard (b) of View B needs
 * *all* of these — an object reachable from an endpoint request/response, webhook, websocket message,
 * error, or a global header/parameter/variable is not exclusively a union variant, so leaf-dropping
 * its fields would corrupt that other usage.
 */
export type UnionBasePropertyDedupeInput = Pick<
    IntermediateRepresentation,
    | "types"
    | "services"
    | "webhookGroups"
...
/**
 * Keys whose values reference types only *incidentally* (example payloads embed concrete
 * variant/type values that are not "usages"). Walking them would wrongly mark a variant object as
 * non-exclusive, so they are skipped by {@link collectApiSurfaceTypeReferences}.
 */
const EXAMPLE_KEYS: ReadonlySet<string> = new Set([
    "v2Examples",
...
/**
 * Deep-walks an API-surface value (endpoints, webhooks, channels, errors, global headers/parameters/
 * variables), collecting every `typeId` it references into `into`. Both `TypeReference.named` and
 * `DeclaredTypeName` carry `typeId`, so any usage — however nested inside containers — is caught,
 * which keeps this robust against future IR shape changes. Example payloads are skipped (they embed
 * type values incidentally). Must NOT be called on `ir.types` (the categorized walk in
 * {@link getOutgoingTypeReferences} handles that, distinguishing variant vs non-variant edges) nor on
 * the package tree (`Package.types` enumerates *every* declared typeId, which would mark everything).
 */
function collectApiSurfaceTypeReferences(value: unknown, into: Set<TypeId>): void {
    if (Array.isArray(value)) {
        for (const item of value) {
            collectApiSurfaceTypeReferences(item, into);
        }
        return;
    }
    ...
    for (const [key, child] of Object.entries(value)) {
        ...
        if (key === "typeId" && typeof child === "string") {
            into.add(child);
            continue;
        }
        collectApiSurfaceTypeReferences(child, into);
    }
}
...
/**
 * Scans the IR once, recording for each typeId which unions reference it as a
 * `samePropertiesAsObject` variant, and whether it is referenced in any non-variant position —
 * both within the type graph (`ir.types`) AND across the API surface (endpoints, webhooks,
 * websocket channels, errors, global headers/parameters/variables). The latter is essential for
 * guard (b): an object reachable from, say, a direct endpoint response is not exclusively a union
 * variant, so its leaf fields must not be dropped.
 */
function buildTypeReferenceInfo(ir: UnionBasePropertyDedupeInput): {
    ...
    for (const [typeId, typeDeclaration] of Object.entries(ir.types)) {
        const { variantEdges, nonVariantEdges } = getOutgoingTypeReferences(typeDeclaration, ir.types);
    ...
    // Every type reachable from the API surface is a non-variant usage (there are no union-variant
    // edges outside union type declarations, which the type-graph walk above already covered).
    for (const apiSurface of [
        ir.services,
        ir.webhookGroups,
        ir.websocketChannels,
        ir.errors,
        ir.headers,
        ...
        collectApiSurfaceTypeReferences(apiSurface, nonVariantReferenced);
    }
 ...
 */
export function computeUnionBasePropertyDedupe(ir: UnionBasePropertyDedupeInput): UnionBasePropertyDedupe {
    const { types } = ir;
    ...
    const { variantReferrers, nonVariantReferenced } = buildTypeReferenceInfo(ir);
 ...
 */
export function addUnionBasePropertyDedupeToIr(ir: UnionBasePropertyDedupeInput): void {
    const { inheritedBasePropertiesByUnion, deferredUnionBasePropertiesByObject } = computeUnionBasePropertyDedupe(ir);
