    ...
    return Type.union({
        ...
        baseProperties:
            union["base-properties"] != null
                ? Object.entries(union["base-properties"])
                      // Filter out any base property that has the same wire key as the discriminant
                      // to avoid duplicate fields when the discriminant is also defined as a base property
                      .filter(([propertyKey]) => propertyKey !== discriminant)
