...
export function convertPathParameters({
    ...
    const convertedParameters = Object.entries(pathParameters).map(([parameterName, parameter]) =>
    ...
    if (paramNamesInUrlOrder.length > 0) {
        const paramMap = new Map(convertedParameters.map((param) => [param.name.originalName, param]));

        const reorderedPathParameters: PathParameter[] = [];
        ...
        for (const paramName of paramNamesInUrlOrder) {
            ...
            if (param) {
                ...
                reorderedPathParameters.push({
                    ...
                    name: file.casingsGenerator.generateName(paramName),
                    valueType: file.parseTypeReference("string"),
                    location,
                    variable: undefined,
                    v2Examples: {
                        userSpecifiedExamples: {},
        ...
        for (const param of convertedParameters) {
            if (!usedParams.has(param.name.originalName)) {
                reorderedPathParameters.push(param);
            }
        }

        return reorderedPathParameters;
    }
