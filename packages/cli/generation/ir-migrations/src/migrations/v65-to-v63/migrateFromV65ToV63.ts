        ...
        return {
            ...
            webhookGroups: mapValues(v65.webhookGroups, (webhookGroup) =>
                webhookGroup.map((webhook) => {
                    ...
                    return rest as unknown as IrVersions.V63.webhooks.Webhook;
                })
            )
...
function convertHttpResponse(response: IrVersions.V65.HttpResponse): IrVersions.V63.http.HttpResponse {
    if (response.body != null && response.body.type === "multipart") {
        // V65 adds "multipart" to HttpResponseBody. V63 does not have this variant.
        // Fall back to fileDownload for backward compatibility.
        return {
            ...response,
            body: IrVersions.V63.http.HttpResponseBody.fileDownload({
                docs: response.body.docs ?? undefined,
                v2Examples: undefined
            })
        } as IrVersions.V63.http.HttpResponse;
    }
