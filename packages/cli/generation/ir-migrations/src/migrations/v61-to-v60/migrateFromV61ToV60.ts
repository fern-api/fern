...
function convertApiAuth(auth: IrVersions.V61.auth.ApiAuth): IrVersions.V60.auth.ApiAuth {
    // Convert ENDPOINT_SPECIFIC to ALL when migrating backwards since v60 doesn't support ENDPOINT_SPECIFIC
    ...
    // For ALL and ANY, we need to ensure we're using the V60 enum type
    return {
        ...auth,
        requirement:
