...
describe("migrateFromV66ToV65", () => {
    ...
    describe("service and endpoint inflation", () => {
        ...
        it("strips viewers from service and endpoints when migrating to V65", () => {
            const service: IrVersions.V66.HttpService = {
                availability: undefined,
                name: {
                    fernFilepath: {
                ...
                transport: undefined,
                audiences: undefined,
                viewers: ["admin", "gardener"]
            };

            const v66IR = createMinimalV66IR({
                services: { service_plants: service }
            });
            const v65IR = V66_TO_V65_MIGRATION.migrateBackwards(v66IR, mockContext);

            const migratedService = v65IR.services["service_plants"];
            expect(migratedService).toBeDefined();
            expect((migratedService as unknown as { viewers?: unknown }).viewers).toBeUndefined();

            const migratedEndpoint = migratedService?.endpoints[0];
            expect(migratedEndpoint).toBeDefined();
            expect((migratedEndpoint as unknown as { viewers?: unknown }).viewers).toBeUndefined();
        });
    });
