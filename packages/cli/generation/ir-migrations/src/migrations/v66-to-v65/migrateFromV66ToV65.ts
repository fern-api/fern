...
function inflateIr(
    ...
    const inflateEndpoint = (ep: IrVersions.V65.HttpEndpoint): IrVersions.V65.HttpEndpoint => ({
        ...
        displayName: ep.displayName != null ? inflateName(ep.displayName) : undefined,
        ...
        switch (rb.type) {
            case "inlinedRequestBody":
                return IrVersions.V65.HttpRequestBody.inlinedRequestBody({
                    ...
                    extraProperties: rb.extraProperties != null ? inflateObjectProperty(rb.extraProperties) : undefined
                });
            case "fileUpload":
    ...
    const inflateWebhook = (wh: IrVersions.V65.webhooks.Webhook): IrVersions.V65.webhooks.Webhook => ({
        ...
        displayName: wh.displayName != null ? inflateName(wh.displayName) : undefined,
        ...
        switch (payload.type) {
            case "inlinedPayload":
                return IrVersions.V65.webhooks.WebhookPayload.inlinedPayload({
                    ...
                    properties: payload.properties.map(inflateObjectProperty)
                });
            default:
                return payload;
        }
        ...
        switch (config.type) {
            case "clientCredentials":
                return IrVersions.V65.OAuthConfiguration.clientCredentials({
                    ...config,
                    clientIdProperty: inflateNameAndWireValue(config.clientIdProperty),
                    clientSecretProperty: inflateNameAndWireValue(config.clientSecretProperty),
                    tokenEndpoint: {
                        ...
                        config.refreshEndpoint != null
                            ? {
                                  ...
                                  requestProperties: {
                                      ...
                                      refreshToken: inflateNameAndWireValue(
    ...
    const inflateChannel = (
        ...
        path: {
            ...ch.path,
            pathParameters: ch.path.pathParameters.map(inflatePathParameter)
        },
        ...
        messages: ch.messages.map((msg) => ({
            ...msg,
            displayName: msg.displayName != null ? inflateName(msg.displayName) : undefined,
        ...
        switch (body.type) {
            case "inlinedBody":
                return IrVersions.V65.websocket.WebSocketMessageBody.inlinedBody({
                    ...
                    properties: body.properties.map(inflateObjectProperty)
                });
            default:
                return body;
        }
    ...
    const { types, errors, services, webhookGroups, websocketChannels, subpackages, rootPackage, ...rest } = ir;
