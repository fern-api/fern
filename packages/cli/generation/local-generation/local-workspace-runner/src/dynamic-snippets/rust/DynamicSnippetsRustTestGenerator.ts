...
export class DynamicSnippetsRustTestGenerator {
    ...
    private async initializeProject(outputDir: AbsoluteFilePath): Promise<AbsoluteFilePath> {
        const absolutePathToOutputDir = join(outputDir, RelativeFilePath.of("dynamic-snippets"));
    ...
    private getTestFilePath({
        ...
        return join(absolutePathToOutputDir, RelativeFilePath.of(`example${idx}.rs`));
    }
}
