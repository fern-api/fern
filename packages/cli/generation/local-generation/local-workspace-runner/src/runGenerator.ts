...
export async function writeFilesToDiskAndRunGenerator({
    ...
    if (deferredExampleGeneration != null) {
        ...
        // The real full IR is atomically renamed into place once ready.
        await writeFile(absolutePathToFullIr, "{}");

        const deferredWorkAndSnippets = async (): Promise<void> => {
            const dStart = Date.now();
            // NOTE: Intentional in-place mutation of `ir` for performance.
            // The IR can be 50MB+ (e.g., Square API). Cloning it would double peak memory
            // and add significant GC pressure. Both `injectAutogeneratedExamples` and
            // the `dynamic` assignment enrich the same IR that is later serialized for v2
            // and used by `onDeferredWorkComplete` for snippet generation.
            // The caller (`GenerationRunner`) does not read from `ir` concurrently during
            // this window — it only accesses `ir` after `writeFilesToDiskAndRunGenerator`
            // resolves, at which point the mutations are complete.
            ...
            ir.dynamic = dynamic;
            const fullIrForV2 = JSON.stringify(ir);
            // Atomic write: write to a temp file then rename into place.
            // `rename()` is atomic on the same filesystem, so the v2 consumer
            // (polling via stat().size) will either see the 2-byte placeholder
            // or the complete IR — never a partially-written file.
            const tmpFullIrPath = absolutePathToFullIr + ".tmp";
            await writeFile(tmpFullIrPath, fullIrForV2);
            await rename(tmpFullIrPath, absolutePathToFullIr);
