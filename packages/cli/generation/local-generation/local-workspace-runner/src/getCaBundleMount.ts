...
export interface CaBundleMount {
    ...
    /**
     * Set when the bundle holds a single certificate. SSL_CERT_FILE and GIT_SSL_CAINFO
     * replace the container's trust store, so a lone corporate root would make git and
     * OpenSSL reject every publicly-signed host.
     */
    warning?: string;
}
...
export function getCaBundleMount(env: NodeJS.ProcessEnv = process.env): CaBundleMount | undefined {
    ...
    return {
        ...
        warning:
            certificateCount === 1
                ? `${FERN_CA_BUNDLE_ENV_VAR} (${hostPath}) contains a single certificate. It replaces the trust store for git and OpenSSL inside the generator container, so it should be a complete bundle (system CA certificates plus your corporate CA) or TLS to public hosts will fail.`
                : undefined
    };
}
