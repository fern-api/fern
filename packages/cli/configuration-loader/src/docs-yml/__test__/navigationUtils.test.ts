...
describe("buildNavigationForDirectory with underscore prefix exclusion", () => {
    ...
    it("should use sibling .mdx file as section overview when it matches a directory name", async () => {
        const mockGetDir = async (path: AbsoluteFilePath) => {
            if (path === ("/test" as AbsoluteFilePath)) {
                return [
                    {
                        type: "file" as const,
                        name: "adapters.mdx",
                        absolutePath: "/test/adapters.mdx" as AbsoluteFilePath,
                        contents: ""
                    },
                    {
                        type: "directory" as const,
                        name: "adapters",
                        absolutePath: "/test/adapters" as AbsoluteFilePath,
                        contents: []
                    },
                    {
                        type: "file" as const,
                        name: "standalone.mdx",
                        absolutePath: "/test/standalone.mdx" as AbsoluteFilePath,
                        contents: ""
                    }
                ];
            }
            if (path === ("/test/adapters" as AbsoluteFilePath)) {
                return [
                    {
                        type: "file" as const,
                        name: "child1.mdx",
                        absolutePath: "/test/adapters/child1.mdx" as AbsoluteFilePath,
                        contents: ""
                    },
                    {
                        type: "file" as const,
                        name: "child2.mdx",
                        absolutePath: "/test/adapters/child2.mdx" as AbsoluteFilePath,
                        contents: ""
                    }
                ];
            }
        ...
        // Should have 2 items: the section (with overview) and the standalone page
        expect(result).toHaveLength(2);
        ...
        expect(section).toMatchObject({
            type: "section",
            title: "Adapters",
            overviewAbsolutePath: "/test/adapters.mdx"
        });
        expect.assert(section?.type === "section");
        expect(section.contents).toHaveLength(2);
    ...
    it("should prefer index.mdx inside directory over sibling .mdx file", async () => {
        const mockGetDir = async (path: AbsoluteFilePath) => {
            if (path === ("/test" as AbsoluteFilePath)) {
                return [
                    {
                        type: "file" as const,
                        name: "guides.mdx",
                        absolutePath: "/test/guides.mdx" as AbsoluteFilePath,
                        contents: ""
                    },
                    {
                        type: "directory" as const,
            ...
            if (path === ("/test/guides" as AbsoluteFilePath)) {
                return [
                    {
                        type: "file" as const,
                        name: "index.mdx",
                    ...
                    },
                    {
                        type: "file" as const,
                        name: "page.mdx",
                        absolutePath: "/test/guides/page.mdx" as AbsoluteFilePath,
                        contents: ""
                    }
                ];
            }
    ...
    it("should handle multiple sibling files matching multiple directories", async () => {
        const mockGetDir = async (path: AbsoluteFilePath) => {
            if (path === ("/test" as AbsoluteFilePath)) {
                return [
                    {
                        type: "file" as const,
                        name: "api.mdx",
                        absolutePath: "/test/api.mdx" as AbsoluteFilePath,
                        contents: ""
                    },
                    {
                        type: "file" as const,
                        name: "cli.mdx",
                        absolutePath: "/test/cli.mdx" as AbsoluteFilePath,
                        contents: ""
                    },
                    {
                        type: "directory" as const,
                    ...
                    },
                    {
                        type: "directory" as const,
                        name: "cli",
                        absolutePath: "/test/cli" as AbsoluteFilePath,
                        contents: []
                    }
                ];
            }
            if (path === ("/test/api" as AbsoluteFilePath)) {
                return [
                    {
                        type: "file" as const,
                        name: "endpoints.mdx",
                        absolutePath: "/test/api/endpoints.mdx" as AbsoluteFilePath,
                        contents: ""
                    }
                ];
            }
            if (path === ("/test/cli" as AbsoluteFilePath)) {
                return [
                    {
                        type: "file" as const,
                        name: "commands.mdx",
                        absolutePath: "/test/cli/commands.mdx" as AbsoluteFilePath,
                        contents: ""
                    }
                ];
            }
        ...
        expect(result).toHaveLength(2);
        const apiSection = result.find((item) => item.type === "section" && item.title === "Api");
        const cliSection = result.find((item) => item.type === "section" && item.title === "Cli");

        expect(apiSection).toMatchObject({
            type: "section",
            overviewAbsolutePath: "/test/api.mdx"
        });
        expect(cliSection).toMatchObject({
            type: "section",
            overviewAbsolutePath: "/test/cli.mdx"
        });
