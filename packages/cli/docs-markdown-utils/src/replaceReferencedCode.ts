...
/**
 * Scans markdown content for <Code src="https://..."/> tags and returns the unique URLs.
 * Used to pre-fetch all external URLs in parallel before processing pages.
 */
export function collectCodeSrcUrls(markdown: string): string[] {
    if (!markdown.includes("<Code")) {
        return [];
    }
    const tags = scanForCodeTags(markdown);
    const urls: string[] = [];
...
export async function replaceReferencedCode({
    ...
    for (const codeTag of codeTagMatches) {
        ...
        try {
            ...
            if (isUrl(src)) {
                // Use pre-fetched cache if available, otherwise fetch inline
