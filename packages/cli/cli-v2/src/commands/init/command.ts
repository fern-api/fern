...
export class InitCommand {
  public async handle(context: Context, args: InitCommand.Args): Promise<void> {
    const fernYmlPath = join(
      context.cwd,
      RelativeFilePath.of(FERN_YML_FILENAME),
    );
  ...
  private async writeFernYml({
    ...
    const specFormat =
      result.apiSource.type === "sample" ? "openapi" : result.apiSource.format;
  ...
  private async writeApiSpec({
    ...
    switch (apiSource.type) {
      case "file":
        ...
        return;
      case "url":
        await this.writeSpecFile(
          context.cwd,
          apiSource.filename,
          apiSource.content,
        );
        context.stderr.info(
          `  ${Icons.success} Created ${SPECS_DIR}/${apiSource.filename}`,
        );
        return;
      case "sample":
        await this.writeSpecFile(
          context.cwd,
          "openapi.yml",
          PETSTORE_OPENAPI_YML,
        );
        context.stderr.info(
          `  ${Icons.success} Created ${SPECS_DIR}/openapi.yml`,
        );
        return;
      default:
  ...
  private printNextSteps({
    ...
    if (result.languages.length === 1 && firstLang != null) {
      ...
      context.stderr.info(
        chalk.cyan(`       fern sdk generate --target ${firstLang}`),
      );
    } else {
  ...
  private async validateArgs({
    ...
    if (!context.isTTY && !args.yes) {
      throw new CliError({
        message:
          "Cannot run interactive init in non-TTY environment. Use --yes for defaults.",
        code: CliError.Code.ConfigError,
      });
    }
  }
