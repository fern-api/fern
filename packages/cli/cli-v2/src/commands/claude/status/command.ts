...
export class StatusCommand {
    public async handle(context: Context, args: StatusCommand.Args): Promise<void> {
        ...
        context.stdout.info("");
        context.stdout.info(`  Marketplace:  ${FERN_CLAUDE_PLUGIN.marketplaceRepo}`);
        context.stdout.info(`  Plugin:       ${FERN_CLAUDE_PLUGIN.pluginName}@${FERN_CLAUDE_PLUGIN.marketplaceName}`);
        context.stdout.info(`  Skills:       ${FERN_SKILLS.length}`);
        context.stdout.info("");

        if (inSession) {
            const entrypointLabel = entrypoint != null ? ` (${entrypoint})` : "";
            context.stdout.info(`  ${Icons.success} Running inside a Claude Code session${entrypointLabel}.`);
            context.stdout.info("");
            context.stdout.info(chalk.dim("  Fern cannot detect from outside whether the plugin is installed."));
            context.stdout.info(chalk.dim("  Inside Claude Code, run /plugin to see installed plugins."));
        } else {
            context.stdout.info(`  ${Icons.info} Not running inside a Claude Code session.`);
            context.stdout.info("");
