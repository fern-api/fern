...
export class InstallCommand {
    public async handle(context: Context, args: InstallCommand.Args): Promise<void> {
        if (args.json) {
            context.stdout.info(
                JSON.stringify(
                    {
        ...
        context.stdout.info(chalk.dim(`  Docs:    ${FERN_CLAUDE_PLUGIN.claudeDocsUrl}`));
        context.stdout.info("");
        context.stdout.info(`  ${Icons.info} Run 'fern claude skills' to see what the plugin can do.`);
    }
}
    ...
    command(
        ...
        (yargs) =>
            yargs
                ...
                .example("$0 claude install", "Print the slash commands to run inside Claude Code")
                .example("$0 claude install --json", "Output marketplace/plugin metadata as JSON")
    );
}
