 ...
 *
 * `description` is omitted so the group is hidden from top-level help —
 * the underlying features are still in beta and not yet ready for general
 * use. Subcommands:
...
export function addMdCommand(cli: Argv<GlobalArgs>): void {
    commandGroup({ cli, name: "md", subcommands: [addGenerateCommand, addCheckCommand] });
}
