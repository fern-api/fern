...
interface SwitchArgs extends GlobalArgs {
    user?: string;
}

export function addSwitchCommand(cli: Argv<GlobalArgs>): void {
    command(cli, "switch", "Switch active account", handleSwitch, (yargs) =>
        yargs.option("user", {
            type: "string",
...
async function handleSwitch(context: Context, args: SwitchArgs): Promise<void> {
    ...
    if (accounts.length === 1) {
        ...
        context.stdout.warn(`${chalk.yellow("⚠")} You only have one account logged in: ${chalk.bold(account.user)}`);
        context.stdout.info("");
    ...
    if (args.user != null) {
        await switchToUser(context, args.user);
        return;
    }
    if (accounts.length === 2) {
        const otherAccount = accounts.find((a) => !a.isActive);
        if (otherAccount != null) {
            await switchToUser(context, otherAccount.user);
        }
        return;
    }
    // If there are more than two accounts, we use an interactive selection.
    await handleInteractiveSwitch(context, accounts);
}

async function switchToUser(context: Context, user: string): Promise<void> {
...
async function handleInteractiveSwitch(
    ...
    await switchToUser(context, selectedUser);
}
