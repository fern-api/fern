...
interface WhoamiArgs extends GlobalArgs {
    json: boolean;
}

export function addWhoamiCommand(cli: Argv<GlobalArgs>): void {
    command(cli, "whoami", "Print the current user", handleWhoami, (yargs) =>
        yargs.option("json", {
            type: "boolean",
            default: false,
...
async function handleWhoami(context: Context, args: WhoamiArgs): Promise<void> {
