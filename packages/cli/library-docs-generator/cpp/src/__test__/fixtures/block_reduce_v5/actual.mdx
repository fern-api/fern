...
The BlockReduce class provides collective  methods for computing a
...
a blocked arrangement  across 128 threads where each thread
...
<CodeBlock links={{"BlockReduce": "/library/api/cub::BlockReduce", "TempStorage": "/library/api/cub::_block_reduce::_temp_storage"}}>
```cpp showLineNumbers={false}
cub::BlockReduce<T, BlockDimX, Algorithm, BlockDimY, BlockDimZ>::BlockReduce(
...
Reference to memory allocation having layout type [TempStorage](/library/api/cub::_block_reduce::_temp_storage)
</ParamField>
...
<CodeBlock links={{"BlockReduce": "/library/api/cub::BlockReduce"}}>
```cpp showLineNumbers={false}
T cub::BlockReduce<T, BlockDimX, Algorithm, BlockDimY, BlockDimZ>::Sum(T input)
```
</CodeBlock>
...
T cub::BlockReduce<T, BlockDimX, Algorithm, BlockDimY, BlockDimZ>::Sum(T(&inputs)[ITEMS_PER_THREAD])
```
</CodeBlock>
...
| `_TempStorage` | `typename InternalBlockReduce::TempStorage` | Shared memory storage layout type for [BlockReduce](/library/api/cub::_block_reduce). |
...
The operations exposed by [BlockReduce](/library/api/cub::_block_reduce) require a temporary memory allocation of this nested type for thread communication. This opaque storage can be allocated directly using the `__shared__` keyword. Alternatively, it can be aliased to externally allocated memory (shared or global) or `union`'d with other storage allocation types to facilitate memory reuse.
