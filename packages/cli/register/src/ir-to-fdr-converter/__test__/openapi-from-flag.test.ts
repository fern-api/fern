...
describe("OpenAPI v3 Parser Pipeline (--from-openapi flag)", () => {
    ...
    it("should detect when AI examples override contains outdated schema properties", async () => {
        // Test case for the critical issue: AI-generated examples in ai_examples_override.yml
        // become stale when the OpenAPI schema changes (properties removed/added)
        // Currently, the system doesn't detect or update these stale examples
        const context = createMockTaskContext();
        const workspace = await loadAPIWorkspace({
            absolutePathToWorkspace: join(
                AbsoluteFilePath.of(__dirname),
                RelativeFilePath.of("fixtures/schema-change-test")
            ),
            context,
            cliVersion: "0.0.0",
            workspaceName: "schema-change-test"
        });
        ...
        // Validate that the schema change issue exists
        const services = Object.values(intermediateRepresentation.services);
        expect(services.length).toBeGreaterThan(0);
        ...
        if (service && typeof service === "object" && "endpoints" in service) {
            const serviceWithEndpoints = service as { endpoints?: Array<any> };
            expect(serviceWithEndpoints.endpoints).toBeDefined();
            expect(serviceWithEndpoints.endpoints?.length).toBe(1); // POST /book
            ...
            // Check for autogenerated examples (should exist since no human examples in OpenAPI)
            const hasAutogenerated = (endpointAny?.autogeneratedExamples?.length || 0) > 0;
            console.log(`Autogenerated examples: ${hasAutogenerated ? endpointAny.autogeneratedExamples.length : 0}`);

            // The critical issue: AI override examples may contain outdated schema properties
            console.log("\n🔍 Checking for schema mismatch in examples:");

            if (hasAutogenerated) {
                const autogenExample = endpointAny.autogeneratedExamples[0];
                const requestExample = autogenExample?.example?.request?.jsonExample;
                const responseExample = autogenExample?.example?.response?.value?.jsonExample;

                console.log("Request example:", JSON.stringify(requestExample, null, 2));
                console.log("Response example:", JSON.stringify(responseExample, null, 2));

                // Check for properties that should NOT exist in current schema
                const hasRemovedProperties =
                    requestExample?.isbn !== undefined ||
                    requestExample?.publishedYear !== undefined ||
                    responseExample?.isbn !== undefined ||
                    responseExample?.publishedYear !== undefined;

                // Check for properties that SHOULD exist in current schema but are missing
                const missingNewProperties =
                    responseExample?.status === undefined || responseExample?.addedDate === undefined;

                console.log(`⚠️  Contains removed properties (isbn, publishedYear): ${hasRemovedProperties}`);
                console.log(`⚠️  Missing new properties (status, addedDate): ${missingNewProperties}`);

                if (hasRemovedProperties) {
                    console.log("🚨 ISSUE: AI override contains properties removed from schema");
                }

                if (missingNewProperties) {
                    console.log("🚨 ISSUE: AI override missing new properties added to schema");
                }

                // This demonstrates the core problem: AI examples don't auto-update with schema
                const hasSchemaIssues = hasRemovedProperties || missingNewProperties;
                console.log(`\n📊 Schema mismatch detected: ${hasSchemaIssues}`);

                // For now, we expect this issue to exist (until we implement the fix)
                // expect(hasSchemaIssues).toBe(true); // TODO: Change to false after implementing fix
            }

            console.log("=== END SCHEMA CHANGE ANALYSIS ===");
        }

        // Snapshot the complete output for regression testing
        await expect(fdrApiDefinition).toMatchFileSnapshot("__snapshots__/schema-change-test-fdr.snap");
        await expect(intermediateRepresentation).toMatchFileSnapshot("__snapshots__/schema-change-test-ir.snap");
    });
