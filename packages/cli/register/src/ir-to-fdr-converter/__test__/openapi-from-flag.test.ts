...
describe("OpenAPI v3 Parser Pipeline (--from-openapi flag)", () => {
    ...
    it("should handle OpenAPI with x-fern-examples, regular examples, and nullable fields", async () => {
        // Test OpenAPI spec with x-fern-examples, regular example, examples with sub-examples,
        const context = createMockTaskContext();
        const workspace = await loadAPIWorkspace({
            absolutePathToWorkspace: join(
                AbsoluteFilePath.of(__dirname),
                RelativeFilePath.of("fixtures/examples-with-nullable")
            ),
            context,
            cliVersion: "0.0.0",
            workspaceName: "examples-with-nullable"
        });
        ...
        // Validate that nullable fields are handled correctly in examples
        expect(intermediateRepresentation.types).toBeDefined();
        const productType = Object.values(intermediateRepresentation.types).find(
            (type) => type.name.name.originalName === "Product"
        );
        ...
        // Snapshot the complete output for regression testing
        await expect(fdrApiDefinition).toMatchFileSnapshot("__snapshots__/examples-with-nullable-fdr.snap");
        await expect(intermediateRepresentation).toMatchFileSnapshot("__snapshots__/examples-with-nullable-ir.snap");
    });
});
