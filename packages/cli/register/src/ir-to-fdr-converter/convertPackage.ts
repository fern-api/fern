...
function convertService(
    ...
    for (const irEndpoint of irService.endpoints) {
        ...
        if (irEndpoint.v2Examples != null) {
            ...
            // We only want to disambiguate multiple user-specified examples.
            const shouldUseUserSpecifiedExampleName = Object.keys(userSpecifiedExamples).length > 1;
            ...
            // Similarly, we only want to disambiguate multiple autogenerated examples.
            const shouldUseAutogeneratedExampleName = Object.keys(autogeneratedExamples).length > 1;
...
function getErrorExamplesFromDeclaration(errorDeclaration: Ir.ErrorDeclaration) {
    return errorDeclaration.v2Examples?.userSpecifiedExamples != null
        ? Object.entries(errorDeclaration.v2Examples.userSpecifiedExamples).map(([name, example]) => ({
              name,
              responseBody: { type: "json" as const, value: example },
              description: undefined
          }))
        : errorDeclaration.v2Examples?.autogeneratedExamples != null
          ? Object.entries(errorDeclaration.v2Examples.autogeneratedExamples).map(([name, example]) => ({
                name,
                responseBody: { type: "json" as const, value: example },
                description: undefined
            }))
          : errorDeclaration.examples.map((irExample) => {
                return {
                    name: irExample.name?.originalName,
