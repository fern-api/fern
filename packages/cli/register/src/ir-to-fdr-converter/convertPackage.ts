...
function convertWebSocketChannel(
    ...
    if (
        ...
        const allEntries = userSpecifiedEntries.length > 0 ? userSpecifiedEntries : autogeneratedEntries;
        for (const [exampleName, exampleValue] of allEntries) {
            examples.push({
                name: exampleName,
                description: "",
                path: exampleValue.baseUrl ?? "",
                pathParameters: exampleValue.pathParameters ?? {},
                queryParameters: exampleValue.queryParameters ?? {},
                headers: exampleValue.headers ?? {},
                messages:
                    exampleValue.messages?.map((message) => ({
