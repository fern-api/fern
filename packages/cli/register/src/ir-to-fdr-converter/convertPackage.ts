...
import { generateEndpointV1Example as generateEndpointExample, getNameString } from "@fern-api/ir-utils";
...
function convertWebhookGroup(webhookGroup: Ir.webhooks.WebhookGroup): FdrCjsSdk.api.v1.register.WebhookDefinition[] {
    return webhookGroup.map((webhook) => {
        ...
        if (webhook.v2Examples != null) {
            for (const [exampleName, example] of Object.entries(webhook.v2Examples.userSpecifiedExamples)) {
                webhookExamples.push({
                    name: example.name != null ? getNameString(example.name) : exampleName,
            ...
            webhookExamples.push(
                ...(webhook.examples?.map((example) => {
                    return {
                        name: example.name != null ? getNameString(example.name) : undefined,
        ...
        return {
            ...
            id: FdrCjsSdk.WebhookId(getNameString(webhook.name)),
            path: [],
            method: webhook.method,
            name: webhook.displayName ?? startCase(getNameString(webhook.name)),
        ...
        const endpoint: FdrCjsSdk.api.v1.register.EndpointDefinition = {
            ...
            environments:
                ir.environments != null
                    ...
                    : undefined,
            id: FdrCjsSdk.EndpointId(getNameString(irEndpoint.name)),
            originalEndpointId: irEndpoint.id,
            name: irEndpoint.displayName ?? startCase(getNameString(irEndpoint.name)),
            path:
                    ...
                    ? {
                          pathParameters: irEndpoint.pathParameters.map(
                              (pathParameter): FdrCjsSdk.api.v1.register.PathParameter => ({
                                  ...
                                  key: FdrCjsSdk.PropertyKey(getNameString(pathParameter.name)),
...
function convertWebSocketChannel(
    ...
    if (
        ...
        if (firstEntry != null) {
            ...
            examples.push({
                name: getNameString(exampleName),
                description: "",
        ...
        examples = channel.examples.map(
            (example): FdrCjsSdk.api.v1.register.ExampleWebSocketSession => ({
                ...
                >((pathParameters, irPathParameterExample) => {
                    pathParameters[FdrCjsSdk.PropertyKey(getNameString(irPathParameterExample.name))] =
    ...
    return {
        ...
        environments:
            ir.environments != null
                ...
                : [],
        id: FdrCjsSdk.WebSocketId(getNameString(channel.name)),
        name: channel.displayName ?? startCase(getNameString(channel.name)),
        path: {
    ...
    const requestBodyShape = Ir.http.HttpRequestBody._visit<FdrCjsSdk.api.v1.register.HttpRequestBodyShape | undefined>(
        irRequest,
        {
            ...
            fileUpload: (fileUpload) => ({
                type: "fileUpload",
                value: {
                    name: getNameString(fileUpload.name),
        ...
        for (const irResponseError of irResponseErrors) {
            ...
            if (errorDeclaration) {
                errors.push({
                    ...
                    name: errorDeclaration.displayName ?? getNameString(errorDeclaration.name.name),
                    availability: undefined,
            ...
            if (errorDeclaration) {
                const properties: FdrCjsSdk.api.v1.register.ObjectProperty[] = [
                    {
                        ...
                        valueType: {
                            ...
                            value: {
                                type: "stringLiteral",
                                value: getNameString(errorDeclaration.discriminantValue.name)
                            }
                        },
                ...
                errors.push({
                    ...
                    examples: errorDeclaration.examples.map((irExample) => {
                        return {
                            name: irExample.name != null ? getNameString(irExample.name) : undefined,
            ...
            ?.map((codeSample) =>
                ExampleCodeSample._visit<FdrCjsSdk.api.v1.register.CustomCodeSample | undefined>(codeSample, {
                    language: (value) => ({
                        ...
                        code: value.code,
                        name: value.name != null ? getNameString(value.name) : undefined,
...
function convertWebhookPayloadWithFileUpload(
    ...
    if (fileUploadPayload != null) {
        return {
            type: {
                type: "formData",
                name: getNameString(fileUploadPayload.name),
