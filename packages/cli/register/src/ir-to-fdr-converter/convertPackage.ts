...
/**
 * Surface the IR endpoint's user-specified v2 examples on the FDR endpoint
 * object so the AI example enhancer can recognise endpoints that already have
 * human-authored OpenAPI examples and skip them.
 *
 * The FDR SDK's `EndpointDefinition` type does not declare a `v2Examples`
 * field — but `hasUserSpecifiedV2Examples` in the AI enhancer reads it via an
 * `as any` cast. Without this marker, every FDR endpoint looks like it has
 * no user-specified examples, the predicate always returns `false`, and the
 * enhancer falls back to the fragile `isExampleAutogenerated` value-shape
 * heuristic. That heuristic incorrectly classifies generic-looking human
 * values (e.g. `name: "name"`) as autogenerated and the enhancer ends up
 * overwriting them. See FER-10006.
 *
 * We deliberately omit `autogeneratedExamples` here: they can be large, and
 * the only consumer that needs the marker (the AI enhancer) cares about the
 * user-specified entries only.
 */
function attachUserSpecifiedV2ExamplesMarker(
    ...
    const endpointWithV2ExamplesMarker = endpoint as FdrCjsSdk.api.v1.register.EndpointDefinition & {
        v2Examples?: {
            userSpecifiedExamples: Record<string, V2HttpEndpointExample>;
            autogeneratedExamples: Record<string, V2HttpEndpointExample>;
        };
    };
...
function convertWebSocketChannel(
    ...
    if (
        ...
        const userSpecifiedEntries = Object.entries(channel.v2Examples.userSpecifiedExamples);
        const autogeneratedEntries = Object.entries(channel.v2Examples.autogeneratedExamples);
        const firstEntry = userSpecifiedEntries.length > 0 ? userSpecifiedEntries[0] : autogeneratedEntries[0];
        if (firstEntry != null) {
            const [exampleName, firstExample] = firstEntry;
            examples.push({
                name: exampleName,
                description: "",
...
// `responseHeaders` was added to the FDR register schema after the pinned
// @fern-api/fdr-sdk release; keep the local extension until the catalog pin is bumped.
type ExampleEndpointCallWithResponseHeaders = FdrCjsSdk.api.v1.register.ExampleEndpointCall & {
    responseHeaders?: Record<string, unknown>;
};

function convertResponseHeaderExamples(headers: Ir.http.HttpHeader[] | undefined): Record<string, unknown> | undefined {
    const responseHeaders: Record<string, unknown> = {};
    for (const header of headers ?? []) {
        const example =
            Object.values(header.v2Examples?.userSpecifiedExamples ?? {})[0] ??
            Object.values(header.v2Examples?.autogeneratedExamples ?? {})[0];
        if (example !== undefined) {
            responseHeaders[getWireValue(header.name)] = example;
        }
    }
    return Object.keys(responseHeaders).length > 0 ? responseHeaders : undefined;
}

// Error-response headers live on the error declaration, not the endpoint's
// `responseHeaders`, so pick the header set that matches the example's status.
function findErrorHeadersByStatusCode({
    irEndpoint,
    ir,
    statusCode
}: {
    ...
    ir: Ir.ir.IntermediateRepresentation;
    statusCode: number;
}): Ir.http.HttpHeader[] | undefined {
    let wildcardMatch: Ir.ErrorDeclaration | undefined;
    for (const responseError of irEndpoint.errors ?? []) {
        ...
        if (
            ...
            Math.floor(error.statusCode / 100) === Math.floor(statusCode / 100)
        ) {
            wildcardMatch = error;
        }
    }
    return wildcardMatch?.headers ?? undefined;
}
...
function convertV2HttpEndpointExample({
    ...
    irService: Ir.http.HttpService;
    ir: Ir.ir.IntermediateRepresentation;
}): ExampleEndpointCallWithResponseHeaders | undefined {
    if (example == null) {
        return undefined;
    }
    ...
    return {
        ...
        responseBodyV3:
            responseBodyValue != null
                ...
                : undefined,
        responseHeaders: convertResponseHeaderExamples(responseHeaders),
...
function convertHttpEndpointExample({
    ...
    const errorHeaders = Ir.http.ExampleResponse._visit(example.response, {
        ok: () => undefined,
    ...
    return {
        ...
        responseHeaders: convertResponseHeaderExamples(errorHeaders ?? irEndpoint.responseHeaders),
