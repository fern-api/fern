...
/**
 * Checks whether a v2 schema example is an OpenAPI ExampleObject (with value, summary,
 * description fields) rather than a raw JSON value. Error examples from the OpenAPI
 * importer store the full ExampleObject so that per-example descriptions are preserved.
 */
function isOpenApiExampleObject(
    example: unknown
): example is { value: unknown; summary?: string; description?: string; externalValue?: string } {
    if (typeof example !== "object" || example === null || Array.isArray(example)) {
        return false;
    }
    const obj = example as Record<string, unknown>;
    return "value" in obj && ("summary" in obj || "description" in obj || "externalValue" in obj);
}

/**
 * Extracts the response body value and optional description from a v2 error example.
 * Handles both formats:
 * - Full OpenAPI ExampleObject: { value: <json>, description?: string, summary?: string }
 * - Raw JSON value (legacy): the example IS the response body
 */
function extractErrorExampleData(example: unknown): { value: unknown; description: string | undefined } {
    if (isOpenApiExampleObject(example)) {
        return {
            value: example.value,
            description: typeof example.description === "string" ? example.description : undefined
        };
    }
    return { value: example, description: undefined };
}
...
function getErrorExamplesFromDeclaration(
    ...
    if (v2UserSpecifiedExamples != null && Object.keys(v2UserSpecifiedExamples).length > 0) {
        return Object.entries(v2UserSpecifiedExamples).map(([name, example]) => {
            const { value, description } = extractErrorExampleData(example);
            return {
                name,
                responseBody: { type: "json" as const, value },
                description
            };
        });
