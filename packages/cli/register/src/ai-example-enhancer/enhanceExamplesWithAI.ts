 ...
 *
 * User-specified examples can live at three levels on an FDR endpoint:
 *   1. Endpoint-level v2Examples.userSpecifiedExamples (from x-fern-examples /
 *      x-code-samples, or synthesized from native `examples:` on schemaless
 *      request bodies).
 *   2. Per-request-body v2Examples.userSpecifiedExamples on
 *      v2RequestBodies.requestBodies[i] (from native OpenAPI `example` /
 *      `examples` on a schema-backed request body mediaType).
 *   3. Per-response-body v2Examples.userSpecifiedExamples on
 *      v2Responses.responses[i].body (from native OpenAPI `example` /
 *      `examples` on a response mediaType).
 *
 * Any of these should disqualify an endpoint from AI enhancement so we don't
 * silently overwrite the user's examples from their spec.
 */
export function hasUserSpecifiedV2Examples(endpointV3: EndpointV3): boolean {
    try {
        ...
        // Check endpoint-level v2Examples for user-specified examples.
        if (
            endpoint.v2Examples?.userSpecifiedExamples &&
            typeof endpoint.v2Examples.userSpecifiedExamples === "object" &&
            Object.keys(endpoint.v2Examples.userSpecifiedExamples).length > 0
        ) {
            return true;
        }

        // Check per-request-body v2Examples for user-specified examples. These
        // are populated from native OpenAPI `examples:` on schema-backed request
        // body mediaTypes, which do not bubble up to endpoint-level v2Examples.
        if (endpoint.v2RequestBodies?.requestBodies) {
            for (const requestBody of endpoint.v2RequestBodies.requestBodies) {
                if (
                    requestBody?.v2Examples?.userSpecifiedExamples &&
                    typeof requestBody.v2Examples.userSpecifiedExamples === "object" &&
                    Object.keys(requestBody.v2Examples.userSpecifiedExamples).length > 0
                ) {
                    return true;
                }
            }
        ...
        // Check per-response-body v2Examples for user-specified examples.
        if (endpoint.v2Responses?.responses) {
            for (const response of endpoint.v2Responses.responses) {
                if (
                    ...
                    Object.keys(response.body.v2Examples.userSpecifiedExamples).length > 0
                ) {
                    return true;
                }
            }
