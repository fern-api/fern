...
function isExampleAutogenerated(example: ExampleV3): boolean {
    ...
    const requestIsGenericValues = isFromAutogeneratedValues(extractExampleValue(example.requestBodyV3));
    const responseIsGenericValues = isFromAutogeneratedValues(extractExampleValue(example.responseBodyV3));

    return hasGenericName && hasEmptyDescription && requestIsGenericValues && responseIsGenericValues;
}

function isFromAutogeneratedValues(obj: unknown): boolean {
    if (obj === null || obj === undefined) {
        return false;
    }

    if (typeof obj === "string") {
        // Check against all Fern default string values
        return (
            obj === Examples.STRING ||
            obj === Examples.BASE64 ||
            obj === Examples.DATE ||
    ...
    if (typeof obj === "number") {
        // Check against all Fern default number values
        return (
            obj === Examples.DOUBLE ||
            obj === Examples.FLOAT ||
            obj === Examples.INT ||
    ...
    if (typeof obj === "boolean") {
        // Check against Fern default boolean value
        return obj === Examples.BOOLEAN;
    }

    if (Array.isArray(obj)) {
        // All items in array must be autogenerated values for array to be considered autogenerated
        return obj.length > 0 && obj.every((item) => isFromAutogeneratedValues(item));
    }

    if (typeof obj === "object") {
        // All values in object must be autogenerated for object to be considered autogenerated
        return Object.values(obj).every((value) => isFromAutogeneratedValues(value));
    }
