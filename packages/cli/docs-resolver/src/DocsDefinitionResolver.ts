...
export class DocsDefinitionResolver {
    ...
    private async toApiSectionNode(
        ...
        if (this.parsedDocsConfig.experimental?.openapiParserV2) {
            ...
            const fernApi = convertIrToApiDefinition(ir, apiDefinitionId, { oauth: item.playground?.oauth });
            ...
            const mergeApiExamples = (
                ...
                const createSnippet = (
                    ...
                    code
                        ? [{ name: example.name, language, code, generated: true, install, description: undefined }]
                        : undefined;
                ...
                const createSnippetsForExample = (example: FdrAPI.api.v1.read.ExampleEndpointCall) => ({
                    go:
                        ...
                        createSnippet(
                            example,
                            "go",
                            example.codeExamples.goSdk.client,
                            example.codeExamples.goSdk.install
                        ),
                    pythonAsync:
                    ...
                    ruby:
                        ...
                        createSnippet(
                            example,
                            "ruby",
                            example.codeExamples.rubySdk.client,
                            example.codeExamples.rubySdk.install
                        ),
                    typescript:
                ...
                const mergeSnippets = (
                    ...
                    ...(snippets.typescript && {
                        typescript: [...(existing.typescript ?? []), ...(snippets.typescript ?? [])].filter(
                            isNonNullish
                        )
                    })
                });
