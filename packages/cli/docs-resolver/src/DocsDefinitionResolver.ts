...
export class DocsDefinitionResolver {
    ...
    public async resolve(): Promise<DocsV1Write.DocsDefinition> {
        ...
        this.taskContext.logger.info(
            `[PERF] collectFilesFromDocsConfig: ${collectTime.toFixed(0)}ms (${filesToUploadSet.size} files)`
        );
        ...
        if (this.pendingApiRegistrations.length > 0) {
            ...
            this.taskContext.logger.info(
                `[PERF] deferredApiRegistrations: ${deferredTime.toFixed(0)}ms (${this.pendingApiRegistrations.length} APIs)`
            );
    ...
    private async toApiSectionNode({
        ...
        if (useV3Parser) {
            try {
                ...
                this.taskContext.logger.info(
                    `[PERF]   v3 getIntermediateRepresentation: ${(performance.now() - irStart).toFixed(0)}ms`
                );
            } catch (error) {
                openapiError = error;
            }
        ...
        if (ir == null) {
            ...
            this.taskContext.logger.info(
                `[PERF]   v1 toFernWorkspace: ${(performance.now() - v1WsStart).toFixed(0)}ms`
            );
            ...
            this.taskContext.logger.info(
                `[PERF]   v1 generateIntermediateRepresentation: ${(performance.now() - v1IrStart).toFixed(0)}ms`
            );
        } else {
            ...
            try {
                ...
                this.taskContext.logger.info(
                    `[PERF]   v3 fallback toFernWorkspace (for snippets): ${(performance.now() - v3WsStart).toFixed(0)}ms`
                );
            } catch (error) {
        ...
        this.taskContext.logger.info(
            `[PERF]   convertIrToApiDefinition: ${(performance.now() - convertStart).toFixed(0)}ms`
        );
        ...
        this.taskContext.logger.info(
            `[PERF] toApiSectionNode DONE: ${(performance.now() - apiSectionStart).toFixed(0)}ms`
        );
