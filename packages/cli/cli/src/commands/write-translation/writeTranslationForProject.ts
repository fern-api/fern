 ...
 */
export function addLanguageSuffixToUrl(
  url: string,
  language: Language,
): string {
  try {
...
function modifyConfig(
  ...
  const configWithLanguage = modifySettingsConfigForLanguage(
    docsConfig,
    language,
  );
...
function modifyInstanceUrlsForLanguage(
  ...
  if (modifiedConfig.instances && Array.isArray(modifiedConfig.instances)) {
    modifiedConfig.instances = modifiedConfig.instances.map(
      (instance: DocsInstance) => {
        const modifiedInstance = { ...instance };

        if (modifiedInstance.url) {
          modifiedInstance.url = addLanguageSuffixToUrl(
            modifiedInstance.url,
            language,
          );
        }
        ...
        if (modifiedInstance["custom-domain"]) {
          ...
          if (typeof customDomain === "string") {
            modifiedInstance["custom-domain"] = addLanguageSuffixToUrl(
              customDomain,
              language,
            );
          } else if (Array.isArray(customDomain)) {
            modifiedInstance["custom-domain"] = customDomain.map(
              (domain: string) => addLanguageSuffixToUrl(domain, language),
            );
          }
        }
...
async function createLanguageSpecificDocsConfig(
  ...
  try {
    ...
    const translatedContent = await transformContentForLanguage(
      transformation,
      context,
      stub,
    );
    ...
    context.logger.debug(
      `Created language-specific docs config: ${modifiedConfigPath}`,
    );
  } catch (error) {
    ...
    context.logger.warn(
      `Failed to create language-specific docs config for ${language}: ${error}`,
    );
  }
}
...
export async function writeTranslationForProject({
  ...
  if (docsWorkspace == null) {
    cliContext.logger.error(
      "No docs workspace found. Please ensure you have a docs.yml file configured.",
    );
    return;
  }
  ...
  await cliContext.runTaskForWorkspace(docsWorkspace, async (context) => {
    ...
    const translationsDirectory = join(
      fernDirectory,
    ...
    if (!sourceLanguage) {
      throw new Error(
        "Unexpected error - first element of languages array is invalid",
      );
    }

    if (!existsSync(translationsDirectory)) {
      context.logger.debug(
        `Creating translations directory at: ${translationsDirectory}`,
      );
    ...
    const originalDocsConfigPath = join(
      fernDirectory,
      RelativeFilePath.of(DOCS_CONFIGURATION_FILENAME),
    );
    ...
    try {
      ...
      const totalFiles = allFilesEntries.length;
      ...
      context.logger.info(
        chalk.cyan(`Processing ${totalFiles} files for translation...`),
      );
      ...
      for (const [filePath, relativePath] of allFilesEntries) {
        ...
        context.logger.info(
          chalk.gray(
            `[${currentFileIndex}/${totalFiles}] Processing: ${relativePath}`,
          ),
        );
        ...
        const fileHasChanged = await hasFileChanged(
          filePath,
          relativePath,
          hashMappings,
        );
        ...
        if (!fileHasChanged) {
          ...
          cliContext.logger.debug(
            `[SKIPPED] ${relativePath} (no changes since last translation)`,
          );
          continue;
        }

        cliContext.logger.debug(
          `[PROCESSING] ${relativePath} (detected changes)`,
        );
        ...
        for (const language of targetLanguages) {
          ...
          cliContext.logger.debug(
            `[COMPLETED] ${relativePath} -> ${language}/${relativePath}`,
          );
        }
      }
      ...
      for (const language of languages) {
        ...
        if (language === sourceLanguage) {
          ...
          context.logger.info(
            chalk.green(
              `✓ Successfully tracked ${language} source language hashes (no directory created)`,
            ),
          );
        } else {
          context.logger.info(
          ...
          context.logger.info(
            chalk.green(
              `✓ Successfully created ${language} translation with content processing`,
            ),
          );
        }
      }
    ...
    const aggregateTotalFiles =
      aggregateStats.filesProcessed + aggregateStats.filesSkipped;
    context.logger.info(
      chalk.cyan(
    ...
    context.logger.info(
      chalk.green(
        `Translations created successfully in: ${translationsDirectory}`,
      ),
    );
    if (targetLanguages.length > 0) {
      context.logger.info(
        `You can now modify the content in each language directory:`,
      );
      for (const language of targetLanguages) {
        context.logger.info(
          chalk.blue(
            `  - ${language}: ${join(translationsDirectory, RelativeFilePath.of(language))}`,
          ),
        );
      }
    }
    ...
    if (hasDocsConfig && targetLanguages.length > 0) {
      context.logger.info(
        "Language-specific docs configurations created with modified instance URLs:",
      );
      for (const language of targetLanguages) {
        context.logger.info(
          `  - ${language}/docs.yml: URLs modified to include ${language} prefix`,
        );
      }
    }
  });
...
async function collectFiles(
  ...
  for (const entry of entries) {
    ...
    const relativePath = relativeBase
      ? join(relativeBase, RelativeFilePath.of(entry))
      : RelativeFilePath.of(entry);

    const stat = statSync(fullPath);

    if (stat.isDirectory()) {
      const subdirectoryFiles = await collectFiles(fullPath, relativePath);
      for (const [subPath, subRelativePath] of Object.entries(
        subdirectoryFiles,
      )) {
