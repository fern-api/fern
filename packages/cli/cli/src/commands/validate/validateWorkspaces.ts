...
export async function validateWorkspaces({
    ...
    const runValidation = async (): Promise<{
        ...
        return { apiResults, docsResult, hasErrors, hasAnyErrors };
    };

    if (outputJson) {
        const { apiResults, docsResult, hasErrors, hasAnyErrors } = await runValidation();
        ...
        const { hasErrors, hasAnyErrors } = await runValidation();
        if (hasErrors || hasAnyErrors) {
            cliContext.failAndThrow();
        }
    }
