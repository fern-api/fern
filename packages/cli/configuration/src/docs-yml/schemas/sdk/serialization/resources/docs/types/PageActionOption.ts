// This file was auto-generated by Fern from our API Definition.

import type * as FernDocsConfig from "../../../../api/index.js";
import * as core from "../../../../core/index.js";
import type * as serializers from "../../../index.js";

export const PageActionOption: core.serialization.Schema<
    serializers.PageActionOption.Raw,
    FernDocsConfig.PageActionOption
> = core.serialization.enum_(["copy-page", "view-as-markdown", "ask-ai", "chatgpt", "claude", "cursor", "claude-code", "vscode"]);

export declare namespace PageActionOption {
    export type Raw = "copy-page" | "view-as-markdown" | "ask-ai" | "chatgpt" | "claude" | "cursor" | "claude-code" | "vscode";
}
