...
export interface ExperimentalConfig {
    ...
    /** When enabled, automatically injects `last-updated` frontmatter into Markdown pages using per-file git history during `fern docs publish`. Uses `git log -G'[^\s]'` to skip whitespace-only commits. API-generated pages are excluded. Pages that already have a `last-updated` value in frontmatter are left unchanged. */
