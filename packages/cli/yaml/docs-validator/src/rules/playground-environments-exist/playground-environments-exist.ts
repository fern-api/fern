        ...
        apiSection: async ({ workspace, context, config }) => {
            ...
            if (nonExistentEnviromentIds.length === 1 ) {
                return [
                    {
                        severity: "error",
                        message: `The API does not contain the ${nonExistentEnviromentIds[0]} environment. ${getExistingEnviromentIds(Array.from(availableEnvironmentIds)) || ""}`
                    }
                ];
            }
            ...
            return [
                {
                    severity: "error",
                    message: `The API does not contain the following enviroments: ${nonExistentEnviromentIds.join(", ")}. ${getExistingEnviromentIds(Array.from(availableEnvironmentIds)) || ""}`
                }
            ];
        }
    })
