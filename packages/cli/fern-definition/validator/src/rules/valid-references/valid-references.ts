...
export const ValidReferencesRule: Rule = {
    ...
    validate: (context: OpenApiRuleContext): OpenApiRuleViolation[] => {
        ...
        if (document.components) {
            ...
            if (document.components.schemas) addRefs("schemas", document.components.schemas);
            if (document.components.responses) addRefs("responses", document.components.responses);
            if (document.components.parameters) addRefs("parameters", document.components.parameters);
            if (document.components.examples) addRefs("examples", document.components.examples);
            if (document.components.requestBodies) addRefs("requestBodies", document.components.requestBodies);
            if (document.components.headers) addRefs("headers", document.components.headers);
            if (document.components.securitySchemes) addRefs("securitySchemes", document.components.securitySchemes);
            if (document.components.links) addRefs("links", document.components.links);
            if (document.components.callbacks) addRefs("callbacks", document.components.callbacks);
        }
