...
```typescript
import { validateOpenApiDocument } from "@fern-api/fern-definition-validator";
...
- `allowExtensions` (boolean): Allow vendor extensions (x-* fields)
- `validateExamples` (boolean): Validate example values against their schemas
- `validateSchemas` (boolean): Perform deep schema validation
