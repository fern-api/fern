...
public partial class ComplexClient
{
    ...
    public Pager<Conversation> SearchAsync(SearchRequest request, RequestOptions? options = null) {
        ...
        var pager = new CursorPager<SearchRequest, RequestOptions?, PaginatedConversationResponse, string?, Conversation>(
            request, options, SearchAsync, (request, cursor) => {
                request.Pagination ??= new ();
                request.Pagination.StartingAfter = cursor;
            }
            , response => response?.Pages?.Next?.StartingAfter, response => response?.Conversations?.ToList()
        );
        return pager;
    }

    private async Task<PaginatedConversationResponse> SearchAsync(SearchRequest request, RequestOptions? options = null, CancellationToken cancellationToken = default) {
        var response = await _client.MakeRequestAsync(new RawClient.JsonApiRequest{ 
                BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Post, Path = "conversations/search", Body = request, ContentType = "application/json", Options = options
