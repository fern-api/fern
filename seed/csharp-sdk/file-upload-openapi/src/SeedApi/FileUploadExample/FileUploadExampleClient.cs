...
public partial class FileUploadExampleClient : IFileUploadExampleClient
{
    ...
    /// </code></example>
    public async Task<string> UploadFileAsync(UploadFileRequest request, RequestOptions? options = null, CancellationToken cancellationToken = default) {
        var multipartFormRequest_ = new MultipartFormRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Post, Path = "upload-file", Options = options};
    ...
    public partial class RawAccessClient
    {
        ...
        /// Upload a file to the database
        /// </summary>
        public async Task<RawResponse<string>> UploadFileAsync(UploadFileRequest request, RequestOptions? options = null, CancellationToken cancellationToken = default) {
            var multipartFormRequest_ = new MultipartFormRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Post, Path = "upload-file", Options = options};
            multipartFormRequest_.AddStringPart("name", request.Name);
            multipartFormRequest_.AddFileParameterPart("file", request.File);
            var response = await _client.SendRequestAsync(multipartFormRequest_, cancellationToken).ConfigureAwait(false);
