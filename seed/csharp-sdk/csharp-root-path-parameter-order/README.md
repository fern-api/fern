...
[![nuget shield](https://img.shields.io/nuget/v/Ferncsharp-root-path-parameter-order)](https://nuget.org/packages/Ferncsharp-root-path-parameter-order)
...
```sh
dotnet add package Ferncsharp-root-path-parameter-order
```
...
await client.Widgets.CreateAsync(new Widget { Name = "name" }, "acme", "v1");
```
...
var result = await client.Widgets.CreateAsync(...).WithRawResponse();
