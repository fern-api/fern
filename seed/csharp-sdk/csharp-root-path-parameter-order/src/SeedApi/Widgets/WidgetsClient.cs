...
public partial class WidgetsClient : IWidgetsClient
{
        ...
        var response = await _client
            .SendRequestAsync(
                new JsonRequest
                {
                    ...
                    Path = string.Format(
                        "/{0}/api/{1}/widgets",
                        ValueConvert.ToPathParameterString(tenant ?? "acme"),
                        ValueConvert.ToPathParameterString(version ?? "v1")
                    ),
                    Body = request,
    ...
    )
    {
        return new WithRawResponseTask<Widget>(
            CreateAsyncCore(request, tenant, version, options, cancellationToken)
        );
    }
}
