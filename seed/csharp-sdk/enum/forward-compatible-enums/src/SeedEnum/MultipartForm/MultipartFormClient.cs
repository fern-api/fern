...
public partial class MultipartFormClient : IMultipartFormClient
{
    ...
    public async Task MultipartFormAsync(MultipartFormRequest request, RequestOptions? options = null, CancellationToken cancellationToken = default) {
        var multipartFormRequest_ = new SeedEnum.Core.MultipartFormRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Post, Path = "multipart", Options = options};
        ...
        var response = await _client.SendRequestAsync(multipartFormRequest_, cancellationToken).ConfigureAwait(false);
    ...
    public partial class RawAccessClient
    {
        ...
        public async Task<RawResponse<object>> MultipartFormAsync(MultipartFormRequest request, RequestOptions? options = null, CancellationToken cancellationToken = default) {
            var multipartFormRequest_ = new SeedEnum.Core.MultipartFormRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Post, Path = "multipart", Options = options};
            multipartFormRequest_.AddJsonPart("color", request.Color);
            multipartFormRequest_.AddJsonPart("maybeColor", request.MaybeColor);
            multipartFormRequest_.AddJsonParts("colorList", request.ColorList);
