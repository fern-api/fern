...
[![nuget shield](https://img.shields.io/nuget/v/Fernhttp-head)](https://nuget.org/packages/Fernhttp-head)
...
```sh
dotnet add package Fernhttp-head
```
...
await client.User.ListAsync(new UserListRequest { Limit = 1 });
```
...
var result = await client.User.ListAsync(...).WithRawResponse();
...
var data = await client.User.ListAsync(...);
```
