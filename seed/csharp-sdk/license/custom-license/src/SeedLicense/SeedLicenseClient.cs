...
public partial class SeedLicenseClient : ISeedLicenseClient
{
    ...
    /// </code></example>
    public async Task GetAsync(RequestOptions? options = null, CancellationToken cancellationToken = default) {
        ...
        }
        {
            ...
            throw new SeedLicenseApiException($"Error with status code {response.StatusCode}", response.StatusCode, responseBody);
        }
    }
    ...
    public partial class RawAccessClient
    {
        ...
        public async Task<RawResponse<object>> GetAsync(RequestOptions? options = null, CancellationToken cancellationToken = default) {
            var response = await _client.SendRequestAsync(new JsonRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Get, Path = "/", Options = options}, cancellationToken).ConfigureAwait(false);
            if (response.StatusCode is >= 200 and < 400)
            {
