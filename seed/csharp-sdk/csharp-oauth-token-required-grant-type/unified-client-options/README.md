...
[![nuget shield](https://img.shields.io/nuget/v/Ferncsharp-oauth-token-required-grant-type)](https://nuget.org/packages/Ferncsharp-oauth-token-required-grant-type)
...
```sh
dotnet add package Ferncsharp-oauth-token-required-grant-type
```
...
```csharp
using SeedCsharpOauthTokenRequiredGrantType;

var client = new SeedCsharpOauthTokenRequiredGrantTypeClient(
    clientOptions: new ClientOptions { ClientId = "client_id", ClientSecret = "client_secret" }
);
...
} catch (SeedCsharpOauthTokenRequiredGrantTypeApiException e) {
...
var result = await client.Auth.CreateOauth2TokenAsync(...).WithRawResponse();
...
var parsedData = await client.Auth.CreateOauth2TokenAsync(...);
