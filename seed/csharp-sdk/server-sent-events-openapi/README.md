...
[![nuget shield](https://img.shields.io/nuget/v/Fernserver-sent-events-openapi)](https://nuget.org/packages/Fernserver-sent-events-openapi)
...
```sh
dotnet add package Fernserver-sent-events-openapi
```
...
var items = client.StreamProtocolNoCollisionAsync(new StreamRequest());
...
var result = await client.StreamProtocolNoCollisionAsync(...).WithRawResponse();
...
var parsedData = await client.StreamProtocolNoCollisionAsync(...);
...
var entityEventPayloadEventType = EntityEventPayloadEventType.Created;

// Using a custom value
var customEntityEventPayloadEventType = EntityEventPayloadEventType.FromCustom("custom-value");

// Using in a switch statement
switch (entityEventPayloadEventType.Value)
{
    case EntityEventPayloadEventType.Values.Created:
        Console.WriteLine("Created");
        break;
    default:
        Console.WriteLine($"Unknown value: {entityEventPayloadEventType.Value}");
        break;
}

// Explicit casting
string entityEventPayloadEventTypeString = (string)EntityEventPayloadEventType.Created;
EntityEventPayloadEventType entityEventPayloadEventTypeFromString = (EntityEventPayloadEventType)"CREATED";
```
