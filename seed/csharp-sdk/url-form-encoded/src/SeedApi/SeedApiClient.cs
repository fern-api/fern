...
public partial class SeedApiClient : ISeedApiClient
{
    ...
    /// );
    /// </code></example>
    public async Task<PostSubmitResponse> SubmitFormDataAsync(PostSubmitRequest request, RequestOptions? options = null, CancellationToken cancellationToken = default) {
        var response = await _client.SendRequestAsync(new FormRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Post, Path = "submit", Body = request, ContentType = "application/x-www-form-urlencoded", Options = options}, cancellationToken).ConfigureAwait(false);
    ...
    public partial class RawAccessClient
    {
        ...
        public async Task<RawResponse<PostSubmitResponse>> SubmitFormDataAsync(PostSubmitRequest request, RequestOptions? options = null, CancellationToken cancellationToken = default) {
            var response = await _client.SendRequestAsync(new FormRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Post, Path = "submit", Body = request, ContentType = "application/x-www-form-urlencoded", Options = options}, cancellationToken).ConfigureAwait(false);
            if (response.StatusCode is >= 200 and < 400)
            {
                var responseBody = await response.Raw.Content.ReadAsStringAsync();
