...
[![nuget shield](https://img.shields.io/nuget/v/Fernserver-url-templating-single-url)](https://nuget.org/packages/Fernserver-url-templating-single-url)
...
```sh
dotnet add package Fernserver-url-templating-single-url
```
...
{
    BaseUrl = SeedApiEnvironment.RegionalApiServer
});
```
...
var result = await client.GetUsersAsync(...).WithRawResponse();
...
var parsedData = await client.GetUsersAsync(...);
