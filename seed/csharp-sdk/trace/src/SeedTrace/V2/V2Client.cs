...
public partial class V2Client : IV2Client
{
    ...
    internal V2Client (RawClient client){
        _client = client;
        Problem = 
        new ProblemClient(_client);
        V3 = 
        new V3Client(_client);
    ...
    public partial class RawAccessClient
    {
        ...
        public async Task<RawResponse<object>> TestAsync(RequestOptions? options = null, CancellationToken cancellationToken = default) {
            var response = await _client.SendRequestAsync(new JsonRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Get, Path = "", Options = options}, cancellationToken).ConfigureAwait(false);
            if (response.StatusCode is >= 200 and < 400)
            {
