...
public partial class ServiceClient : IServiceClient
{
    ...
    /// </code></example>
    public async Task<SeedExamples.Exception> GetExceptionAsync(string notificationId, RequestOptions? options = null, CancellationToken cancellationToken = default) {
        var response = await _client.SendRequestAsync(new JsonRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Get, Path = string.Format("/file/notification/{0}", ValueConvert.ToPathParameterString(notificationId)), Options = options}, cancellationToken).ConfigureAwait(false);
    ...
    public partial class RawAccessClient
    {
        ...
        public async Task<RawResponse<SeedExamples.Exception>> GetExceptionAsync(string notificationId, RequestOptions? options = null, CancellationToken cancellationToken = default) {
            var response = await _client.SendRequestAsync(new JsonRequest {BaseUrl = _client.Options.BaseUrl, Method = HttpMethod.Get, Path = string.Format("/file/notification/{0}", ValueConvert.ToPathParameterString(notificationId)), Options = options}, cancellationToken).ConfigureAwait(false);
            if (response.StatusCode is >= 200 and < 400)
            {
                var responseBody = await response.Raw.Content.ReadAsStringAsync();
