...
[![nuget shield](https://img.shields.io/nuget/v/Ferncsharp-path-param-order)](https://nuget.org/packages/Ferncsharp-path-param-order)
...
```sh
dotnet add package Ferncsharp-path-param-order
```
...
try {
    var response = await client.SetApprovedBillAsync(...);
} catch (SeedCsharpPathParamOrderApiException e) {
...
var result = await client.SetApprovedBillAsync(...).WithRawResponse();
...
var parsedData = await client.SetApprovedBillAsync(...);
