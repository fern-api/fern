...
{
    private UnionWithBaseProperties(string type, object? value)
    {
        Type = type;
        Value = value;
    }
    ...
    public override bool Equals(object? obj)
    {
        ...
        if (ReferenceEquals(this, obj))
            return true;
        if (obj is not UnionWithBaseProperties other)
            return false;
    ...
    public static implicit operator UnionWithBaseProperties(
        SeedUndiscriminatedUnions.NamedMetadata value
    ) => new("namedMetadata", value);

    public static implicit operator UnionWithBaseProperties(Dictionary<string, object?>? value) =>
        new("optionalMetadata", value);
    ...
    internal sealed class JsonConverter : JsonConverter<UnionWithBaseProperties>
    {
        ...
        )
        {
            ...
            if (reader.TokenType == JsonTokenType.StartObject)
            {
                ...
                foreach (var (key, type) in types)
                {
                    try
                    {
                        ...
                        if (value != null)
                        {
                            UnionWithBaseProperties result = new(key, value);
                            return result;
                        }
                    }
                    catch (JsonException)
            ...
            throw new JsonException(
                $"Cannot deserialize JSON token {reader.TokenType} into UnionWithBaseProperties"
            );
        }
        ...
        )
        {
            ...
            UnionWithBaseProperties result = new("string", stringValue);
            return result;
        }
