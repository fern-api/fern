...
public partial class SeedExhaustiveClient
{
    ...
    public SeedExhaustiveClient (string token, ClientOptions? clientOptions = null){
        try (){
            ...
            clientOptions.ExceptionHandler = new ExceptionHandler(new SeedExhaustiveExceptionInterceptor());
            ...
            Endpoints = 
            new EndpointsClient(_client);
            InlinedRequests = 
            new InlinedRequestsClient(_client);
            NoAuth = 
            new NoAuthClient(_client);
            NoReqBody = 
            new NoReqBodyClient(_client);
            ReqWithHeaders = 
            new ReqWithHeadersClient(_client);
        }
        catch ((Exception ex)){
