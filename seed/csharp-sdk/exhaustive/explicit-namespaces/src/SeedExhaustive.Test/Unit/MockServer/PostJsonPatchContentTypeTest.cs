...
public class PostJsonPatchContentTypeTest : BaseMockServerTest
{
    ...
    public void MockServerTest() {
        ...
        Server.Given(WireMock.RequestBuilders.Request.Create().WithPath("/foo/bar").WithHeader("Content-Type", "application/json-patch+json").UsingPost().WithBodyAsJson(requestJson))

        .RespondWith(WireMock.ResponseBuilders.Response.Create()
        .WithStatusCode(200)
        );

        Assert.DoesNotThrowAsync(async () => await Client.Endpoints.ContentType.PostJsonPatchContentTypeAsync(new ObjectWithOptionalField{ 
