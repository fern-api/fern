...
public class PostWithObjectBodyandResponseTest : BaseWireTest
{
    ...
    public async Task WireTest() {
        ...
        Server.Given(WireMock.RequestBuilders.Request.Create().WithPath("/req-bodies/object").UsingPost().WithBodyAsJson(requestJson))

        .RespondWith(WireMock.ResponseBuilders.Response.Create()
        .WithStatusCode(200)
        .WithBody(mockResponse));

        var response = await Client.InlinedRequests.PostWithObjectBodyandResponseAsync(new PostWithObjectBodynew PostWithObjectBody{ 
                String = "string", Integer = 1, NestedObject = new ObjectWithOptionalFieldnew ObjectWithOptionalField{ 
