...
public partial class PaginationClient : IPaginationClient
{
    ...
    private async Task<WithRawResponse<EndpointsPaginatedResponse>> ListItemsAsyncCore(
    ...
    /// </code></example>
    public WithRawResponseTask<EndpointsPaginatedResponse> ListItemsAsync(
        ListItemsPaginationRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<EndpointsPaginatedResponse>(
            ListItemsAsyncCore(request, options, cancellationToken)
        );
    }
}
