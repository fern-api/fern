...
public partial class EndpointsContainerClient : IEndpointsContainerClient
{
    ...
    private async Task<
        WithRawResponse<Dictionary<string, TypesMixedType>>
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<Dictionary<string, TypesMixedType>>(
                    responseBody
                )!;
                return new WithRawResponse<Dictionary<string, TypesMixedType>>()
                {
                    Data = responseData,
                    RawResponse = new RawResponse()
                    {
    ...
    {
        return new WithRawResponseTask<Dictionary<string, TypesMixedType>>(
            EndpointsContainerGetAndReturnMapOfPrimToUndiscriminatedUnionAsyncCore(
                request,
                options,
                cancellationToken
