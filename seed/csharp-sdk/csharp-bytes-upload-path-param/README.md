...
[![nuget shield](https://img.shields.io/nuget/v/Ferncsharp-bytes-upload-path-param)](https://nuget.org/packages/Ferncsharp-bytes-upload-path-param)
...
```sh
dotnet add package Ferncsharp-bytes-upload-path-param
```
...
try {
    var response = await client.Service.UploadWithPathParamAsync(...);
} catch (SeedCsharpBytesUploadPathParamApiException e) {
...
var result = await client.Service.UploadWithPathParamAsync(...).WithRawResponse();
...
var parsedData = await client.Service.UploadWithPathParamAsync(...);
