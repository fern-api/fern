...
public partial class SeedApiClient : ISeedApiClient
{
    ...
    public SeedApiClient(ClientOptions? clientOptions = null)
    {
        ...
        var platformHeaders = new Headers(
            new Dictionary<string, string>()
            {
                ...
                { "User-Agent", "Ferncli-multi-spec/0.0.1" },
            }
        );
    ...
    private async Task<WithRawResponse<IEnumerable<Invoice>>> ListInvoicesAsyncCore(
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<IEnumerable<Invoice>>(responseBody)!;
                return new WithRawResponse<IEnumerable<Invoice>>()
                {
                    Data = responseData,
                    RawResponse = new RawResponse()
                    {
    ...
    private async Task<WithRawResponse<Invoice>> GetInvoiceAsyncCore(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<Invoice>(responseBody)!;
                return new WithRawResponse<Invoice>()
                {
                    Data = responseData,
                    RawResponse = new RawResponse()
                    {
    ...
    /// <example><code>
    /// await client.ListInvoicesAsync();
    /// </code></example>
    public WithRawResponseTask<IEnumerable<Invoice>> ListInvoicesAsync(
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<IEnumerable<Invoice>>(
            ListInvoicesAsyncCore(options, cancellationToken)
        );
    }

    /// <example><code>
    /// await client.GetInvoiceAsync(new GetInvoiceRequest { InvoiceId = "invoiceId" });
    /// </code></example>
    public WithRawResponseTask<Invoice> GetInvoiceAsync(
        GetInvoiceRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<Invoice>(
            GetInvoiceAsyncCore(request, options, cancellationToken)
        );
    }
}
