...
class ObjectClient
{
     ...
     * @throws SeedApiException
     */
    public function getAndReturnNestedWithRequiredField(string $stringValue, GetAndReturnNestedWithRequiredFieldObjectRequest $request, ?array $options = null): ?TypesNestedObjectWithRequiredField
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? '',
                    path: "object/get-and-return-nested-with-required-field/{$stringValue}",
                    method: HttpMethod::POST,
                    body: $request->body,
                ),
                $options,
