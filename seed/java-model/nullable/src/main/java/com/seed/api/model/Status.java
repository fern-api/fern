...
public final class Status {
  ...
  public static Status active(StatusActive value) {
    return new Status(new ActiveValue(value));
  }

  public static Status archived(StatusArchived value) {
    return new Status(new ArchivedValue(value));
  }

  public static Status softDeleted(StatusSoftDeleted value) {
  ...
  public Optional<StatusActive> getActive() {
    if (isActive()) {
      return Optional.of(((ActiveValue) value).value);
    }
    return Optional.empty();
  }

  public Optional<StatusArchived> getArchived() {
    if (isArchived()) {
  ...
  public Optional<StatusSoftDeleted> getSoftDeleted() {
    if (isSoftDeleted()) {
  ...
  public interface Visitor<T> {
    T visitActive(StatusActive active);

    T visitArchived(StatusArchived archived);

    T visitSoftDeleted(StatusSoftDeleted softDeleted);
  ...
  private static final class ActiveValue implements Value {
    ...
    private ActiveValue(StatusActive value) {
      this.value = value;
    }

    @java.lang.Override
    public <T> T visit(Visitor<T> visitor) {
      return visitor.visitActive(value);
    }

    @java.lang.Override
    public boolean equals(Object other) {
      if (this == other) return true;
      return other instanceof ActiveValue && equalTo((ActiveValue) other);
    }

    private boolean equalTo(ActiveValue other) {
  ...
  private static final class ArchivedValue implements Value {
    ...
    private StatusArchived value;

    @JsonCreator(
        mode = JsonCreator.Mode.PROPERTIES
    )
    private ArchivedValue() {
    }

    private ArchivedValue(StatusArchived value) {
      this.value = value;
    }
  ...
  private static final class SoftDeletedValue implements Value {
    ...
    private StatusSoftDeleted value;

    @JsonCreator(
        mode = JsonCreator.Mode.PROPERTIES
    )
    private SoftDeletedValue() {
    }

    private SoftDeletedValue(StatusSoftDeleted value) {
      this.value = value;
    }
