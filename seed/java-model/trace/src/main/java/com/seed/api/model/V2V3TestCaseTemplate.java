/**
 * This file was auto-generated by Fern from our API Definition.
 */

package com.seed.api.model;

import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonSetter;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.seed.api.core.ObjectMappers;
import java.lang.Object;
import java.lang.String;
import java.util.Objects;

@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(
    builder = V2V3TestCaseTemplate.Builder.class
)
public final class V2V3TestCaseTemplate {
  private final String templateId;

  private final String name;

  private final V2V3TestCaseImplementation implementation;

  private V2V3TestCaseTemplate(String templateId, String name,
      V2V3TestCaseImplementation implementation) {
    this.templateId = templateId;
    this.name = name;
    this.implementation = implementation;
  }

  @JsonProperty("templateId")
  public String getTemplateId() {
    return templateId;
  }

  @JsonProperty("name")
  public String getName() {
    return name;
  }

  @JsonProperty("implementation")
  public V2V3TestCaseImplementation getImplementation() {
    return implementation;
  }

  @java.lang.Override
  public boolean equals(Object other) {
    if (this == other) return true;
    return other instanceof V2V3TestCaseTemplate && equalTo((V2V3TestCaseTemplate) other);
  }

  private boolean equalTo(V2V3TestCaseTemplate other) {
    return templateId.equals(other.templateId) && name.equals(other.name) && implementation.equals(other.implementation);
  }

  @java.lang.Override
  public int hashCode() {
    return Objects.hash(this.templateId, this.name, this.implementation);
  }

  @java.lang.Override
  public String toString() {
    return ObjectMappers.stringify(this);
  }

  public static TemplateIdStage builder() {
    return new Builder();
  }

  public interface TemplateIdStage {
    NameStage templateId(String templateId);

    Builder from(V2V3TestCaseTemplate other);
  }

  public interface NameStage {
    ImplementationStage name(String name);
  }

  public interface ImplementationStage {
    _FinalStage implementation(V2V3TestCaseImplementation implementation);
  }

  public interface _FinalStage {
    V2V3TestCaseTemplate build();
  }

  @JsonIgnoreProperties(
      ignoreUnknown = true
  )
  public static final class Builder implements TemplateIdStage, NameStage, ImplementationStage, _FinalStage {
    private String templateId;

    private String name;

    private V2V3TestCaseImplementation implementation;

    private Builder() {
    }

    @java.lang.Override
    public Builder from(V2V3TestCaseTemplate other) {
      templateId(other.getTemplateId());
      name(other.getName());
      implementation(other.getImplementation());
      return this;
    }

    @java.lang.Override
    @JsonSetter("templateId")
    public NameStage templateId(String templateId) {
      this.templateId = Objects.requireNonNull(templateId, "templateId must not be null");
      return this;
    }

    @java.lang.Override
    @JsonSetter("name")
    public ImplementationStage name(String name) {
      this.name = Objects.requireNonNull(name, "name must not be null");
      return this;
    }

    @java.lang.Override
    @JsonSetter("implementation")
    public _FinalStage implementation(V2V3TestCaseImplementation implementation) {
      this.implementation = Objects.requireNonNull(implementation, "implementation must not be null");
      return this;
    }

    @java.lang.Override
    public V2V3TestCaseTemplate build() {
      return new V2V3TestCaseTemplate(templateId, name, implementation);
    }
  }
}
