# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import typing

import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel, update_forward_refs
from .search_request_query import SearchRequestQuery
from .starting_after_paging import StartingAfterPaging


class SearchRequest(UniversalBaseModel):
    pagination: typing.Optional[StartingAfterPaging] = None
    query: SearchRequestQuery

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


from .multiple_filter_search_request import MultipleFilterSearchRequest  # noqa: E402, I001

update_forward_refs(SearchRequest, MultipleFilterSearchRequest=MultipleFilterSearchRequest)
