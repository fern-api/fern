...
async def test_send_list(client: SeedEnum, async_client: AsyncSeedEnum) -> None:
    # Type ignore to avoid mypy complaining about the function not being meant to return a value
    assert (
        client.query_param.send_list(operand=">", operand_or_color="red")  # type: ignore[func-returns-value]
        is None
    )

    assert (
        await async_client.query_param.send_list(operand=">", operand_or_color="red")  # type: ignore[func-returns-value]
        is None
    )
