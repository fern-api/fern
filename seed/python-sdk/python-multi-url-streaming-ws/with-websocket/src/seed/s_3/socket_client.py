...
class AsyncS3SocketClient(EventEmitterMixin):
    ...
    async def __aiter__(self):
        async for message in self._websocket:
            if isinstance(message, bytes):
                ...
                yield parse_obj_as(S3SocketClientResponse, json.loads(message))  # type: ignore
        ...
        try:
            async for raw_message in self._websocket:
                if isinstance(raw_message, bytes):
                    ...
                    parsed = parse_obj_as(S3SocketClientResponse, json_data)  # type: ignore
    ...
    async def send_send(self, message: SendRequest) -> None:
        """
        Send a message to the websocket connection.
        The message will be sent as a SendRequest.
        """
        await self._send_model(message)

    async def recv(self) -> S3SocketClientResponse:
        """
        ...
        return parse_obj_as(S3SocketClientResponse, json_data)  # type: ignore
...
class S3SocketClient(EventEmitterMixin):
    ...
    def send_send(self, message: SendRequest) -> None:
        """
        Send a message to the websocket connection.
        The message will be sent as a SendRequest.
        """
    ...
    def recv(self) -> S3SocketClientResponse:
        """
        Receive a message from the websocket connection.
        """
