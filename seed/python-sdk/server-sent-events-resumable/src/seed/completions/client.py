...
class CompletionsClient:
    ...
    def stream_non_resumable(
        ...
        response = client.completions.stream_non_resumable(
            query="bar",
        )
        for chunk in response:
            yield chunk
        """
        with self._raw_client.stream_non_resumable(query=query, request_options=request_options) as r:
            yield from r.data
...
class AsyncCompletionsClient:
    ...
    async def stream(
        ...
        from seed import AsyncSeedServerSentEventsResumable

        client = AsyncSeedServerSentEventsResumable(
            base_url="https://yourhost.com/path/to/api",
        )
    ...
    async def stream_non_resumable(
        self, *, query: str, request_options: typing.Optional[RequestOptions] = None
    ) -> typing.AsyncIterator[StreamedCompletion]:
        """
        Parameters
        ...
        async def main() -> None:
            response = await client.completions.stream_non_resumable(
                query="bar",
            )
            async for chunk in response:
                yield chunk
        ...
        asyncio.run(main())
        """
        async with self._raw_client.stream_non_resumable(query=query, request_options=request_options) as r:
            async for _chunk in r.data:
                yield _chunk
