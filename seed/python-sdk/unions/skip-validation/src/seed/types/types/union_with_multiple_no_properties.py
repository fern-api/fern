# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2
from ...core.unchecked_base_model import UncheckedBaseModel, UnionMetadata


class UnionWithMultipleNoProperties_Foo(UncheckedBaseModel):
    """
    Examples
    --------
    from seed.types import UnionWithMultipleNoProperties_Foo

    UnionWithMultipleNoProperties_Foo(
        name="example",
    )
    """

    type: typing.Literal["foo"] = "foo"
    name: str

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


class UnionWithMultipleNoProperties_Empty1(UncheckedBaseModel):
    """
    Examples
    --------
    from seed.types import UnionWithMultipleNoProperties_Foo

    UnionWithMultipleNoProperties_Foo(
        name="example",
    )
    """

    type: typing.Literal["empty1"] = "empty1"

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


class UnionWithMultipleNoProperties_Empty2(UncheckedBaseModel):
    """
    Examples
    --------
    from seed.types import UnionWithMultipleNoProperties_Foo

    UnionWithMultipleNoProperties_Foo(
        name="example",
    )
    """

    type: typing.Literal["empty2"] = "empty2"

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


"""
from seed.types import UnionWithMultipleNoProperties_Foo

UnionWithMultipleNoProperties_Foo(
    name="example",
)
"""
UnionWithMultipleNoProperties = typing_extensions.Annotated[
    typing.Union[
        UnionWithMultipleNoProperties_Foo, UnionWithMultipleNoProperties_Empty1, UnionWithMultipleNoProperties_Empty2
    ],
    UnionMetadata(discriminant="type"),
]
