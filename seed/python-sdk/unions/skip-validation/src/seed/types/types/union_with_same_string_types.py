# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2
from ...core.unchecked_base_model import UncheckedBaseModel, UnionMetadata


class UnionWithSameStringTypes_CustomFormat(UncheckedBaseModel):
    value: str
    type: typing.Literal["customFormat"] = "customFormat"

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True


class UnionWithSameStringTypes_RegularString(UncheckedBaseModel):
    value: str
    type: typing.Literal["regularString"] = "regularString"

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True


class UnionWithSameStringTypes_PatternString(UncheckedBaseModel):
    value: str
    type: typing.Literal["patternString"] = "patternString"

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True


"""
from seed.types import UnionWithSameStringTypes_CustomFormat

UnionWithSameStringTypes_CustomFormat(value="custom-123")
"""
UnionWithSameStringTypes = typing_extensions.Annotated[
    typing.Union[
        UnionWithSameStringTypes_CustomFormat,
        UnionWithSameStringTypes_RegularString,
        UnionWithSameStringTypes_PatternString,
    ],
    UnionMetadata(discriminant="type"),
]
