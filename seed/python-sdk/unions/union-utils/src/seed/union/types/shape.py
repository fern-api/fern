...
class Shape(UniversalRootModel):
    ...
    if IS_PYDANTIC_V2:
        root: typing_extensions.Annotated[typing.Union[_Shape.Circle, _Shape.Square], pydantic.Field(discriminator="type")]
        def get_as_union(self) -> typing.Union[_Shape.Circle, _Shape.Square]:
            return self.root
    else:
        __root__: typing_extensions.Annotated[typing.Union[_Shape.Circle, _Shape.Square], pydantic.Field(discriminator="type")]
        def get_as_union(self) -> typing.Union[_Shape.Circle, _Shape.Square]:
    ...
    def visit(self, circle: typing.Callable[[union_types_circle_Circle], T_Result], square: typing.Callable[[union_types_square_Square], T_Result]) -> T_Result:
        unioned_value = self.get_as_union()
        if unioned_value.type == "circle":
            return circle(
            union_types_circle_Circle(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
        if unioned_value.type == "square":
            return square(
            union_types_square_Square(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
