...
class Metadata extends JsonSerializableType
{
     ...
     * @param MetadataHtml $html
     * @return Metadata
     */
    public static function html(array $extra, array $tags, MetadataHtml $html): Metadata
    {
        return new Metadata([
            'extra' => $extra,
            'tags' => $tags,
     ...
     * @param array<string, string> $extra
     * @param array<string> $tags
     * @param MetadataMarkdown $markdown
     * @return Metadata
     */
    public static function markdown(array $extra, array $tags, MetadataMarkdown $markdown): Metadata
    {
        return new Metadata([
            'extra' => $extra,
            'tags' => $tags,
    ...
    {
        return $this->value instanceof MetadataHtml && $this->type === 'html';
    }

    /**
     * @return MetadataHtml
     */
    public function asHtml(): MetadataHtml
    {
        if (!($this->value instanceof MetadataHtml && $this->type === 'html')) {
            throw new Exception(
    ...
    {
        return $this->value instanceof MetadataMarkdown && $this->type === 'markdown';
    }

    /**
     * @return MetadataMarkdown
     */
    public function asMarkdown(): MetadataMarkdown
    {
        if (!($this->value instanceof MetadataMarkdown && $this->type === 'markdown')) {
            throw new Exception(
    ...
    public function jsonSerialize(): array
    {
        ...
        switch ($this->type) {
            case 'html':
                ...
                break;
            case 'markdown':
                $value = $this->asMarkdown()->jsonSerialize();
                $result = array_merge($value, $result);
                break;
            case '_unknown':
            default:
    ...
    public static function jsonDeserialize(array $data): static
    {
        ...
        switch ($type) {
            case 'html':
                $args['value'] = MetadataHtml::jsonDeserialize($data);
                break;
            case 'markdown':
                $args['value'] = MetadataMarkdown::jsonDeserialize($data);
                break;
            case '_unknown':
            default:
                $args['type'] = '_unknown';
