...
impl UnexpectedLanguageError {
    pub fn builder() -> UnexpectedLanguageErrorBuilder {
        <UnexpectedLanguageErrorBuilder as Default>::default()
    }
}
...
impl UnexpectedLanguageErrorBuilder {
    ...
    pub fn build(self) -> Result<UnexpectedLanguageError, BuildError> {
        Ok(UnexpectedLanguageError {
            expected_language: self.expected_language.ok_or_else(|| BuildError::missing_field("expected_language"))?,
            actual_language: self.actual_language.ok_or_else(|| BuildError::missing_field("actual_language"))?,
        })
    }
}
