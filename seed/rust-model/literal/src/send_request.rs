...
pub struct SendRequest {
    pub prompt: SendRequestPrompt,
    #[serde(default)]
    pub query: String,
    #[serde(default)]
    pub stream: bool,
    pub ending: SendRequestEnding,
...
pub struct SendRequestBuilder {
    prompt: Option<SendRequestPrompt>,
    query: Option<String>,
    stream: Option<bool>,
    ending: Option<SendRequestEnding>,
...
impl SendRequestBuilder {
    pub fn prompt(mut self, value: SendRequestPrompt) -> Self {
    ...
    pub fn ending(mut self, value: SendRequestEnding) -> Self {
        self.ending = Some(value);
        self
    }
    ...
    pub fn build(self) -> Result<SendRequest, BuildError> {
        Ok(SendRequest {
            ...
            ending: self.ending.ok_or_else(|| BuildError::missing_field("ending"))?,
            context: self.context.ok_or_else(|| BuildError::missing_field("context"))?,
            maybe_context: self.maybe_context,
            container_object: self.container_object.ok_or_else(|| BuildError::missing_field("container_object"))?,
        })
    }
}
