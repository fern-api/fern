...
impl SendLiteralsInlinedRequestBuilder {
    ...
    pub fn build(self) -> Result<SendLiteralsInlinedRequest, BuildError> {
        Ok(SendLiteralsInlinedRequest {
            ...
            aliased_context: self.aliased_context.ok_or_else(|| BuildError::missing_field("aliased_context"))?,
            maybe_context: self.maybe_context,
            object_with_literal: self.object_with_literal.ok_or_else(|| BuildError::missing_field("object_with_literal"))?,
        })
    }
}
