...
```swift
dependencies: [
    .package(url: "https://github.com/server-sent-events-resumable/fern", from: "0.0.1"),
]
```
...
do {
    let response = try await client.completions.stream(...)
    // Handle successful response
} catch let error as ServerSentEventsResumableError {
    switch error {
...
let request = Requests.StreamCompletionRequest(
    ...
)
```
