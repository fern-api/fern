...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as MultiUrlEnvironmentError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected MultiUrlEnvironmentError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected MultiUrlEnvironmentError, got \(error)")
        }
    }
