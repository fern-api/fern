...
@Suite("Client Retry Tests") struct ClientRetryTests {
    @Test func testRetryOn408RequestTimeout() async throws {
        ...
        let response = try await client.user.head(requestOptions: RequestOptions(additionalHeaders: stub.headers))
    ...
    @Test func testEndpointLevelMaxRetriesOverride() async throws {
        ...
        let response = try await client.user.head(requestOptions: RequestOptions(maxRetries: 5, additionalHeaders: stub.headers))
