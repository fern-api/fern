...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw MultiUrlEnvironmentNoDefaultError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw MultiUrlEnvironmentNoDefaultError.decodingError(error)
        }
    }
    ...
    private func buildRequestURL(
        ...
        let resolvedBaseURL = baseURLId.flatMap { clientConfig.baseURLs?[$0] } ?? clientConfig.baseURL
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: MultiUrlEnvironmentNoDefaultError?
                ...
                } else if let existingClientError = error as? MultiUrlEnvironmentNoDefaultError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw MultiUrlEnvironmentNoDefaultError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> MultiUrlEnvironmentNoDefaultError
    {
        ...
        return MultiUrlEnvironmentNoDefaultError.httpError(httpError)
    }
}
