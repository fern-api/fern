...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw QueryParametersError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw QueryParametersError.decodingError(error)
        }
    }
    ...
    private static func validateHTTPS(url: Foundation.URL) throws {
        ...
        throw QueryParametersError.networkError(
            NSError(
                domain: "HTTPClient",
                code: -1,
                userInfo: [
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: QueryParametersError?
                ...
                } else if let existingClientError = error as? QueryParametersError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw QueryParametersError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> QueryParametersError
    {
        ...
        return QueryParametersError.httpError(httpError)
    }
}
