...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw ObjectsWithImportsError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw ObjectsWithImportsError.decodingError(error)
        }
    }
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: ObjectsWithImportsError?
                ...
                } else if let existingClientError = error as? ObjectsWithImportsError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw ObjectsWithImportsError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> ObjectsWithImportsError
    {
        ...
        return ObjectsWithImportsError.httpError(httpError)
    }
}
