...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as OauthClientCredentialsEnvironmentVariablesError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected OauthClientCredentialsEnvironmentVariablesError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected OauthClientCredentialsEnvironmentVariablesError, got \(error)")
        }
    }
