...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as LiteralUserAgentError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected LiteralUserAgentError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected LiteralUserAgentError, got \(error)")
        }
    }
