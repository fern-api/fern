...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as SingleUrlEnvironmentNoDefaultError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected SingleUrlEnvironmentNoDefaultError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected SingleUrlEnvironmentNoDefaultError, got \(error)")
        }
    }
