...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as PhpGlobalHeaderEnvError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected PhpGlobalHeaderEnvError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected PhpGlobalHeaderEnvError, got \(error)")
        }
    }
