...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
            _ = try await client.service.upload(
                request: Data("data".utf8),
                requestOptions: RequestOptions(additionalHeaders: stub.headers)
            )

            Issue.record("Expected error to be thrown")
        } catch let error as BytesUploadError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected BytesUploadError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected BytesUploadError, got \(error)")
        }
    }
