...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw EndpointSecurityAuthError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw EndpointSecurityAuthError.decodingError(error)
        }
    }
    ...
    private static func validateHTTPS(url: Foundation.URL) throws {
        ...
        throw EndpointSecurityAuthError.networkError(
            NSError(
                domain: "HTTPClient",
                code: -1,
                userInfo: [
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: EndpointSecurityAuthError?
                ...
                } else if let existingClientError = error as? EndpointSecurityAuthError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw EndpointSecurityAuthError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> EndpointSecurityAuthError
    {
        ...
        return EndpointSecurityAuthError.httpError(httpError)
    }
}
