...
```swift
dependencies: [
    .package(url: "https://github.com/websocket/fern", from: "0.0.1"),
]
```
...
do {
    let response = try await client.status.getStatus(...)
...
```swift
try await client.status.getStatus(..., requestOptions: .init(
    additionalHeaders: [
