...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as BasicAuthEnvironmentVariablesError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected BasicAuthEnvironmentVariablesError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected BasicAuthEnvironmentVariablesError, got \(error)")
        }
    }
