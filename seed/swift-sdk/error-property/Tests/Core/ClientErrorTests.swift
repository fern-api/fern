...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
            _ = try await client.propertyBasedError.throwError(requestOptions: RequestOptions(additionalHeaders: stub.headers))

            Issue.record("Expected error to be thrown")
        } catch let error as ErrorPropertyError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected ErrorPropertyError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected ErrorPropertyError, got \(error)")
        }
    }
