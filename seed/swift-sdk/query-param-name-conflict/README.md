...
```swift
dependencies: [
    .package(url: "https://github.com/query-param-name-conflict/fern", from: "0.0.1"),
]
```
...
do {
    let response = try await client.bulkUpdateTasks(...)
...
let request = Requests.BulkUpdateTasksRequest(
    ...
)
```
...
```swift
try await client.bulkUpdateTasks(..., requestOptions: .init(
    additionalHeaders: [
