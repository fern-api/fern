...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
            _ = try await client.getUnion(requestOptions: RequestOptions(additionalHeaders: stub.headers))

            Issue.record("Expected error to be thrown")
        } catch let error as UndiscriminatedUnionWithResponsePropertyError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected UndiscriminatedUnionWithResponsePropertyError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected UndiscriminatedUnionWithResponsePropertyError, got \(error)")
        }
    }
