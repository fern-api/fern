...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as ServerSentEventsError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected ServerSentEventsError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected ServerSentEventsError, got \(error)")
        }
    }
