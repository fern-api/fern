...
let client = SingleUrlEnvironmentDefaultClient(token: "YOUR_API_KEY")

do {
    let response = try await client.dummy.getDummy(...)
    // Handle successful response
} catch let error as SingleUrlEnvironmentDefaultError {
    switch error {
...
```swift
try await client.dummy.getDummy(..., requestOptions: .init(
    additionalHeaders: [
