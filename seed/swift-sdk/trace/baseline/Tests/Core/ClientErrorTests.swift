...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
            _ = try await client.v2.test(requestOptions: RequestOptions(additionalHeaders: stub.headers))

            Issue.record("Expected error to be thrown")
        } catch let error as TraceError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected TraceError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected TraceError, got \(error)")
        }
    }
