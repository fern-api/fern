...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as OauthClientCredentialsMandatoryAuthError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected OauthClientCredentialsMandatoryAuthError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected OauthClientCredentialsMandatoryAuthError, got \(error)")
        }
    }
