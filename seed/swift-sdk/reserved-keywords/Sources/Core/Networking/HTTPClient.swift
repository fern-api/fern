...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw NurseryApiError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw NurseryApiError.decodingError(error)
        }
    }
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: NurseryApiError?
                ...
                } else if let existingClientError = error as? NurseryApiError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw NurseryApiError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> NurseryApiError
    {
        ...
        return NurseryApiError.httpError(httpError)
    }
}
