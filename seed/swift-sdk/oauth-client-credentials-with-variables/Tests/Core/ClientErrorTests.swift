...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as OauthClientCredentialsWithVariablesError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected OauthClientCredentialsWithVariablesError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected OauthClientCredentialsWithVariablesError, got \(error)")
        }
    }
