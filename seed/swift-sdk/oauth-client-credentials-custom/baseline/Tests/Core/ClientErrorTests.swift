...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as OauthClientCredentialsError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected OauthClientCredentialsError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected OauthClientCredentialsError, got \(error)")
        }
    }
