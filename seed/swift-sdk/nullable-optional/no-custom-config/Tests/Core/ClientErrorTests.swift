...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as NullableOptionalError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected NullableOptionalError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected NullableOptionalError, got \(error)")
        }
    }
