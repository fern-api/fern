...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw IdempotencyHeadersError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw IdempotencyHeadersError.decodingError(error)
        }
    }
    ...
    private static func validateHTTPS(url: Foundation.URL) throws {
        ...
        throw IdempotencyHeadersError.networkError(
            NSError(
                domain: "HTTPClient",
                code: -1,
                userInfo: [
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: IdempotencyHeadersError?
                ...
                } else if let existingClientError = error as? IdempotencyHeadersError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw IdempotencyHeadersError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> IdempotencyHeadersError
    {
        ...
        return IdempotencyHeadersError.httpError(httpError)
    }
}
