...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw HeaderTokenError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw HeaderTokenError.decodingError(error)
        }
    }
    ...
    private static func validateHTTPS(url: Foundation.URL) throws {
        ...
        throw HeaderTokenError.networkError(
            NSError(
                domain: "HTTPClient",
                code: -1,
                userInfo: [
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: HeaderTokenError?
                ...
                } else if let existingClientError = error as? HeaderTokenError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw HeaderTokenError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> HeaderTokenError
    {
        ...
        return HeaderTokenError.httpError(httpError)
    }
}
