...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw BasicAuthError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw BasicAuthError.decodingError(error)
        }
    }
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: BasicAuthError?
                ...
                } else if let existingClientError = error as? BasicAuthError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw BasicAuthError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> BasicAuthError
    {
        ...
        return BasicAuthError.httpError(httpError)
    }
}
