...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
            _ = try await client.service.endpoint(requestOptions: RequestOptions(additionalHeaders: stub.headers))

            Issue.record("Expected error to be thrown")
        } catch let error as AcceptError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected AcceptError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected AcceptError, got \(error)")
        }
    }
