...
```swift
dependencies: [
    .package(url: "https://github.com/pagination-uri-path/fern", from: "0.0.1"),
]
```
...
let client = PaginationUriPathClient(token: "YOUR_API_KEY")

do {
    let response = try await client.users.listWithUriPagination(...)
...
```swift
try await client.users.listWithUriPagination(..., requestOptions: .init(
    additionalHeaders: [
