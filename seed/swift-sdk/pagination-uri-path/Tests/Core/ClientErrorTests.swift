...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as PaginationUriPathError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected PaginationUriPathError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected PaginationUriPathError, got \(error)")
        }
    }
