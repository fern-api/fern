...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as InferredAuthImplicitNoExpiryError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected InferredAuthImplicitNoExpiryError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected InferredAuthImplicitNoExpiryError, got \(error)")
        }
    }
