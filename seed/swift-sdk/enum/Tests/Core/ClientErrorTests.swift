...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
            _ = try await client.headers.send(requestOptions: RequestOptions(additionalHeaders: stub.headers))

            Issue.record("Expected error to be thrown")
        } catch let error as EnumError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected EnumError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected EnumError, got \(error)")
        }
    }
