...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw CrossPackageTypeNamesError.invalidResponse
            }
        }
        ...
        do {
            ...
            throw CrossPackageTypeNamesError.decodingError(error)
        }
    }
    ...
    private static func validateHTTPS(url: Foundation.URL) throws {
        ...
        throw CrossPackageTypeNamesError.networkError(
            NSError(
                domain: "HTTPClient",
                code: -1,
                userInfo: [
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                let clientError: CrossPackageTypeNamesError?
                ...
                } else if let existingClientError = error as? CrossPackageTypeNamesError {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw CrossPackageTypeNamesError.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> CrossPackageTypeNamesError
    {
        ...
        return CrossPackageTypeNamesError.httpError(httpError)
    }
}
