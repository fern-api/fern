...
@Suite("Client Error & HTTP Error Tests") struct ClientErrorTests {
    ...
    @Test func testClientErrorFor400BadRequest() async throws {
        ...
        do {
        ...
        } catch let error as UnionQueryParametersError {
            guard case .httpError(let httpError) = error else {
                Issue.record("Expected UnionQueryParametersError.httpError, got \(error)")
                return
            }
            ...
            Issue.record("Expected UnionQueryParametersError, got \(error)")
        }
    }
