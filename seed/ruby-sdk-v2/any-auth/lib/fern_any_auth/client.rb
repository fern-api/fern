...
module FernAnyAuth
  class Client
    ...
    def initialize(client_id:, client_secret:, base_url: nil, token: ENV.fetch("MY_TOKEN", nil), api_key: ENV.fetch("MY_API_KEY", nil))
      ...
      auth_raw_client = FernAnyAuth::Internal::Http::RawClient.new(
        base_url: base_url,
        headers: {
      ...
      auth_client = FernAnyAuth::Auth::Client.new(client: auth_raw_client)

      # Create the auth provider with the auth client and credentials
      @auth_provider = FernAnyAuth::Internal::InferredAuthProvider.new(
        auth_client: auth_client,
        options: { base_url: base_url, client_id: client_id, client_secret: client_secret }
      )

      @raw_client = FernAnyAuth::Internal::Http::RawClient.new(
        base_url: base_url,
        headers: {
    ...
    # @return [FernAnyAuth::Auth::Client]
    def auth
      @auth ||= FernAnyAuth::Auth::Client.new(client: @raw_client)
    end

    # @return [FernAnyAuth::User::Client]
    def user
      @user ||= FernAnyAuth::User::Client.new(client: @raw_client)
    end
  end
end
