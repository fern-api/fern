  ...
  module Playlist
    class Client
      ...
      def get_playlists(request_options: {}, **params)
        ...
        code = response.code.to_i
        if code.between?(200, 299)
          Seed::Internal::Types::Utils.coerce(Internal::Types::Array[Seed::Playlist::Types::Playlist], JSON.parse(response.body, symbolize_names: true))
        else
          error_class = Seed::Errors::ResponseError.subclass_for_code(code)
          raise error_class.new(response.body, code: code)
        end
      ...
      def update_playlist(request_options: {}, **params)
        ...
        code = response.code.to_i
        if code.between?(200, 299)
          Seed::Internal::Types::Utils.coerce(Seed::Playlist::Types::Playlist, JSON.parse(response.body, symbolize_names: true))
        else
          error_class = Seed::Errors::ResponseError.subclass_for_code(code)
          raise error_class.new(response.body, code: code)
        end
