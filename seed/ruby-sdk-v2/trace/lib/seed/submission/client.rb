  ...
  module Submission
    class Client
      ...
      def get_execution_session(request_options: {}, **params)
        ...
        code = response.code.to_i
        if code.between?(200, 299)
          Seed::Internal::Types::Utils.coerce(Seed::Submission::Types::ExecutionSessionResponse, (response.body.to_s.empty? ? nil : JSON.parse(response.body, symbolize_names: true)))
        else
          error_class = Seed::Errors::ResponseError.subclass_for_code(code)
          raise error_class.new(response.body, code: code)
        end
