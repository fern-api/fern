...
module FernWebsocketInferredAuth
  class Client
    ...
    def initialize(client_id:, client_secret:, x_api_key:, base_url: nil, scope: nil)
      ...
      auth_raw_client = FernWebsocketInferredAuth::Internal::Http::RawClient.new(
        base_url: base_url,
        headers: {
      ...
      auth_client = FernWebsocketInferredAuth::Auth::Client.new(client: auth_raw_client)

      # Create the auth provider with the auth client and credentials
      @auth_provider = FernWebsocketInferredAuth::Internal::InferredAuthProvider.new(
        auth_client: auth_client,
        options: { base_url: base_url, client_id: client_id, client_secret: client_secret, scope: scope, x_api_key: x_api_key }
      )

      @raw_client = FernWebsocketInferredAuth::Internal::Http::RawClient.new(
        base_url: base_url,
        headers: {
    ...
    # @return [FernWebsocketInferredAuth::Auth::Client]
    def auth
      @auth ||= FernWebsocketInferredAuth::Auth::Client.new(client: @raw_client)
    end
  end
end
