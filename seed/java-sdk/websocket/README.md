...
[![Maven Central](https://img.shields.io/maven-central/v/com.fern/websocket)](https://central.sonatype.com/artifact/com.fern/websocket)
...
```groovy
dependencies {
  implementation 'com.fern:websocket:0.0.1'
}
```
...
public class Example {
    public static void main(String[] args) {
        SeedWebsocketClient client = SeedWebsocketClient
            .builder()
            .build();
...
try{
    client.status().getStatus(...);
} catch (SeedWebsocketApiException e){
...
var ws = client.empty().emptyRealtime().emptyRealtimeWebSocket();
ws.connect(EmptyRealtimeConnectOptions.builder().build()).join();
...
```java
SeedWebsocketHttpResponse response = client.status().withRawResponse().getStatus(...);
