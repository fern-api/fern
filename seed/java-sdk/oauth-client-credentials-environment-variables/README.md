...
```groovy
dependencies {
  implementation 'com.fern:oauth-client-credentials-environment-variables:0.0.1'
}
```
...
```java
SeedOauthClientCredentialsEnvironmentVariablesClient client = SeedOauthClientCredentialsEnvironmentVariablesClient.builder()
...
```java
SeedOauthClientCredentialsEnvironmentVariablesHttpResponse response = client.auth().withRawResponse().getTokenWithClientCredentials(...);
