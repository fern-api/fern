...
[![Maven Central](https://img.shields.io/maven-central/v/com.fern/java-endpoint-security-token-subpackage)](https://central.sonatype.com/artifact/com.fern/java-endpoint-security-token-subpackage)
...
```groovy
dependencies {
  implementation 'com.fern:java-endpoint-security-token-subpackage:0.0.1'
}
```
...
<dependency>
  ...
  <artifactId>java-endpoint-security-token-subpackage</artifactId>
...
try{
    client.token().getToken(...);
} catch (SeedJavaEndpointSecurityTokenSubpackageApiException e){
...
```java
SeedJavaEndpointSecurityTokenSubpackageHttpResponse response = client.token().withRawResponse().getToken(...);
