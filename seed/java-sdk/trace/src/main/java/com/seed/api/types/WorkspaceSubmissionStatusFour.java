/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.seed.api.types;

import com.fasterxml.jackson.annotation.JsonAnyGetter;
import com.fasterxml.jackson.annotation.JsonAnySetter;
import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonSetter;
import com.fasterxml.jackson.annotation.Nulls;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.seed.api.core.ObjectMappers;
import java.util.HashMap;
import java.util.Map;
import java.util.Objects;
import java.util.Optional;
import org.jetbrains.annotations.NotNull;

@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = WorkspaceSubmissionStatusFour.Builder.class)
public final class WorkspaceSubmissionStatusFour implements IWorkspaceRunDetails {
    private final Optional<ExceptionV2> exceptionV2;

    private final Optional<ExceptionInfo> exception;

    private final String stdout;

    private final WorkspaceSubmissionStatusFourType type;

    private final Map<String, Object> additionalProperties;

    private WorkspaceSubmissionStatusFour(
            Optional<ExceptionV2> exceptionV2,
            Optional<ExceptionInfo> exception,
            String stdout,
            WorkspaceSubmissionStatusFourType type,
            Map<String, Object> additionalProperties) {
        this.exceptionV2 = exceptionV2;
        this.exception = exception;
        this.stdout = stdout;
        this.type = type;
        this.additionalProperties = additionalProperties;
    }

    @JsonProperty("exceptionV2")
    @java.lang.Override
    public Optional<ExceptionV2> getExceptionV2() {
        return exceptionV2;
    }

    @JsonProperty("exception")
    @java.lang.Override
    public Optional<ExceptionInfo> getException() {
        return exception;
    }

    @JsonProperty("stdout")
    @java.lang.Override
    public String getStdout() {
        return stdout;
    }

    @JsonProperty("type")
    public WorkspaceSubmissionStatusFourType getType() {
        return type;
    }

    @java.lang.Override
    public boolean equals(Object other) {
        if (this == other) return true;
        return other instanceof WorkspaceSubmissionStatusFour && equalTo((WorkspaceSubmissionStatusFour) other);
    }

    @JsonAnyGetter
    public Map<String, Object> getAdditionalProperties() {
        return this.additionalProperties;
    }

    private boolean equalTo(WorkspaceSubmissionStatusFour other) {
        return exceptionV2.equals(other.exceptionV2)
                && exception.equals(other.exception)
                && stdout.equals(other.stdout)
                && type.equals(other.type);
    }

    @java.lang.Override
    public int hashCode() {
        return Objects.hash(this.exceptionV2, this.exception, this.stdout, this.type);
    }

    @java.lang.Override
    public String toString() {
        return ObjectMappers.stringify(this);
    }

    public static StdoutStage builder() {
        return new Builder();
    }

    public interface StdoutStage {
        TypeStage stdout(@NotNull String stdout);

        Builder from(WorkspaceSubmissionStatusFour other);
    }

    public interface TypeStage {
        _FinalStage type(@NotNull WorkspaceSubmissionStatusFourType type);
    }

    public interface _FinalStage {
        WorkspaceSubmissionStatusFour build();

        _FinalStage additionalProperty(String key, Object value);

        _FinalStage additionalProperties(Map<String, Object> additionalProperties);

        _FinalStage exceptionV2(Optional<ExceptionV2> exceptionV2);

        _FinalStage exceptionV2(ExceptionV2 exceptionV2);

        _FinalStage exception(Optional<ExceptionInfo> exception);

        _FinalStage exception(ExceptionInfo exception);
    }

    @JsonIgnoreProperties(ignoreUnknown = true)
    public static final class Builder implements StdoutStage, TypeStage, _FinalStage {
        private String stdout;

        private WorkspaceSubmissionStatusFourType type;

        private Optional<ExceptionInfo> exception = Optional.empty();

        private Optional<ExceptionV2> exceptionV2 = Optional.empty();

        @JsonAnySetter
        private Map<String, Object> additionalProperties = new HashMap<>();

        private Builder() {}

        @java.lang.Override
        public Builder from(WorkspaceSubmissionStatusFour other) {
            exceptionV2(other.getExceptionV2());
            exception(other.getException());
            stdout(other.getStdout());
            type(other.getType());
            return this;
        }

        @java.lang.Override
        @JsonSetter("stdout")
        public TypeStage stdout(@NotNull String stdout) {
            this.stdout = Objects.requireNonNull(stdout, "stdout must not be null");
            return this;
        }

        @java.lang.Override
        @JsonSetter("type")
        public _FinalStage type(@NotNull WorkspaceSubmissionStatusFourType type) {
            this.type = Objects.requireNonNull(type, "type must not be null");
            return this;
        }

        @java.lang.Override
        public _FinalStage exception(ExceptionInfo exception) {
            this.exception = Optional.ofNullable(exception);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "exception", nulls = Nulls.SKIP)
        public _FinalStage exception(Optional<ExceptionInfo> exception) {
            this.exception = exception;
            return this;
        }

        @java.lang.Override
        public _FinalStage exceptionV2(ExceptionV2 exceptionV2) {
            this.exceptionV2 = Optional.ofNullable(exceptionV2);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "exceptionV2", nulls = Nulls.SKIP)
        public _FinalStage exceptionV2(Optional<ExceptionV2> exceptionV2) {
            this.exceptionV2 = exceptionV2;
            return this;
        }

        @java.lang.Override
        public WorkspaceSubmissionStatusFour build() {
            return new WorkspaceSubmissionStatusFour(exceptionV2, exception, stdout, type, additionalProperties);
        }

        @java.lang.Override
        public Builder additionalProperty(String key, Object value) {
            this.additionalProperties.put(key, value);
            return this;
        }

        @java.lang.Override
        public Builder additionalProperties(Map<String, Object> additionalProperties) {
            this.additionalProperties.putAll(additionalProperties);
            return this;
        }
    }
}
