...
public final class UnionWithTime {
    ...
    public static UnionWithTime value(UnionWithTimeValue value) {
        return new UnionWithTime(new ValueValue(value));
    }

    public static UnionWithTime date(UnionWithTimeDate value) {
        return new UnionWithTime(new DateValue(value));
    }

    public static UnionWithTime datetime(UnionWithTimeDatetime value) {
    ...
    public Optional<UnionWithTimeValue> getValue() {
        if (isValue()) {
    ...
    public Optional<UnionWithTimeDate> getDate() {
        if (isDate()) {
    ...
    public Optional<UnionWithTimeDatetime> getDatetime() {
        if (isDatetime()) {
    ...
    public boolean equals(Object other) {
        ...
        return other instanceof UnionWithTime && value.equals(((UnionWithTime) other).value);
    }
    ...
    public interface Visitor<T> {
        T visitValue(UnionWithTimeValue value);

        T visitDate(UnionWithTimeDate date);

        T visitDatetime(UnionWithTimeDatetime datetime);
    ...
    private static final class ValueValue implements Value {
        ...
        private UnionWithTimeValue value;

        @JsonCreator(mode = JsonCreator.Mode.PROPERTIES)
        private ValueValue() {}

        private ValueValue(UnionWithTimeValue value) {
            this.value = value;
        }
    ...
    private static final class DateValue implements Value {
        @JsonUnwrapped
        @JsonIgnoreProperties(value = "type", allowSetters = true)
        private UnionWithTimeDate value;

        @JsonCreator(mode = JsonCreator.Mode.PROPERTIES)
        private DateValue() {}

        private DateValue(UnionWithTimeDate value) {
            this.value = value;
        }
    ...
    private static final class DatetimeValue implements Value {
        @JsonUnwrapped
        @JsonIgnoreProperties(value = "type", allowSetters = true)
        private UnionWithTimeDatetime value;

        @JsonCreator(mode = JsonCreator.Mode.PROPERTIES)
        private DatetimeValue() {}

        private DatetimeValue(UnionWithTimeDatetime value) {
            this.value = value;
        }
