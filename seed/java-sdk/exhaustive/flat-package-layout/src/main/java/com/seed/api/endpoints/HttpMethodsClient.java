/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.seed.api.endpoints;

import com.seed.api.core.ClientOptions;
import com.seed.api.core.RequestOptions;
import com.seed.api.endpoints.types.TestDeleteHttpMethodsRequest;
import com.seed.api.endpoints.types.TestGetHttpMethodsRequest;
import com.seed.api.endpoints.types.TestPatchHttpMethodsRequest;
import com.seed.api.endpoints.types.TestPutHttpMethodsRequest;
import com.seed.api.types.TypesObjectWithOptionalField;
import com.seed.api.types.TypesObjectWithRequiredField;

public class HttpMethodsClient {
    protected final ClientOptions clientOptions;

    private final RawHttpMethodsClient rawClient;

    public HttpMethodsClient(ClientOptions clientOptions) {
        this.clientOptions = clientOptions;
        this.rawClient = new RawHttpMethodsClient(clientOptions);
    }

    /**
     * Get responses with HTTP metadata like headers
     */
    public RawHttpMethodsClient withRawResponse() {
        return this.rawClient;
    }

    public String testGet(String id) {
        return this.rawClient.testGet(id).body();
    }

    public String testGet(String id, RequestOptions requestOptions) {
        return this.rawClient.testGet(id, requestOptions).body();
    }

    public String testGet(String id, TestGetHttpMethodsRequest request) {
        return this.rawClient.testGet(id, request).body();
    }

    public String testGet(String id, TestGetHttpMethodsRequest request, RequestOptions requestOptions) {
        return this.rawClient.testGet(id, request, requestOptions).body();
    }

    public TypesObjectWithOptionalField testPut(String id, TestPutHttpMethodsRequest request) {
        return this.rawClient.testPut(id, request).body();
    }

    public TypesObjectWithOptionalField testPut(
            String id, TestPutHttpMethodsRequest request, RequestOptions requestOptions) {
        return this.rawClient.testPut(id, request, requestOptions).body();
    }

    public boolean testDelete(String id) {
        return this.rawClient.testDelete(id).body();
    }

    public boolean testDelete(String id, RequestOptions requestOptions) {
        return this.rawClient.testDelete(id, requestOptions).body();
    }

    public boolean testDelete(String id, TestDeleteHttpMethodsRequest request) {
        return this.rawClient.testDelete(id, request).body();
    }

    public boolean testDelete(String id, TestDeleteHttpMethodsRequest request, RequestOptions requestOptions) {
        return this.rawClient.testDelete(id, request, requestOptions).body();
    }

    public TypesObjectWithOptionalField testPatch(String id, TestPatchHttpMethodsRequest request) {
        return this.rawClient.testPatch(id, request).body();
    }

    public TypesObjectWithOptionalField testPatch(
            String id, TestPatchHttpMethodsRequest request, RequestOptions requestOptions) {
        return this.rawClient.testPatch(id, request, requestOptions).body();
    }

    public TypesObjectWithOptionalField testPost(TypesObjectWithRequiredField request) {
        return this.rawClient.testPost(request).body();
    }

    public TypesObjectWithOptionalField testPost(TypesObjectWithRequiredField request, RequestOptions requestOptions) {
        return this.rawClient.testPost(request, requestOptions).body();
    }
}
