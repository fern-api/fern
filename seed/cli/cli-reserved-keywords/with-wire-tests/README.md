# Reserved Keyword CLI CLI

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Reserved%20Keyword%20CLI%2FCLI)

Command-line interface for the Reserved Keyword CLI API.
...
```bash
cargo build --release
./target/release/reserved-keyword-cli --help
```

## Authentication

This API requires authentication. Run `reserved-keyword-cli --help` for details.
...
```bash
reserved-keyword-cli --help
```

Call an API endpoint:

```bash
reserved-keyword-cli <resource> <method>
```

Run `reserved-keyword-cli <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `reserved-keyword-cli <resource> <method>`). Run `reserved-keyword-cli <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
reserved-keyword-cli <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|-------------|
| `RESERVED_KEYWORD_CLI_BASE_URL` | Override the API base URL | yes |
| `RESERVED_KEYWORD_CLI_OUTPUT` | Default output format when `--format` is not passed | yes |
| `RESERVED_KEYWORD_CLI_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `RESERVED_KEYWORD_CLI_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `RESERVED_KEYWORD_CLI_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `RESERVED_KEYWORD_CLI_PROXY` | HTTP(S) proxy URL | no |
| `RESERVED_KEYWORD_CLI_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `RESERVED_KEYWORD_CLI_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `reserved-keyword-cli profiles set <profile> RESERVED_KEYWORD_CLI_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
reserved-keyword-cli <resource> <method> --format json | jq

# Keep the human rendering even when piped
reserved-keyword-cli <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
reserved-keyword-cli --help --format json | jq '.operations | length'
```
...
```bash
reserved-keyword-cli completion <bash|zsh|fish|powershell>
```
