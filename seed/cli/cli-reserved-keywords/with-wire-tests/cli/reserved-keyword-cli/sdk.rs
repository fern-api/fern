...
impl reserved_keyword_cli_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> reserved_keyword_cli_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = reserved_keyword_cli_sdk::ClientConfig::default();
    let http_client = reserved_keyword_cli_sdk::HttpClient::with_executor(
        adapter as Arc<dyn reserved_keyword_cli_sdk::RequestExecutor>,
        config.clone(),
    );
    reserved_keyword_cli_sdk::api::ApiClient {
        config,
        models: reserved_keyword_cli_sdk::api::ModelsClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, reserved_keyword_cli_sdk::ApiError>>,
{
...
fn convert_api_error(e: reserved_keyword_cli_sdk::ApiError) -> CliError {
    match e {
        reserved_keyword_cli_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        reserved_keyword_cli_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        reserved_keyword_cli_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
