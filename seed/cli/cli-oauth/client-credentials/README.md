...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=OAuth%20Test%20API%2FCLI)

Command-line interface for the OAuth Test API.
...
```bash
cargo build --release
./target/release/oauth-test --help
```
...
```bash
oauth-test <resource> <method>
```

Run `oauth-test <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `oauth-test <resource> <method>`). Run `oauth-test <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
oauth-test <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|-------------|
| `OAUTH_TEST_BASE_URL` | Override the API base URL | yes |
| `OAUTH_TEST_OUTPUT` | Default output format when `--format` is not passed | yes |
| `OAUTH_TEST_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `OAUTH_TEST_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `OAUTH_TEST_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `OAUTH_TEST_PROXY` | HTTP(S) proxy URL | no |
| `OAUTH_TEST_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `OAUTH_TEST_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `oauth-test profiles set <profile> OAUTH_TEST_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
oauth-test <resource> <method> --format json | jq

# Keep the human rendering even when piped
oauth-test <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
oauth-test --help --format json | jq '.operations | length'
```
...
```bash
oauth-test completion <bash|zsh|fish|powershell>
```
