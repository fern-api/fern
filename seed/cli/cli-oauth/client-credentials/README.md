...
|----------|-------------|
| `OAUTH_TEST_BASE_URL` | Override the API base URL |
| `OAUTH_TEST_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) |
| `OAUTH_TEST_INSECURE=1` | Skip TLS verification (debugging only) |
| `OAUTH_TEST_PROXY` | HTTP(S) proxy URL |
| `OAUTH_TEST_TIMEOUT_SECS` | Total request timeout in seconds |
