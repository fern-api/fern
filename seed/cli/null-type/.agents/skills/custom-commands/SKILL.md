...
The `null-type` CLI supports user-authored custom commands compiled
...
cli/null-type/sdk.rs       ← Generated bridge: client() + block_on()
...
fn handle_fetch(args: FetchArgs, ctx: &AppContext) -> Result<(), CliError> {
    ...
    let result = super::sdk::block_on(
        client.conversations.get(&args.id),
    )?;
...
| Field | Type |
|-------|------|
| `client.conversations` | `null_type_sdk::api::ConversationsClient` |
| `client.users` | `null_type_sdk::api::UsersClient` |
...
| `cli/null-type/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
null-type hello world              # top-level custom command
null-type admin users list --limit 5  # nested under custom namespace
null-type fetch my-id --format json   # output cohesion
null-type fetch my-id --dry-run       # dry-run preview
```
