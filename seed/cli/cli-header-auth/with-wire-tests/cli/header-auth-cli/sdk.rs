...
impl header_auth_cli_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> header_auth_cli_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = header_auth_cli_sdk::ClientConfig::default();
    let http_client = header_auth_cli_sdk::HttpClient::with_executor(
        adapter as Arc<dyn header_auth_cli_sdk::RequestExecutor>,
        config.clone(),
    );
    header_auth_cli_sdk::api::ApiClient {
        config,
        widgets: header_auth_cli_sdk::api::WidgetsClient { http_client: http_client.clone() },
        system: header_auth_cli_sdk::api::SystemClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, header_auth_cli_sdk::ApiError>>,
{
...
fn convert_api_error(e: header_auth_cli_sdk::ApiError) -> CliError {
    match e {
        header_auth_cli_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        header_auth_cli_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        header_auth_cli_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
