...
The `webhook-audience-test` CLI supports user-authored custom commands compiled
...
cli/webhook-audience-test/sdk.rs       ← Generated bridge: client() + block_on()
...
| `cli/webhook-audience-test/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
webhook-audience-test hello world              # top-level custom command
webhook-audience-test admin users list --limit 5  # nested under custom namespace
webhook-audience-test fetch my-id --format json   # output cohesion
webhook-audience-test fetch my-id --dry-run       # dry-run preview
```
