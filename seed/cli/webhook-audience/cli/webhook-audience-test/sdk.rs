...
impl webhook_audience_test_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> Result<webhook_audience_test_sdk::api::ApiClient, CliError> {
    let executor = ctx.build_sdk_executor()?;
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = webhook_audience_test_sdk::ClientConfig::default();
    let http_client = webhook_audience_test_sdk::HttpClient::with_executor(
        adapter as Arc<dyn webhook_audience_test_sdk::RequestExecutor>,
        config.clone(),
    );
    Ok(webhook_audience_test_sdk::api::ApiClient {
        config,
...
where
    F: Future<Output = Result<T, webhook_audience_test_sdk::ApiError>>,
{
...
fn convert_api_error(e: webhook_audience_test_sdk::ApiError) -> CliError {
    match e {
        webhook_audience_test_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        webhook_audience_test_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        webhook_audience_test_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
