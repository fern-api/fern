...
async fn run_case(id: &str) {
    ...
    if let Some(setup) = &manifest.login_token_setup {
        ...
        let home = &auth_home;
        let mut login = tokio::process::Command::new(env!("CARGO_BIN_EXE_login-flow"));
        login
    ...
    let mut cmd = tokio::process::Command::new(env!("CARGO_BIN_EXE_login-flow"));
    cmd.args(&args);
...
async fn wire_exchange() {
    run_case("exchange").await;
}

#[tokio::test]
async fn wire_health() {
    run_case("health").await;
}
...
#[tokio::test]
async fn wire_exchange_error() {
    run_case("exchange_error").await;
}

#[tokio::test]
async fn wire_health_error() {
    run_case("health_error").await;
}
