...
impl login_flow_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> login_flow_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = login_flow_sdk::ClientConfig::default();
    let http_client = login_flow_sdk::HttpClient::with_executor(
        adapter as Arc<dyn login_flow_sdk::RequestExecutor>,
        config.clone(),
    );
    login_flow_sdk::api::ApiClient {
        config,
        widgets: login_flow_sdk::api::WidgetsClient { http_client: http_client.clone() },
        tokens: login_flow_sdk::api::TokensClient { http_client: http_client.clone() },
        system: login_flow_sdk::api::SystemClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, login_flow_sdk::ApiError>>,
{
...
fn convert_api_error(e: login_flow_sdk::ApiError) -> CliError {
    match e {
        login_flow_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        login_flow_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        login_flow_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
