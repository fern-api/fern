...
impl acme_versioned_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> Result<acme_versioned_sdk::api::ApiClient, CliError> {
    let executor = ctx.build_sdk_executor()?;
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = acme_versioned_sdk::ClientConfig::default();
    let http_client = acme_versioned_sdk::HttpClient::with_executor(
        adapter as Arc<dyn acme_versioned_sdk::RequestExecutor>,
        config.clone(),
    );
    Ok(acme_versioned_sdk::api::ApiClient {
        config,
        v1: acme_versioned_sdk::api::V1Client { http_client: http_client.clone() },
        v2: acme_versioned_sdk::api::V2Client { http_client: http_client.clone() },
    })
}
...
where
    F: Future<Output = Result<T, acme_versioned_sdk::ApiError>>,
{
...
fn convert_api_error(e: acme_versioned_sdk::ApiError) -> CliError {
    match e {
        acme_versioned_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        acme_versioned_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        acme_versioned_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
