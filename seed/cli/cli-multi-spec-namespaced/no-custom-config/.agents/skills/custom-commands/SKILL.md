...
The `acme-versioned` CLI supports user-authored custom commands compiled
...
| Field | Type |
|-------|------|
| `client.v1` | `acme_versioned_sdk::api::V1Client` |
| `client.v2` | `acme_versioned_sdk::api::V2Client` |
...
```bash
cargo build
acme-versioned hello world              # top-level custom command
acme-versioned admin users list --limit 5  # nested under custom namespace
acme-versioned fetch my-id --format json   # output cohesion
acme-versioned fetch my-id --dry-run       # dry-run preview
```
