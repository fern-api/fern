...
/// Resolve the consumer-supplied `User-Agent` suffix from the
/// `--user-agent-suffix` flag. Returns the flag value if present, else
/// `None` — in which case [`crate::http::HttpConfig`] falls back to the
/// `<NAME>_USER_AGENT_SUFFIX` env var. Keeping the env fallback in
/// `HttpConfig` means the flag simply takes precedence when both are set.
