...
async fn run_case(id: &str) {
    ...
    if let Some(setup) = &manifest.login_token_setup {
        ...
        let home = &auth_home;
        let mut login = tokio::process::Command::new(env!("CARGO_BIN_EXE_acme-versioned"));
        login
    ...
    let mut cmd = tokio::process::Command::new(env!("CARGO_BIN_EXE_acme-versioned"));
    cmd.args(&args);
...
#[tokio::test]
async fn wire_listusers() {
    run_case("listusers").await;
}

#[tokio::test]
async fn wire_listaccounts() {
    run_case("listaccounts").await;
}

#[tokio::test]
async fn wire_listusers_error() {
    run_case("listusers_error").await;
}

#[tokio::test]
async fn wire_listaccounts_error() {
    run_case("listaccounts_error").await;
}
