...
The `x-fern-default-test` CLI supports user-authored custom commands compiled
...
```bash
cargo build
x-fern-default-test hello world              # top-level custom command
x-fern-default-test admin users list --limit 5  # nested under custom namespace
x-fern-default-test fetch my-id --format json   # output cohesion
x-fern-default-test fetch my-id --dry-run       # dry-run preview
```
