...
impl x_fern_default_test_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> x_fern_default_test_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = x_fern_default_test_sdk::ClientConfig::default();
    let http_client = x_fern_default_test_sdk::HttpClient::with_executor(
        adapter as Arc<dyn x_fern_default_test_sdk::RequestExecutor>,
        config.clone(),
    );
    x_fern_default_test_sdk::api::ApiClient {
        config,
        http_client: x_fern_default_test_sdk::api::HttpClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, x_fern_default_test_sdk::ApiError>>,
{
...
fn convert_api_error(e: x_fern_default_test_sdk::ApiError) -> CliError {
    match e {
        x_fern_default_test_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        x_fern_default_test_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        x_fern_default_test_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
