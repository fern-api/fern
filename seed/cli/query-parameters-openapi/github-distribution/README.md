...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=https%3A%2F%2Fgithub.com%2Fquery-parameters-openapi%2Ffern)
...
```bash
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/query-parameters-openapi/fern/releases/latest/download/fern-cli-sdk-installer.sh | sh
```

### PowerShell (Windows)

```powershell
powershell -ExecutionPolicy ByPass -c "irm https://github.com/query-parameters-openapi/fern/releases/latest/download/fern-cli-sdk-installer.ps1 | iex"
```

### Homebrew (macOS / Linux)

```bash
brew install acme/tap/query-parameters-api
```
...
scoop install query-parameters-api
```
...
|----------|-------------|-------------|
| `QUERY_PARAMETERS_API_BASE_URL` | Override the API base URL | yes |
| `QUERY_PARAMETERS_API_OUTPUT` | Default output format when `--format` is not passed | yes |
| `QUERY_PARAMETERS_API_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `QUERY_PARAMETERS_API_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `QUERY_PARAMETERS_API_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `QUERY_PARAMETERS_API_PROXY` | HTTP(S) proxy URL | no |
| `QUERY_PARAMETERS_API_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `QUERY_PARAMETERS_API_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `query-parameters-api profiles set <profile> QUERY_PARAMETERS_API_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
query-parameters-api <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
query-parameters-api --help --format json | jq '.operations | length'
```
