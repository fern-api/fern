...
The `query-parameters-api` CLI supports user-authored custom commands compiled
...
cli/query-parameters-api/sdk.rs       ← Generated bridge: client() + block_on()
...
| `cli/query-parameters-api/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
query-parameters-api hello world              # top-level custom command
query-parameters-api admin users list --limit 5  # nested under custom namespace
query-parameters-api fetch my-id --format json   # output cohesion
query-parameters-api fetch my-id --dry-run       # dry-run preview
```
