...
impl oauth_api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> oauth_api_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = oauth_api_sdk::ClientConfig::default();
    let http_client = oauth_api_sdk::HttpClient::with_executor(
        adapter as Arc<dyn oauth_api_sdk::RequestExecutor>,
        config.clone(),
    );
    oauth_api_sdk::api::ApiClient {
        config,
        http_client: http_client.clone(),
        oauth: oauth_api_sdk::api::OauthClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, oauth_api_sdk::ApiError>>,
{
...
fn convert_api_error(e: oauth_api_sdk::ApiError) -> CliError {
    match e {
        oauth_api_sdk::ApiError::Http { status, message } => {
            fern_cli_sdk::error::api_error_from_body(status, &message)
        }
        oauth_api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        oauth_api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
