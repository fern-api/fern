...
impl api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> api_sdk::api::ApiClient {
    ...
    let config = api_sdk::ClientConfig {
    ...
    api_sdk::api::ApiClient {
        config,
        file_upload_example: api_sdk::api::FileUploadExampleClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, api_sdk::ApiError>>,
{
...
fn convert_api_error(e: api_sdk::ApiError) -> CliError {
    match e {
        api_sdk::ApiError::Http { status, message } => {
            fern_cli_sdk::error::api_error_from_body(status, &message)
        }
        api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
