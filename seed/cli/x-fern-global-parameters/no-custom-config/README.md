...
```bash
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/<org>/<repo>/releases/latest/download/test-x-fern-global-parameters-extension-installer.sh | sh
```

### PowerShell (Windows)

```powershell
powershell -ExecutionPolicy ByPass -c "irm https://github.com/<org>/<repo>/releases/latest/download/test-x-fern-global-parameters-extension-installer.ps1 | iex"
```
...
```bash
cargo build --release
./target/release/test-x-fern-global-parameters-extension --help
```

## Authentication

This API requires authentication. Run `test-x-fern-global-parameters-extension --help` for details.
...
```bash
test-x-fern-global-parameters-extension --help
```

Call an API endpoint:

```bash
test-x-fern-global-parameters-extension <resource> <method>
```

Run `test-x-fern-global-parameters-extension <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `test-x-fern-global-parameters-extension <resource> <method>`). Run `test-x-fern-global-parameters-extension <resource> --help` to see available methods.
...
test-x-fern-global-parameters-extension <resource> <method> --format json | jq

# Machine-readable catalog of every operation
test-x-fern-global-parameters-extension --help --format json | jq 'length'
```
...
```bash
test-x-fern-global-parameters-extension completion <bash|zsh|fish|powershell>
```
