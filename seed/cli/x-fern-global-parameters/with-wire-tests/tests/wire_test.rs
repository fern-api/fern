...
async fn run_case(id: &str) {
    ...
    if let Some(setup) = &manifest.login_token_setup {
        ...
        let home = &auth_home;
        let mut login = tokio::process::Command::new(env!("CARGO_BIN_EXE_test-x-fern-global-parameters-extension"));
        login
    ...
    let mut cmd = tokio::process::Command::new(env!("CARGO_BIN_EXE_test-x-fern-global-parameters-extension"));
    cmd.args(&args);
