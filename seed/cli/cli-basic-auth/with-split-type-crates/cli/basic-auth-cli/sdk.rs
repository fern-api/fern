...
impl basic_auth_cli_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> basic_auth_cli_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = basic_auth_cli_sdk::ClientConfig::default();
    let http_client = basic_auth_cli_sdk::HttpClient::with_executor(
        adapter as Arc<dyn basic_auth_cli_sdk::RequestExecutor>,
        config.clone(),
    );
    basic_auth_cli_sdk::api::ApiClient {
        config,
        widgets: basic_auth_cli_sdk::api::WidgetsClient { http_client: http_client.clone() },
        system: basic_auth_cli_sdk::api::SystemClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, basic_auth_cli_sdk::ApiError>>,
{
...
fn convert_api_error(e: basic_auth_cli_sdk::ApiError) -> CliError {
    match e {
        basic_auth_cli_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        basic_auth_cli_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        basic_auth_cli_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
