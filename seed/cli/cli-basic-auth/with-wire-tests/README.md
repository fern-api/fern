...
| `--page-all` | Auto-paginate and stream results (paginated operations only) |
...
basic-auth-cli <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
basic-auth-cli --help --format json | jq '.operations | length'
```
