# Server URL Templating API CLI

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Server%20URL%20Templating%20API%2FCLI)
...
```bash
cargo build --release
./target/release/server-url-templating-api --help
```
...
|----------|-------------|-------------|
| `SERVER_URL_TEMPLATING_API_BASE_URL` | Override the API base URL | yes |
| `SERVER_URL_TEMPLATING_API_OUTPUT` | Default output format when `--format` is not passed | yes |
| `SERVER_URL_TEMPLATING_API_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `SERVER_URL_TEMPLATING_API_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `SERVER_URL_TEMPLATING_API_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `SERVER_URL_TEMPLATING_API_PROXY` | HTTP(S) proxy URL | no |
| `SERVER_URL_TEMPLATING_API_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `SERVER_URL_TEMPLATING_API_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `server-url-templating-api profiles set <profile> SERVER_URL_TEMPLATING_API_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
server-url-templating-api <resource> <method> --format json | jq

# Keep the human rendering even when piped
server-url-templating-api <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
server-url-templating-api --help --format json | jq '.operations | length'
```
...
```bash
server-url-templating-api completion <bash|zsh|fish|powershell>
```
