...
impl server_url_templating_api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> Result<server_url_templating_api_sdk::api::ApiClient, CliError> {
    let executor = ctx.build_sdk_executor()?;
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = server_url_templating_api_sdk::ClientConfig::default();
    let http_client = server_url_templating_api_sdk::HttpClient::with_executor(
        adapter as Arc<dyn server_url_templating_api_sdk::RequestExecutor>,
        config.clone(),
    );
    Ok(server_url_templating_api_sdk::api::ApiClient {
        config,
...
where
    F: Future<Output = Result<T, server_url_templating_api_sdk::ApiError>>,
{
...
fn convert_api_error(e: server_url_templating_api_sdk::ApiError) -> CliError {
    match e {
        server_url_templating_api_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        server_url_templating_api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        server_url_templating_api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
