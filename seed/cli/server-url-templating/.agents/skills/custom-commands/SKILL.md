...
The `server-url-templating-api` CLI supports user-authored custom commands compiled
...
| `cli/server-url-templating-api/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
server-url-templating-api hello world              # top-level custom command
server-url-templating-api admin users list --limit 5  # nested under custom namespace
server-url-templating-api fetch my-id --format json   # output cohesion
server-url-templating-api fetch my-id --dry-run       # dry-run preview
```
