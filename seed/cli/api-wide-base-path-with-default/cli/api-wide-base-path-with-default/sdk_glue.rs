...
impl api_wide_base_path_with_default_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> api_wide_base_path_with_default_sdk::api::ApiClient {
...
where
    F: Future<Output = Result<T, api_wide_base_path_with_default_sdk::ApiError>>,
{
...
fn convert_api_error(e: api_wide_base_path_with_default_sdk::ApiError) -> CliError {
    match e {
        api_wide_base_path_with_default_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        api_wide_base_path_with_default_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        api_wide_base_path_with_default_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
