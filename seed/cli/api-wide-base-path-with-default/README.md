...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=api-wide-base-path-with-default%2FCLI)
...
```bash
cargo build --release
./target/release/api-wide-base-path-with-default --help
```
...
|----------|-------------|-------------|
| `API_WIDE_BASE_PATH_WITH_DEFAULT_BASE_URL` | Override the API base URL | yes |
| `API_WIDE_BASE_PATH_WITH_DEFAULT_OUTPUT` | Default output format when `--format` is not passed | yes |
| `API_WIDE_BASE_PATH_WITH_DEFAULT_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `API_WIDE_BASE_PATH_WITH_DEFAULT_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `API_WIDE_BASE_PATH_WITH_DEFAULT_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `API_WIDE_BASE_PATH_WITH_DEFAULT_PROXY` | HTTP(S) proxy URL | no |
| `API_WIDE_BASE_PATH_WITH_DEFAULT_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `API_WIDE_BASE_PATH_WITH_DEFAULT_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `api-wide-base-path-with-default profiles set <profile> API_WIDE_BASE_PATH_WITH_DEFAULT_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
api-wide-base-path-with-default <resource> <method> --format json | jq

# Keep the human rendering even when piped
api-wide-base-path-with-default <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
api-wide-base-path-with-default --help --format json | jq '.operations | length'
```
...
```bash
api-wide-base-path-with-default completion <bash|zsh|fish|powershell>
```
