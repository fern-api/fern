...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Webhooks%20CLI%2FCLI)

Command-line interface for the Webhooks CLI API.
...
```bash
cargo build --release
./target/release/webhooks-cli --help
```

## Authentication

This API requires authentication. Run `webhooks-cli --help` for details.
...
```bash
webhooks-cli <resource> <method>
```

Run `webhooks-cli <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `webhooks-cli <resource> <method>`). Run `webhooks-cli <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
webhooks-cli <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|
| `WEBHOOKS_CLI_BASE_URL` | Override the API base URL |
| `WEBHOOKS_CLI_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) |
| `WEBHOOKS_CLI_INSECURE=1` | Skip TLS verification (debugging only) |
| `WEBHOOKS_CLI_PROXY` | HTTP(S) proxy URL |
| `WEBHOOKS_CLI_TIMEOUT_SECS` | Total request timeout in seconds |
...
webhooks-cli <resource> <method> --format json | jq

# Keep the human rendering even when piped
webhooks-cli <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
webhooks-cli --help --format json | jq '.operations | length'
```
...
```bash
webhooks-cli completion <bash|zsh|fish|powershell>
```
