...
impl query_param_name_conflict_api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> Result<query_param_name_conflict_api_sdk::api::ApiClient, CliError> {
    let executor = ctx.build_sdk_executor()?;
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = query_param_name_conflict_api_sdk::ClientConfig::default();
    let http_client = query_param_name_conflict_api_sdk::HttpClient::with_executor(
        adapter as Arc<dyn query_param_name_conflict_api_sdk::RequestExecutor>,
        config.clone(),
    );
    Ok(query_param_name_conflict_api_sdk::api::ApiClient {
        config,
...
where
    F: Future<Output = Result<T, query_param_name_conflict_api_sdk::ApiError>>,
{
...
fn convert_api_error(e: query_param_name_conflict_api_sdk::ApiError) -> CliError {
    match e {
        query_param_name_conflict_api_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        query_param_name_conflict_api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        query_param_name_conflict_api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
