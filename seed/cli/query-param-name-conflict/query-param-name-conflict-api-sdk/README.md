...
[![crates.io shield](https://img.shields.io/crates/v/query_param_name_conflict_api_sdk)](https://crates.io/crates/query_param_name_conflict_api_sdk)
...
```toml
[dependencies]
query_param_name_conflict_api_sdk = "0.1.0"
```

Or install via cargo:

```sh
cargo add query_param_name_conflict_api_sdk
```
...
```rust
match client.bulk_update_tasks(None)?.await {
    Ok(response) => {
...
let request = BulkUpdateTasksRequest {
    ...
};
```
...
```rust
let response = client.bulk_update_tasks(
