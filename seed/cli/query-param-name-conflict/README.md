...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Query%20Param%20Name%20Conflict%20API%2FCLI)
...
|----------|-------------|-------------|
| `QUERY_PARAM_NAME_CONFLICT_API_BASE_URL` | Override the API base URL | yes |
| `QUERY_PARAM_NAME_CONFLICT_API_OUTPUT` | Default output format when `--format` is not passed | yes |
| `QUERY_PARAM_NAME_CONFLICT_API_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `QUERY_PARAM_NAME_CONFLICT_API_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `QUERY_PARAM_NAME_CONFLICT_API_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `QUERY_PARAM_NAME_CONFLICT_API_PROXY` | HTTP(S) proxy URL | no |
| `QUERY_PARAM_NAME_CONFLICT_API_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `QUERY_PARAM_NAME_CONFLICT_API_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `query-param-name-conflict-api profiles set <profile> QUERY_PARAM_NAME_CONFLICT_API_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
query-param-name-conflict-api <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
query-param-name-conflict-api --help --format json | jq '.operations | length'
```
