...
The `query-param-name-conflict-api` CLI supports user-authored custom commands compiled
...
cli/query-param-name-conflict-api/sdk.rs       ← Generated bridge: client() + block_on()
...
| `cli/query-param-name-conflict-api/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
query-param-name-conflict-api hello world              # top-level custom command
query-param-name-conflict-api admin users list --limit 5  # nested under custom namespace
query-param-name-conflict-api fetch my-id --format json   # output cohesion
query-param-name-conflict-api fetch my-id --dry-run       # dry-run preview
```
