...
The `acme-cli` CLI supports user-authored custom commands compiled
...
cli/acme-cli/sdk.rs       ← Generated bridge: client() + block_on()
...
| `cli/acme-cli/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
acme-cli hello world              # top-level custom command
acme-cli admin users list --limit 5  # nested under custom namespace
acme-cli fetch my-id --format json   # output cohesion
acme-cli fetch my-id --dry-run       # dry-run preview
```
