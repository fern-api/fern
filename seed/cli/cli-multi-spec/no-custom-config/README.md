...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Users%20API%2FCLI)
...
acme-cli <resource> <method> --format json | jq

# Keep the human rendering even when piped
acme-cli <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
acme-cli --help --format json | jq '.operations | length'
```
...
```bash
acme-cli completion <bash|zsh|fish|powershell>
```
