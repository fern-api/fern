...
impl acme_cli_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> acme_cli_sdk::api::ApiClient {
...
where
    F: Future<Output = Result<T, acme_cli_sdk::ApiError>>,
{
...
fn convert_api_error(e: acme_cli_sdk::ApiError) -> CliError {
    match e {
        acme_cli_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        acme_cli_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        acme_cli_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
