...
|----------|-------------|-------------|
| `VERSIONED_STORE_BASE_URL` | Override the API base URL | yes |
| `VERSIONED_STORE_OUTPUT` | Default output format when `--format` is not passed | yes |
| `VERSIONED_STORE_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `VERSIONED_STORE_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `VERSIONED_STORE_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `VERSIONED_STORE_PROXY` | HTTP(S) proxy URL | no |
| `VERSIONED_STORE_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `VERSIONED_STORE_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `versioned-store profiles set <profile> VERSIONED_STORE_RETRIES=3`; the others apply to the whole process and are read from the environment only.
