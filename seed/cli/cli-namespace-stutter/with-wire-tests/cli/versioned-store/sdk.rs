...
impl versioned_store_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> versioned_store_sdk::api::ApiClient {
    ...
    versioned_store_sdk::api::ApiClient {
        config,
        v1: versioned_store_sdk::api::V1Client {
            http_client: http_client.clone(),
            v1: versioned_store_sdk::api::resources::v1::V1Client2 { http_client: http_client.clone() },
        },
    }
}
...
where
    F: Future<Output = Result<T, versioned_store_sdk::ApiError>>,
{
...
fn convert_api_error(e: versioned_store_sdk::ApiError) -> CliError {
    match e {
        versioned_store_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        versioned_store_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        versioned_store_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
