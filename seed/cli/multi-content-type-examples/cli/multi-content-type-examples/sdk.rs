...
impl multi_content_type_examples_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> multi_content_type_examples_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = multi_content_type_examples_sdk::ClientConfig::default();
    let http_client = multi_content_type_examples_sdk::HttpClient::with_executor(
        adapter as Arc<dyn multi_content_type_examples_sdk::RequestExecutor>,
        config.clone(),
    );
    multi_content_type_examples_sdk::api::ApiClient {
        config,
        clients: multi_content_type_examples_sdk::api::ClientsClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, multi_content_type_examples_sdk::ApiError>>,
{
...
fn convert_api_error(e: multi_content_type_examples_sdk::ApiError) -> CliError {
    match e {
        multi_content_type_examples_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        multi_content_type_examples_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        multi_content_type_examples_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
