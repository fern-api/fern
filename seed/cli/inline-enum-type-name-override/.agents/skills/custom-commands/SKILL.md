...
The `inline-enum-type-name-override` CLI supports user-authored custom commands compiled
...
| Field | Type |
|-------|------|
| `client.reporting` | `inline_enum_type_name_override_sdk::api::ReportingClient` |
...
```bash
cargo build
inline-enum-type-name-override hello world              # top-level custom command
inline-enum-type-name-override admin users list --limit 5  # nested under custom namespace
inline-enum-type-name-override fetch my-id --format json   # output cohesion
inline-enum-type-name-override fetch my-id --dry-run       # dry-run preview
```
