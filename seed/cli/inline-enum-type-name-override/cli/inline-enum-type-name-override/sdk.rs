...
impl inline_enum_type_name_override_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> Result<inline_enum_type_name_override_sdk::api::ApiClient, CliError> {
    let executor = ctx.build_sdk_executor()?;
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = inline_enum_type_name_override_sdk::ClientConfig::default();
    let http_client = inline_enum_type_name_override_sdk::HttpClient::with_executor(
        adapter as Arc<dyn inline_enum_type_name_override_sdk::RequestExecutor>,
        config.clone(),
    );
    Ok(inline_enum_type_name_override_sdk::api::ApiClient {
        config,
        reporting: inline_enum_type_name_override_sdk::api::ReportingClient { http_client: http_client.clone() },
    })
}
...
where
    F: Future<Output = Result<T, inline_enum_type_name_override_sdk::ApiError>>,
{
...
fn convert_api_error(e: inline_enum_type_name_override_sdk::ApiError) -> CliError {
    match e {
        inline_enum_type_name_override_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        inline_enum_type_name_override_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        inline_enum_type_name_override_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
