# Inline Enum Type Name Override CLI

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Inline%20Enum%20Type%20Name%20Override%2FCLI)

Command-line interface for the Inline Enum Type Name Override API.
...
```bash
cargo build --release
./target/release/inline-enum-type-name-override --help
```

## Authentication

This API requires authentication. Run `inline-enum-type-name-override --help` for details.
...
```bash
inline-enum-type-name-override --help
```

Call an API endpoint:

```bash
inline-enum-type-name-override <resource> <method>
```

Run `inline-enum-type-name-override <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `inline-enum-type-name-override <resource> <method>`). Run `inline-enum-type-name-override <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
inline-enum-type-name-override <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|-------------|
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_BASE_URL` | Override the API base URL | yes |
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_OUTPUT` | Default output format when `--format` is not passed | yes |
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_PROXY` | HTTP(S) proxy URL | no |
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `INLINE_ENUM_TYPE_NAME_OVERRIDE_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `inline-enum-type-name-override profiles set <profile> INLINE_ENUM_TYPE_NAME_OVERRIDE_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
inline-enum-type-name-override <resource> <method> --format json | jq

# Keep the human rendering even when piped
inline-enum-type-name-override <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
inline-enum-type-name-override --help --format json | jq '.operations | length'
```
...
```bash
inline-enum-type-name-override completion <bash|zsh|fish|powershell>
```
