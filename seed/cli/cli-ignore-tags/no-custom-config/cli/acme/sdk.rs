...
impl acme_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> acme_sdk::api::ApiClient {
    ...
    let config = acme_sdk::ClientConfig {
        base_url: ctx.effective_base_url(),
        ..Default::default()
    };
    let http_client = acme_sdk::HttpClient::with_executor(
        adapter as Arc<dyn acme_sdk::RequestExecutor>,
        config.clone(),
    );
    acme_sdk::api::ApiClient {
        config,
        knowledge: acme_sdk::api::KnowledgeClient { http_client: http_client.clone() },
        messages: acme_sdk::api::MessagesClient {
            http_client: http_client.clone(),
            messages: acme_sdk::api::resources::messages::MessagesClient2 { http_client: http_client.clone() },
        },
    }
}
...
where
    F: Future<Output = Result<T, acme_sdk::ApiError>>,
{
...
fn convert_api_error(e: acme_sdk::ApiError) -> CliError {
    match e {
        acme_sdk::ApiError::Http { status, message } => {
            fern_cli_sdk::error::api_error_from_body(status, &message)
        }
        acme_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        acme_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
