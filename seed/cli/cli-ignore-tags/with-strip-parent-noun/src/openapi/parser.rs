...
/// Split an identifier into lowercase words regardless of casing style:
/// `CreateMessage`, `createMessage`, `create_message`, `create.message` and
/// `Create Message` all yield `["create", "message"]`. Unlike [`tokenize`],
/// PascalCase is split too. Uppercase runs and digits stay attached
/// (`ListImportsV2` → `["list", "imports", "v2"]`, `APIKeys` → `["api", "keys"]`).
...
/// Reduce a token to a crude singular so `Messages` matches `Message` and
/// `Policies` matches `Policy`. Only used for equality comparisons between
/// a parent group's tokens and an operationId's tokens.
fn singular_key(token: &str) -> String {
    if let Some(stem) = token.strip_suffix("ies") {
        return format!("{stem}y");
    }
    if token.ends_with("ss") || token.len() <= 1 {
        return token.to_string();
    }
    token.strip_suffix('s').unwrap_or(token).to_string()
}

/// Strip a parent group's noun from anywhere in an operationId, so
/// `parent="Messages", operationId="CreateMessage"` → `create` and
/// `parent="knowledge", operationId="ListKnowledgeBases"` → `list-bases`.
///
/// Every token of the parent must appear in the operationId (singular /
/// plural insensitive) for anything to be removed; a partial overlap leaves
/// the operationId untouched. If stripping would consume every token the
/// original is kept, so a leaf is never empty.
fn strip_parent_noun(operation_id: &str, parent: &str) -> String {
    let parent_keys: HashSet<String> = tokenize_words(parent)
        .iter()
        .map(|t| singular_key(t))
        .collect();
    let op_tokens = tokenize_words(operation_id);
    if parent_keys.is_empty() {
        return operation_id.to_string();
    }
    let op_keys: HashSet<String> = op_tokens.iter().map(|t| singular_key(t)).collect();
    if !parent_keys.is_subset(&op_keys) {
        return operation_id.to_string();
    }
    let kept: Vec<&str> = op_tokens
        .iter()
        .filter(|t| !parent_keys.contains(&singular_key(t)))
        .map(String::as_str)
        .collect();
    if kept.is_empty() {
        return operation_id.to_string();
...
/// Per-spec naming controls applied while lowering operations into the
/// command tree. The generator sets these from `generators.yml`; the
/// defaults reproduce the historical behavior exactly.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct SpecNamingOptions {
    /// When set, tag-derived grouping is disabled: operations without an
    /// explicit `x-fern-sdk-group-name` land directly under this group,
    /// which is the namespace the spec is mounted under. Mirrors the Fern
    /// importer's `ignore-tags` setting, where such operations sit at the
    /// namespace root instead of in a per-tag subpackage.
    pub flatten_into: Option<String>,
    /// Strip the parent group's noun from operationId-derived leaves
    /// (`CreateMessage` under `messages` → `create`). Explicit
    /// `x-fern-sdk-method-name` values are never touched.
    pub strip_parent_noun: bool,
}
...
pub fn load_openapi_spec_from_value(
    ...
    load_openapi_spec_from_value_with_naming(value, cli_name, &SpecNamingOptions::default())
}

/// Like [`load_openapi_spec_from_value`], with explicit [`SpecNamingOptions`].
pub fn load_openapi_spec_from_value_with_naming(
    value: serde_yaml::Value,
    cli_name: &str,
...
"#;
    ...
    #[test]
    fn test_strip_parent_noun_removes_noun_anywhere_plural_insensitive() {
        assert_eq!(strip_parent_noun("CreateMessage", "Messages"), "create");
        assert_eq!(
            strip_parent_noun("ListKnowledgeBases", "knowledge"),
            "list-bases"
        );
        assert_eq!(
            strip_parent_noun("FetchKnowledgeBase", "KnowledgeBases"),
            "fetch"
        );
        assert_eq!(strip_parent_noun("wallet.list", "Wallets"), "list");
        assert_eq!(strip_parent_noun("listPolicy", "Policies"), "list");
        // Partial overlap of a multi-token parent leaves the id alone.
        assert_eq!(
            strip_parent_noun("ListBases", "KnowledgeBases"),
            "ListBases"
        );
        // No overlap: untouched.
        assert_eq!(
            strip_parent_noun("FetchMetadata", "Insights"),
            "FetchMetadata"
        );
        // Never strip down to nothing.
        assert_eq!(strip_parent_noun("Messages", "Messages"), "Messages");
        // Version tokens are ordinary tokens and are preserved.
        assert_eq!(
            strip_parent_noun("ListProfileImportsV2", "memory"),
            "ListProfileImportsV2"
        );
    }

    const NAMING_FIXTURE: &str = r#"
openapi: "3.0.0"
info: { title: T, version: "1.0" }
servers: [{ url: "https://x.com" }]
paths:
  ...
  /v1/Explicit:
    get:
      tags: [KnowledgeBases]
      operationId: ListKnowledgeExplicit
      x-fern-sdk-group-name: [admin]
      x-fern-sdk-method-name: list-knowledge-explicit
      responses: { "200": { description: ok } }
  /v1/Untagged:
    get:
      operationId: FetchUntagged
      responses: { "200": { description: ok } }
"#;

    fn load_with_naming(naming: SpecNamingOptions) -> RestDescription {
        let value: serde_yaml::Value = serde_yaml::from_str(NAMING_FIXTURE).unwrap();
        load_openapi_spec_from_value_with_naming(value, "t", &naming).unwrap()
    }

    #[test]
    fn test_default_naming_is_unchanged() {
        let doc = load_with_naming(SpecNamingOptions::default());
        let kb = &doc.resources["knowledge-bases"];
        assert!(kb.methods.contains_key("create-knowledge"));
        assert!(kb.methods.contains_key("list-knowledge-bases"));
        assert!(kb.methods.contains_key("list-knowledge-chunks"));
        assert!(doc.resources["admin"]
            .methods
            .contains_key("list-knowledge-explicit"));
        assert!(doc.resources["v1"].methods.contains_key("fetch-untagged"));
    }

    #[test]
    fn test_flatten_into_puts_tagged_and_untagged_ops_under_namespace_verbatim() {
        let doc = load_with_naming(SpecNamingOptions {
            flatten_into: Some("knowledge".to_string()),
            strip_parent_noun: false,
        });
        assert!(!doc.resources.contains_key("knowledge-bases"));
        assert!(!doc.resources.contains_key("v1"));
        let ns = &doc.resources["knowledge"];
        // operationIds kept verbatim (SDK parity), only kebab-cased.
        assert!(ns.methods.contains_key("create-knowledge"));
        assert!(ns.methods.contains_key("list-knowledge-bases"));
        assert!(ns.methods.contains_key("list-knowledge-chunks"));
        assert!(ns.methods.contains_key("fetch-untagged"));
        // Explicit x-fern-sdk-group-name still wins over flattening.
        assert!(doc.resources["admin"]
            .methods
            .contains_key("list-knowledge-explicit"));
    }
    ...
    #[test]
    fn test_flatten_into_preserves_raw_namespace_key() {
        let doc = load_with_naming(SpecNamingOptions {
            flatten_into: Some("bulk_messages".to_string()),
            strip_parent_noun: false,
        });
        assert!(doc.resources.contains_key("bulk_messages"));
    }

    #[test]
    fn test_strip_parent_noun_with_tag_grouping() {
        let doc = load_with_naming(SpecNamingOptions {
            flatten_into: None,
            strip_parent_noun: true,
        });
        let kb = &doc.resources["knowledge-bases"];
        // `CreateKnowledge` only shares `knowledge` with `KnowledgeBases` → untouched.
        assert!(kb.methods.contains_key("create-knowledge"));
        assert!(kb.methods.contains_key("list"));
        assert!(kb.methods.contains_key("list-knowledge-chunks"));
        assert!(doc.resources["admin"]
            .methods
            .contains_key("list-knowledge-explicit"));
    ...
    #[test]
    fn test_strip_parent_noun_with_flatten_uses_namespace_as_parent() {
        let doc = load_with_naming(SpecNamingOptions {
            flatten_into: Some("knowledge".to_string()),
            strip_parent_noun: true,
        });
        let ns = &doc.resources["knowledge"];
        assert!(ns.methods.contains_key("create"));
        assert!(ns.methods.contains_key("list-bases"));
        assert!(ns.methods.contains_key("list-chunks"));
        assert!(ns.methods.contains_key("fetch-untagged"));
        assert!(doc.resources["admin"]
            .methods
            .contains_key("list-knowledge-explicit"));
