...
impl OpenApiBinding {
    ...
    /// Ignore operation tags when grouping the most recently added spec
    /// (Fern's `ignore-tags`). See [`CliApp::ignore_tags`](crate::openapi::app::CliApp::ignore_tags).
    pub fn ignore_tags(mut self) -> Self {
        self.inner = self.inner.ignore_tags();
        self
    }

    /// Strip the parent group's noun from operationId-derived command names.
    /// See [`CliApp::strip_parent_noun`](crate::openapi::app::CliApp::strip_parent_noun).
    pub fn strip_parent_noun(mut self) -> Self {
        self.inner = self.inner.strip_parent_noun();
        self
    }
