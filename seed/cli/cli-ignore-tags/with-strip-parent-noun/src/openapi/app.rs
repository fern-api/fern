...
use crate::openapi::parser::SpecNamingOptions;
...
pub(crate) struct SpecEntry {
    ...
    /// Disable tag-derived grouping for this spec (Fern's `ignore-tags`).
    /// Only takes effect when the spec is mounted under a prefix, since the
    /// flattened operations need a namespace to land in.
    ignore_tags: bool,
}
...
pub struct CliApp {
    ...
    /// Strip the parent group's noun from operationId-derived command
    /// names across every spec. See [`CliApp::strip_parent_noun`].
    strip_parent_noun: bool,
...
impl CliApp {
    ...
    /// mirroring `settings.ignore-tags` in `generators.yml`. Operations
    /// without an explicit `x-fern-sdk-group-name` become direct
    /// subcommands of the spec's namespace instead of a per-tag group, and
    /// their operationIds are used verbatim (kebab-cased), matching the
    /// method names the Fern SDK generators produce for the same setting.
    ///
    /// Has no effect on a spec added with a bare [`spec`](Self::spec): there
    /// is no namespace to flatten into, so tag grouping is kept.
    ///
    /// # Panics
    ///
    /// Panics if called before `.spec()` or `.spec_under()`.
    pub fn ignore_tags(mut self) -> Self {
        let entry = self
            .specs
            .last_mut()
            .expect("ignore_tags() called before spec(); add a spec first");
        entry.ignore_tags = true;
        self
    }

    /// Strip the parent group's noun from operationId-derived command
    /// names: `CreateMessage` under `messages` becomes `create`,
    /// `ListKnowledgeBases` under `knowledge` becomes `list-bases`. The
    /// parent is the namespace when tags are ignored, otherwise the tag.
    /// Explicit `x-fern-sdk-method-name` values are never altered, and an
    /// operationId is left as-is when stripping would leave nothing.
    pub fn strip_parent_noun(mut self) -> Self {
        self.strip_parent_noun = true;
        self
    }
    ...
    pub(crate) fn build_doc(&self) -> Result<RestDescription, CliError> {
        ...
        for entry in &self.specs {
            ...
            let flatten_into = match (entry.ignore_tags, entry.prefix_path.last()) {
                (true, Some(namespace)) => Some(namespace.clone()),
                (true, None) => {
                    tracing::warn!(
                        "ignore_tags is set on a spec without a namespace; tag grouping is kept"
                    );
                    None
                }
                (false, _) => None,
            };
            let naming = SpecNamingOptions {
                flatten_into,
                strip_parent_noun: self.strip_parent_noun,
            };
            let spec_doc = crate::openapi::parser::load_openapi_spec_from_value_with_naming(
                value, &self.name, &naming,
            )?;
...
"#;
    ...
    #[test]
    fn test_ignore_tags_flattens_tag_groups_under_namespace() {
        let doc = CliApp::new("t")
            .spec_under("bulk_messages", IGNORE_TAGS_SPEC)
            .ignore_tags()
            .build_doc()
            .unwrap();
        let ns = &doc.resources["bulk_messages"];
        assert!(
            ns.resources.is_empty(),
            "tag groups must be flattened: {:?}",
            ns.resources.keys()
        );
        assert!(ns.methods.contains_key("create-message"));
        assert!(ns.methods.contains_key("list-media"));
    }

    #[test]
    fn test_ignore_tags_with_strip_parent_noun() {
        let doc = CliApp::new("t")
            .spec_under("messages", IGNORE_TAGS_SPEC)
            .ignore_tags()
            .strip_parent_noun()
            .build_doc()
            .unwrap();
        let ns = &doc.resources["messages"];
        assert!(ns.methods.contains_key("create"));
        assert!(ns.methods.contains_key("list-media"));
    }

    #[test]
    fn test_ignore_tags_without_namespace_keeps_tag_groups() {
        let doc = CliApp::new("t")
            .spec(IGNORE_TAGS_SPEC)
            .ignore_tags()
            .build_doc()
            .unwrap();
        assert!(doc.resources["messages"]
            .methods
            .contains_key("create-message"));
        assert!(doc.resources["media"].methods.contains_key("list-media"));
    }
