...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Knowledge%20API%2FCLI)

Command-line interface for the Knowledge API.
...
```bash
acme <resource> <method>
```

Run `acme <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `acme <resource> <method>`). Run `acme <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
acme <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|
| `ACME_BASE_URL` | Override the API base URL |
| `ACME_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) |
| `ACME_INSECURE=1` | Skip TLS verification (debugging only) |
| `ACME_PROXY` | HTTP(S) proxy URL |
| `ACME_TIMEOUT_SECS` | Total request timeout in seconds |
...
acme <resource> <method> --format json | jq

# Keep the human rendering even when piped
acme <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
acme --help --format json | jq '.operations | length'
```
...
```bash
acme completion <bash|zsh|fish|powershell>
```
