...
impl nullable_request_body_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> Result<nullable_request_body_sdk::api::ApiClient, CliError> {
    let executor = ctx.build_sdk_executor()?;
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = nullable_request_body_sdk::ClientConfig::default();
    let http_client = nullable_request_body_sdk::HttpClient::with_executor(
        adapter as Arc<dyn nullable_request_body_sdk::RequestExecutor>,
        config.clone(),
    );
    Ok(nullable_request_body_sdk::api::ApiClient {
        config,
        test_group: nullable_request_body_sdk::api::TestGroupClient { http_client: http_client.clone() },
    })
}
...
where
    F: Future<Output = Result<T, nullable_request_body_sdk::ApiError>>,
{
...
fn convert_api_error(e: nullable_request_body_sdk::ApiError) -> CliError {
    match e {
        nullable_request_body_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        nullable_request_body_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        nullable_request_body_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
