# nullable-request-body CLI

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=nullable-request-body%2FCLI)
...
```bash
cargo build --release
./target/release/nullable-request-body --help
```
...
|----------|-------------|-------------|
| `NULLABLE_REQUEST_BODY_BASE_URL` | Override the API base URL | yes |
| `NULLABLE_REQUEST_BODY_OUTPUT` | Default output format when `--format` is not passed | yes |
| `NULLABLE_REQUEST_BODY_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `NULLABLE_REQUEST_BODY_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `NULLABLE_REQUEST_BODY_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `NULLABLE_REQUEST_BODY_PROXY` | HTTP(S) proxy URL | no |
| `NULLABLE_REQUEST_BODY_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `NULLABLE_REQUEST_BODY_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `nullable-request-body profiles set <profile> NULLABLE_REQUEST_BODY_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
nullable-request-body <resource> <method> --format json | jq

# Keep the human rendering even when piped
nullable-request-body <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
nullable-request-body --help --format json | jq '.operations | length'
```
...
```bash
nullable-request-body completion <bash|zsh|fish|powershell>
```
