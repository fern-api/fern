...
|----------|-------------|
| `PATH_PARAM_BODY_COLLISION_BASE_URL` | Override the API base URL |
| `PATH_PARAM_BODY_COLLISION_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) |
| `PATH_PARAM_BODY_COLLISION_INSECURE=1` | Skip TLS verification (debugging only) |
| `PATH_PARAM_BODY_COLLISION_PROXY` | HTTP(S) proxy URL |
| `PATH_PARAM_BODY_COLLISION_TIMEOUT_SECS` | Total request timeout in seconds |
