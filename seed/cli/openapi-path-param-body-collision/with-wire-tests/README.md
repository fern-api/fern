# Path Param Body Collision CLI

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Path%20Param%20Body%20Collision%2FCLI)

Command-line interface for the Path Param Body Collision API.
...
```bash
cargo build --release
./target/release/path-param-body-collision --help
```

## Authentication

This API requires authentication. Run `path-param-body-collision --help` for details.
...
```bash
path-param-body-collision --help
```

Call an API endpoint:

```bash
path-param-body-collision <resource> <method>
```

Run `path-param-body-collision <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `path-param-body-collision <resource> <method>`). Run `path-param-body-collision <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
path-param-body-collision <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|-------------|
| `PATH_PARAM_BODY_COLLISION_BASE_URL` | Override the API base URL | yes |
| `PATH_PARAM_BODY_COLLISION_OUTPUT` | Default output format when `--format` is not passed | yes |
| `PATH_PARAM_BODY_COLLISION_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `PATH_PARAM_BODY_COLLISION_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `PATH_PARAM_BODY_COLLISION_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `PATH_PARAM_BODY_COLLISION_PROXY` | HTTP(S) proxy URL | no |
| `PATH_PARAM_BODY_COLLISION_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `PATH_PARAM_BODY_COLLISION_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `path-param-body-collision profiles set <profile> PATH_PARAM_BODY_COLLISION_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
path-param-body-collision <resource> <method> --format json | jq

# Keep the human rendering even when piped
path-param-body-collision <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
path-param-body-collision --help --format json | jq '.operations | length'
```
...
```bash
path-param-body-collision completion <bash|zsh|fish|powershell>
```
