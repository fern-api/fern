...
async fn run_case(id: &str) {
    ...
    if let Some(setup) = &manifest.login_token_setup {
        ...
        let home = &auth_home;
        let mut login = tokio::process::Command::new(env!("CARGO_BIN_EXE_path-param-body-collision"));
        login
    ...
    let mut cmd = tokio::process::Command::new(env!("CARGO_BIN_EXE_path-param-body-collision"));
    cmd.args(&args);
...
#[tokio::test]
async fn wire_updateprofileidentifier() {
    run_case("updateprofileidentifier").await;
}

#[tokio::test]
async fn wire_updateprofileidentifier_error() {
    run_case("updateprofileidentifier_error").await;
}
