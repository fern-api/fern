...
impl path_param_body_collision_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> path_param_body_collision_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = path_param_body_collision_sdk::ClientConfig::default();
    let http_client = path_param_body_collision_sdk::HttpClient::with_executor(
        adapter as Arc<dyn path_param_body_collision_sdk::RequestExecutor>,
        config.clone(),
    );
    path_param_body_collision_sdk::api::ApiClient {
        config,
...
where
    F: Future<Output = Result<T, path_param_body_collision_sdk::ApiError>>,
{
...
fn convert_api_error(e: path_param_body_collision_sdk::ApiError) -> CliError {
    match e {
        path_param_body_collision_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        path_param_body_collision_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        path_param_body_collision_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
