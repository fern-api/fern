//! AsyncAPI Overlay support (v1.0.0).
//!
//! Applies overlays (modeled on the [OpenAPI Overlays
//! specification](https://spec.openapis.org/overlay/latest.html)) to an
//! AsyncAPI document represented as a generic JSON value. Each overlay
//! contains a list of *actions* whose `target` is a JSONPath (RFC 9535)
//! expression. Actions either **update** (deep-merge) or **remove**
//! matched nodes.
//!
//! Per the no-shared-abstractions rule (see `AGENTS.md`), this module is a
//! direct duplicate of `src/openapi/overlay.rs`. The semantics — JSONPath
//! resolution, deep-merge, array-append, root targeting, sequential
//! application — are identical; only the surrounding context (AsyncAPI
//! instead of OpenAPI) differs.
...
/// Apply an overlay document to an AsyncAPI spec represented as a JSON value.
///
...
/// Parse an AsyncAPI spec string (YAML or JSON) into a `serde_json::Value`,
/// apply a list of overlay strings, and return the modified JSON value
/// serialised back to a YAML string suitable for the AsyncAPI parser.
pub fn apply_overlays_to_spec(
    ...
    let yaml_value: serde_yaml::Value = serde_yaml::from_str(spec_yaml)
        .map_err(|e| CliError::Discovery(format!("Failed to parse AsyncAPI spec: {e}")))?;
...
"#;
    ...
    // -----------------------------------------------------------------------
    // AsyncAPI-specific overlay tests — exercise update / remove / zero-match
    // against a parsed AsyncAPI document. Mirrors the structure of the
    // openapi fixture tests but stays inside the AsyncAPI path.
    // -----------------------------------------------------------------------

    const SAMPLE_ASYNCAPI_YAML: &str = r##"
asyncapi: "2.6.0"
info:
  title: Sample
  version: "1.0"
...
"##;
    ...
    fn parse_asyncapi_yaml(yaml: &str) -> serde_json::Value {
        let yaml_value: serde_yaml::Value = serde_yaml::from_str(yaml)
            .expect("YAML must parse for overlay tests");
        yaml_to_json(yaml_value)
    }

    #[test]
    fn overlay_update_action_deep_merges_into_parsed_asyncapi() {
        let doc = parse_asyncapi_yaml(SAMPLE_ASYNCAPI_YAML);
        let overlay = make_overlay(vec![update_action(
            "$.channels.AgentMessages",
            json!({ "description": "Updated description" }),
        )]);

        let result = apply_overlay(&doc, &overlay).unwrap();
        assert_eq!(
            result["channels"]["AgentMessages"]["description"],
            "Updated description"
        );
        // Pre-existing children preserved (subscribe block intact)
        assert!(result["channels"]["AgentMessages"]["subscribe"].is_object());
    }

    #[test]
    fn overlay_remove_action_removes_node_from_parsed_asyncapi() {
        let doc = parse_asyncapi_yaml(SAMPLE_ASYNCAPI_YAML);
        let overlay = make_overlay(vec![remove_action(
            "$.channels.AgentMessages.subscribe",
        )]);

        let result = apply_overlay(&doc, &overlay).unwrap();
        assert!(
            result["channels"]["AgentMessages"]
                .get("subscribe")
                .is_none(),
            "subscribe should be removed: {:?}",
            result["channels"]["AgentMessages"]
        );
        // Sibling description survives the removal.
        assert_eq!(
            result["channels"]["AgentMessages"]["description"],
    ...
    #[test]
    fn overlay_jsonpath_filter_edge_case_no_matches() {
        let doc = parse_asyncapi_yaml(SAMPLE_ASYNCAPI_YAML);
        let overlay = make_overlay(vec![update_action(
            "$.channels.AgentMessages.subscribe.message.oneOf[?(@.name == 'DoesNotExist')]",
            json!({ "description": "should not be applied" }),
        )]);

        let result = apply_overlay(&doc, &overlay).expect(
            "zero-match overlay must succeed without error",
        );
