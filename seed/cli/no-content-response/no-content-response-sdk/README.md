...
[![crates.io shield](https://img.shields.io/crates/v/no_content_response_sdk)](https://crates.io/crates/no_content_response_sdk)
...
```toml
[dependencies]
no_content_response_sdk = "0.1.0"
```

Or install via cargo:

```sh
cargo add no_content_response_sdk
```
...
```rust
match client.contacts.create(None)?.await {
    Ok(response) => {
...
```rust
let response = client.contacts.create(
