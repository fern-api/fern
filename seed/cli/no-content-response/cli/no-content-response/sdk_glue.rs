...
impl no_content_response_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> no_content_response_sdk::api::ApiClient {
...
where
    F: Future<Output = Result<T, no_content_response_sdk::ApiError>>,
{
...
fn convert_api_error(e: no_content_response_sdk::ApiError) -> CliError {
    match e {
        no_content_response_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        no_content_response_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        no_content_response_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
