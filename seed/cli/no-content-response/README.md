...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=no-content-response%2FCLI)
...
|----------|-------------|-------------|
| `NO_CONTENT_RESPONSE_BASE_URL` | Override the API base URL | yes |
| `NO_CONTENT_RESPONSE_OUTPUT` | Default output format when `--format` is not passed | yes |
| `NO_CONTENT_RESPONSE_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `NO_CONTENT_RESPONSE_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `NO_CONTENT_RESPONSE_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `NO_CONTENT_RESPONSE_PROXY` | HTTP(S) proxy URL | no |
| `NO_CONTENT_RESPONSE_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `NO_CONTENT_RESPONSE_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `no-content-response profiles set <profile> NO_CONTENT_RESPONSE_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
no-content-response <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
no-content-response --help --format json | jq '.operations | length'
```
