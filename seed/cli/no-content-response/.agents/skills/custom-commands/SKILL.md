...
The `no-content-response` CLI supports user-authored custom commands compiled
...
| Field | Type |
|-------|------|
| `client.contacts` | `no_content_response_sdk::api::ContactsClient` |
...
| `cli/no-content-response/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
no-content-response hello world              # top-level custom command
no-content-response admin users list --limit 5  # nested under custom namespace
no-content-response fetch my-id --format json   # output cohesion
no-content-response fetch my-id --dry-run       # dry-run preview
```
