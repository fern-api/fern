# Multi Scheme Routing CLI CLI

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Multi%20Scheme%20Routing%20CLI%2FCLI)

Command-line interface for the Multi Scheme Routing CLI API.
...
```bash
cargo build --release
./target/release/multi-scheme-cli --help
```
...
```bash
export MULTI_SCHEME_CLI_API_KEY="<your api key>"
```
...
```bash
multi-scheme-cli --help
```

Call an API endpoint:

```bash
multi-scheme-cli <resource> <method>
```

Run `multi-scheme-cli <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `multi-scheme-cli <resource> <method>`). Run `multi-scheme-cli <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
multi-scheme-cli <resource> <method> --json '{"key": "value"}'
```
...
multi-scheme-cli <resource> <method> --format json | jq

# Keep the human rendering even when piped
multi-scheme-cli <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
multi-scheme-cli --help --format json | jq '.operations | length'
```
...
```bash
multi-scheme-cli completion <bash|zsh|fish|powershell>
```
