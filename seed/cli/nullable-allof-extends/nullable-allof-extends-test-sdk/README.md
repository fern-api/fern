...
[![crates.io shield](https://img.shields.io/crates/v/nullable_allof_extends_test_sdk)](https://crates.io/crates/nullable_allof_extends_test_sdk)
...
```toml
[dependencies]
nullable_allof_extends_test_sdk = "0.1.0"
```

Or install via cargo:

```sh
cargo add nullable_allof_extends_test_sdk
```
...
```rust
match client.create_test(None)?.await {
    Ok(response) => {
...
```rust
let response = client.create_test(
