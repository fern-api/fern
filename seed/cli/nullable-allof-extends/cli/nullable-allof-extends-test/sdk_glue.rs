...
impl nullable_allof_extends_test_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> nullable_allof_extends_test_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = nullable_allof_extends_test_sdk::ClientConfig::default();
    let http_client = nullable_allof_extends_test_sdk::HttpClient::with_executor(
        adapter as Arc<dyn nullable_allof_extends_test_sdk::RequestExecutor>,
        config.clone(),
    );
    nullable_allof_extends_test_sdk::api::ApiClient {
        config,
        http_client: nullable_allof_extends_test_sdk::api::HttpClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, nullable_allof_extends_test_sdk::ApiError>>,
{
...
fn convert_api_error(e: nullable_allof_extends_test_sdk::ApiError) -> CliError {
    match e {
        nullable_allof_extends_test_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        nullable_allof_extends_test_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        nullable_allof_extends_test_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
