...
impl RootObjectBuilder {
    ...
    pub fn build(self) -> Result<RootObject, BuildError> {
        Ok(RootObject {
            normal_object_fields: self.normal_object_fields.ok_or_else(|| BuildError::missing_field("normal_object_fields"))?,
            nullable_object_fields: self.nullable_object_fields.ok_or_else(|| BuildError::missing_field("nullable_object_fields"))?,
        })
    }
}
