...
|----------|-------------|-------------|
| `ALLOF_COMPOSITION_BASE_URL` | Override the API base URL | yes |
| `ALLOF_COMPOSITION_OUTPUT` | Default output format when `--format` is not passed | yes |
| `ALLOF_COMPOSITION_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `ALLOF_COMPOSITION_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `ALLOF_COMPOSITION_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `ALLOF_COMPOSITION_PROXY` | HTTP(S) proxy URL | no |
| `ALLOF_COMPOSITION_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `ALLOF_COMPOSITION_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `allof-composition profiles set <profile> ALLOF_COMPOSITION_RETRIES=3`; the others apply to the whole process and are read from the environment only.
