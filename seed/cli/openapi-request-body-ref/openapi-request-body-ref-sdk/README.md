...
[![crates.io shield](https://img.shields.io/crates/v/openapi_request_body_ref_sdk)](https://crates.io/crates/openapi_request_body_ref_sdk)
...
```toml
[dependencies]
openapi_request_body_ref_sdk = "0.1.0"
```

Or install via cargo:

```sh
cargo add openapi_request_body_ref_sdk
```
...
```rust
match client.vendor.create_vendor(None)?.await {
    Ok(response) => {
...
```rust
let response = client.vendor.create_vendor(
