...
The `server-sent-events-openapi` CLI supports user-authored custom commands compiled
...
cli/server-sent-events-openapi/sdk.rs       ← Generated bridge: client() + block_on()
...
| `cli/server-sent-events-openapi/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
server-sent-events-openapi hello world              # top-level custom command
server-sent-events-openapi admin users list --limit 5  # nested under custom namespace
server-sent-events-openapi fetch my-id --format json   # output cohesion
server-sent-events-openapi fetch my-id --dry-run       # dry-run preview
```
