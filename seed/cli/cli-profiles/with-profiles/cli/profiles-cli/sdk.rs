...
impl profiles_cli_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> profiles_cli_sdk::api::ApiClient {
    ...
    let config = profiles_cli_sdk::ClientConfig {
        base_url: ctx.effective_base_url(),
        ..Default::default()
    };
    let http_client = profiles_cli_sdk::HttpClient::with_executor(
        adapter as Arc<dyn profiles_cli_sdk::RequestExecutor>,
        config.clone(),
    );
    profiles_cli_sdk::api::ApiClient {
        config,
        messages: profiles_cli_sdk::api::MessagesClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, profiles_cli_sdk::ApiError>>,
{
...
fn convert_api_error(e: profiles_cli_sdk::ApiError) -> CliError {
    match e {
        profiles_cli_sdk::ApiError::Http { status, message } => {
            fern_cli_sdk::error::api_error_from_body(status, &message)
        }
        profiles_cli_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        profiles_cli_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
