...
impl multi_url_environment_reference_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> multi_url_environment_reference_sdk::api::ApiClient {
...
where
    F: Future<Output = Result<T, multi_url_environment_reference_sdk::ApiError>>,
{
...
fn convert_api_error(e: multi_url_environment_reference_sdk::ApiError) -> CliError {
    match e {
        multi_url_environment_reference_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        multi_url_environment_reference_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        multi_url_environment_reference_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
