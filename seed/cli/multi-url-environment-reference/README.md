# multi-url-environment-reference CLI

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=multi-url-environment-reference%2FCLI)
...
```bash
cargo build --release
./target/release/multi-url-environment-reference --help
```
...
```bash
export MULTI_URL_ENVIRONMENT_REFERENCE_TOKEN="<your token>"
```
...
|----------|-------------|-------------|
| `MULTI_URL_ENVIRONMENT_REFERENCE_BASE_URL` | Override the API base URL | yes |
| `MULTI_URL_ENVIRONMENT_REFERENCE_OUTPUT` | Default output format when `--format` is not passed | yes |
| `MULTI_URL_ENVIRONMENT_REFERENCE_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `MULTI_URL_ENVIRONMENT_REFERENCE_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `MULTI_URL_ENVIRONMENT_REFERENCE_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `MULTI_URL_ENVIRONMENT_REFERENCE_PROXY` | HTTP(S) proxy URL | no |
| `MULTI_URL_ENVIRONMENT_REFERENCE_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `MULTI_URL_ENVIRONMENT_REFERENCE_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `multi-url-environment-reference profiles set <profile> MULTI_URL_ENVIRONMENT_REFERENCE_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
multi-url-environment-reference <resource> <method> --format json | jq

# Keep the human rendering even when piped
multi-url-environment-reference <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
multi-url-environment-reference --help --format json | jq '.operations | length'
```
...
```bash
multi-url-environment-reference completion <bash|zsh|fish|powershell>
```
