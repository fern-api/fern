...
impl optional_referenced_request_bodies_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> optional_referenced_request_bodies_sdk::api::ApiClient {
    ...
    optional_referenced_request_bodies_sdk::api::ApiClient {
        config,
...
where
    F: Future<Output = Result<T, optional_referenced_request_bodies_sdk::ApiError>>,
{
...
fn convert_api_error(e: optional_referenced_request_bodies_sdk::ApiError) -> CliError {
    match e {
        optional_referenced_request_bodies_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        optional_referenced_request_bodies_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        optional_referenced_request_bodies_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
