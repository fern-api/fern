...
The `oauth-client-credentials-openapi` CLI supports user-authored custom commands compiled
...
| Field | Type |
|-------|------|
| `client.identity` | `oauth_client_credentials_openapi_sdk::api::IdentityClient` |
| `client.plants` | `oauth_client_credentials_openapi_sdk::api::PlantsClient` |
...
```bash
cargo build
oauth-client-credentials-openapi hello world              # top-level custom command
oauth-client-credentials-openapi admin users list --limit 5  # nested under custom namespace
oauth-client-credentials-openapi fetch my-id --format json   # output cohesion
oauth-client-credentials-openapi fetch my-id --dry-run       # dry-run preview
```
