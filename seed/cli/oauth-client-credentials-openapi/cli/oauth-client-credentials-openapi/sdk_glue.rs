...
impl oauth_client_credentials_openapi_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> oauth_client_credentials_openapi_sdk::api::ApiClient {
...
where
    F: Future<Output = Result<T, oauth_client_credentials_openapi_sdk::ApiError>>,
{
...
fn convert_api_error(e: oauth_client_credentials_openapi_sdk::ApiError) -> CliError {
    match e {
        oauth_client_credentials_openapi_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        oauth_client_credentials_openapi_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        oauth_client_credentials_openapi_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
