...
impl plant_store_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> plant_store_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = plant_store_sdk::ClientConfig::default();
    let http_client = plant_store_sdk::HttpClient::with_executor(
        adapter as Arc<dyn plant_store_sdk::RequestExecutor>,
        config.clone(),
    );
    plant_store_sdk::api::ApiClient {
        config,
        identity: plant_store_sdk::api::IdentityClient { http_client: http_client.clone() },
        plants: plant_store_sdk::api::PlantsClient { http_client: http_client.clone() },
    }
}
...
where
    F: Future<Output = Result<T, plant_store_sdk::ApiError>>,
{
...
fn convert_api_error(e: plant_store_sdk::ApiError) -> CliError {
    match e {
        plant_store_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        plant_store_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        plant_store_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
