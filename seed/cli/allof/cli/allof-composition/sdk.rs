...
impl allof_composition_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
where
    F: Future<Output = Result<T, allof_composition_sdk::ApiError>>,
{
...
fn convert_api_error(e: allof_composition_sdk::ApiError) -> CliError {
    match e {
        allof_composition_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
