...
The `allof-composition` CLI supports user-authored custom commands compiled
...
cli/allof-composition/sdk.rs       ← Generated bridge: client() + block_on()
...
| `cli/allof-composition/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
allof-composition hello world              # top-level custom command
allof-composition admin users list --limit 5  # nested under custom namespace
allof-composition fetch my-id --format json   # output cohesion
allof-composition fetch my-id --dry-run       # dry-run preview
```
