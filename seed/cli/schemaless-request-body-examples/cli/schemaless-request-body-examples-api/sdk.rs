...
impl schemaless_request_body_examples_api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> Result<schemaless_request_body_examples_api_sdk::api::ApiClient, CliError> {
    let executor = ctx.build_sdk_executor()?;
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = schemaless_request_body_examples_api_sdk::ClientConfig::default();
    let http_client = schemaless_request_body_examples_api_sdk::HttpClient::with_executor(
        adapter as Arc<dyn schemaless_request_body_examples_api_sdk::RequestExecutor>,
        config.clone(),
    );
    Ok(schemaless_request_body_examples_api_sdk::api::ApiClient {
        config,
...
where
    F: Future<Output = Result<T, schemaless_request_body_examples_api_sdk::ApiError>>,
{
...
fn convert_api_error(e: schemaless_request_body_examples_api_sdk::ApiError) -> CliError {
    match e {
        schemaless_request_body_examples_api_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        schemaless_request_body_examples_api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        schemaless_request_body_examples_api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
