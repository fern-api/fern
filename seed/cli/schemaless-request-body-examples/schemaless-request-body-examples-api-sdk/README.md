...
[![crates.io shield](https://img.shields.io/crates/v/schemaless_request_body_examples_api_sdk)](https://crates.io/crates/schemaless_request_body_examples_api_sdk)
...
```toml
[dependencies]
schemaless_request_body_examples_api_sdk = "0.1.0"
```

Or install via cargo:

```sh
cargo add schemaless_request_body_examples_api_sdk
```
...
```rust
match client.create_plant(None)?.await {
    Ok(response) => {
...
```rust
let response = client.create_plant(
