...
impl discriminated_union_with_nested_oneof_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn client(ctx: &AppContext) -> discriminated_union_with_nested_oneof_sdk::api::ApiClient {
    let executor = ctx.build_sdk_executor();
    let adapter = Arc::new(CliExecutorAdapter(executor));
    let config = discriminated_union_with_nested_oneof_sdk::ClientConfig::default();
    let http_client = discriminated_union_with_nested_oneof_sdk::HttpClient::with_executor(
        adapter as Arc<dyn discriminated_union_with_nested_oneof_sdk::RequestExecutor>,
        config.clone(),
    );
    discriminated_union_with_nested_oneof_sdk::api::ApiClient {
        config,
...
where
    F: Future<Output = Result<T, discriminated_union_with_nested_oneof_sdk::ApiError>>,
{
...
fn convert_api_error(e: discriminated_union_with_nested_oneof_sdk::ApiError) -> CliError {
    match e {
        discriminated_union_with_nested_oneof_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        discriminated_union_with_nested_oneof_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        discriminated_union_with_nested_oneof_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
