...
|----------|-------------|
| `PLANT_STORE_API_BASE_URL` | Override the API base URL |
| `PLANT_STORE_API_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) |
| `PLANT_STORE_API_INSECURE=1` | Skip TLS verification (debugging only) |
| `PLANT_STORE_API_PROXY` | HTTP(S) proxy URL |
| `PLANT_STORE_API_TIMEOUT_SECS` | Total request timeout in seconds |
