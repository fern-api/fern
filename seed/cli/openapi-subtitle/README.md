...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Plant%20Store%20API%2FCLI)

Command-line interface for the Plant Store API.
...
```bash
cargo build --release
./target/release/plant-store-api --help
```

## Authentication

This API requires authentication. Run `plant-store-api --help` for details.
...
```bash
plant-store-api --help
```

Call an API endpoint:

```bash
plant-store-api <resource> <method>
```

Run `plant-store-api <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `plant-store-api <resource> <method>`). Run `plant-store-api <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
plant-store-api <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|-------------|
| `PLANT_STORE_API_BASE_URL` | Override the API base URL | yes |
| `PLANT_STORE_API_OUTPUT` | Default output format when `--format` is not passed | yes |
| `PLANT_STORE_API_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `PLANT_STORE_API_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `PLANT_STORE_API_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `PLANT_STORE_API_PROXY` | HTTP(S) proxy URL | no |
| `PLANT_STORE_API_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `PLANT_STORE_API_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `plant-store-api profiles set <profile> PLANT_STORE_API_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
plant-store-api <resource> <method> --format json | jq

# Keep the human rendering even when piped
plant-store-api <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
plant-store-api --help --format json | jq '.operations | length'
```
...
```bash
plant-store-api completion <bash|zsh|fish|powershell>
```
