...
The `plant-store-api` CLI supports user-authored custom commands compiled
...
cli/plant-store-api/sdk.rs       ← Generated bridge: client() + block_on()
...
| `cli/plant-store-api/sdk.rs` | Yes | Bridges AppContext → SDK client |
...
```bash
cargo build
plant-store-api hello world              # top-level custom command
plant-store-api admin users list --limit 5  # nested under custom namespace
plant-store-api fetch my-id --format json   # output cohesion
plant-store-api fetch my-id --dry-run       # dry-run preview
```
