...
impl plant_store_api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> plant_store_api_sdk::api::ApiClient {
...
where
    F: Future<Output = Result<T, plant_store_api_sdk::ApiError>>,
{
...
fn convert_api_error(e: plant_store_api_sdk::ApiError) -> CliError {
    match e {
        plant_store_api_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        plant_store_api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        plant_store_api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
