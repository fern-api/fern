...
impl url_form_encoded_api_sdk::RequestExecutor for CliExecutorAdapter {
    fn execute(
        &self,
...
pub fn sdk_client(ctx: &AppContext) -> url_form_encoded_api_sdk::api::ApiClient {
...
where
    F: Future<Output = Result<T, url_form_encoded_api_sdk::ApiError>>,
{
...
fn convert_api_error(e: url_form_encoded_api_sdk::ApiError) -> CliError {
    match e {
        url_form_encoded_api_sdk::ApiError::Http { status, message } => CliError::Api {
            code: status,
            message,
            reason: http_status_reason(status).to_string(),
        },
        url_form_encoded_api_sdk::ApiError::Network(err) => {
            CliError::Other(anyhow::anyhow!("SDK network error: {err}"))
        }
        url_form_encoded_api_sdk::ApiError::Executor(boxed) => match boxed.downcast::<SdkError>() {
