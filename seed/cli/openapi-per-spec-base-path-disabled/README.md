...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-CLI%20generated%20by%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=OAuth%20API%2FCLI)

Command-line interface for the OAuth API.
...
```bash
export OAUTH_API_CLIENT_ID="<your OAuth client credential>"
export OAUTH_API_CLIENT_SECRET="<your OAuth client credential>"
```
...
```bash
oauth-api <resource> <method>
```

Run `oauth-api <resource> --help` to see available methods for a resource.

## Usage

Every API resource appears as a subcommand (e.g. `oauth-api <resource> <method>`). Run `oauth-api <resource> --help` to see available methods.

Provide request parameters as flags or as JSON:

```bash
oauth-api <resource> <method> --json '{"key": "value"}'
```
...
|----------|-------------|-------------|
| `OAUTH_API_BASE_URL` | Override the API base URL | yes |
| `OAUTH_API_OUTPUT` | Default output format when `--format` is not passed | yes |
| `OAUTH_API_RETRIES` | Retry attempts for failed requests (`--retries` wins) | yes |
| `OAUTH_API_CA_BUNDLE` | Path to PEM file with extra trust roots (or `SSL_CERT_FILE`) | no |
| `OAUTH_API_INSECURE=1` | Skip TLS verification (debugging only) | no |
| `OAUTH_API_PROXY` | HTTP(S) proxy URL | no |
| `OAUTH_API_TIMEOUT_SECS` | Total request timeout in seconds | no |
| `OAUTH_API_CONNECT_TIMEOUT_SECS` | Connection timeout in seconds | no |

Variables marked "yes" can also be stored on a profile with `oauth-api profiles set <profile> OAUTH_API_RETRIES=3`; the others apply to the whole process and are read from the environment only.
...
oauth-api <resource> <method> --format json | jq

# Keep the human rendering even when piped
oauth-api <resource> <method> --human | less

# Machine-readable catalog of every operation (same as --schema)
oauth-api --help --format json | jq '.operations | length'
```
...
```bash
oauth-api completion <bash|zsh|fish|powershell>
```
