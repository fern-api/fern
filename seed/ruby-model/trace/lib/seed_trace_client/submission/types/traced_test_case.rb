...
module SeedTraceClient
  class Submission
    class TracedTestCase
      attr_reader :result, :trace_responses_size, :additional_properties, :_field_set
      ...
      def initialize(result:, trace_responses_size:, additional_properties: nil)
        # @type [SeedTraceClient::Submission::TestCaseResultWithStdout]
        @result = result
        # @type [Integer]
        @trace_responses_size = trace_responses_size
        @_field_set = { "result": @result, "traceResponsesSize": @trace_responses_size }.reject do |_k, v|
          v == OMIT
        end
      end
