...
public record QueryColumn
{
    ...
    internal static QueryColumn FromProto(ProtoDataV1Grpc.QueryColumn value) {
        return new QueryColumn{Values = value.Values?.ToList() ?? Enumerable.Empty<float>(), TopK = value.TopK, Namespace = value.Namespace, Filter = value.Filter != null ? Metadata.FromProto(value.Filter) : null, IndexedData = value.IndexedData != null ? IndexedData.FromProto(value.IndexedData) : null};
    }
