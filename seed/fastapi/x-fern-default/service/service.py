# This file was auto-generated by Fern from our API Definition.

import abc
import functools
import inspect
import logging
import typing

import fastapi
from ..core.abstract_fern_service import AbstractFernService
from ..core.exceptions.fern_http_exception import FernHTTPException
from ..core.route_args import get_route_args
from ..types.test_get_response import TestGetResponse


class AbstractRootService(AbstractFernService):
    """
    AbstractRootService is an abstract class containing the methods that you should implement.

    Each method is associated with an API route, which will be registered
    with FastAPI when you register your implementation using Fern's register()
    function.
    """

    @abc.abstractmethod
    def test_get(
        self, *, region: str, limit: typing.Optional[str] = None, api_version: typing.Optional[str] = None
    ) -> TestGetResponse: ...

    """
    Below are internal methods used by Fern to register your implementation.
    You can ignore them.
    """

    @classmethod
    def _init_fern(cls, router: fastapi.APIRouter) -> None:
        cls.__init_test_get(router=router)

    @classmethod
    def __init_test_get(cls, router: fastapi.APIRouter) -> None:
        endpoint_function = inspect.signature(cls.test_get)
        type_hints = typing.get_type_hints(cls.test_get)

        new_parameters: typing.List[inspect.Parameter] = []
        for index, (parameter_name, parameter) in enumerate(endpoint_function.parameters.items()):
            # Get the resolved type hint for this parameter, as fastapi does not handle forward refs in all cases
            resolved_annotation = type_hints.get(parameter_name, parameter.annotation)

            if index == 0:
                new_parameters.append(parameter.replace(default=fastapi.Depends(cls)))
            elif parameter_name == "region":
                new_parameters.append(
                    parameter.replace(annotation=typing.Annotated[resolved_annotation, fastapi.Path()])
                )
            elif parameter_name == "limit":
                new_parameters.append(
                    parameter.replace(annotation=typing.Annotated[resolved_annotation, fastapi.Query()], default=None)
                )
            elif parameter_name == "api_version":
                new_parameters.append(
                    parameter.replace(
                        annotation=typing.Annotated[resolved_annotation, fastapi.Header(alias="X-API-Version")],
                        default=None,
                    )
                )
            else:
                new_parameters.append(parameter)
        setattr(cls.test_get, "__signature__", endpoint_function.replace(parameters=new_parameters))

        @functools.wraps(cls.test_get)
        def wrapper(*args: typing.Any, **kwargs: typing.Any) -> TestGetResponse:
            try:
                return cls.test_get(*args, **kwargs)
            except FernHTTPException as e:
                logging.getLogger(f"{cls.__module__}.{cls.__name__}").warn(
                    f"Endpoint 'test_get' unexpectedly threw {e.__class__.__name__}. "
                    + f"If this was intentional, please add {e.__class__.__name__} to "
                    + "the endpoint's errors list in your Fern Definition."
                )
                raise e

        router.get(
            path="/test/{region}/resource",
            response_model=None,
            description=AbstractRootService.test_get.__doc__,
            **get_route_args(cls.test_get, default_tag=""),
        )(wrapper)
