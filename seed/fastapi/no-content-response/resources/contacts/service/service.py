# This file was auto-generated by Fern from our API Definition.

import abc
import functools
import inspect
import logging
import typing

import fastapi
from ....core.abstract_fern_service import AbstractFernService
from ....core.exceptions.fern_http_exception import FernHTTPException
from ....core.route_args import get_route_args
from ....types.contact import Contact
from .create_contact_request import CreateContactRequest


class AbstractContactsService(AbstractFernService):
    """
    AbstractContactsService is an abstract class containing the methods that you should implement.

    Each method is associated with an API route, which will be registered
    with FastAPI when you register your implementation using Fern's register()
    function.
    """

    @abc.abstractmethod
    def create(self, *, body: CreateContactRequest) -> typing.Optional[Contact]:
        """
        Creates a new contact. Returns 200 with the contact or 204 with no content.
        """
        ...

    @abc.abstractmethod
    def get(self, *, id: str) -> Contact:
        """
        Gets a contact by ID. Returns 200 with the contact.
        """
        ...

    """
    Below are internal methods used by Fern to register your implementation.
    You can ignore them.
    """

    @classmethod
    def _init_fern(cls, router: fastapi.APIRouter) -> None:
        cls.__init_create(router=router)
        cls.__init_get(router=router)

    @classmethod
    def __init_create(cls, router: fastapi.APIRouter) -> None:
        endpoint_function = inspect.signature(cls.create)
        type_hints = typing.get_type_hints(cls.create)

        new_parameters: typing.List[inspect.Parameter] = []
        for index, (parameter_name, parameter) in enumerate(endpoint_function.parameters.items()):
            # Get the resolved type hint for this parameter, as fastapi does not handle forward refs in all cases
            resolved_annotation = type_hints.get(parameter_name, parameter.annotation)

            if index == 0:
                new_parameters.append(parameter.replace(default=fastapi.Depends(cls)))
            elif parameter_name == "body":
                new_parameters.append(
                    parameter.replace(annotation=typing.Annotated[resolved_annotation, fastapi.Body()])
                )
            else:
                new_parameters.append(parameter)
        setattr(cls.create, "__signature__", endpoint_function.replace(parameters=new_parameters))

        @functools.wraps(cls.create)
        def wrapper(*args: typing.Any, **kwargs: typing.Any) -> typing.Optional[Contact]:
            try:
                return cls.create(*args, **kwargs)
            except FernHTTPException as e:
                logging.getLogger(f"{cls.__module__}.{cls.__name__}").warn(
                    f"Endpoint 'create' unexpectedly threw {e.__class__.__name__}. "
                    + f"If this was intentional, please add {e.__class__.__name__} to "
                    + "the endpoint's errors list in your Fern Definition."
                )
                raise e

        router.post(
            path="/contacts",
            response_model=None,
            description=AbstractContactsService.create.__doc__,
            **get_route_args(cls.create, default_tag="contacts"),
        )(wrapper)

    @classmethod
    def __init_get(cls, router: fastapi.APIRouter) -> None:
        endpoint_function = inspect.signature(cls.get)
        type_hints = typing.get_type_hints(cls.get)

        new_parameters: typing.List[inspect.Parameter] = []
        for index, (parameter_name, parameter) in enumerate(endpoint_function.parameters.items()):
            # Get the resolved type hint for this parameter, as fastapi does not handle forward refs in all cases
            resolved_annotation = type_hints.get(parameter_name, parameter.annotation)

            if index == 0:
                new_parameters.append(parameter.replace(default=fastapi.Depends(cls)))
            elif parameter_name == "id":
                new_parameters.append(
                    parameter.replace(annotation=typing.Annotated[resolved_annotation, fastapi.Path()])
                )
            else:
                new_parameters.append(parameter)
        setattr(cls.get, "__signature__", endpoint_function.replace(parameters=new_parameters))

        @functools.wraps(cls.get)
        def wrapper(*args: typing.Any, **kwargs: typing.Any) -> Contact:
            try:
                return cls.get(*args, **kwargs)
            except FernHTTPException as e:
                logging.getLogger(f"{cls.__module__}.{cls.__name__}").warn(
                    f"Endpoint 'get' unexpectedly threw {e.__class__.__name__}. "
                    + f"If this was intentional, please add {e.__class__.__name__} to "
                    + "the endpoint's errors list in your Fern Definition."
                )
                raise e

        router.get(
            path="/contacts/{id}",
            response_model=None,
            description=AbstractContactsService.get.__doc__,
            **get_route_args(cls.get, default_tag="contacts"),
        )(wrapper)
