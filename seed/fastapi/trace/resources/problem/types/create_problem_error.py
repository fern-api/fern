...
class _Factory:
    ...
    def generic(self, value: GenericCreateProblemError) -> CreateProblemError:
        if IS_PYDANTIC_V2:
            return CreateProblemError(root=_CreateProblemError.Generic(**value.dict(exclude_unset=True), error_type="generic"))  # type: ignore
        else:
            return CreateProblemError(__root__=_CreateProblemError.Generic(**value.dict(exclude_unset=True), error_type="generic"))  # type: ignore
class CreateProblemError(UniversalRootModel):
    ...
    def visit(self, generic: typing.Callable[[GenericCreateProblemError], T_Result]) -> T_Result:
        ...
        if unioned_value.error_type == "generic":
            return generic(
            GenericCreateProblemError(**unioned_value.dict(exclude_unset=True, exclude={"_type"})))
