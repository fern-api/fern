...
class _Factory:
    ...
    def running(self, value: RunningSubmissionState) -> WorkspaceSubmissionUpdateInfo:
        if IS_PYDANTIC_V2:
            return WorkspaceSubmissionUpdateInfo(root=_WorkspaceSubmissionUpdateInfo.Running(type="running", value=value))  # type: ignore
        else:
            return WorkspaceSubmissionUpdateInfo(__root__=_WorkspaceSubmissionUpdateInfo.Running(type="running", value=value))  # type: ignore
    
    def ran(self, value: WorkspaceRunDetails) -> WorkspaceSubmissionUpdateInfo:
        if IS_PYDANTIC_V2:
            return WorkspaceSubmissionUpdateInfo(root=_WorkspaceSubmissionUpdateInfo.Ran(**value.dict(exclude_unset=True), type="ran"))  # type: ignore
        else:
            return WorkspaceSubmissionUpdateInfo(__root__=_WorkspaceSubmissionUpdateInfo.Ran(**value.dict(exclude_unset=True), type="ran"))  # type: ignore
    ...
    def traced_v_2(self, value: WorkspaceTracedUpdate) -> WorkspaceSubmissionUpdateInfo:
        if IS_PYDANTIC_V2:
            return WorkspaceSubmissionUpdateInfo(root=_WorkspaceSubmissionUpdateInfo.TracedV2(**value.dict(exclude_unset=True), type="tracedV2"))  # type: ignore
        else:
            return WorkspaceSubmissionUpdateInfo(__root__=_WorkspaceSubmissionUpdateInfo.TracedV2(**value.dict(exclude_unset=True), type="tracedV2"))  # type: ignore
    
    def errored(self, value: ErrorInfo) -> WorkspaceSubmissionUpdateInfo:
        if IS_PYDANTIC_V2:
            return WorkspaceSubmissionUpdateInfo(root=_WorkspaceSubmissionUpdateInfo.Errored(type="errored", value=value))  # type: ignore
        else:
            return WorkspaceSubmissionUpdateInfo(__root__=_WorkspaceSubmissionUpdateInfo.Errored(type="errored", value=value))  # type: ignore
...
class WorkspaceSubmissionUpdateInfo(UniversalRootModel):
    ...
    def visit(self, running: typing.Callable[[RunningSubmissionState], T_Result], ran: typing.Callable[[WorkspaceRunDetails], T_Result], stopped: typing.Callable[[], T_Result], traced: typing.Callable[[], T_Result], traced_v_2: typing.Callable[[WorkspaceTracedUpdate], T_Result], errored: typing.Callable[[ErrorInfo], T_Result], finished: typing.Callable[[], T_Result]) -> T_Result:
        ...
        if unioned_value.type == "ran":
            return ran(
            WorkspaceRunDetails(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
        ...
        if unioned_value.type == "tracedV2":
            return traced_v_2(
            WorkspaceTracedUpdate(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
