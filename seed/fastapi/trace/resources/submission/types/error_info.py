...
class ErrorInfo(UniversalRootModel):
    ...
    if IS_PYDANTIC_V2:
        root: typing_extensions.Annotated[typing.Union[_ErrorInfo.CompileError, _ErrorInfo.RuntimeError, _ErrorInfo.InternalError], pydantic.Field(discriminator="type")]
        def get_as_union(self) -> typing.Union[_ErrorInfo.CompileError, _ErrorInfo.RuntimeError, _ErrorInfo.InternalError]:
            return self.root
    else:
        __root__: typing_extensions.Annotated[typing.Union[_ErrorInfo.CompileError, _ErrorInfo.RuntimeError, _ErrorInfo.InternalError], pydantic.Field(discriminator="type")]
        def get_as_union(self) -> typing.Union[_ErrorInfo.CompileError, _ErrorInfo.RuntimeError, _ErrorInfo.InternalError]:
    ...
    def visit(self, compile_error: typing.Callable[[resources_submission_types_compile_error_CompileError], T_Result], runtime_error: typing.Callable[[resources_submission_types_runtime_error_RuntimeError], T_Result], internal_error: typing.Callable[[resources_submission_types_internal_error_InternalError], T_Result]) -> T_Result:
        unioned_value = self.get_as_union()
        if unioned_value.type == "compileError":
            return compile_error(
            resources_submission_types_compile_error_CompileError(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
        if unioned_value.type == "runtimeError":
            return runtime_error(
            resources_submission_types_runtime_error_RuntimeError(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
        if unioned_value.type == "internalError":
            return internal_error(
            resources_submission_types_internal_error_InternalError(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
class _ErrorInfo:
