...
class _Factory:
    ...
    def metadata(self, value: resources_commons_resources_types_types_metadata_Metadata) -> EventInfo:
        if IS_PYDANTIC_V2:
            return EventInfo(root=_EventInfo.Metadata(**value.dict(exclude_unset=True), type="metadata"))  # type: ignore
        else:
            return EventInfo(__root__=_EventInfo.Metadata(**value.dict(exclude_unset=True), type="metadata"))  # type: ignore
...
class EventInfo(UniversalRootModel):
    ...
    if IS_PYDANTIC_V2:
        root: typing_extensions.Annotated[typing.Union[_EventInfo.Metadata, _EventInfo.Tag], pydantic.Field(discriminator="type")]
        def get_as_union(self) -> typing.Union[_EventInfo.Metadata, _EventInfo.Tag]:
            return self.root
    else:
        __root__: typing_extensions.Annotated[typing.Union[_EventInfo.Metadata, _EventInfo.Tag], pydantic.Field(discriminator="type")]
        def get_as_union(self) -> typing.Union[_EventInfo.Metadata, _EventInfo.Tag]:
    ...
    def visit(self, metadata: typing.Callable[[resources_commons_resources_types_types_metadata_Metadata], T_Result], tag: typing.Callable[[resources_commons_resources_types_types_tag_Tag], T_Result]) -> T_Result:
        unioned_value = self.get_as_union()
        if unioned_value.type == "metadata":
            return metadata(
            resources_commons_resources_types_types_metadata_Metadata(**unioned_value.dict(exclude_unset=True, exclude={"type"})))
