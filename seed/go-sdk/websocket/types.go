...
var (
	flushedEventFieldType = big.NewInt(1 << 0)
)

type FlushedEvent struct {
	Type FlushedEventType `json:"type" url:"type"`
...
func (f *FlushedEvent) GetType() FlushedEventType {
	if f == nil {
		return ""
	}
	return f.Type
...
// this prevents an empty or null value for this field from being omitted during serialization.
func (f *FlushedEvent) SetType(type_ FlushedEventType) {
	f.Type = type_
	f.require(flushedEventFieldType)
}

func (f *FlushedEvent) UnmarshalJSON(data []byte) error {
	type unmarshaler FlushedEvent
	var value unmarshaler
	if err := json.Unmarshal(data, &value); err != nil {
		return err
	}
	*f = FlushedEvent(value)
...
type FlushedEventType string

const (
	FlushedEventTypeFlushed FlushedEventType = "flushed"
)

func NewFlushedEventTypeFromString(s string) (FlushedEventType, error) {
	switch s {
	case "flushed":
		return FlushedEventTypeFlushed, nil
	}
	var t FlushedEventType
	return "", fmt.Errorf("%s is not a valid %T", s, t)
}

func (f FlushedEventType) Ptr() *FlushedEventType {
	return &f
}
...
var (
	transcriptEventFieldType = big.NewInt(1 << 0)
	transcriptEventFieldData = big.NewInt(1 << 1)
)

type TranscriptEvent struct {
	Type TranscriptEventType `json:"type" url:"type"`
...
func (t *TranscriptEvent) GetType() TranscriptEventType {
	if t == nil {
		return ""
	}
	return t.Type
...
// SetType sets the Type field and marks it as non-optional;
// this prevents an empty or null value for this field from being omitted during serialization.
func (t *TranscriptEvent) SetType(type_ TranscriptEventType) {
	t.Type = type_
	t.require(transcriptEventFieldType)
}
...
func (t *TranscriptEvent) UnmarshalJSON(data []byte) error {
	type unmarshaler TranscriptEvent
	var value unmarshaler
	if err := json.Unmarshal(data, &value); err != nil {
		return err
	}
	*t = TranscriptEvent(value)
...
const (
	TranscriptEventTypeTranscript TranscriptEventType = "transcript"
)

func NewTranscriptEventTypeFromString(s string) (TranscriptEventType, error) {
	switch s {
	case "transcript":
		return TranscriptEventTypeTranscript, nil
	}
	var t TranscriptEventType
	return "", fmt.Errorf("%s is not a valid %T", s, t)
}

func (t TranscriptEventType) Ptr() *TranscriptEventType {
	return &t
}
