...
func TestCustomAuthGetWithCustomAuthWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/custom-auth")).WillReturnResponse(
	...
	_, invocationErr := client.CustomAuth.GetWithCustomAuth(
		context.TODO(),
	)
...
func TestCustomAuthPostWithCustomAuthWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/custom-auth")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			true,
		).WithStatus(http.StatusOK),
	)
	...
	_, invocationErr := client.CustomAuth.PostWithCustomAuth(
		context.TODO(),
		request,
	)
