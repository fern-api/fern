...
func TestEndpointsContentTypePostJsonPatchContentTypeWithWireMock(
	...
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/foo/bar")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
...
func TestEndpointsContentTypePostJsonPatchContentWithCharsetTypeWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/foo/baz")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{},
		).WithStatus(http.StatusOK),
	)
