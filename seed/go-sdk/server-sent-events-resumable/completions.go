...
var (
	streamCompletionRequestNonResumableFieldQuery = big.NewInt(1 << 0)
)

type StreamCompletionRequestNonResumable struct {
	Query string `json:"query" url:"-"`
...
// SetQuery sets the Query field and marks it as non-optional;
// this prevents an empty or null value for this field from being omitted during serialization.
func (s *StreamCompletionRequestNonResumable) SetQuery(query string) {
	s.Query = query
	s.require(streamCompletionRequestNonResumableFieldQuery)
}

func (s *StreamCompletionRequestNonResumable) UnmarshalJSON(data []byte) error {
	type unmarshaler StreamCompletionRequestNonResumable
	var body unmarshaler
	if err := json.Unmarshal(data, &body); err != nil {
		return err
	}
	*s = StreamCompletionRequestNonResumable(body)
	return nil
}

func (s *StreamCompletionRequestNonResumable) MarshalJSON() ([]byte, error) {
	type embed StreamCompletionRequestNonResumable
	var marshaler = struct {
		embed
	}{
		embed: embed(*s),
