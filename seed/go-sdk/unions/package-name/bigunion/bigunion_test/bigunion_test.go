...
func TestBigunionGetWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/{id}")).WithPathParam(
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"type": "normalSweet", "value": "value", "id": "id", "created-at": "2024-01-15T09:30:00Z", "archived-at": "2024-01-15T09:30:00Z"},
...
func TestBigunionUpdateWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	...
	_, invocationErr := client.Bigunion.Update(
		context.TODO(),
		request,
	)
...
func TestBigunionUpdateManyWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/many")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"string": true},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
	...
	_, invocationErr := client.Bigunion.UpdateMany(
		context.TODO(),
		request,
	)
