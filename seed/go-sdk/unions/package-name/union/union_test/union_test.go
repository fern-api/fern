...
func TestBigunionGetWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"type": "circle", "radius": 1.1, "id": "id"},
	...
	_, invocationErr := client.Bigunion.Get(
		context.TODO(),
		"id",
	)
...
func TestUnionUpdateWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			true,
		).WithStatus(http.StatusOK),
	)
	...
	_, invocationErr := client.Union.Update(
		context.TODO(),
		request,
	)
