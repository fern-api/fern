...
func (i *InferredAuthProvider) SetToken(accessToken string, expiresIn int) {
	...
	if expiresIn > 0 {
		...
		// Unlike OAuth (which treats missing expiry as "never expires"), inferred auth
		// applies a default expiry to ensure periodic token refresh, since token endpoints
		// that omit expiry typically still issue short-lived tokens.
