...
```go
response, err := client.Reports.Create(...)
if err != nil {
...
```go
response, err := client.Reports.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.Reports.Create(ctx, ...)
```
...
response, err := client.Reports.Create(ctx, request, ...)
```
