...
var (
	generateRequestFieldStream    = big.NewInt(1 << 0)
	generateRequestFieldNumEvents = big.NewInt(1 << 1)
)
...
func (g *GenerateRequest) SetStream(stream bool) {
	g.Stream = stream
	g.require(generateRequestFieldStream)
}

// SetNumEvents sets the NumEvents field and marks it as non-optional;
// this prevents an empty or null value for this field from being omitted during serialization.
func (g *GenerateRequest) SetNumEvents(numEvents int) {
	g.NumEvents = numEvents
	g.require(generateRequestFieldNumEvents)
}
