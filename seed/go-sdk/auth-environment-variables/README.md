...
```go
response, err := client.Service.WithRawResponse.GetWithApiKey(...)
if err != nil {
    return err
}
...
response, err := client.Service.GetWithApiKey(ctx, ...)
```
...
response, err := client.Service.GetWithApiKey(ctx, request, ...)
```
