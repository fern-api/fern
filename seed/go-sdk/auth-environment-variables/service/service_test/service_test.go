...
func TestServiceGetWithApiKeyWithWireMock(
	t *testing.T,
) {
	...
	_, invocationErr := client.Service.GetWithApiKey(
		context.TODO(),
	)
...
func TestServiceGetWithHeaderWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/apiKeyInHeader")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			"string",
		).WithStatus(http.StatusOK),
	)
	...
	_, invocationErr := client.Service.GetWithHeader(
		context.TODO(),
		request,
	)
