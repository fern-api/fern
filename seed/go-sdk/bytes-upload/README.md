...
```go
response, err := client.Service.Upload(...)
if err != nil {
...
```go
response, err := client.Service.WithRawResponse.Upload(...)
if err != nil {
    return err
}
...
response, err := client.Service.Upload(ctx, ...)
```
...
response, err := client.Service.Upload(ctx, request, ...)
```
