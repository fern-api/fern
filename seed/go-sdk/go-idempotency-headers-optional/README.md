...
```go
response, err := client.Service.Create(...)
if err != nil {
...
```go
response, err := client.Service.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.Service.Create(ctx, ...)
```
...
response, err := client.Service.Create(ctx, request, ...)
```
