...
```go
response, err := client.WithRawResponse.UpdateProfileIdentifier(...)
if err != nil {
    return err
}
...
response, err := client.UpdateProfileIdentifier(ctx, ...)
```
...
response, err := client.UpdateProfileIdentifier(ctx, request, ...)
```
