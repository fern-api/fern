...
func TestFolderServiceEndpointWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/service")).WillReturnResponse(
	...
	invocationErr := client.Folder.Service.Endpoint(
		context.TODO(),
	)
...
func TestFolderServiceUnknownRequestWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/service")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{},
		).WithStatus(http.StatusOK),
	)
	...
	invocationErr := client.Folder.Service.UnknownRequest(
		context.TODO(),
		request,
	)
