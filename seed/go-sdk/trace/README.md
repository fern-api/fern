...
```go
client := client.NewClient(
    option.WithBaseURL(trace.Environments.Prod),
)
```
...
```go
response, err := client.Admin.UpdateTestSubmissionStatus(...)
if err != nil {
...
```go
response, err := client.Admin.WithRawResponse.UpdateTestSubmissionStatus(...)
if err != nil {
    return err
}
...
response, err := client.Admin.UpdateTestSubmissionStatus(ctx, ...)
```
...
response, err := client.Admin.UpdateTestSubmissionStatus(ctx, request, ...)
```
