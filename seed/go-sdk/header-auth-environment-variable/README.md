...
client := client.NewClient(
    option.WithHeaderTokenAuth("YOUR_HEADER_VALUE"),
...
```go
response, err := client.Service.WithRawResponse.GetWithBearerToken(...)
if err != nil {
    return err
}
...
response, err := client.Service.GetWithBearerToken(ctx, ...)
```
...
response, err := client.Service.GetWithBearerToken(ctx, request, ...)
```
