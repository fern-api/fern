...
```go
response, err := client.Widgets.Create(...)
if err != nil {
...
```go
response, err := client.Widgets.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.Widgets.Create(ctx, ...)
```
...
response, err := client.Widgets.Create(ctx, request, ...)
```
