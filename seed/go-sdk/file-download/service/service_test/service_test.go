...
func TestServiceSimpleWithWireMock(
	...
	invocationErr := client.Service.Simple(
		...
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestServiceSimpleWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestServiceSimpleWithWireMock", "POST", "/snippet", nil, 1)
}

func TestServiceDownloadFileWithWireMock(
	t *testing.T,
) {
	WireMockBaseURL := os.Getenv("WIREMOCK_URL")
	if WireMockBaseURL == "" {
	...
	response, invocationErr := client.Service.WithRawResponse.DownloadFile(
		context.TODO(),
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestServiceDownloadFileWithWireMock"}},
		),
	)
	...
	VerifyRequestCount(t, "TestServiceDownloadFileWithWireMock", "POST", "/", nil, 1)
}
