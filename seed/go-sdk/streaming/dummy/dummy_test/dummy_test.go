...
func TestDummyGenerateStreamWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/generate-stream")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
                    "$schema": "https://json-schema.org/draft/2020-12/schema",
                    "type": "object",
                    "required": ["stream", "num_events"],
                    "properties": {
                        "stream": {"type": "boolean", "const": true}, "num_events": {"type": "integer"}
                    },
	...
	_, invocationErr := client.Dummy.GenerateStream(
		context.TODO(),
		request,
	)
...
func TestDummyGenerateWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/generate")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
                    "$schema": "https://json-schema.org/draft/2020-12/schema",
                    "type": "object",
                    "required": ["stream", "num_events"],
                    "properties": {
                        "stream": {"type": "boolean", "const": false}, "num_events": {"type": "integer"}
                    },
                    "additionalProperties": true
                }`, "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
	...
	_, invocationErr := client.Dummy.Generate(
		context.TODO(),
		request,
	)
