...
```go
response, err := client.Contacts.Create(...)
if err != nil {
...
```go
response, err := client.Contacts.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.Contacts.Create(ctx, ...)
```
...
response, err := client.Contacts.Create(ctx, request, ...)
```
