...
const (
	...
	// defaultInferredAuthExpirySeconds is used when the inferred auth response doesn't include an expires_in value.
	defaultInferredAuthExpirySeconds = 3600 // 1 hour fallback
)

// InferredAuthProvider manages inferred authentication tokens, including caching and automatic refresh.
// It works similarly to OAuthTokenProvider but for inferred auth schemes where tokens are obtained
// from a dedicated endpoint using other authentication (like API keys).
...
func (i *InferredAuthProvider) SetToken(accessToken string, expiresIn int) {
	...
	if expiresIn > 0 {
		...
		// No expiration info, use default with buffer
		effectiveExpiresIn := defaultInferredAuthExpirySeconds - (expirationBufferMinutes * 60)
		if effectiveExpiresIn < 0 {
			effectiveExpiresIn = 0
		}
		i.expiresAt = time.Now().Add(time.Duration(effectiveExpiresIn) * time.Second)
