...
const (
	// inferredAuthBufferMinutes is subtracted from the token expiration time
	// to ensure we refresh the token before it actually expires.
	inferredAuthBufferMinutes = 2

	// DefaultInferredAuthExpirySeconds is used when the token response doesn't include an expiry value.
...
func (i *InferredAuthProvider) SetToken(accessToken string, expiresIn int) {
	...
	if expiresIn > 0 {
		bufferSeconds := inferredAuthBufferMinutes * 60
		...
		// Use default expiry when no expiry info is provided
		effectiveExpiresIn := DefaultInferredAuthExpirySeconds - (inferredAuthBufferMinutes * 60)
		if effectiveExpiresIn < 0 {
			effectiveExpiresIn = 0
		}
		i.expiresAt = time.Now().Add(time.Duration(effectiveExpiresIn) * time.Second)
