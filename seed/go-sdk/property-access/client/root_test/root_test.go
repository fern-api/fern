...
func TestCreateUserWithWireMock(
	t *testing.T,
) {
	...
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/users")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"id": "id", "email": "email", "password": "password", "profile": map[string]interface{}{"name": "name", "verification": map[string]interface{}{"verified": "verified"}, "ssn": "ssn"}},
	...
	_, invocationErr := client.CreateUser(
		context.TODO(),
		request,
	)
