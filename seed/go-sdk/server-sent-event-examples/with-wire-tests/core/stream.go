...
// EventUnmarshalFunc is a function that unmarshals SSE event data based on the
// event type from the SSE envelope. This is used for protocol-level discrimination
// where the union discriminant comes from the SSE event: field rather than from
// within the JSON data payload.
type EventUnmarshalFunc[T any] func(eventType string, data []byte) (T, error)

// Stream represents a stream of messages sent from a server.
type Stream[T any] struct {
	reader         streamReader
	closer         io.Closer
	eventUnmarshal EventUnmarshalFunc[T]
}
...
// WithEventDiscriminator configures the SSE stream reader to track the
// SSE event field value. When used with WithEventUnmarshalFunc, the event
// type is passed to the custom unmarshal function for protocol-level discrimination.
...
// NewStreamWithEventUnmarshal constructs a new Stream that uses a custom event
// unmarshal function for protocol-level SSE discrimination. The eventUnmarshal
// function receives the SSE event type and raw data bytes, and returns the
// deserialized value.
func NewStreamWithEventUnmarshal[T any](response *http.Response, eventUnmarshal EventUnmarshalFunc[T], opts ...StreamOption) *Stream[T] {
	options := new(streamOptions)
	for _, opt := range opts {
		opt(options)
	}
...
func (s Stream[T]) Recv() (T, error) {
	var value T
	// For protocol-level SSE discrimination, use the event-aware reader
	// to get both the event type and data, then delegate to the custom
	// unmarshal function.
	if s.eventUnmarshal != nil {
		if sseReader, ok := s.reader.(*SseStreamReader); ok {
			eventType, data, err := sseReader.ReadEventFromStream()
			if err != nil {
				return value, err
			}
			return s.eventUnmarshal(eventType, data)
		}
	}
	rawBytes, err := s.reader.ReadFromStream()
	if err != nil {
		return value, err
	}
...
// ReadEventFromStream reads the next SSE event and returns both the event type
// and the data payload. This is used for protocol-level discrimination where
// the event type determines which variant to deserialize into.
func (s *SseStreamReader) ReadEventFromStream() (string, []byte, error) {
	event, err := s.nextEvent()
	if err != nil {
		return "", nil, err
	}
	...
	return string(event.event), event.data, nil
}
