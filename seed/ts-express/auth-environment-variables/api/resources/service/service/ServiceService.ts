    ...
    public toRouter(): express.Router {
        ...
        this.router.get("/apiKeyInHeader", async (req, res, next) => {
            try {
                ...
                if (error instanceof errors.SeedAuthEnvironmentVariablesError) {
                    console.warn(
                        `Endpoint 'getWithHeader' unexpectedly threw ${error.constructor.name}. If this was intentional, please add ${error.constructor.name} to the endpoint's errors list in your Fern Definition.`,
                    );
                    await error.send(res);
                } else {
                    res.status(500).json("Internal Server Error");
