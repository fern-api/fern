...
[![crates.io shield](https://img.shields.io/crates/v/seed_nursery_api)](https://crates.io/crates/seed_nursery_api)
...
```rust
use seed_nursery_api::{ClientConfig, NurseryApiClient};
...
```rust
use seed_nursery_api::{ClientError, ClientConfig, NurseryApiClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = NurseryApiClient::new(config)?;
...
async fn main() {
    ...
    let client = NurseryApiClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.package.test().await?;
