...
```rust
use seed_simple_api::{ClientError, ClientConfig, SimpleApiClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = SimpleApiClient::new(config)?;
...
async fn main() {
    ...
    let client = SimpleApiClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.user.get().await?;
