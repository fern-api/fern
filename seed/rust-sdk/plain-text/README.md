...
[![crates.io shield](https://img.shields.io/crates/v/seed_plain_text)](https://crates.io/crates/seed_plain_text)
...
```rust
use seed_plain_text::{ClientError, ClientConfig, PlainTextClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = PlainTextClient::new(config)?;
...
async fn main() {
    ...
    let client = PlainTextClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.service.get_text().await?;
