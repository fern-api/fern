...
[![crates.io shield](https://img.shields.io/crates/v/seed_validation)](https://crates.io/crates/seed_validation)
...
```rust
use seed_validation::{ClientConfig, ValidationClient};
...
```rust
use seed_validation::{ClientError, ClientConfig, ValidationClient};
...
async fn main() {
    ...
    let client = ValidationClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.create().await?;
