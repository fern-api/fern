...
[![crates.io shield](https://img.shields.io/crates/v/seed_extends)](https://crates.io/crates/seed_extends)
...
```rust
use seed_extends::{ClientConfig, ExtendsClient};
...
```rust
use seed_extends::{ClientError, ClientConfig, ExtendsClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = ExtendsClient::new(config)?;
...
async fn main() {
    ...
    let client = ExtendsClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.extended_inline_request_body().await?;
