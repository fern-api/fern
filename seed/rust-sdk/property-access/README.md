...
```rust
use seed_property_access::{ApiError, ClientConfig, PropertyAccessClient};
...
async fn main() -> Result<(), ApiError> {
    ...
    let client = PropertyAccessClient::new(config)?;
...
async fn main() {
    ...
    let client = PropertyAccessClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.create_user().await?;
