...
[![crates.io shield](https://img.shields.io/crates/v/seed_rust_streaming_custom_payload)](https://crates.io/crates/seed_rust_streaming_custom_payload)
...
```toml
[dependencies]
seed_rust_streaming_custom_payload = "0.0.1"
```

Or install via cargo:

```sh
cargo add seed_rust_streaming_custom_payload
```
...
```rust
match client.chunks.stream(None)?.await {
    Ok(response) => {
...
```rust
let response = client.chunks.stream(
