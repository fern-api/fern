...
impl QueryparamClient {
    ...
    pub async fn sendlist(&self, request: &SendlistQueryRequest, options: Option<RequestOptions>) -> Result<(), ApiError> {
        self.http_client.execute_request(
            Method::POST,
            "query-list",
            None,
            QueryBuilder::new().serialize_array("operand", request.operand.clone()).serialize_array("maybeOperand", request.maybe_operand.clone()).serialize_array("operandOrColor", request.operand_or_color.clone()).serialize_array("maybeOperandOrColor", request.maybe_operand_or_color.clone())
            .build(),
            options,
        ).await
    }
