...
[![crates.io shield](https://img.shields.io/crates/v/seed_enum)](https://crates.io/crates/seed_enum)
...
```rust
use seed_enum::{ClientError, ClientConfig, EnumClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = EnumClient::new(config)?;
...
async fn main() {
    ...
    let client = EnumClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.headers.send().await?;
