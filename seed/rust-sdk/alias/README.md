...
[![crates.io shield](https://img.shields.io/crates/v/seed_alias)](https://crates.io/crates/seed_alias)
...
```rust
use seed_alias::{ClientError, ClientConfig, AliasClient};
...
async fn main() {
    ...
    let client = AliasClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.get().await?;
