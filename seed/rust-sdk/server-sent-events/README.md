...
[![crates.io shield](https://img.shields.io/crates/v/seed_server_sent_events)](https://crates.io/crates/seed_server_sent_events)
...
```rust
use seed_server_sent_events::{ClientError, ClientConfig, ServerSentEventsClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = ServerSentEventsClient::new(config)?;
...
async fn main() {
    ...
    let client = ServerSentEventsClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.completions.stream().await?;
