...
[![crates.io shield](https://img.shields.io/crates/v/seed_php_global_header_env)](https://crates.io/crates/seed_php_global_header_env)
...
```toml
[dependencies]
seed_php_global_header_env = "0.0.1"
```

Or install via cargo:

```sh
cargo add seed_php_global_header_env
```
...
```rust
match client.service.get_with_api_version(None)?.await {
    Ok(response) => {
...
```rust
let response = client.service.get_with_api_version(
