...
[![crates.io shield](https://img.shields.io/crates/v/seed_accept)](https://crates.io/crates/seed_accept)

The Seed Rust library provides convenient access to the Seed APIs from Rust.
force some more updates
## Installation
...
```rust
use seed_accept::{ClientConfig, AcceptClient};
...
```rust
use seed_accept::{ApiError, ClientConfig, AcceptClient};
...
async fn main() {
    ...
    let client = AcceptClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.service.endpoint().await?;
