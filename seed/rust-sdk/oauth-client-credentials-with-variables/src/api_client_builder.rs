...
impl ApiClientBuilder {
    ...
    pub fn build(self) -> Result<OauthClientCredentialsWithVariablesClient, ClientError> {
        ...
        match (api_key, bearer_token, username, password) {
            // API Key auth
            (Some(key), None, None, None) => OauthClientCredentialsWithVariablesClient::new(self.config, Some(key)),
            // Bearer token auth  
            (None, Some(token), None, None) => OauthClientCredentialsWithVariablesClient::new(self.config, Some(token)),
            // Basic auth
            (None, None, Some(user), pass) => OauthClientCredentialsWithVariablesClient::new(self.config, Some(user), pass),
            // Multiple auth methods
            (Some(key), Some(token), None, None) => OauthClientCredentialsWithVariablesClient::new(self.config, Some(key), Some(token)),
            // No auth or unknown combination - try single parameter
            _ => OauthClientCredentialsWithVariablesClient::new(self.config),
        }
    }
}