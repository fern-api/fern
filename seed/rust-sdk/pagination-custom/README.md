...
```rust
use seed_pagination::{ClientError, ClientConfig, PaginationClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = PaginationClient::new(config)?;
...
async fn main() {
    ...
    let client = PaginationClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.users.list_usernames_custom().await?;
