...
```rust
use seed_streaming::{ClientConfig, StreamingClient};
...
```rust
use seed_streaming::{ClientError, ClientConfig, StreamingClient};
...
async fn main() {
    ...
    let client = StreamingClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.dummy.generate_stream().await?;
