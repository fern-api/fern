...
```rust
use seed_multi_line_docs::{ClientError, ClientConfig, MultiLineDocsClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = MultiLineDocsClient::new(config)?;
...
async fn main() {
    ...
    let client = MultiLineDocsClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.user.create_user().await?;
