...
[![crates.io shield](https://img.shields.io/crates/v/seed_bearer_token_environment_variable)](https://crates.io/crates/seed_bearer_token_environment_variable)
...
```toml
[dependencies]
seed_bearer_token_environment_variable = "0.1.0"
```
...
```rust
use seed_bearer_token_environment_variable::{ClientError, ClientConfig, BearerTokenEnvironmentVariableClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = BearerTokenEnvironmentVariableClient::new(config)?;
...
async fn main() {
    ...
    let client = BearerTokenEnvironmentVariableClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.service.get_with_bearer_token().await?;
