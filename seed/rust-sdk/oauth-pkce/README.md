...
[![crates.io shield](https://img.shields.io/crates/v/seed_oauth_pkce)](https://crates.io/crates/seed_oauth_pkce)
...
```toml
[dependencies]
seed_oauth_pkce = "0.0.1"
```

Or install via cargo:

```sh
cargo add seed_oauth_pkce
```
...
```rust
match client.oauth.authorize(None)?.await {
    Ok(response) => {
...
```rust
let response = client.oauth.authorize(
