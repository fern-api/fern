...
async fn main() {
    ...
    client.optional_send_optional_body(Some(todo!("Unhandled type reference"))).await;
}
```
...
```rust
use seed_objects_with_imports::{ApiError, ClientConfig, ObjectsWithImportsClient};
...
async fn main() -> Result<(), ApiError> {
    ...
    let client = ObjectsWithImportsClient::new(config)?;
...
async fn main() {
    ...
    let client = ObjectsWithImportsClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.optional.send_optional_body().await?;
