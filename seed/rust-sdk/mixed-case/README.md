...
[![crates.io shield](https://img.shields.io/crates/v/seed_mixed_case)](https://crates.io/crates/seed_mixed_case)
...
```rust
use seed_mixed_case::{ClientError, ClientConfig, MixedCaseClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = MixedCaseClient::new(config)?;
...
async fn main() {
    ...
    let client = MixedCaseClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.service.get_resource().await?;
