...
[![crates.io shield](https://img.shields.io/crates/v/seed_union_query_parameters)](https://crates.io/crates/seed_union_query_parameters)
...
```toml
[dependencies]
seed_union_query_parameters = "0.0.1"
```

Or install via cargo:

```sh
cargo add seed_union_query_parameters
```
...
```rust
match client.events.subscribe(None)?.await {
    Ok(response) => {
...
```rust
let response = client.events.subscribe(
