...
```toml
[dependencies]
seed_cross_package_type_names = "0.1.0"
```
...
```rust
use seed_cross_package_type_names::{ClientError, ClientConfig, CrossPackageTypeNamesClient};
...
async fn main() -> Result<(), ClientError> {
    ...
    let client = CrossPackageTypeNamesClient::new(config)?;
...
async fn main() {
    ...
    let client = CrossPackageTypeNamesClient::new(config).expect("Failed to build client");
    let mut paginated_stream = client.foo.find().await?;
