...
impl NullableOptionalClient2 {
    ...
    ///
    /// JSON response from the API
    pub async fn update_complex_profile(&self, profile_id: &String, request: &UpdateComplexProfileRequest, options: Option<RequestOptions>) -> Result<ComplexProfile, ApiError> {
        self.http_client.execute_request(
            Method::PATCH,
            &format!("/api/profiles/complex/{}", profile_id),
            Some(serde_json::to_value(request).unwrap_or_default()),
    ...
    ///
    /// JSON response from the API
    pub async fn update_tags(&self, user_id: &String, request: &UpdateTagsRequest, options: Option<RequestOptions>) -> Result<Vec<String>, ApiError> {
        self.http_client.execute_request(
            Method::PUT,
