...
impl SubmissionClient {
    ...
    ///
    /// JSON response from the API
    pub async fn get_execution_session(&self, session_id: &str, options: Option<RequestOptions>) -> Result<Option<ExecutionSessionResponse>, ApiError> {
        self.http_client.execute_request(
            Method::GET,
    ...
    ///
    /// Empty response
    pub async fn stop_execution_session(&self, session_id: &str, options: Option<RequestOptions>) -> Result<(), ApiError> {
        self.http_client.execute_request(
            Method::DELETE,
