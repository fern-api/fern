# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import datetime as dt
import typing

import pydantic
import typing_extensions
from .core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .core.serialization import FieldMetadata
from .entity_event_payload_event_type import EntityEventPayloadEventType


class StreamProtocolWithFlatSchemaResponse_Heartbeat(UniversalBaseModel):
    event: typing.Literal["heartbeat"] = "heartbeat"
    timestamp: typing.Optional[dt.datetime] = None

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow")  # type: ignore # Pydantic v2
    else:

        class Config:
            extra = pydantic.Extra.allow


class StreamProtocolWithFlatSchemaResponse_Entity(UniversalBaseModel):
    event: typing.Literal["entity"] = "entity"
    entity_id: typing_extensions.Annotated[
        typing.Optional[str], FieldMetadata(alias="entityId"), pydantic.Field(alias="entityId")
    ] = None
    event_type: typing_extensions.Annotated[
        typing.Optional[EntityEventPayloadEventType],
        FieldMetadata(alias="eventType"),
        pydantic.Field(alias="eventType"),
    ] = None
    updated_time: typing_extensions.Annotated[
        typing.Optional[dt.datetime], FieldMetadata(alias="updatedTime"), pydantic.Field(alias="updatedTime")
    ] = None

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow")  # type: ignore # Pydantic v2
    else:

        class Config:
            extra = pydantic.Extra.allow


StreamProtocolWithFlatSchemaResponse = typing_extensions.Annotated[
    typing.Union[StreamProtocolWithFlatSchemaResponse_Heartbeat, StreamProtocolWithFlatSchemaResponse_Entity],
    pydantic.Field(discriminator="event"),
]
