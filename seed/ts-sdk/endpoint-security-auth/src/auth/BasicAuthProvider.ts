...
export class BasicAuthProvider implements core.AuthProvider {
    ...
    public static canCreate(options: Partial<BasicAuthProvider.Options>): boolean {
        return (options?.[WRAPPER_PROPERTY]?.[USERNAME_PARAM] != null || process.env?.[ENV_USERNAME] != null) && (options?.[WRAPPER_PROPERTY]?.[PASSWORD_PARAM] != null || process.env?.[ENV_PASSWORD] != null);
    }
    ...
    public async getAuthRequest({ endpointMetadata }: {
            endpointMetadata?: core.EndpointMetadata;
                ...
                if (username == null) { throw new errors.SeedEndpointSecurityAuthError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_USERNAME }); }
                const password = 
                    (await core.EndpointSupplier.get(this.options[WRAPPER_PROPERTY]?.[PASSWORD_PARAM], { endpointMetadata })) ??
                    process.env?.[ENV_PASSWORD];
                if (password == null) { throw new errors.SeedEndpointSecurityAuthError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_PASSWORD }); }
...
export namespace BasicAuthProvider {
    ...
    export type AuthOptions = {
                [WRAPPER_PROPERTY]?: { [USERNAME_PARAM]?: core.EndpointSupplier<string> | undefined; [PASSWORD_PARAM]?: core.EndpointSupplier<string> | undefined };
            };
