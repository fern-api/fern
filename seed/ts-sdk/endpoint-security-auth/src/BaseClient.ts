...
} & RoutingAuthProvider.AuthOptions<[BearerAuthProvider.AuthOptions, HeaderAuthProvider.AuthOptions, OAuthAuthProvider.AuthOptions, BasicAuthProvider.AuthOptions, InferredAuthProvider.AuthOptions]>;
...
export function normalizeClientOptions<T extends BaseClientOptions = BaseClientOptions>(
    ...
    const headers = mergeHeaders(
        { "X-Fern-Language": "JavaScript", "X-Fern-SDK-Name": "@fern/endpoint-security-auth", "X-Fern-SDK-Version": "0.0.1", "User-Agent": "@fern/endpoint-security-auth/0.0.1", "X-Fern-Runtime": core.RUNTIME.type, "X-Fern-Runtime-Version": core.RUNTIME.version },
        options?.headers
    );
...
export function normalizeClientOptionsWithAuth<T extends BaseClientOptions = BaseClientOptions>(
    ...
    normalized.authProvider ??= RoutingAuthProvider.createInstance(normalizedWithNoOpAuthProvider, [BearerAuthProvider, HeaderAuthProvider, OAuthAuthProvider, BasicAuthProvider, InferredAuthProvider]);
    return normalized;
}
