...
export class RealtimeClient {
    ...
    public async connect(args: RealtimeClient.ConnectArgs): Promise<RealtimeSocket> {
        const { session_id: sessionId, model, temperature, protocols, queryParams, headers, debug, reconnectAttempts, connectionTimeoutInSeconds, abortSignal } = args;
        const _queryParams: Record<string, unknown> = {
            model,
            temperature
        };
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        let _headers: Record<string, unknown> = mergeHeaders(_authRequest.headers, this._options?.headers, headers);
