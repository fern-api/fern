...
export class ServiceClient {
    ...
    private async __check(id: string, requestOptions?: ServiceClient.RequestOptions): Promise<core.WithRawResponse<void>> {
        ...
        const _response = await core.fetcher({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), `/check/${core.url.encodePathParam(id)}`),
            method: "GET",
            headers: _headers,
        ...
        switch (_response.error.reason) {
            ...
            case "timeout": throw new errors.SeedExamplesTimeoutError("Timeout exceeded when calling GET /check/{id}.");
    ...
    private async __ping(requestOptions?: ServiceClient.RequestOptions): Promise<core.WithRawResponse<boolean>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, mergeOnlyDefinedHeaders({ "Authorization": await this._getAuthorizationHeader() }), requestOptions?.headers);
        const _response = await core.fetcher({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), "/ping"),
            method: "GET",
            headers: _headers,
            queryParameters: requestOptions?.queryParams,
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
        ...
        switch (_response.error.reason) {
            ...
            case "timeout": throw new errors.SeedExamplesTimeoutError("Timeout exceeded when calling GET /ping.");
            case "unknown": throw new errors.SeedExamplesError({
                message: _response.error.errorMessage,
                rawResponse: _response.rawResponse
            });
