...
[![npm shield](https://img.shields.io/npm/v/@fern/bytes)](https://www.npmjs.com/package/@fern/bytes)
...
```typescript
import { SeedBytesError } from "@fern/bytes";

try {
    await client.service.upload(...);
} catch (err) {
    if (err instanceof SeedBytesError) {
...
const client = new SeedBytesClient({
    ...
