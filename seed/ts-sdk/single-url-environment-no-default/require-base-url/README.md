...
[![npm shield](https://img.shields.io/npm/v/@fern/single-url-environment-no-default)](https://www.npmjs.com/package/@fern/single-url-environment-no-default)
...
```sh
npm i -s @fern/single-url-environment-no-default
```
...
const client = new SeedSingleUrlEnvironmentNoDefaultClient({ baseUrl: "YOUR_BASE_URL", token: "YOUR_TOKEN" });
...
```typescript
import { DummyClient } from '@fern/single-url-environment-no-default/dummy';
...
```typescript
import { SeedSingleUrlEnvironmentNoDefaultClient, logging } from "@fern/single-url-environment-no-default";
