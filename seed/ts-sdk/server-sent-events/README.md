...
const client = new SeedServerSentEventsClient({ baseUrl: "YOUR_BASE_URL" });
...
```typescript
import { CompletionsClient } from '@fern/server-sent-events/completions';
...
```typescript
import { SeedServerSentEventsClient, logging } from "@fern/server-sent-events";
