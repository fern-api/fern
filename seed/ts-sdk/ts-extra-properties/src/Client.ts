...
export class SeedApiClient {
    ...
    private async __getUser(requestOptions?: SeedApiClient.RequestOptions): Promise<core.WithRawResponse<SeedApi.User>> {
     ...
     *     })
     */
    public createUser(request: SeedApi.CreateUserRequest, requestOptions?: SeedApiClient.RequestOptions): core.HttpResponsePromise<SeedApi.User> {
        return core.HttpResponsePromise.fromPromise(this.__createUser(request, requestOptions));
    }

    private async __createUser(request: SeedApi.CreateUserRequest, requestOptions?: SeedApiClient.RequestOptions): Promise<core.WithRawResponse<SeedApi.User>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, requestOptions?.headers);
        const _response = await core.fetcher({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), "user"),
            method: "POST",
            ...
            queryString: core.url.queryBuilder().mergeAdditional(requestOptions?.queryParams).build(),
            requestType: "json",
            body: serializers.CreateUserRequest.jsonOrThrow(request, { unrecognizedObjectKeys: "strip", omitUndefined: true }),
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
