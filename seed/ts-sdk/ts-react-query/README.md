...
[![npm shield](https://img.shields.io/npm/v/@fern/ts-react-query)](https://www.npmjs.com/package/@fern/ts-react-query)
...
```typescript
import { SeedApiError } from "@fern/ts-react-query";
...
```typescript
const { data, rawResponse } = await client.user.create(...).withRawResponse();
...
```typescript
import { SeedApiClient, logging } from "@fern/ts-react-query";
