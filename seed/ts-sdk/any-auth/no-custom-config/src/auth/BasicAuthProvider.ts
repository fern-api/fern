...
export class BasicAuthProvider implements core.AuthProvider {
    ...
    public async getAuthRequest({ endpointMetadata }: {
            endpointMetadata?: core.EndpointMetadata;
                ...
                if (username == null) { throw new errors.SeedAnyAuthError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_USERNAME }); }
                const password = 
                    (await core.Supplier.get(this.options[WRAPPER_PROPERTY]?.[PASSWORD_PARAM])) ??
                    process.env?.[ENV_PASSWORD];
                if (password == null) { throw new errors.SeedAnyAuthError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_PASSWORD }); }
...
export namespace BasicAuthProvider {
    ...
    export type AuthOptions = {
                [WRAPPER_PROPERTY]?: { [USERNAME_PARAM]?: core.Supplier<string> | undefined; [PASSWORD_PARAM]?: core.Supplier<string> | undefined };
            };
