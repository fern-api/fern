    ...
    > {
        ...
        if (_response.ok) {
            return {
                data: {
                    ok: true,
                    body: serializers.tsExpressCasing.MovieId.parseOrThrow(_response.body, {
    ...
    > {
        ...
        const _response = await core.fetcher({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    ((await core.Supplier.get(this._options.environment)) ?? environments.SeedApiEnvironment.Production)
                        .base,
                `movies/${core.url.encodePathParam(serializers.tsExpressCasing.MovieId.jsonOrThrow(movieId, { omitUndefined: true }))}`,
            ),
            method: "GET",
            headers: _headers,
            queryString: core.url.queryBuilder().mergeAdditional(requestOptions?.queryParams).build(),
        ...
        if (_response.ok) {
            return {
                data: {
                    ok: true,
                    body: serializers.tsExpressCasing.Movie.parseOrThrow(_response.body, {
                        unrecognizedObjectKeys: "passthrough",
                        allowUnrecognizedUnionMembers: true,
                        allowUnrecognizedEnumValues: true,
                        skipValidation: true,
            ...
            switch (_response.error.statusCode) {
                case 404:
                    return {
                        data: {
                            ok: false,
                            error: SeedApi.tsExpressCasing.imdb.getMovie.Error.notFoundError(_response.error.body),
                            rawResponse: _response.rawResponse,
                        },
                        rawResponse: _response.rawResponse,
                    };
