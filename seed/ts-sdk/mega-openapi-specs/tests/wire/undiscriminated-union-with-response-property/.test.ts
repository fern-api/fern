// This file was auto-generated by Fern from our API Definition.

import { SeedApiClient } from "../../../src/Client";
import { mockServerPool } from "../../mock-server/MockServerPool";

describe("Client", () => {
    test("getUnion", async () => {
        const server = mockServerPool.createServer();
        const client = new SeedApiClient({
            maxRetries: 0,
            token: "test",
            apiVersion: "test",
            apiKey: "test",
            environment: { base: server.baseUrl, auth: server.baseUrl, upload: server.baseUrl },
        });

        const rawResponseBody = { data: { type: "A", valueA: "valueA" } };

        server.mockEndpoint().get("/union").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.undiscriminatedUnionWithResponseProperty..getUnion();
        expect(response).toEqual(rawResponseBody);
    });

    test("listUnions", async () => {
        const server = mockServerPool.createServer();
        const client = new SeedApiClient({
            maxRetries: 0,
            token: "test",
            apiVersion: "test",
            apiKey: "test",
            environment: { base: server.baseUrl, auth: server.baseUrl, upload: server.baseUrl },
        });

        const rawResponseBody = { data: [{ type: "A", valueA: "valueA" }] };

        server.mockEndpoint().get("/unions").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.undiscriminatedUnionWithResponseProperty..listUnions();
        expect(response).toEqual(rawResponseBody);
    });
});
