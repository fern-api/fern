// This file was auto-generated by Fern from our API Definition.

import type * as SeedApi from "../../../../api/index.js";
import * as core from "../../../../core/index.js";
import type * as serializers from "../../../index.js";

export const V2FileInfoV2: core.serialization.ObjectSchema<
    serializers.trace.V2FileInfoV2.Raw,
    SeedApi.trace.V2FileInfoV2
> = core.serialization.object({
    filename: core.serialization.string(),
    directory: core.serialization.string(),
    contents: core.serialization.string(),
    editable: core.serialization.boolean(),
});

export declare namespace V2FileInfoV2 {
    export interface Raw {
        filename: string;
        directory: string;
        contents: string;
        editable: boolean;
    }
}
