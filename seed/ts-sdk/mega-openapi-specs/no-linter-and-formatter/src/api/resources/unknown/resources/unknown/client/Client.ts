...
export class UnknownClient {
    ...
    private async __post(request?: unknown, requestOptions?: UnknownClient.RequestOptions): Promise<core.WithRawResponse<unknown[]>> {
     ...
     *     })
     */
    public postObject(request: SeedApi.unknown.MyObject, requestOptions?: UnknownClient.RequestOptions): core.HttpResponsePromise<unknown[]> {
        return core.HttpResponsePromise.fromPromise(this.__postObject(request, requestOptions));
    }

    private async __postObject(request: SeedApi.unknown.MyObject, requestOptions?: UnknownClient.RequestOptions): Promise<core.WithRawResponse<unknown[]>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, mergeOnlyDefinedHeaders({ "X-API-Version": requestOptions?.apiVersion ?? (this._options?.apiVersion ?? "2024-02-08"), "X-Api-Key": requestOptions?.apiKey ?? this._options?.apiKey }), requestOptions?.headers);
        const _response = await core.fetcher({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? (await core.Supplier.get(this._options.environment) ?? environments.SeedApiEnvironment.Production).base, "with-object"),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
            queryString: core.url.queryBuilder().mergeAdditional(requestOptions?.queryParams).build(),
