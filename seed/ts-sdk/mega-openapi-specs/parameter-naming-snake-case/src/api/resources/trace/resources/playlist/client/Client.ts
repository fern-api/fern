...
export class PlaylistClient {
    ...
    private async __createPlaylist(
        ...
        const { service_param: serviceParam, datetime, optional_datetime: optionalDatetime, body: _body } = request;
    ...
    private async __getPlaylist(
        ...
        const { service_param: serviceParam, playlist_id: playlistId } = request;
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            this._options?.headers,
            mergeOnlyDefinedHeaders({
                "X-API-Version": requestOptions?.apiVersion ?? this._options?.apiVersion ?? "2024-02-08",
    ...
    private async __updatePlaylist(
        ...
        const { service_param: serviceParam, playlist_id: playlistId, body: _body } = request;
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
            this._options?.headers,
