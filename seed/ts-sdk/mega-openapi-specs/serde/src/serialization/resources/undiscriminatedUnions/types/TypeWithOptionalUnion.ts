// This file was auto-generated by Fern from our API Definition.

import type * as SeedApi from "../../../../api/index.js";
import * as core from "../../../../core/index.js";
import type * as serializers from "../../../index.js";
import { MyUnion } from "./MyUnion.js";

export const TypeWithOptionalUnion: core.serialization.ObjectSchema<
    serializers.undiscriminatedUnions.TypeWithOptionalUnion.Raw,
    SeedApi.undiscriminatedUnions.TypeWithOptionalUnion
> = core.serialization.object({
    myUnion: MyUnion.optionalNullable(),
});

export declare namespace TypeWithOptionalUnion {
    export interface Raw {
        myUnion?: (MyUnion.Raw | null | undefined) | null;
    }
}
