...
describe("join", () => {
    describe("basic functionality", () => {
        ...
        it("should handle multiple segments", () => {
            expect(join("base", "path1", "path2", "path3")).toBe(
                "base/path1/path2/path3",
            );
            expect(join("base/", "/path1/", "/path2/", "/path3/")).toBe(
                "base/path1/path2/path3/",
            );
    ...
    describe("URL handling", () => {
        it("should handle absolute URLs", () => {
            expect(join("https://example.com", "api", "v1")).toBe(
                "https://example.com/api/v1",
            );
            expect(join("https://example.com/", "/api/", "/v1/")).toBe(
                "https://example.com/api/v1/",
            );
            expect(join("https://example.com/base", "api", "v1")).toBe(
                "https://example.com/base/api/v1",
            );
        });

        it("should preserve URL query parameters and fragments", () => {
            expect(join("https://example.com?query=1", "api")).toBe(
                "https://example.com/api?query=1",
            );
            expect(join("https://example.com#fragment", "api")).toBe(
        ...
        it("should handle different protocols", () => {
            expect(join("http://example.com", "api")).toBe(
                "http://example.com/api",
            );
            expect(join("ftp://example.com", "files")).toBe(
                "ftp://example.com/files",
            );
            expect(join("ws://example.com", "socket")).toBe(
    ...
    describe("edge cases", () => {
        ...
        it("should handle complex nested paths", () => {
            expect(join("api/v1/", "/users/", "/123/", "/profile")).toBe(
    ...
    describe("real-world scenarios", () => {
        it("should handle API endpoint construction", () => {
            ...
            expect(join(baseUrl, "users", "123", "posts")).toBe(
                "https://api.example.com/v1/users/123/posts",
            );
        });

        it("should handle file path construction", () => {
            expect(join("/var/www", "html", "assets", "images")).toBe(
                "/var/www/html/assets/images",
            );
        });

        it("should handle relative path construction", () => {
            expect(join("../parent", "child", "grandchild")).toBe(
                "../parent/child/grandchild",
            );
        });

        it("should handle Windows-style paths", () => {
            expect(join("C:\\Users", "Documents", "file.txt")).toBe(
    ...
    describe("performance scenarios", () => {
        ...
        it("should handle long URLs", () => {
            ...
            expect(join("https://example.com", longPath)).toBe(
    ...
    describe("trailing slash preservation", () => {
        it("should preserve trailing slash on final result when base has trailing slash and no segments", () => {
            expect(join("https://api.example.com/")).toBe(
                "https://api.example.com/",
            );
            expect(join("https://api.example.com/v1/")).toBe(
                "https://api.example.com/v1/",
            );
        });

        it("should preserve trailing slash when last segment has trailing slash", () => {
            expect(join("https://api.example.com", "users/")).toBe(
