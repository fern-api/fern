...
export class UserClient {
    ...
    private async __getUsername(request: SeedQueryParameters.GetUsersRequest, requestOptions?: UserClient.RequestOptions): Promise<core.WithRawResponse<SeedQueryParameters.User>> {
        ...
        const _queryParams: Record<string, unknown> = {
            ...
            bytes,
            user: serializers.User.jsonOrThrow(user, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, omitUndefined: true, breadcrumbsPrefix: ["request", "user"] }),
            userList: toJson(userList),
            optionalDeadline: optionalDeadline?.toISOString(),
            keyValue: toJson(keyValue),
            optionalString,
            nestedUser: serializers.NestedUser.jsonOrThrow(nestedUser, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, omitUndefined: true, breadcrumbsPrefix: ["request", "nestedUser"] }),
            optionalUser: optionalUser != null ? serializers.User.jsonOrThrow(optionalUser, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, omitUndefined: true, breadcrumbsPrefix: ["request", "optionalUser"] }) : optionalUser,
            excludeUser: Array.isArray(excludeUser) ? await Promise.all(excludeUser.map(async (item) => serializers.User.jsonOrThrow(item, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, omitUndefined: true, breadcrumbsPrefix: ["request", "excludeUser"] }))) : serializers.User.jsonOrThrow(excludeUser, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, omitUndefined: true, breadcrumbsPrefix: ["request", "excludeUser"] }),
            filter
        };
