...
export class UnionClient {
    ...
    private async __get(id: string, requestOptions?: UnionClient.RequestOptions): Promise<core.WithRawResponse<SeedUnions.Shape>> {
        ...
        if (_response.ok) {
            return { data: serializers.Shape.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
    ...
    private async __update(request: SeedUnions.Shape, requestOptions?: UnionClient.RequestOptions): Promise<core.WithRawResponse<boolean>> {
        ...
        const _response = await core.fetcher({
            ...
            queryString: core.url.queryBuilder().mergeAdditional(requestOptions?.queryParams).build(),
            requestType: "json",
            body: serializers.Shape.jsonOrThrow(request, { unrecognizedObjectKeys: "strip", omitUndefined: true }),
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
            fetchFn: this._options?.fetch,
        ...
        if (_response.ok) {
            return { data: serializers.union.update.Response.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
