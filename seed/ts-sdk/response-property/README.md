...
[![npm shield](https://img.shields.io/npm/v/@fern/response-property)](https://www.npmjs.com/package/@fern/response-property)
...
```sh
npm i -s @fern/response-property
```
...
const client = new SeedResponsePropertyClient({ baseUrl: "YOUR_BASE_URL" });
...
try {
    ...
    if (err instanceof SeedResponsePropertyError) {
...
```typescript
import { ServiceClient } from '@fern/response-property/service';
...
```typescript
const { data, rawResponse } = await client.service.getMovie(...).withRawResponse();
...
```typescript
import { SeedResponsePropertyClient, logging } from "@fern/response-property";
