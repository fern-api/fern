...
[![npm shield](https://img.shields.io/npm/v/@fern/ts-oauth-token-optional)](https://www.npmjs.com/package/@fern/ts-oauth-token-optional)
...
```sh
npm i -s @fern/ts-oauth-token-optional
```
...
```typescript
import { SeedTsOauthTokenOptionalClient } from "@fern/ts-oauth-token-optional";
...
```typescript
import { SeedTsOauthTokenOptional } from "@fern/ts-oauth-token-optional";

const request: SeedTsOauthTokenOptional.CreateOauth2TokenRequest = {
    ...
};
```
...
```typescript
import { SeedTsOauthTokenOptionalError } from "@fern/ts-oauth-token-optional";

try {
    await client.auth.createOauth2Token(...);
} catch (err) {
    if (err instanceof SeedTsOauthTokenOptionalError) {
...
```typescript
import { AuthClient } from '@fern/ts-oauth-token-optional/auth';
...
```typescript
const { data, rawResponse } = await client.auth.createOauth2Token(...).withRawResponse();
...
```typescript
import { SeedTsOauthTokenOptionalClient, logging } from "@fern/ts-oauth-token-optional";
