// This file was auto-generated by Fern from our API Definition.

import { SeedApiClient } from "../../src/Client";
import { mockServerPool } from "../mock-server/MockServerPool";

describe("EndpointsPaginationClient", () => {
    test("endpoints_pagination_listItems", async () => {
        const server = mockServerPool.createServer();
        const client = new SeedApiClient({ maxRetries: 0, token: "test", environment: server.baseUrl });

        const rawResponseBody = { items: [{ string: "string" }], next: "next" };

        server.mockEndpoint().get("/pagination").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.endpointsPagination.endpointsPaginationListItems();
        expect(response).toEqual({
            items: [
                {
                    string: "string",
                },
            ],
            next: "next",
        });
    });
});
