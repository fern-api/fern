// This file was auto-generated by Fern from our API Definition.

import * as SeedExhaustive from "../../../src/api/index";
import { SeedExhaustiveClient } from "../../../src/Client";
import { mockServerPool } from "../../mock-server/MockServerPool";

describe("UnionClient", () => {
    test("getAndReturnUnion", async () => {
        const server = mockServerPool.createServer();
        const client = new SeedExhaustiveClient({ maxRetries: 0, token: "test", environment: server.baseUrl });
        const rawRequestBody = { animal: "dog", name: "name", likesToWoof: true };
        const rawResponseBody = { animal: "dog", name: "name", likesToWoof: true };

        server
            .mockEndpoint()
            .post("/union")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.endpoints.union.getAndReturnUnion(
            SeedExhaustive.types.Animal.dog({
                name: "name",
                likesToWoof: true,
            }),
        );
        expect(response).toEqual({
            body: SeedExhaustive.types.Animal.dog({
                name: "name",
                likesToWoof: true,
            }),
            ok: true,
            headers: expect.any(Object),
            rawResponse: expect.any(Object),
        });
    });

    test("getAndReturnObjectWithUnionField", async () => {
        const server = mockServerPool.createServer();
        const client = new SeedExhaustiveClient({ maxRetries: 0, token: "test", environment: server.baseUrl });
        const rawRequestBody = { name: "name", animal: { animal: "dog", name: "name", likesToWoof: true } };
        const rawResponseBody = { name: "name", animal: { animal: "dog", name: "name", likesToWoof: true } };

        server
            .mockEndpoint()
            .post("/union/object")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.endpoints.union.getAndReturnObjectWithUnionField({
            name: "name",
            animal: SeedExhaustive.types.Animal.dog({
                name: "name",
                likesToWoof: true,
            }),
        });
        expect(response).toEqual({
            body: {
                name: "name",
                animal: SeedExhaustive.types.Animal.dog({
                    name: "name",
                    likesToWoof: true,
                }),
            },
            ok: true,
            headers: expect.any(Object),
            rawResponse: expect.any(Object),
        });
    });
});
