import { stringify } from "../../../../src/test-packagePath/core/qs.js";

describe("Test qs stringify", () => {
    describe("Basic functionality", () => {
        ...
        it("should return empty string for primitive values", () => {
            expect(stringify("hello")).toBe("");
            expect(stringify(42)).toBe("");
    ...
    describe("Array handling", () => {
        it("should handle arrays with indices format (default)", () => {
            ...
            expect(stringify(obj)).toBe("items%5B0%5D=a&items%5B1%5D=b&items%5B2%5D=c");
        });

        it("should handle arrays with repeat format", () => {
            const obj = { items: ["a", "b", "c"] };
            expect(stringify(obj, { arrayFormat: "repeat" })).toBe("items=a&items=b&items=c");
        });
        ...
        it("should handle arrays with mixed types", () => {
            ...
            expect(stringify(obj)).toBe("mixed%5B0%5D=string&mixed%5B1%5D=42&mixed%5B2%5D=true&mixed%5B3%5D=false");
        });

        it("should handle arrays with objects", () => {
            const obj = { users: [{ name: "John" }, { name: "Jane" }] };
            expect(stringify(obj)).toBe("users%5B0%5D%5Bname%5D=John&users%5B1%5D%5Bname%5D=Jane");
        });

        it("should handle arrays with objects in repeat format", () => {
            const obj = { users: [{ name: "John" }, { name: "Jane" }] };
            expect(stringify(obj, { arrayFormat: "repeat" })).toBe("users%5Bname%5D=John&users%5Bname%5D=Jane");
        });
    });
    ...
    describe("Nested objects", () => {
        it("should handle nested objects", () => {
            ...
            expect(stringify(obj)).toBe("user%5Bname%5D=John&user%5Bage%5D=30");
        });
    ...
    describe("Encoding", () => {
        it("should encode by default", () => {
            ...
            expect(stringify(obj)).toBe("name=John%20Doe&email=john%40example.com");
        });

        it("should not encode when encode is false", () => {
            const obj = { name: "John Doe", email: "john@example.com" };
            expect(stringify(obj, { encode: false })).toBe("name=John Doe&email=john@example.com");
        });

        it("should encode special characters in keys", () => {
            const obj = { "user name": "John", "email[primary]": "john@example.com" };
            expect(stringify(obj)).toBe("user%20name=John&email%5Bprimary%5D=john%40example.com");
        });

        it("should not encode special characters in keys when encode is false", () => {
            const obj = { "user name": "John", "email[primary]": "john@example.com" };
            expect(stringify(obj, { encode: false })).toBe("user name=John&email[primary]=john@example.com");
        });
    });
    ...
    describe("Mixed scenarios", () => {
        ...
        it("should handle arrays with null/undefined values", () => {
            ...
            expect(stringify(obj)).toBe("items%5B0%5D=a&items%5B1%5D=&items%5B2%5D=c&items%5B4%5D=e");
        });

        it("should handle objects with null/undefined values", () => {
            const obj = { name: "John", age: null, email: undefined, active: true };
            expect(stringify(obj)).toBe("name=John&age=&active=true");
        });
    });
    ...
    describe("Edge cases", () => {
        it("should handle numeric keys", () => {
            ...
            expect(stringify(obj)).toBe("0=zero&1=one");
        });

        it("should handle boolean values in objects", () => {
            const obj = { enabled: true, disabled: false };
            expect(stringify(obj)).toBe("enabled=true&disabled=false");
        });

        it("should handle empty strings", () => {
            const obj = { name: "", description: "test" };
            expect(stringify(obj)).toBe("name=&description=test");
        });

        it("should handle zero values", () => {
            const obj = { count: 0, price: 0.0 };
            expect(stringify(obj)).toBe("count=0&price=0");
        });

        it("should handle arrays with empty strings", () => {
            const obj = { items: ["a", "", "c"] };
            expect(stringify(obj)).toBe("items%5B0%5D=a&items%5B1%5D=&items%5B2%5D=c");
        });
    });
    ...
    describe("Options combinations", () => {
        it("should respect both arrayFormat and encode options", () => {
            ...
            expect(stringify(obj, { arrayFormat: "repeat", encode: false })).toBe("items=a & b&items=c & d");
        });

        it("should use default options when none provided", () => {
            const obj = { items: ["a", "b"] };
            expect(stringify(obj)).toBe("items%5B0%5D=a&items%5B1%5D=b");
        });
