...
export class PaginationClient {
     ...
     *     })
     */
    public async listItems(request: SeedExhaustive.ListItemsRequest = {}, requestOptions?: PaginationClient.RequestOptions): Promise<core.Page<SeedExhaustive.types.ObjectWithRequiredField, SeedExhaustive.endpoints.PaginatedResponse>> {
        const list = core.HttpResponsePromise.interceptFunction(async (request: SeedExhaustive.ListItemsRequest): Promise<core.WithRawResponse<SeedExhaustive.endpoints.PaginatedResponse>> => { const { cursor, limit } = request; const _queryParams: Record<string, unknown> = {
            cursor,
            limit
