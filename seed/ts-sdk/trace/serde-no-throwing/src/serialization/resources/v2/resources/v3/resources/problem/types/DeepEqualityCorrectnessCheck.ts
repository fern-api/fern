// This file was auto-generated by Fern from our API Definition.

import * as serializers from "../../../../../../../index.js";
import * as SeedTrace from "../../../../../../../../api/index.js";
import * as core from "../../../../../../../../core/index.js";
import { ParameterId } from "./ParameterId.js";

export const DeepEqualityCorrectnessCheck: core.serialization.ObjectSchema<serializers.v2.v3.DeepEqualityCorrectnessCheck.Raw, SeedTrace.v2.v3.DeepEqualityCorrectnessCheck> = core.serialization.object({
        "expectedValueParameterId": ParameterId
    });

export declare namespace DeepEqualityCorrectnessCheck {
    export interface Raw {
        expectedValueParameterId: ParameterId.Raw;
    }
}
