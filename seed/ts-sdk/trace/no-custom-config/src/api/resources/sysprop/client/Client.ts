...
export class SyspropClient {
     ...
     */
    public getNumWarmInstances(requestOptions?: SyspropClient.RequestOptions): core.HttpResponsePromise<Partial<Record<SeedTrace.Language, number>>> {
        return core.HttpResponsePromise.fromPromise(this.__getNumWarmInstances(requestOptions));
    }

    private async __getNumWarmInstances(requestOptions?: SyspropClient.RequestOptions): Promise<core.WithRawResponse<Partial<Record<SeedTrace.Language, number>>>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, mergeOnlyDefinedHeaders({ "X-Random-Header": requestOptions?.xRandomHeader ?? this._options?.xRandomHeader }), requestOptions?.headers);
        const _response = await core.fetcher({
        ...
        if (_response.ok) {
            return { data: _response.body as Partial<Record<SeedTrace.Language, number>>, rawResponse: _response.rawResponse };
        }
