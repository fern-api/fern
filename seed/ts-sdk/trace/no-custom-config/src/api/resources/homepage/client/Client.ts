...
export class Homepage {
    ...
    public async getHomepageProblems(requestOptions?: Homepage.RequestOptions): Promise<SeedTrace.ProblemId[]> {
        const _response = await core.fetcher({
            url: urlJoin(await core.Supplier.get(this._options.environment) ?? environments.SeedTraceEnvironment.Prod, "/homepage-problems"),
            method: "GET",
            headers: {
        ...
        if (_response.ok) {
            return serializers.homepage.getHomepageProblems.Response.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, breadcrumbsPrefix: ["response"] });
        }
     ...
     */
    public async setHomepageProblems(request: SeedTrace.ProblemId[], requestOptions?: Homepage.RequestOptions): Promise<void> {
        const _response = await core.fetcher({
            url: urlJoin(await core.Supplier.get(this._options.environment) ?? environments.SeedTraceEnvironment.Prod, "/homepage-problems"),
            method: "POST",
            headers: {
            ...
            contentType: "application/json",
            requestType: "json",
            body: serializers.homepage.setHomepageProblems.Request.jsonOrThrow(request, { unrecognizedObjectKeys: "strip" }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? (requestOptions.timeoutInSeconds * 1000) : 60000,
            maxRetries: requestOptions?.maxRetries,
            abortSignal: requestOptions?.abortSignal
        });
