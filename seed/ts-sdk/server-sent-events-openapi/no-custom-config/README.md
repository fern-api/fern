...
[![npm shield](https://img.shields.io/npm/v/@fern/server-sent-events-openapi)](https://www.npmjs.com/package/@fern/server-sent-events-openapi)
...
```sh
npm i -s @fern/server-sent-events-openapi
```
...
```typescript
import { SeedApiError } from "@fern/server-sent-events-openapi";

try {
    await client.streamProtocolNoCollision(...);
} catch (err) {
...
```typescript
import { SeedApiClient } from "@fern/server-sent-events-openapi";
...
```typescript
const { data, rawResponse } = await client.streamProtocolNoCollision(...).withRawResponse();
...
```typescript
import { SeedApiClient, logging } from "@fern/server-sent-events-openapi";
