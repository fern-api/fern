...
[![npm shield](https://img.shields.io/npm/v/@fern/literal-user-agent)](https://www.npmjs.com/package/@fern/literal-user-agent)
...
```sh
npm i -s @fern/literal-user-agent
```
...
```typescript
import { SeedLiteralUserAgentError } from "@fern/literal-user-agent";

try {
    await client.ping(...);
} catch (err) {
    if (err instanceof SeedLiteralUserAgentError) {
...
const client = new SeedLiteralUserAgentClient({
    ...
    headers: {
...
```typescript
const { data, rawResponse } = await client.ping(...).withRawResponse();
...
```typescript
import { SeedLiteralUserAgentClient, logging } from "@fern/literal-user-agent";
