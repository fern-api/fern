...
```typescript
import { SeedInferredAuthImplicitError } from "@fern/inferred-auth-implicit-reference";

try {
    await client.auth.getTokenWithClientCredentials(...);
} catch (err) {
    if (err instanceof SeedInferredAuthImplicitError) {
...
```typescript
import { SeedInferredAuthImplicitClient, logging } from "@fern/inferred-auth-implicit-reference";
