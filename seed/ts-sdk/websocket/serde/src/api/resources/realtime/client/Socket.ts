...
export class RealtimeSocket {
    ...
    private handleMessage: (event: { data: string }) => void = event => {
            ...
            const parsedResponse = serializers.RealtimeSocketResponse.parse(data, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, omitUndefined: true });
    ...
    public sendSend(message: SeedWebsocket.SendEvent): void {
        ...
        const jsonPayload = SendEvent.jsonOrThrow(message, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, omitUndefined: true });
        this.socket.send(JSON.stringify(jsonPayload));
    }

    public sendSendSnakeCase(message: SeedWebsocket.SendSnakeCase): void {
        this.assertSocketIsOpen();
        const jsonPayload = SendSnakeCase.jsonOrThrow(message, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, omitUndefined: true });
        this.socket.send(JSON.stringify(jsonPayload));
    }

    public sendSend2(message: SeedWebsocket.SendEvent2): void {
        this.assertSocketIsOpen();
        const jsonPayload = SendEvent2.jsonOrThrow(message, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, omitUndefined: true });
        this.socket.send(JSON.stringify(jsonPayload));
    }
