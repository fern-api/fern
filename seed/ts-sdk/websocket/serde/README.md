...
[![npm shield](https://img.shields.io/npm/v/@fern/websocket)](https://www.npmjs.com/package/@fern/websocket)
...
```typescript
import { SeedWebsocketError } from "@fern/websocket";

try {
    await client.status.getStatus(...);
} catch (err) {
    if (err instanceof SeedWebsocketError) {
...
```typescript
import { StatusClient } from '@fern/websocket/status';
...
const client = new SeedWebsocketClient({
    ...
    headers: {
...
```typescript
const { data, rawResponse } = await client.status.getStatus(...).withRawResponse();
...
```typescript
import { SeedWebsocketClient, logging } from "@fern/websocket";
