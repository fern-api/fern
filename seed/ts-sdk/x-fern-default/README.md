...
[![npm shield](https://img.shields.io/npm/v/@fern/x-fern-default)](https://www.npmjs.com/package/@fern/x-fern-default)
...
const client = new SeedApiClient({ baseUrl: "YOUR_BASE_URL", apiVersion: "2024-02-08" });
...
```typescript
import { SeedApi } from "@fern/x-fern-default";

const request: SeedApi.TestGetRequest = {
    ...
};
```
...
```typescript
import { SeedApiError } from "@fern/x-fern-default";
...
```typescript
const { data, rawResponse } = await client.testGet(...).withRawResponse();
...
```typescript
import { SeedApiClient, logging } from "@fern/x-fern-default";
