...
[![npm shield](https://img.shields.io/npm/v/@fern/no-content)](https://www.npmjs.com/package/@fern/no-content)
...
```typescript
import { SeedApiClient } from "@fern/no-content";
...
```typescript
import { SeedApi } from "@fern/no-content";

const request: SeedApi.PlantsGetRequest = {
    ...
};
```
...
```typescript
import { SeedApiError } from "@fern/no-content";
...
```typescript
const { data, rawResponse } = await client.plantsGet(...).withRawResponse();
...
```typescript
import { SeedApiClient, logging } from "@fern/no-content";
