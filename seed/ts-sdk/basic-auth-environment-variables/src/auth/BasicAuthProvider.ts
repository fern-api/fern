...
export class BasicAuthProvider implements core.AuthProvider {
    ...
    public async getAuthRequest({ endpointMetadata }: {
            endpointMetadata?: core.EndpointMetadata;
                ...
                if (username == null) { throw new errors.SeedBasicAuthEnvironmentVariablesError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_USERNAME }); }
                const accessToken = 
                    (await core.Supplier.get(this.options[PASSWORD_PARAM])) ??
                    process.env?.[ENV_PASSWORD];
                if (accessToken == null) { throw new errors.SeedBasicAuthEnvironmentVariablesError({ message: BasicAuthProvider.AUTH_CONFIG_ERROR_MESSAGE_PASSWORD }); }
