...
export class ServiceClient {
    ...
    private async __getWithBearerToken(requestOptions?: ServiceClient.RequestOptions): Promise<core.WithRawResponse<string>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, mergeOnlyDefinedHeaders({ "Authorization": await this._getAuthorizationHeader(), "X-API-Version": requestOptions?.version ?? "1.0.0" }), requestOptions?.headers);
        ...
        switch (_response.error.reason) {
            ...
            case "timeout": throw new errors.SeedBearerTokenEnvironmentVariableTimeoutError("Timeout exceeded when calling GET /apiKey.");
    ...
    protected async _getAuthorizationHeader(): Promise<string> {
        ...
        if (bearer == null) {
            throw new errors.SeedBearerTokenEnvironmentVariableError({
                message: "Please specify a bearer by either passing it in to the constructor or initializing a COURIER_API_KEY environment variable"
            });
        }
