...
export class OrganizationsClient {
    ...
    private async __getOrganization(organization_id: string, requestOptions?: OrganizationsClient.RequestOptions): Promise<core.WithRawResponse<SeedPathParameters.Organization>> {
        ...
        if (_response.ok) {
            return { data: serializers.Organization.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
     ...
     */
    public getOrganizationUser(organization_id: string, user_id: string, request: SeedPathParameters.GetOrganizationUserRequest = {}, requestOptions?: OrganizationsClient.RequestOptions): core.HttpResponsePromise<SeedPathParameters.User> {
        return core.HttpResponsePromise.fromPromise(this.__getOrganizationUser(organization_id, user_id, request, requestOptions));
    }

    private async __getOrganizationUser(organization_id: string, user_id: string, request: SeedPathParameters.GetOrganizationUserRequest = {}, requestOptions?: OrganizationsClient.RequestOptions): Promise<core.WithRawResponse<SeedPathParameters.User>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, requestOptions?.headers);
        const _response = await core.fetcher({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), `/${core.url.encodePathParam(this._options.tenant_id)}/organizations/${core.url.encodePathParam(organization_id)}/users/${core.url.encodePathParam(user_id)}`),
            method: "GET",
            headers: _headers,
            queryParameters: requestOptions?.queryParams,
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
        ...
        if (_response.ok) {
            return { data: serializers.User.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
    ...
    private async __searchOrganizations(organization_id: string, request: SeedPathParameters.SearchOrganizationsRequest = {}, requestOptions?: OrganizationsClient.RequestOptions): Promise<core.WithRawResponse<SeedPathParameters.Organization[]>> {
        ...
        if (_response.ok) {
            return { data: serializers.organizations.searchOrganizations.Response.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
