...
[![npm shield](https://img.shields.io/npm/v/@fern/ts-error-name-collision)](https://www.npmjs.com/package/@fern/ts-error-name-collision)
...
```sh
npm i -s @fern/ts-error-name-collision
```
...
```typescript
import { SeedErrorsError } from "@fern/ts-error-name-collision";

try {
    await client.simple.fooWithoutEndpointError(...);
} catch (err) {
    if (err instanceof SeedErrorsError) {
...
```typescript
import { SimpleClient } from '@fern/ts-error-name-collision/simple';
...
```typescript
const { data, rawResponse } = await client.simple.fooWithoutEndpointError(...).withRawResponse();
...
```typescript
import { SeedErrors, logging } from "@fern/ts-error-name-collision";
