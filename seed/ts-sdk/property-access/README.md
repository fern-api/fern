...
```typescript
import { SeedCsharpAccessError } from "@fern/property-access";

try {
    await client.createUser(...);
} catch (err) {
    if (err instanceof SeedCsharpAccessError) {
...
const client = new SeedCsharpAccessClient({
    ...
