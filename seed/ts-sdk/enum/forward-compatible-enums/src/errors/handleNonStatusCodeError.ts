...
export function handleNonStatusCodeError(error: core.Fetcher.Error, rawResponse: core.RawResponse, method: string, path: string): never {
    switch (error.reason) {
        ...
        case "body-is-null": throw new errors.SeedEnumError({
            statusCode: error.statusCode,
            rawResponse: rawResponse
        });
        case "timeout": throw new errors.SeedEnumTimeoutError(`Timeout exceeded when calling ${method} ${path}.`);
        case "unknown": throw new errors.SeedEnumError({
            message: error.errorMessage,
            rawResponse: rawResponse
        });
        default: throw new errors.SeedEnumError({
