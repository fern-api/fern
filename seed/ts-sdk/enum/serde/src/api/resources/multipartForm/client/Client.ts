...
export class MultipartFormClient {
    ...
    private async __multipartForm(request: SeedEnum.MultipartFormRequest, requestOptions?: MultipartFormClient.RequestOptions): Promise<core.WithRawResponse<void>> {
        ...
        _body.append("color", serializers.Color.jsonOrThrow(request.color, { unrecognizedObjectKeys: "strip", omitUndefined: true }));
        if (request.maybeColor != null) {
            _body.append("maybeColor", serializers.Color.jsonOrThrow(request.maybeColor, { unrecognizedObjectKeys: "strip", omitUndefined: true }));
        }

        for (const _item of request.colorList) {
            _body.append("colorList", serializers.Color.jsonOrThrow(_item, { unrecognizedObjectKeys: "strip", omitUndefined: true }));
        }

        if (request.maybeColorList != null) {
            for (const _item of request.maybeColorList) {
                _body.append("maybeColorList", serializers.Color.jsonOrThrow(_item, { unrecognizedObjectKeys: "strip", omitUndefined: true }));
            }
        }
