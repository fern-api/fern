...
[![npm shield](https://img.shields.io/npm/v/@fern/header-auth)](https://www.npmjs.com/package/@fern/header-auth)
...
const client = new SeedHeaderTokenClient({ environment: "YOUR_BASE_URL", headerTokenAuth: "YOUR_HEADER_TOKEN_AUTH" });
...
```typescript
import { SeedHeaderTokenError } from "@fern/header-auth";

try {
    await client.service.getWithBearerToken(...);
} catch (err) {
    if (err instanceof SeedHeaderTokenError) {
...
```typescript
import { ServiceClient } from '@fern/header-auth/service';
...
```typescript
import { SeedHeaderTokenClient, logging } from "@fern/header-auth";
