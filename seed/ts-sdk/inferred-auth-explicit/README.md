...
[![npm shield](https://img.shields.io/npm/v/@fern/inferred-auth-explicit)](https://www.npmjs.com/package/@fern/inferred-auth-explicit)
...
```sh
npm i -s @fern/inferred-auth-explicit
```
...
const client = new SeedInferredAuthExplicitClient({ baseUrl: "YOUR_BASE_URL" });
...
```typescript
import { SeedInferredAuthExplicit } from "@fern/inferred-auth-explicit";

const request: SeedInferredAuthExplicit.GetTokenRequest = {
    ...
};
```
...
```typescript
import { SeedInferredAuthExplicitError } from "@fern/inferred-auth-explicit";

try {
    await client.auth.getTokenWithClientCredentials(...);
} catch (err) {
    if (err instanceof SeedInferredAuthExplicitError) {
...
```typescript
import { AuthClient } from '@fern/inferred-auth-explicit/auth';
...
```typescript
import { SeedInferredAuthExplicitClient, logging } from "@fern/inferred-auth-explicit";
