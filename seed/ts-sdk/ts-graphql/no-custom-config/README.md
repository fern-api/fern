...
[![npm shield](https://img.shields.io/npm/v/@fern/ts-graphql)](https://www.npmjs.com/package/@fern/ts-graphql)
...
```typescript
const { data } = await client.query.viewer({ id: true, name: true });
...
```typescript
for await (const node of client.query.paginate.viewerPosts({ first: 50 }, { id: true, title: true })) {
    console.log(node);
}
```
...
```typescript
const { data, errors } = await client.query.viewer({ id: true, name: true });
if (errors != null && errors.length > 0) {
    // handle GraphQL operation errors (data may still be partially present)
}

// Or opt into exceptions:
const strict = await client.query.viewer({ id: true, name: true }, { throwOnError: true });
```
...
```typescript
import { SeedApiClient } from "@fern/ts-graphql";
...
```typescript
import { SeedApi } from "@fern/ts-graphql";

const request: SeedApi.UserRequest = {
    ...
};
```
...
```typescript
import { SeedApiError } from "@fern/ts-graphql";
...
```typescript
import { QueryClient } from '@fern/ts-graphql/query';
...
const response = await client.query.viewer(..., {
    headers: {
        'X-Custom-Header': 'custom value'
    }
});
...
```typescript
const { data, rawResponse } = await client.query.viewer(...).withRawResponse();
...
```typescript
import { SeedApiClient, logging } from "@fern/ts-graphql";
