...
describe("Stream", () => {
    ...
    describe("SSE event-level discrimination (inject discriminator)", () => {
        ...
        it("should parse id: and retry: fields and expose via events()", async () => {
            ...
            expect(events).toEqual([
                { data: { type: "completion", content: "hello" }, event: "completion", id: "evt-1", retry: 3000 },
                { data: { type: "completion", content: "world" }, event: "completion", id: "evt-2", retry: 3000 },
            ]);
            expect(stream.lastEventId).toBe("evt-2");
        });
        ...
        it("should persist lastEventId across events per SSE spec", async () => {
            ...
            expect(events).toEqual([
                { data: { type: "a", v: 1 }, event: "a", id: "persistent-id", retry: undefined },
                { data: { type: "b", v: 2 }, event: "b", id: "persistent-id", retry: undefined },
            ]);
        });

        it("should ignore invalid retry values", async () => {
            const mockStream = createReadableStream(['event: a\nretry: not-a-number\ndata: {"v": 1}\n\n']);
            const stream = new Stream({
                stream: mockStream,
                parse: async (val: unknown) => val,
                eventShape: { type: "sse", eventDiscriminator: "type" },
            ...
            expect(events).toEqual([{ data: { type: "a", v: 1 }, event: "a", id: undefined, retry: undefined }]);
        });
