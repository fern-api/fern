...
[![npm shield](https://img.shields.io/npm/v/@fern/server-sent-events-resumable)](https://www.npmjs.com/package/@fern/server-sent-events-resumable)
...
```sh
npm i -s @fern/server-sent-events-resumable
```
...
```typescript
import { SeedServerSentEventsResumable } from "@fern/server-sent-events-resumable";

const request: SeedServerSentEventsResumable.StreamCompletionRequest = {
    ...
};
```
...
```typescript
import { SeedServerSentEventsResumableError } from "@fern/server-sent-events-resumable";

try {
    await client.completions.stream(...);
} catch (err) {
    if (err instanceof SeedServerSentEventsResumableError) {
...
```typescript
import { CompletionsClient } from '@fern/server-sent-events-resumable/completions';

const client = new CompletionsClient({...});
```
...
const client = new SeedServerSentEventsResumableClient({
    ...
    headers: {
...
```typescript
import { SeedServerSentEventsResumableClient, logging } from "@fern/server-sent-events-resumable";
