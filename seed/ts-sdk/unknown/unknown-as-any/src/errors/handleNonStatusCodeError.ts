...
export function handleNonStatusCodeError(error: core.Fetcher.Error, rawResponse: core.RawResponse, method: string, path: string): never {
    switch (error.reason) {
        ...
        case "body-is-null": throw new errors.SeedUnknownAsAnyError({
            statusCode: error.statusCode,
            rawResponse: rawResponse
        });
        case "timeout": throw new errors.SeedUnknownAsAnyTimeoutError(`Timeout exceeded when calling ${method} ${path}.`, {
            cause: error.cause
        });
        ...
        default: throw new errors.SeedUnknownAsAnyError({
