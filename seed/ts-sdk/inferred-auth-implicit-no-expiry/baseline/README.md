...
[![npm shield](https://img.shields.io/npm/v/@fern/inferred-auth-implicit-no-expiry)](https://www.npmjs.com/package/@fern/inferred-auth-implicit-no-expiry)
...
```sh
npm i -s @fern/inferred-auth-implicit-no-expiry
```
...
```typescript
import { SeedInferredAuthImplicitNoExpiry } from "@fern/inferred-auth-implicit-no-expiry";

const request: SeedInferredAuthImplicitNoExpiry.GetTokenRequest = {
    ...
};
```
...
```typescript
import { SeedInferredAuthImplicitNoExpiryError } from "@fern/inferred-auth-implicit-no-expiry";

try {
    await client.auth.getTokenWithClientCredentials(...);
} catch (err) {
    if (err instanceof SeedInferredAuthImplicitNoExpiryError) {
...
```typescript
import { AuthClient } from '@fern/inferred-auth-implicit-no-expiry/auth';
...
```typescript
import { SeedInferredAuthImplicitNoExpiryClient, logging } from "@fern/inferred-auth-implicit-no-expiry";
