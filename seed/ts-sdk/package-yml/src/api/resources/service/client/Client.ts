...
export class ServiceClient {
    ...
    private async __nop(nestedId: string, requestOptions?: ServiceClient.RequestOptions): Promise<core.WithRawResponse<void>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, requestOptions?.headers);
        const _response = await core.fetcher({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), `/${core.url.encodePathParam(this._options.id)}//${core.url.encodePathParam(nestedId)}`),
    method: "GET",
    headers: _headers,
...
switch (_response.error.reason) {
    ...
    case "timeout": throw new errors.SeedPackageYmlTimeoutError("Timeout exceeded when calling GET /{id}/{nestedId}.");
