...
class BClient:
    ...
    def get_movie(self, movie_id: MovieId) -> Movie:
        _response = httpx.request(
            "GET", urllib.parse.urljoin(f"{self._environment.server_b}/", f"movie/movie/{movie_id}")
        )
    ...
    def delete_movie(self, movie_id: MovieId) -> None:
        _response = httpx.request("DELETE", urllib.parse.urljoin(f"{self._environment.server_b}/", f"movie/{movie_id}"))
        if 200 <= _response.status_code < 300:
            return
        if _response.status_code == 404:
            raise MovieNotFoundError(pydantic.parse_obj_as(MovieId, _response.json()))  # type: ignore
...
class AsyncBClient:
    ...
    async def delete_movie(self, movie_id: MovieId) -> None:
        async with httpx.AsyncClient() as _client:
            _response = await _client.request(
                "DELETE", urllib.parse.urljoin(f"{self._environment.server_b}/", f"movie/{movie_id}")
            )
        if 200 <= _response.status_code < 300:
            return
        if _response.status_code == 404:
