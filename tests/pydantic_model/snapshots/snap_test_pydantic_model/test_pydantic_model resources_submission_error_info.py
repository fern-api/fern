...
from .compile_error import CompileError as resources_submission_compile_error_CompileError
from .internal_error import InternalError as resources_submission_internal_error_InternalError
from .runtime_error import RuntimeError as resources_submission_runtime_error_RuntimeError

T_Result = typing.TypeVar("T_Result")


class _Factory:
    def compile_error(self, value: resources_submission_compile_error_CompileError) -> ErrorInfo:
        return ErrorInfo(__root__=_ErrorInfo.CompileError(**dict(value), type="compileError"))

    def runtime_error(self, value: resources_submission_runtime_error_RuntimeError) -> ErrorInfo:
        return ErrorInfo(__root__=_ErrorInfo.RuntimeError(**dict(value), type="runtimeError"))

    def internal_error(self, value: resources_submission_internal_error_InternalError) -> ErrorInfo:
...
class ErrorInfo(pydantic.BaseModel):
    ...
    def visit(
        self,
        compile_error: typing.Callable[[resources_submission_compile_error_CompileError], T_Result],
        runtime_error: typing.Callable[[resources_submission_runtime_error_RuntimeError], T_Result],
        internal_error: typing.Callable[[resources_submission_internal_error_InternalError], T_Result],
    ) -> T_Result:
...
class _ErrorInfo:
    class CompileError(resources_submission_compile_error_CompileError):
        type: typing_extensions.Literal["compileError"]

        class Config:
            frozen = True

    class RuntimeError(resources_submission_runtime_error_RuntimeError):
        type: typing_extensions.Literal["runtimeError"]

        class Config:
            frozen = True

    class InternalError(resources_submission_internal_error_InternalError):
