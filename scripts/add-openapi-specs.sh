...
for dir in fern/apis/*/; do
    ...
    # When updating generators.yml, skip APIs that already have a specs section
    if [ "$UPDATE_GENYML" = true ] && grep -q "specs:" "$gen_file" 2>/dev/null; then
        skipped=$((skipped + 1))
        continue
    fi
    ...
    # Optionally update generators.yml to reference the spec
    if [ "$UPDATE_GENYML" = true ]; then
        api_block="api:\n  specs:\n    - openapi: ./openapi.yml"
