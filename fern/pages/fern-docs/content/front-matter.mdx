...
- `reference`: Full-width content, no table of contents. Designed for API documentation and reference material.
- `overview`: Full-width, no table of contents. Ideal for landing pages and section overviews.
- `guide`: Standard layout with table of contents sidebar on the right. Suitable for guides, tutorials, and structured content.
- `reference`: Full-width content, no table of contents. Designed for API documentation and reference material.
- `page`: Full-screen, no table of contents or sidebar navigation. Use for standalone pages like contact forms or legal pages.
- `custom`: No predefined styling, allows full layout customization using CSS. Table of contents and sidebar navigation are hidden.
