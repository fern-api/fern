...
There are several different ways that you can customize the SDK with Fern. See below for 
specific examples. 

### Custom logic

If you want to provide any logic in your SDK that goes beyond hitting the REST API, you can 
do so by leveraging `.fernignore`. Add any of your custom files to the SDK repository, and 
list them out in `.fernignore`. `fernignore` functions just like a `gitignore` but it tells
the generators which files to ignore on successive generations. 
...
<Note>
  While the examples above only show TypeScript and Python, they can be extrapolated to 
  any language. 
</Note>

### Custom SDK methods

Fern also allows you to add custom methods to the SDK itself (e.g. `client.my_method()` ). While the specifics
are slightly different for each language, the underlying principle is the same: extension. You can inherit the 
Fern generated client and add **whatever methods you want**. Fern's generated SDK classes all have protected variables 
that can be accessed by subclasses for this reason. 
