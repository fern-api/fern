...
You can also use the `--help` flag after each Fern CLI command for specific help on that command.
...
Options:
  --help         Show help                                             [boolean]
  --log-level      [choices: "debug", "info", "warn", "error"] [default: "info"]
  --device-code  Use device code authorization        [boolean] [default: false]
```
</CodeBlock>