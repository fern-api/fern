...
## Version your APIs
To configure versioning for your API reference, first set up each API version as its own API (configured as [multiple APIs](/learn/api-definition/introduction/what-is-the-fern-folder#multiple-apis)). Then, reference the API using the `api-name` in its relevant version navigation. 
