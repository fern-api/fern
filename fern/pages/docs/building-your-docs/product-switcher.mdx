...
You can modify the border radius and add borders to make the selectors more prominent and better integrated with your site's design aesthetic with `border: 1px solid var(--border);`.
...
Use `display: grid;` to enable a grid layout with `grid-template-columns: repeat(N, 1fr);` where `N` is the number of columns in the grid.
