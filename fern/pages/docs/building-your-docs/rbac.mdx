...
The viewers are inherited by nested pieces of content. For example, if a section can only be viewed by `admins`, then all the
...
browser session to have a cooke caleld `FERN_TOKEN`. If the cooke is not present, the user will be redirected to your company's
login page.
