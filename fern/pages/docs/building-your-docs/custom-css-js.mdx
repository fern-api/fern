...
Let's say you want to change the default text that appears within the search bar of your docs site. First, we'll inspect `inspect elements` within Chrome Developer Tools to identify that the attribute we want to change is `.bg-search-dialog .form-input`. Then we can write a file, in this case called `search-dialog-text.js` to control the default text in the search bar.

<CodeBlock title="fern/docs/assets/search-dialog-text.js">
```js
// check for the search bar and change the placeholder text
 function changeSearch() {
   var searchInput = document.querySelector('.bg-search-dialog .form-input');
   if (searchInput) {
 ...
 function observePageChanges() {
   ...
   const callback = function(mutationsList, _observer) {
       for (let mutation of mutationsList) {
           // if the page changes, change the search placeholder
 ...
 // wait for a click event on the search placeholder, or for a / keypress
 // then wait for the searchbar to appear 
 document.addEventListener("click", function(event) {
   console.log(event.target)
   if (
       event.target.classList.contains('search-placeholder') || 
       event.target.classList.contains('fern-search-bar') || 
       event.target.classList.contains('size-icon-md') ||
       event.target.classList.contains('keyboard-shortcut-hint') ||
...
Then we'll add this file to our `docs.yml`:

<CodeBlock title="docs.yml">
```yaml
js:
    - path: ./docs/assets/search-dialog-text.js
