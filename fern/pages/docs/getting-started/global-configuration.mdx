...
An `instance` is the backend of a distinct docs website. Each instance has a unique set of content (such as a Landing Page, Guides, and API Reference) published to a unique domain. It is most common to use multiple instances on the Enterprise plan.
