---
title: 'Commands'
description: 'Complete reference for all Fern CLI commands'
subtitle: 'Explore Fern CLI commands.'
---
...
fern generate                    # Generate and publish SDKs for the default group (configured in generators.yml)

# Working with specific SDK groups
fern generate --group python-sdk --preview  # Preview Python SDK
fern generate --group ts-sdk                # Generate TypeScript SDK
...
<AccordionGroup>
  ...
  <Accordion title="Setting up SDK generation">
    ...
    2. Configure your generators in [`generators.yml`](/learn/generate-sdks/configuration)
    
    3. Generate SDKs:
    ```bash
    fern generate --preview  # Preview changes locally in .preview/ folder
...
|---------|-------------|----------------|
| [`fern init`](#fern-init) | Create new Fern project | Starting new API project |
| [`fern check`](#fern-check) | Validate API definition & config | Development validation, CI checks |
| [`fern upgrade`](#fern-upgrade) | Update Fern CLI & generators | Keeping tools up to date |

### Documentation Commands
| Command | Description | Common Use Case |
|---------|-------------|----------------|
| [`fern docs dev`](#fern-docs-dev) | Run local docs preview server | Local docs development |
| [`fern docs broken-links`](#fern-docs-broken-links) | Validate documentation links | Quality assurance |
| [`fern generate --docs`](#fern-generate---docs) | Build & publish documentation | Release docs updates |

### SDK Generation Commands  
| Command | Description | Common Use Case |
|---------|-------------|----------------|
| [`fern generate`](#fern-generate) | Build & publish SDKs | Release SDK updates |
| [`fern write-definition`](#fern-write-definition) | Convert OpenAPI to [Fern Definition](/learn/overview) | API migration |
| [`fern write-overrides`](#fern-write-overrides) | Create OpenAPI customizations | Enhance OpenAPI specs |
...
<AccordionGroup>
  <Accordion title="fern generate">
    ...
    ```bash
    fern generate [--group <group>] [--api <api>] [--version <version>] [--local] [--keepDocker] [--preview]
    ```
    </CodeBlock>
