...
- Fix: Several bugfixes were made to related to literal properties. If a literal is
  used as a query parameter, header, path parameter, or request parameter, the user
...
- Icon is now available as a markdown component. For details on usage, see the [Icon component documentation](/learn/docs/components/icons).
