...
If you wish to use a custom domain (such as `https://example.com`), or a subdomain or subpath on your custom domain (such as `https://docs.example.com` or `https://example.com/docs`), visit the [Custom domain](/fern-docs/config/branding/custom-domain) page. 
