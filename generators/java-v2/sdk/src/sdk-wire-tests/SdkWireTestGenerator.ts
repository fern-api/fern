...
export class SdkWireTestGenerator {
    ...
    private applyEndpointSpecificTransformations(
        ...
        if (isStreamingEndpoint) {
            ...
            // Handle V2 API specific request type transformations
            transformedSnippet = transformedSnippet.replace(/\bV2ChatStreamRequest\b/g, "V2ChatRequest");
            transformedSnippet = transformedSnippet.replace(/\bV2GenerateStreamRequest\b/g, "V2GenerateRequest");
        }
        ...
        // Fix empty union factory method calls by adding sample parameters
        transformedSnippet = transformedSnippet.replace(/(\w+)\.of\(\s*\)/g, (match, className) => {
            if (className.includes("DocumentsItem")) {
                return `${className}.of("sample document text")`;
            }
            return `${className}.of("sample value")`;
        });

        // TODO: Fix at source - Dynamic snippets should not generate invalid .stream(boolean) calls
        // These method calls don't exist on request builders - root cause: snippet generation assumes
        // streaming is controlled by request properties rather than separate endpoint methods
        transformedSnippet = transformedSnippet.replace(/\.stream\(true\)/g, "");
        transformedSnippet = transformedSnippet.replace(/\.stream\(false\)/g, "");

        // TODO: Fix at source - Dynamic snippets are generating wrong method names
        // Root cause: API spec method name mapping issue in dynamic IR generation
        // Different APIs use different query parameter method names
        const packageName = this.context.getRootPackageName();
        if (packageName.includes("auth0") || packageName.includes("mgmt")) {
            // Auth0 APIs use .q() method
            transformedSnippet = transformedSnippet.replace(/\.query\(/g, ".q(");
        } else {
            // Other APIs like Cohere use .query() method
            transformedSnippet = transformedSnippet.replace(/\.q\(/g, ".query(");
        }

        // TODO: Fix at source - Java dynamic snippets should generate proper generic types
        // Root cause: Type inference in snippet generation not handling Map<String, Object> correctly
        transformedSnippet = transformedSnippet.replace(
            /Stream\.fromJson\(Map\.class,/g,
            "Stream.<Map<String, Object>>fromJson(Map.class,"
        );
        transformedSnippet = transformedSnippet.replace(
            /Stream\.fromSse\(Map\.class,/g,
            "Stream.<Map<String, Object>>fromSse(Map.class,"
        );

        // Fix incorrect request class names generated by dynamic snippets
        // The dynamic snippets incorrectly generate double "Stream" suffixes for streaming endpoints
        if (transformedSnippet.includes("ChatStreamStreamRequest")) {
            console.log("[TRANSFORM DEBUG] Found ChatStreamStreamRequest, replacing...");
            transformedSnippet = transformedSnippet.replace(/\bChatStreamStreamRequest\b/g, "ChatStreamRequest");
        }
        if (transformedSnippet.includes("GenerateStreamStreamRequest")) {
            console.log("[TRANSFORM DEBUG] Found GenerateStreamStreamRequest, replacing...");
            transformedSnippet = transformedSnippet.replace(
                /\bGenerateStreamStreamRequest\b/g,
