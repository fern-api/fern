...
export class WireTestDataExtractor {
    ...
    public getTestExamples(endpoint: FernIr.HttpEndpoint): WireTestExample[] {
        ...
        // For SSE endpoints, prefer autogenerated examples when they have richer
        // event coverage (more union variants) than user-specified examples.
        const useAutoForSse = this.hasBetterAutoGeneratedSseExamples(endpoint);
    ...
    private hasBetterAutoGeneratedSseExamples(endpoint: FernIr.HttpEndpoint): boolean {
        const getSseCount = (example: FernIr.ExampleEndpointCall): number => {
        ...
        const maxUser = Math.max(0, ...userExamples.map(getSseCount));
        const maxAuto = Math.max(0, ...autoExamples.map(getSseCount));
        return maxAuto > maxUser;
    }
        ...
        return response._visit({
            ok: (value) => {
                return value._visit({
                    ...
                    stream: () => undefined,
                    sse: (events) => {
                        // Format SSE events as an SSE protocol string for the mock response body.
                        // The Java client parses this as a text/event-stream response.
                        return events
                            .map((event) => {
                                const eventName = event.event;
                                const eventData = event.data?.jsonExample ?? {};
