...
export class WireTestDataExtractor {
    ...
    public getTestExamples(endpoint: FernIr.HttpEndpoint): WireTestExample[] {
        const bestExample = this.getBestEndpointExample(endpoint);
        const examples: WireTestExample[] = [];

        if (bestExample != null) {
            const extracted = this.extractTestExample(bestExample, endpoint);
            if (extracted) {
     ...
     * autogenerated examples with richer multi-variant event coverage.
     */
    private getBestEndpointExample(endpoint: FernIr.HttpEndpoint): FernIr.ExampleEndpointCall | undefined {
        ...
        if (userExample != null && autoExample != null) {
            const userSseCount = this.getSseEventCount(userExample);
            const autoSseCount = this.getSseEventCount(autoExample);
