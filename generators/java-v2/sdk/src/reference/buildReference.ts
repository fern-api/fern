...
function getEndpointReferencesForService({
  ...
  return service.endpoints
    .map((endpoint) => {
      let example: FernIr.ExampleEndpointCall | undefined;
      if (endpoint.userSpecifiedExamples.length > 0) {
        example = endpoint.userSpecifiedExamples[0]?.example;
      }
      if (example == null) {
        example = endpoint.autogeneratedExamples[0]?.example;
      }
      if (example == null) {
        return undefined;
      }
      const singleEndpointSnippet =
        context.snippetGenerator.getSingleEndpointSnippet({
          endpoint,
          example,
        });
    ...
    .filter(
      (endpoint): endpoint is FernGeneratorCli.EndpointReference => !!endpoint,
    );
}
...
function getEndpointReferenceDescription(
  endpoint: FernIr.HttpEndpoint,
): string | undefined {
...
function getAccessFromRootClient({
  ...
  const servicePath = service.name.fernFilepath.allParts.map((part) =>
    context.caseConverter.camelSafe(part),
  );
  return servicePath.length > 0
    ? `${clientVariableName}.${servicePath.join(".")}`
    : clientVariableName;
}
...
function getEndpointMethodName({
  ...
  return context.caseConverter.camelSafe(endpoint.name);
}
...
function getReferenceEndpointInvocationParameters({
  ...
  endpoint.allPathParameters.forEach((pathParam) => {
    parameters.push(context.caseConverter.camelSafe(pathParam.name));
  });
...
function getEndpointParameters({
  ...
  endpoint.queryParameters.forEach((queryParam) => {
    parameters.push({
      name: context.caseConverter.camelSafe(queryParam.name),
  ...
  endpoint.headers.forEach((header) => {
    parameters.push({
      name: context.caseConverter.camelSafe(header.name),
  ...
  if (
    ...
    endpoint.requestBody.properties.forEach((property) => {
      parameters.push({
        name: context.caseConverter.camelSafe(property.name),
...
function getJavaTypeString({
  ...
  try {
    const javaType = context.javaTypeMapper.convert({
...
function getJavaTypeStringForParameter(
  returnType: unknown,
  context: SdkGeneratorContext,
): string {
  const javaType = returnType as java.Type;
...
function getSectionTitle({
  ...
  return (
    ...
    service.name.fernFilepath.allParts
      .map((part) => context.caseConverter.pascalSafe(part))
      .join(" ")
  );
}
