...
export class DynamicTypeLiteralMapper {
                    ...
                    return swift.Expression.contextualMethodCall({
                        ...
                        arguments_: [
                            swift.functionArgument({
                                value: swift.Expression.boolLiteral(typeReference.value.value)
                            })
                        ]
                    });
                } else {
    ...
    private convertObject({
        ...
        // Reorder properties to match the Swift struct initializer order:
        // [...extendedProperties, ...ownProperties]. The dynamic IR may provide
        // properties in a different order (e.g. own first when hasSelfInterface=true).
        const orderedProperties = this.getOrderedObjectProperties(object_);
    ...
    /**
     * Reorders object properties so that inherited (extended) properties come before
     * own properties, matching the Swift struct initializer order generated by ObjectGenerator.
     */
    private getOrderedObjectProperties(object_: FernIr.dynamic.ObjectType): FernIr.dynamic.NamedParameter[] {
        // The `extends` field is present at runtime (populated by DynamicSnippetsConverter)
        // but may not be in the TypeScript type definition for older SDK versions.
        const extendedTypeIds = (object_ as { extends?: string[] }).extends;
        if (extendedTypeIds == null || extendedTypeIds.length === 0) {
            return object_.properties;
        }

        // Collect all extended property wire values by resolving parent types recursively.
        const extendedPropertyWireValues = new Set<string>();
        this.collectExtendedPropertyWireValues(extendedTypeIds, extendedPropertyWireValues);

        // Partition into extended and own properties, preserving relative order within each group.
        const extendedProperties: FernIr.dynamic.NamedParameter[] = [];
        const ownProperties: FernIr.dynamic.NamedParameter[] = [];
        for (const prop of object_.properties) {
            if (extendedPropertyWireValues.has(prop.name.wireValue)) {
                extendedProperties.push(prop);
            } else {
                ownProperties.push(prop);
            }
        }

        return [...extendedProperties, ...ownProperties];
    }

    /**
     * Recursively collects wire values of properties from extended (parent) types.
     */
    private collectExtendedPropertyWireValues(extendedTypeIds: string[], wireValues: Set<string>): void {
        for (const typeId of extendedTypeIds) {
            const namedType = this.context.resolveNamedType({ typeId });
            if (namedType == null || namedType.type !== "object") {
                continue;
            }
            // Recursively collect from grandparent types first.
            const parentExtends = (namedType as { extends?: string[] }).extends;
            if (parentExtends != null && parentExtends.length > 0) {
                this.collectExtendedPropertyWireValues(parentExtends, wireValues);
            }
            for (const prop of namedType.properties) {
                wireValues.add(prop.name.wireValue);
            }
        }
    }
