...
final class HTTPClient: Swift.Sendable {
    ...
    func performRequest<T: Swift.Decodable>(
        ...
        if responseType == Foundation.Data.self {
            if let data = data as? T {
                ...
                throw <%= errorEnumName %>.invalidResponse
            }
        }
        ...
        do {
            ...
            throw <%= errorEnumName %>.decodingError(error)
        }
    }
    ...
    private func executeRequestWithURLSession(
        ...
        for attempt in 0...maxRetries {
            do {
                ...
                } else if let existingClientError = error as? <%= errorEnumName %> {
                ...
                if attempt >= maxRetries || clientError != nil {
                    if let clientError {
                        ...
                        throw <%= errorEnumName %>.networkError(error)
                    }
                }
    ...
    private func makeErrorFromResponse(statusCode: Swift.Int, data: Foundation.Data) -> <%= errorEnumName %>
    {
        ...
        return <%= errorEnumName %>.httpError(httpError)
    }
}
