...
describe("SentryClient (base-generator)", () => {
    ...
    it("throws when SENTRY_RELEASE is not set but SENTRY_DSN is configured", () => {
        delete process.env.SENTRY_RELEASE;

        expect(() => new SentryClient(DEFAULT_ARGS)).toThrow(
            "SENTRY_RELEASE must be set when SENTRY_DSN is configured"
        );
    });

    it("throws when SENTRY_ENVIRONMENT is not set but SENTRY_DSN is configured", () => {
        delete process.env.SENTRY_ENVIRONMENT;

        expect(() => new SentryClient(DEFAULT_ARGS)).toThrow(
            "SENTRY_ENVIRONMENT must be set when SENTRY_DSN is configured"
        );
    });
});
