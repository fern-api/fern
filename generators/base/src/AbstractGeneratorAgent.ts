...
export abstract class AbstractGeneratorAgent<GeneratorContext extends AbstractGeneratorContext> {
    ...
    public async generateReadme({
        ...
        this.logger.debug(`AbstractGeneratorAgent.generateReadme: Feature config loaded with ${featureConfig.features?.length ?? 0} features`);
    ...
    public async generateReference(builder: ReferenceConfigBuilder): Promise<string> {
        ...
        this.logger.debug(`AbstractGeneratorAgent.generateReference: Reference config built with ${referenceConfig.sections?.length ?? 0} sections, calling CLI...`);
    ...
    protected async readFeatureConfig(): Promise<FernGeneratorCli.FeatureConfig> {
        ...
        try {
            ...
            this.logger.debug(`AbstractGeneratorAgent.readFeatureConfig: Loaded ${loaded.features?.length ?? 0} features`);
            return loaded;
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.logger.debug(`AbstractGeneratorAgent.readFeatureConfig: Failed to read feature config: ${errorMessage}`);
            throw error;
        }
    }
    ...
    private async getFeaturesConfig(): Promise<string> {
        ...
        this.logger.debug(`AbstractGeneratorAgent.getFeaturesConfig: Searching for features.yml in ${FEATURES_CONFIG_PATHS.length} paths...`);
        for (const each of FEATURES_CONFIG_PATHS) {
            try {
                ...
                if (rawContents.length !== 0) {
                    this.logger.debug(`AbstractGeneratorAgent.getFeaturesConfig: Found features.yml at ${each} (${rawContents.length} bytes)`);
                    return rawContents;
                }
                this.logger.debug(`AbstractGeneratorAgent.getFeaturesConfig: File at ${each} is empty, skipping`);
            } catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
                this.logger.debug(`AbstractGeneratorAgent.getFeaturesConfig: Path ${each} not accessible: ${errorMessage}`);
            }
        }
