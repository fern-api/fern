...
export class EndpointSnippetsGenerator {
    ...
    public getSingleEndpointSnippet({
        ...
        if (snippets) {
            const allSnippets = [...snippets.userSpecified, ...snippets.autogenerated];

            // First try to find by example ID (canonical identifier)
            const exampleId = example.id;
            const idMatchedSnippets = allSnippets.filter((s) => s.exampleIdentifier === exampleId);
            if (idMatchedSnippets.length > 0) {
                // Prefer snippets with request body data
                const withBody = idMatchedSnippets.find((s) => s.hasRequestBody);
                return withBody ?? idMatchedSnippets[0];
            }
            ...
            if (nameMatchedSnippets.length > 0) {
                // Prefer snippets with request body data
                const withBody = nameMatchedSnippets.find((s) => s.hasRequestBody);
                return withBody ?? nameMatchedSnippets[0];
            }

            // If no exact match, prefer any snippet with request body data
            const anyWithBody = allSnippets.find((s) => s.hasRequestBody);
            if (anyWithBody) {
                return anyWithBody;
            }

            // Absolute fallback: first available snippet
            const fallbackSnippet = allSnippets[0];
            if (fallbackSnippet) {
    ...
    private async generateSingleEndpointSnippetFromDynamic({
        ...
        try {
            ...
            return {
                ...
                hasRequestBody: example.requestBody != null
            };
        } catch (error) {
            return null;
        }
    ...
    private extractMethodCall(fullSnippet: string): string {
        // Extract just the method call from the full Ruby snippet
        ...
        for (const line of lines) {
            ...
            if (inMethodCall) {
                ...
                // Count parentheses to determine when the method call ends
                ...
                // If we hit the end of the line and parentheses are balanced, we're done
