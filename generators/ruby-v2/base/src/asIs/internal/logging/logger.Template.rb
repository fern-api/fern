    ...
    module Logging
      ...
      class Logger
        ...
        def self.from(config)
          ...
          if config.is_a?(LogConfig)
            ...
            new(
              ...
              silent: config.key?(:silent) ? config[:silent] : (config.key?("silent") ? config["silent"] : true)
            )
          else
            default
          end
        ...
        def debug(message, **kwargs)
          @logger.debug(message, **kwargs) if debug?
        end

        # @param message [String]
        def info(message, **kwargs)
          @logger.info(message, **kwargs) if info?
        end

        # @param message [String]
        def warn(message, **kwargs)
          @logger.warn(message, **kwargs) if warn?
        end

        # @param message [String]
        def error(message, **kwargs)
          @logger.error(message, **kwargs) if error?
        end
