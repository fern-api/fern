...
export class GeneratedDefaultEndpointImplementation implements GeneratedEndpointImplementation {
    ...
    public getStatements(context: SdkContext): ts.Statement[] {
        ...
        if (paginationInfo != null && paginationInfo.type === "custom") {
            ...
            statements.push(...buildStatements);

            // Build the Fetcher.Args object (the prepared HTTP request)
            const fetcherArgs = this.buildFetcherArgs(context, buildStatements);
            statements.push(
                ts.factory.createVariableStatement(
                    undefined,
                    ts.factory.createVariableDeclarationList(
        ...
        const body = [
            ...(this.generateEndpointMetadata
                ...
                : []),
            ...buildStatements,
            ...this.invokeFetcherAndReturnResponse(context, buildStatements)
        ];
    ...
    private buildFetcherArgs(context: SdkContext, buildStatements: ts.Statement[]): ts.Expression {
        ...
        // Only include headers if _headers was actually generated by generateHeaders
        ...
        if (hasEndpointSpecificHeaders && requestArgs.headers != null) {
    ...
    public invokeFetcherAndReturnResponse(context: SdkContext, buildStatements?: ts.Statement[]): ts.Statement[] {
        return [...this.invokeFetcher(context, buildStatements), ...this.response.getReturnResponseStatements(context)];
    }
    ...
    private invokeFetcher(context: SdkContext, buildStatements?: ts.Statement[]): ts.Statement[] {
        const requestArgs = this.request.getFetcherRequestArgs(context);
        // Only include headers if _headers was actually generated by generateHeaders
        const hasEndpointSpecificHeaders =
            buildStatements != null &&
            buildStatements.some(
                (stmt) =>
                    ts.isVariableStatement(stmt) &&
                    stmt.declarationList.declarations.some(
                        (decl) => ts.isIdentifier(decl.name) && decl.name.text === HEADERS_VAR_NAME
        ...
        const fetcherArgs: Fetcher.Args = {
            ...requestArgs,
            ...(hasEndpointSpecificHeaders ? {} : { headers: undefined }),
            url: this.getReferenceToBaseUrl(context),
            method: ts.factory.createStringLiteral(this.endpoint.method),
