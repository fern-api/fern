...
export function generateHeaders({
    ...
    // When the idempotency key is auto-generated (not declared as an endpoint header), the header
    // name and generation are the same for every eligible endpoint, so we merge in the shared
    // `getIdempotencyHeaders()` helper rather than re-emitting the header name + generator call here.
    const shouldMergeAutoGeneratedIdempotencyKey = autoGenerateIdempotencyKey && !wrappedDeclaredIdempotencyKey;
    ...
    if (shouldMergeAutoGeneratedIdempotencyKey) {
        context.importsManager.addImportFromRoot("core/idempotency", {
            namedImports: ["getIdempotencyHeaders"]
        });
        mergeHeadersArgs.push(
            ts.factory.createCallExpression(ts.factory.createIdentifier("getIdempotencyHeaders"), undefined, [])
        );
    }
