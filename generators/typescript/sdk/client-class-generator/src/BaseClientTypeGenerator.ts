...
import { ExportedFilePath, getParameterNameForRootPathParameter, getPropertyKey, getTextOfTsNode } from "@fern-typescript/commons";
import { SdkContext } from "@fern-typescript/contexts";

import { InterfaceDeclarationStructure, PropertySignatureStructure, StructureKind, ts } from "ts-morph";
...
export class BaseClientTypeGenerator {
    ...
    public static readonly TIMEOUT_IN_SECONDS_REQUEST_OPTION_PROPERTY_NAME = TIMEOUT_IN_SECONDS_REQUEST_OPTION_PROPERTY_NAME;
    ...
    public writeToFile(context: SdkContext): void {
        context.sourceFile.addInterface(this.generateBaseClientOptionsInterface(context));
        context.sourceFile.addInterface(this.generateBaseRequestOptionsInterface(context));
        if(this.generateIdempotentRequestOptions){
            context.sourceFile.addInterface(this.generateBaseIdempotentRequestOptionsInterface(context));
        }
    }
    
        private generateBaseClientOptionsInterface(context: SdkContext): InterfaceDeclarationStructure {
    ...
    private generateBaseRequestOptionsInterface(context: SdkContext): InterfaceDeclarationStructure {
        const supplier = context.coreUtilities.fetcher.SupplierOrEndpointSupplier;
        ...
        private generateBaseIdempotentRequestOptionsInterface(context: SdkContext): InterfaceDeclarationStructure {
            const properties: PropertySignatureStructure[] = [];
