...
export class ReactQueryGenerator {
     ...
     */
    private generateQueryKeysTestFile(
        ...
        if (queryEndpoints.length >= 2) {
            ...
            for (const ep of queryEndpoints.slice(0, 3)) {
                const fnName = `${ep.endpointSafeName}QueryKey`;
                const callArg =
                    ep.paramType === "no-param"
                        ? ``
                        : ep.paramType === "single-param"
                          ? `{} as any`
                          : `"x"${ep.hasRequestBody ? ", {} as any" : ""}`;
    ...
    private generateOptionsFactoriesTestFile(
        ...
        if (multiParamQuery != null) {
            ...
            lines.push(`            const client = createMockClient();`);
            lines.push(
                `            const options = ${multiParamQuery.endpointSafeName}Options(${callArgs.join(", ")});`
            );
            lines.push(`            expect(options).toHaveProperty("queryKey");`);
            lines.push(`            expect(options).toHaveProperty("queryFn");`);
            lines.push(`            expect(typeof options.queryFn).toBe("function");`);
        ...
        if (singleParamQuery != null) {
            ...
            lines.push(`            const request = { test: "value" };`);
            lines.push(
                `            const options = ${singleParamQuery.endpointSafeName}Options(client, request as any);`
            );
        ...
        if (noParamQuery != null) {
            ...
            lines.push(
                `            const opts2 = ${noParamQuery.endpointSafeName}Options(client, { timeout: 5000 } as any);`
            );
        ...
        if (singleParamQuery != null) {
            ...
            lines.push(`            const request = { test: "value" };`);
            lines.push(
                `            const opts1 = ${singleParamQuery.endpointSafeName}Options(client, request as any);`
            );
            lines.push(
                `            const opts2 = ${singleParamQuery.endpointSafeName}Options(client, request as any, { timeout: 5000 } as any);`
            );
            lines.push(`            expect(opts1.queryKey).toEqual(opts2.queryKey);`);
            lines.push(`        });`);
            lines.push(``);
        ...
        if (singleParamMutation != null) {
            ...
            lines.push(`            const client = createMockClient();`);
            lines.push(
                `            const options = ${singleParamMutation.endpointSafeName}MutationOptions(client, { timeout: 5000 } as any);`
            );
            lines.push(`            expect(options).toHaveProperty("mutationFn");`);
            lines.push(`        });`);
            lines.push(`    });`);
        ...
        if (noParamMutation != null) {
            ...
            lines.push(`            const client = createMockClient();`);
            lines.push(
                `            const options = ${noParamMutation.endpointSafeName}MutationOptions(client, { timeout: 3000 } as any);`
            );
            lines.push(`            expect(options).toHaveProperty("mutationFn");`);
            lines.push(`        });`);
            lines.push(`    });`);
    ...
    private generateHooksTestFile(
        ...
        const buildMockLevel = (groups: ServiceGroup[], depth: number): void => {
            ...
            for (const g of groups) {
                if (g.servicePath.length <= depth) {
                    ...
                    const pathPart = g.servicePath[depth];
                    if (pathPart == null) {
                        continue;
                    }
                    const seg = pathPart.unsafeName;
            ...
            for (const g of leafGroups) {
                for (const { endpoint } of g.endpoints) {
                    lines.push(
                        `${indent}${endpoint.name.camelCase.unsafeName}: vi.fn().mockResolvedValue("mock-result"),`
                    );
                }
            }
        ...
        if (noParamQuery != null) {
            const hookName = `use${noParamQuery.endpointSafeName.charAt(0).toUpperCase() + noParamQuery.endpointSafeName.slice(1)}`;
            const clientPath =
                noParamQuery.servicePath.length > 0
        ...
        if (singleParamQuery != null) {
            const hookName = `use${singleParamQuery.endpointSafeName.charAt(0).toUpperCase() + singleParamQuery.endpointSafeName.slice(1)}`;
            const clientPath =
                singleParamQuery.servicePath.length > 0
            ...
            lines.push(`            await waitFor(() => expect(result.current.isSuccess).toBe(true));`);
            lines.push(
                `            expect(${clientPath}.${singleParamQuery.endpointUnsafeName}).toHaveBeenCalledWith(request, undefined);`
            );
            lines.push(`        });`);
            lines.push(`    });`);
            lines.push(``);
        ...
        if (multiParamQuery != null) {
            const hookName = `use${multiParamQuery.endpointSafeName.charAt(0).toUpperCase() + multiParamQuery.endpointSafeName.slice(1)}`;
            const clientPath =
                multiParamQuery.servicePath.length > 0
        ...
        if (noParamMutation != null) {
            const hookName = `use${noParamMutation.endpointSafeName.charAt(0).toUpperCase() + noParamMutation.endpointSafeName.slice(1)}Mutation`;
            const clientPath =
                noParamMutation.servicePath.length > 0
        ...
        if (singleParamMutation != null) {
            const hookName = `use${singleParamMutation.endpointSafeName.charAt(0).toUpperCase() + singleParamMutation.endpointSafeName.slice(1)}Mutation`;
            const clientPath =
                singleParamMutation.servicePath.length > 0
            ...
            lines.push(`            await waitFor(() => expect(result.current.isSuccess).toBe(true));`);
            lines.push(
                `            expect(${clientPath}.${singleParamMutation.endpointUnsafeName}).toHaveBeenCalledWith("test-value", undefined);`
            );
            lines.push(`        });`);
            lines.push(`    });`);
            lines.push(``);
        ...
        if (multiParamMutation != null) {
            const hookName = `use${multiParamMutation.endpointSafeName.charAt(0).toUpperCase() + multiParamMutation.endpointSafeName.slice(1)}Mutation`;
            const clientPath =
                multiParamMutation.servicePath.length > 0
            ...
            lines.push(`            await waitFor(() => expect(result.current.isSuccess).toBe(true));`);
            lines.push(
                `            expect(${clientPath}.${multiParamMutation.endpointUnsafeName}).toHaveBeenCalledWith("path", "body");`
            );
            lines.push(`        });`);
            lines.push(`    });`);
            lines.push(``);
    ...
    private generateContextTestFile(): string {
        ...
        lines.push(``);
        lines.push(
            `import { ClientProvider, useClient } from "../../../${this.relativePackagePath}/react-query/context.js";`
        );
        lines.push(``);
     ...
     */
    private generateInvalidationTestFile(
        queryEndpoints: Array<{
            endpointSafeName: string;
            importPath: string;
            keySegments: string[];
        ...
        if (singleParamEp != null) {
            const cap =
                singleParamEp.endpointSafeName.charAt(0).toUpperCase() + singleParamEp.endpointSafeName.slice(1);
        ...
        if (multiParamEp != null) {
            ...
            lines.push(
                `            expect(spy).toHaveBeenCalledWith({ queryKey: [...${prefixKey}, ${expectedKeyArgs.join(", ")}] });`
            );
            lines.push(`        });`);
            lines.push(``);
