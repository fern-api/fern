...
export class TestGenerator {
    ...
    public getTestFile(service: FernIr.HttpService): ExportedFilePath {
        ...
        const filename = `${service.name.fernFilepath.file != null ? this.caseConverter.camelUnsafe(service.name.fernFilepath.file) : "main"}.test.ts`;
...
`;
    ...
    public getRequestExample(request: FernIr.ExampleRequestBody | undefined): Code | undefined {
        if (!request) {
            return undefined;
        }
        const requestExample = request._visit({
            inlinedRequestBody: (value) => {
                return code`${literalOf(
                    Object.fromEntries([
                        ...getUnusedPropertiesFromJsonExample(value.jsonExample, value, this.caseConverter),
                        ...value.properties
                            .map<[string, Code]>((p) => {
                                return [
                                    getWireValue(p.name),
            ...
            named: (value) => {
                return value.shape._visit<Code>({
                    ...
                    object: (value) => {
                        return code`${literalOf(
                            Object.fromEntries([
                                ...getUnusedPropertiesFromJsonExample(jsonExample, value, this.caseConverter),
                                ...value.properties
                                    .filter((p) => {
