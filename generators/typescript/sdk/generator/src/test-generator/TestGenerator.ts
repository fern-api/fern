...
`;
    ...
    /**
     * Whether this service would produce any wire test cases. Used to avoid emitting an empty
     * test file (e.g. for GraphQL services, whose endpoints have no examples) — vitest fails a
     * suite/file that contains no tests.
     */
    public serviceHasWireTests(service: FernIr.HttpService): boolean {
        return service.endpoints.some((endpoint) => this.endpointHasWireTests(endpoint));
    }

    private endpointHasWireTests(endpoint: FernIr.HttpEndpoint): boolean {
...
`;
    ...
    private shouldBuildTest(endpoint: FernIr.HttpEndpoint): boolean {
        ...
        // not match what the SDK actually sends. Skip wire tests for GraphQL until examples carry
        // the envelope. (Read the transport discriminant structurally: the published IR types this
        // generator compiles against may predate the `graphql` variant.)
        const transportType: string | undefined = endpoint.transport?.type;
