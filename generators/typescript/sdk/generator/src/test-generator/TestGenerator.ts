...
`;
    ...
    public buildFile(
        ...
        // The reserved-name set is shared with BaseClientContextImpl so the two never drift.
        const reservedClientOptionNames = BaseClientContextImpl.getReservedClientOptionNames({
            hasEnvironmentOption: !this.requireDefaultEnvironment
        });
        (this.ir.globalParameters ?? []).forEach((param) => {
            // Path-location globals are not exposed as client options (mirrors
            // BaseClientContextImpl), so the test client must not set them.
