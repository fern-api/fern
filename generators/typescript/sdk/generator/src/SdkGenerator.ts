...
export class SdkGenerator {
    ...
    public async generate(): Promise<TypescriptProject> {
        ...
        if (this.config.includeSerdeLayer) {
            // Snapshot the keys added during serde generation so we can detect
            // whether any serialization file was emitted. We can't rely on
            // `getDescendantSourceFiles()` because withSourceFile eagerly
            // extracts and deletes source files from the ts-morph project
            // after text extraction. Snapshotting the exact keys (instead of
            // just the count) also guards against a future caller that
            // concurrently adds unrelated entries to filePrefixes during
            // serde generation.
            const prefixKeysBefore = new Set(this.filePrefixes.keys());
            this.generateTypeSchemas();
            this.generatePerEndpointSerdeFiles();
            const addedSerdeKey = [...this.filePrefixes.keys()].some(
                (key) => !prefixKeysBefore.has(key) && key.includes("/serialization/")
            );
            if (addedSerdeKey) {
                exportSerde = true;
