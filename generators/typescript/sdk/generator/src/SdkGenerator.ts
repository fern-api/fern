...
import { GeneratedSdkClientClassImpl } from "@fern-typescript/sdk-client-class-generator/src/GeneratedSdkClientClassImpl";
...
export class SdkGenerator {
    ...
    private getWebsocketClientsToGenerate(): Record<WebSocketChannelId, WebSocketChannel> {
        console.log("Getting websocket clients to generate");
        if (this.config.enableInlineTypes) {
            return Object.fromEntries(
    ...
    private generateTypeSchemas(): { generated: boolean } {
        ...
        let generated = false;
        for (const typeDeclaration of Object.values(this.getTypesToGenerate())) {
            // console.log(`Processing type declaration full: ${JSON.stringify(typeDeclaration, null, 2)}`);
            // console.log("Processing type declaration:", typeDeclaration.name);
            const filepath = this.typeSchemaDeclarationReferencer.getExportedFilepath(typeDeclaration.name);
            // console.log("generateTypeSchemas filepath:", JSON.stringify(filepath, null, 2));
            this.withSourceFile({
    ...
    private generateWebsocketClients() {
        ...
        for (const websocketClient of Object.values(this.getWebsocketClientsToGenerate())) {
            ...
            console.log("Websocket filepathStr:", filepathStr);
                ...
                run: ({ sourceFile, importsManager }) => {
                   ...
                   // const context = this.generateSdkContext({ sourceFile, importsManager });
                    ...
                    // ]);
                        ...
                        // build the interfaces first like interface Options {
                        // see https://github.com/HumeAI/hume-typescript-sdk/blob/ca322c8ecbafd9bb5832715ad381bf214413644e/src/api/resources/empathicVoice/resources/chat/client/Client.ts#L9

                    /*
                    // Step 0) get some code to print anywhere [done]
                    // Step 1) generate a Client.ts per channel. [IN PROGRESS]
    ...
    private withSourceFile({
        ...
        const filepathStr = convertExportedFilePathToFilePath(filepath);
        this.context.logger.debug(`[TS] Generating : ${filepathStr}`); // file path print here
    ...
    private async withRawFile({
        ...
        const filepathStr = convertExportedFilePathToFilePath(filepath);
        this.context.logger.debug(`Generating maybe here: ${filepathStr}`);
        await run({
