...
export class SdkGenerator {
    ...
    public async generate(): Promise<TypescriptProject> {
        ...
        if (this.config.includeSerdeLayer) {
            const prefixCountBefore = this.filePrefixes.size;
            this.generateTypeSchemas();
            this.generatePerEndpointSerdeFiles();
            // Check if any serialization files were generated by comparing
            // filePrefixes count. We can't use getDescendantSourceFiles()
            // because source files are eagerly extracted and deleted from
            // the ts-morph project after text extraction.
            if (this.filePrefixes.size > prefixCountBefore) {
                exportSerde = true;
