 ...
 * @template TResponse The type of the API response.
 */
export class CustomPager<TItem, TRequest, TResponse extends PayrocPaginatedResponse<TItem>>
    implements BiPager<TItem, TResponse>, AsyncIterable<TItem>
{
    ...
    private async sendRequestAndHandleResponse(request: TRequest): Promise<Page<TItem, TResponse>> {
        ...
        const parsed = await CustomPager.parseHttpCallAsync<TItem, TRequest, TResponse>(request, response, rawResponse);
    ...
    private static async parseHttpCallAsync<TItem, TRequest, TResponse extends PayrocPaginatedResponse<TItem>>(
        ...
        const prevUri = CustomPager.getLinkUri(response, CustomPager.PREVIOUS_REL);
        const hasPreviousPage = prevUri !== undefined;
        const previousPageRequest = prevUri ? CustomPager.cloneRequestWithNewUri(request, prevUri) : undefined;

        const nextUri = CustomPager.getLinkUri(response, CustomPager.NEXT_REL);
        const hasNextPage = nextUri !== undefined;
        const nextPageRequest = nextUri ? CustomPager.cloneRequestWithNewUri(request, nextUri) : undefined;
        ...
        const page = new Page<TItem, TResponse>({
            ...
            loadPage: () => {
                throw new Error("loadPage should not be called on CustomPager pages");
            }
        });
    ...
    public static async create<TItem, TRequest, TResponse extends PayrocPaginatedResponse<TItem>>(args: {
        ...
        const parsed = await CustomPager.parseHttpCallAsync<TItem, TRequest, TResponse>(
