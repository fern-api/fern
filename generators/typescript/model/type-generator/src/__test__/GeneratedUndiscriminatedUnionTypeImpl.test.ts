...
describe("GeneratedUndiscriminatedUnionTypeImpl", () => {
    describe("generateStatements / writeToFile", () => {
        ...
        it("avoids collisions between inline base properties and request/response variants", () => {
            const variantType = createDeclaredTypeName("Variant");
            const inlineRequestType = createDeclaredTypeName("InlineRequest");
            const inlineResponseType = createDeclaredTypeName("InlineResponse");
            const createReference = (typeName: FernIr.DeclaredTypeName) =>
            ...
            const createContext = (
                variantTypeReference = namedTypeRefNodeWithVariants("Variant", "Variant.Request", "Variant.Response")
            ) => {
                const context = createMockBaseContext({
                    typeRefOverrides: new Map([[variantType.typeId, variantTypeReference]]),
                    getTypeDeclarationFn: (typeName) => ({
                        name: typeName,
                        shape: FernIr.Type.object({
                            properties: [],
                        ...
                        availability: undefined,
                        source: undefined,
                        inline: typeName.typeId !== variantType.typeId
                    }),
                    getGeneratedTypeFn: (_typeName, typeNameOverride) => ({
                        type: "object",
                        generateStatements: () => [`export interface ${typeNameOverride} { value: string; }`]
                    })
                });
                context.type.getReferenceToInlinePropertyType = (
                    ...
                    typeRef.type === "named"
                        ? namedTypeRefNode(`${parentTypeName}.${propertyName}`)
            ...
            const baseProperties = [
                ...
                createObjectProperty("request", createReference(inlineRequestType)),
                createObjectProperty("response", createReference(inlineResponseType))
            ];
            const generatorWithVariants = createGenerator({
                typeName: "UnionWithReservedBaseProperties",
                members: [createUnionMember({ type: createReference(variantType) })],
                baseProperties,
                enableInlineTypes: true,
                generateReadWriteOnlyTypes: true
            });

            const output = serializeStatements(generatorWithVariants, createContext());

            expect(output).toContain("request: UnionWithReservedBaseProperties.RequestProperty;");
            expect(output).toContain("response: UnionWithReservedBaseProperties.ResponseProperty;");
            expect(output).toContain("export interface RequestProperty { value: string; }");
            expect(output).toContain("export interface ResponseProperty { value: string; }");
            expect(output).toContain("export type Request =");
            expect(output).toContain("export type Response =");
            expect(output).not.toContain("export interface Request {");
            expect(output).not.toContain("export interface Response {");

            const generatorWithoutVariants = createGenerator({
                typeName: "UnionWithReservedBaseProperties",
                members: [createUnionMember({ type: createReference(variantType) })],
                baseProperties,
                enableInlineTypes: true,
            ...
            const outputWithoutVariants = serializeStatements(generatorWithoutVariants, createContext());

            expect(outputWithoutVariants).toContain("request: UnionWithReservedBaseProperties.Request;");
            expect(outputWithoutVariants).toContain("response: UnionWithReservedBaseProperties.Response;");
            expect(outputWithoutVariants).toContain("export interface Request { value: string; }");
            expect(outputWithoutVariants).toContain("export interface Response { value: string; }");
            expect(outputWithoutVariants).not.toContain("export type Request =");
            expect(outputWithoutVariants).not.toContain("export type Response =");

            const requestOnlyVariant: TypeReferenceNode = {
                ...namedTypeRefNode("Variant"),
                requestTypeNode: ts.factory.createTypeReferenceNode("Variant.Request"),
                requestTypeNodeWithoutUndefined: ts.factory.createTypeReferenceNode("Variant.Request")
            };
            const outputWithRequestVariant = serializeStatements(
                generatorWithVariants,
                createContext(requestOnlyVariant)
            );

            expect(outputWithRequestVariant).toContain("request: UnionWithReservedBaseProperties.RequestProperty;");
            expect(outputWithRequestVariant).toContain("response: UnionWithReservedBaseProperties.Response;");
            expect(outputWithRequestVariant).toContain("export type Request =");
            expect(outputWithRequestVariant).not.toContain("export type Response =");
        });
