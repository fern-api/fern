...
describe("GeneratedObjectTypeImpl", () => {
    ...
    describe("generateModule", () => {
        ...
        it("uses the innermost container name for nested inline map values", () => {
            const inlineTypeName = createDeclaredTypeName("InlineItem");
            ...
            const context = createMockBaseContext({
                typeDeclarationOverrides: new Map([[inlineTypeName.typeId, inlineTypeDeclaration]])
            });
            let generatedTypeName: string | undefined;
            context.type.getGeneratedType = (_typeName: FernIr.DeclaredTypeName, typeNameOverride?: string) => ({
                type: "object",
                generateStatements: () => {
                    generatedTypeName = typeNameOverride;
                    return [];
                }
            });

            const inlineReference = FernIr.TypeReference.named({
                typeId: inlineTypeName.typeId,
                fernFilepath: inlineTypeName.fernFilepath,
                name: inlineTypeName.name,
                displayName: undefined,
                    ...
                    createObjectProperty(
                        ...
                        FernIr.TypeReference.container(
                            FernIr.ContainerType.map({
                                ...
                                valueType: FernIr.TypeReference.container(FernIr.ContainerType.list(inlineReference))
                            })
                        )
                    )
                ],
            ...
            generator.generateModule(context);
            expect(generatedTypeName).toBe("Item");
        });
    });
