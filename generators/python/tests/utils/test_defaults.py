from tests.utils.example_models.types.resources.types.object_with_defaults import (
    ObjectWithDefaults,
)
...
def test_defaulted_object() -> None:
    obj_with_none = ObjectWithDefaults(
        decimal=None, string=None, required_string="something else"
    )
...
def test_with_non_none_setting() -> None:
    obj_with_defaults = ObjectWithDefaults(
        decimal=3.14, string="hello", required_string="something else"
    )
