...
def test_default_aiohttp_client_without_dependency():
    ...
    with patch.dict('sys.modules', {'httpx_aiohttp': None}):
        # Import after patching - use the correct module path
        from core_utilities.shared.http_client import DefaultAioHttpClient
...
def test_default_aiohttp_client_with_dependency():
    ...
    # Create a proper mock that can be subclassed
    class MockHttpxAiohttpClient:
        def __init__(self, **kwargs):
            self.init_kwargs = kwargs
            MockHttpxAiohttpClient.last_instance = self

    MockHttpxAiohttpClient.last_instance = None

    mock_httpx_aiohttp = MagicMock()
    mock_httpx_aiohttp.HttpxAiohttpClient = MockHttpxAiohttpClient
    ...
    with patch.dict('sys.modules', {'httpx_aiohttp': mock_httpx_aiohttp}):
        ...
        for mod in modules_to_remove:
            if mod != 'httpx_aiohttp' and mod in sys.modules:
                del sys.modules[mod]
        ...
        assert MockHttpxAiohttpClient.last_instance is not None
...
def test_default_aiohttp_client_applies_defaults():
    ...
    with patch.dict('sys.modules', {'httpx_aiohttp': mock_httpx_aiohttp}):
        ...
        assert MockHttpxAiohttpClient.last_instance is not None
        call_kwargs = MockHttpxAiohttpClient.last_instance.init_kwargs
...
def test_default_async_httpx_client_export():
    ...
    from core_utilities.shared.http_client import DefaultAsyncHttpxClient
