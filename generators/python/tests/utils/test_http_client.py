...
from core_utilities.shared.http_client import (
    ...
    HttpClient,
    _build_url,
    _maybe_filter_none_from_multipart_data,
    get_request_body,
...
def test_multipart_data_serializes_dict_values() -> None:
    """Test that dict values in multipart data are JSON-serialized."""
    data = {"name": "test", "process_rule": {"indexing": "high_quality"}}
...
def test_multipart_data_preserves_non_dict_values() -> None:
    """Test that str, int, list values in multipart data are NOT modified."""
    data = {"name": "test", "count": 42, "tags": ["a", "b"]}
    fake_files = [("file", ("test.txt", b"hello", "text/plain"))]
    result = _maybe_filter_none_from_multipart_data(data, fake_files, None)
    assert result == {"name": "test", "count": 42, "tags": ["a", "b"]}


def test_multipart_data_filters_none_and_serializes_dicts() -> None:
    """Test that None values are removed AND dict values are serialized."""
...
def test_multipart_data_no_change_without_files() -> None:
    """Test that data is returned as-is when no files are present."""
    data = {"name": "test", "config": {"key": "val"}}
    result = _maybe_filter_none_from_multipart_data(data, None, None)
    assert result == data
    assert isinstance(result["config"], dict)  # Not serialized


def test_multipart_data_serializes_dicts_with_force_multipart() -> None:
...
def test_http_client_serializes_dict_data_with_files() -> None:
    """Integration test: HttpClient serializes dict values in data when files are present."""
    dummy_client = _DummySyncClient()
    http_client = HttpClient(
        httpx_client=dummy_client,  # type: ignore[arg-type]
        base_timeout=lambda: None,
    ...
    data_body = dummy_client.last_request_kwargs["data"]
    assert data_body["name"] == "test"
    assert data_body["process_rule"] == json.dumps({"indexing": "high_quality"})


@pytest.mark.asyncio
async def test_async_http_client_serializes_dict_data_with_files() -> None:
    """Integration test: AsyncHttpClient serializes dict values in data when files are present."""
    dummy_client = _DummyAsyncClient()
    http_client = AsyncHttpClient(
        httpx_client=dummy_client,  # type: ignore[arg-type]
        base_timeout=lambda: None,
