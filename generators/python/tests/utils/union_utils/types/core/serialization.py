...
def convert_and_respect_annotation_metadata(
    *,
    ...
    if (
        ...
        and isinstance(object_, pydantic.BaseModel)
    ):
        return _convert_mapping(object_.dict(), clean_type, direction)
    # TypedDicts
...
def _convert_mapping(
    ...
    for key, value in object_.items():
        ...
        if type_ is None:
            ...
            converted_object[_alias_key(key, type_, direction)] = (
                convert_and_respect_annotation_metadata(
...
def _get_alias_to_field_name(type_: typing.Any) -> typing.Dict[str, str]:
    field_to_hint = typing_extensions.get_type_hints(type_, include_extras=True)
    aliases = {}
    for field, hint in field_to_hint.items():
        maybe_alias = _get_alias_from_type(hint)
        if maybe_alias is not None:
            aliases[_get_alias_from_type(hint)] = field
    return aliases
...
def _alias_key(
    key: str, type_: typing.Any, direction: typing.Literal["read", "write"]
) -> str:
    if direction == "read":
        # If you're reading in objects with aliases, you want to be able to get the field name given the alias
        # so we leverage `_get_alias_to_field_name` to get that mapping and try to pull the given key
        return _get_alias_to_field_name(type_).get(key, key)
