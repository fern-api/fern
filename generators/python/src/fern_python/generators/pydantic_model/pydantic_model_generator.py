...
class PydanticModelGenerator(AbstractGenerator):
    ...
    def _generate_circular_cluster(
        ...
        for type_id in sorted_cluster:
            ...
            # Also create stub file for request variant if it exists (as_request=True)
            if self._should_generate_typedict(context=context, type_=type_decl.shape):
                request_filepath = context.get_filepath_for_type_id(type_id=type_id, as_request=True)
                request_class_name = context.get_class_name_for_type_id(type_id=type_id, as_request=True)
                
                request_stub_content = (
                    f"# This file was auto-generated by Fern from our API Definition.\n\n"
                    f"from ..{consolidated_filename} import {class_name} as {request_class_name}\n\n"
                    f'__all__ = ["{request_class_name}"]\n'
                )
                
                request_stub_filepath = project.get_relative_source_file_filepath(request_filepath)
                project.add_file(request_stub_filepath, request_stub_content)
