...
class TypeDeclarationSnippetGeneratorBuilder:
    ...
    def get_generator(
        ...
        return TypeDeclarationSnippetGenerator(
            ...
            enum=lambda name, example: EnumSnippetGenerator(
                snippet_writer=self._snippet_writer, name=name, example=example, use_str_enums=self._context.use_str_enums
            ).generate_snippet(),
            ...
            undiscriminated_union=lambda name, example: self._get_undiscriminated_union_snippet_generator(name, example),
        )
    ...
    def _get_undiscriminated_union_snippet_generator(self, name: ir_types.DeclaredTypeName, example: ir_types.ExampleUndiscriminatedUnionType) -> Optional[AST.Expression]:
        if self._context.use_typeddict_requests:
