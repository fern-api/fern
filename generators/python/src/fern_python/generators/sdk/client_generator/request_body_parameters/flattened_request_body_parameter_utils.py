...
def get_json_body_for_inlined_request(context: SdkGeneratorContext, properties: List[ir_types.InlinedRequestBodyProperty], are_any_properties_optional: bool) -> Optional[AST.Expression]:
        ...
        def write(writer: AST.NodeWriter) -> None:
            ...
            with writer.indent():
                for property in properties:
                    property_name = _get_property_name(property)
                    possible_literal_value = context.get_literal_value(property.value_type)
...
def get_pre_fetch_statements_for_inlined_request(context: SdkGeneratorContext, properties: List[ir_types.InlinedRequestBodyProperty], are_any_properties_optional: bool) -> Optional[AST.CodeWriter]:
        ...
        if len(required_properties) == 0:
            ...
            with writer.indent():
                for required_property in required_properties:
                    literal_value = context.get_literal_value(reference=required_property.value_type)
                    if literal_value is not None and type(literal_value) is str:
                        ...
                        writer.write_line(
                            f'"{required_property.name.wire_value}": {_get_property_name(required_property)},'
                        )
            writer.write_line("}")

        for optional_property in optional_properties:
            writer.write_line(
                f"if {_get_property_name(optional_property)} is not {DEFAULT_BODY_PARAMETER_VALUE}:"
            )
            with writer.indent():
                writer.write_line(
                    f'{_REQUEST_VARIABLE_NAME}["{optional_property.name.wire_value}"] = {_get_property_name(optional_property)}'
                )

    return AST.CodeWriter(write)

def are_any_properties_optional_in_inlined_request(context: SdkGeneratorContext, properties: List[ir_types.InlinedRequestBodyProperty]) -> bool:
    return any(
