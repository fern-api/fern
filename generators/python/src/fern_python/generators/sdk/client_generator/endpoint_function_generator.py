...
class EndpointFunctionGenerator:
    ...
    def _get_stream_func_return_type(self) -> AST.TypeHint:
        underlying_type = self._get_response_body_underlying_type(response_body=self._endpoint.response.body, is_async=self._is_async)
        underlying_type_wrapped = AST.TypeHint.async_iterator(underlying_type) if self._is_async else AST.TypeHint.iterator(underlying_type)
        return self._get_response_type(self._is_async, underlying_type_wrapped)
        ...
        def write(writer: AST.NodeWriter) -> None:
            ...
            if self._endpoint.sdk_request is not None and self._endpoint.sdk_request.stream_parameter is not None:
                ...
                if is_streaming:
                    ...
                    writer.write_node(
                        AST.FunctionDeclaration(
                            name=EndpointFunctionGenerator.STREAM_FUNC_NAME,
                            signature=AST.FunctionSignature(
                                return_type=self._get_stream_func_return_type(),
                            ),
                            body=AST.CodeWriter(write_stream_generator),
                            is_async=self._is_async,
                        )
                    ...
                    writer.write_node(self._get_response_manager_type(self._is_async))
                    writer.write(f"(stream_func={EndpointFunctionGenerator.STREAM_FUNC_NAME})")
                else:
                    httpx_request = get_httpx_request(is_streaming=False, response_code_writer=response_code_writer)
                    writer.write_node(httpx_request)
    ...
    def _get_streaming_parameter_response_body_type(
        ...
        stream_response_wrapped = AST.TypeHint.async_iterator(stream_response) if is_async else AST.TypeHint.iterator(stream_response)
    ...
    def _get_response_body_type(
        ...
        underlying_type = self._get_response_body_underlying_type(response_body, is_async, streaming_parameter)

        if not self._is_raw_client:
            if response_body and is_streaming_endpoint(self._endpoint):
                return AST.TypeHint.async_iterator(underlying_type) if is_async else AST.TypeHint.iterator(underlying_type)
            return underlying_type

        if response_body and is_streaming_endpoint(self._endpoint):
            return self._get_response_manager_type(is_async, underlying_type)
        return self._get_response_type(is_async, underlying_type)
    ...
    def _get_response_body_underlying_type(
        ...
        return response_body.visit(
            file_download=lambda _: self._get_file_download_response_body_type(),
            json=lambda json_response: self._get_json_response_body_type(json_response),
            streaming=lambda stream_response: self._get_streaming_response_body_type(stream_response=stream_response),
            text=lambda _: AST.TypeHint.str_(),
            ...
            bytes=lambda _: AST.TypeHint.bytes(),
        ) if response_body is not None else AST.TypeHint.none()
    ...
    def _get_response_manager_type(self, is_async: bool, underlying_type: Optional[AST.TypeHint] = None) -> AST.TypeHint:
        return AST.TypeHint(
            type=AST.ClassReference(
                qualified_name_excluding_import=(),
                import_=AST.ReferenceImport(
                    module=AST.Module.local(*self._context.core_utilities._module_path, "http_response"),
                    named_import="AsyncHttpResponseStreamManager" if is_async else "HttpResponseStreamManager",
                ),
            ),
            type_parameters=[AST.TypeParameter(underlying_type)] if underlying_type is not None else None,
        )

    def _get_response_type(self, is_async: bool, underlying_type: Optional[AST.TypeHint] = None) -> AST.TypeHint:
        return AST.TypeHint(
            type=AST.ClassReference(
                qualified_name_excluding_import=(),
                import_=AST.ReferenceImport(
    ...
    def generate_wrapper_function(self) -> AST.FunctionDeclaration:
        ...
        def write_method_body(writer: AST.NodeWriter) -> None:
            maybe_async = "async " if self._is_async else ""
            maybe_await = "await " if self._is_async else ""
            raw_client_method_name = get_endpoint_name(self._endpoint)
            client_call = f"{maybe_await}self._raw_client.{raw_client_method_name}("
            if is_streaming_endpoint(self._endpoint):
                writer.write(f"{maybe_async}with {client_call}")
                with writer.indent():
                    for param in parameters:
                        if param:
                            writer.write_line(f"{param},")
                writer.write_line(") as r:")
                with writer.indent():
                    writer.write_line("return r.data")
            else:
                writer.write_line(f"_response = {client_call}")
                with writer.indent():
                    for param in parameters:
                        if param:
                            writer.write_line(f"{param},")
                writer.write_line(")")
                writer.write_line("return _response.data")
...
def is_streaming_endpoint(endpoint: ir_types.HttpEndpoint) -> bool:
    return endpoint.response is not None and endpoint.response.body and (
        endpoint.response.body.get_as_union().type == "streaming" or 
        endpoint.response.body.get_as_union().type == "fileDownload" or (
