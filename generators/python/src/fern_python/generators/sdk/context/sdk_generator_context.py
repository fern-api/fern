...
class SdkGeneratorContext(ABC):
    def __init__(
        ...
        *,
        ...
        _has_paginated_endpoints = any(map(lambda service: any(map(lambda ep: ep.pagination is not None, service.endpoints)), ir.services.values()))
        self.core_utilities = CoreUtilities(allow_skipping_validation=custom_config.pydantic_config.skip_validation, has_paginated_endpoints=_has_paginated_endpoints)
