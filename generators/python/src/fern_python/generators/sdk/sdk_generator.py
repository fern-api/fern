...
class SdkGenerator(AbstractGenerator):
    ...
    def run(
        ...
        *,
        ...
        # If exported_filename differs from filename, generate a re-export wrapper
    ...
    def _generate_exported_client_wrapper(
        ...
        actual_filename = custom_config.client_filename or custom_config.client.filename
        generated_module = actual_filename.removesuffix(".py")
        exported_module = custom_config.client.exported_filename.removesuffix(".py")

        generated_sync_class = context.get_class_name_for_generated_root_client()
        generated_async_class = "Async" + generated_sync_class
        exported_sync_class = context.get_class_name_for_exported_root_client()
        exported_async_class = "Async" + exported_sync_class

        if generated_sync_class != exported_sync_class:
            sync_import = f"from .{generated_module} import {generated_sync_class} as {exported_sync_class}"
            async_import = f"from .{generated_module} import {generated_async_class} as {exported_async_class}"
        else:
            sync_import = f"from .{generated_module} import {generated_sync_class}"
            async_import = f"from .{generated_module} import {generated_async_class}"
        ...
        contents = (
            ...
            f"{async_import}\n"
            "\n"
            f'__all__ = ["{exported_async_class}", "{exported_sync_class}"]\n'
        )
        ...
        filepath_on_disk = project.get_source_file_filepath(filepath, include_src_root=True)
        project.add_file(filepath_on_disk, contents)
