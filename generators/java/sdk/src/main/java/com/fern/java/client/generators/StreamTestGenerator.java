...
public final class StreamTestGenerator extends AbstractFileGenerator {
    ...
    public StreamTestGenerator(AbstractGeneratorContext<?, ?> generatorContext) {
        super(ClassName.get(generatorContext.getPoetClassNameFactory().getRootPackage(), STREAM_TEST_CLASS_NAME), generatorContext);
    }
    ...
    private MethodSpec createJsonStreamTest(ClassName streamClassName, ClassName testAnnotation) {
        return MethodSpec.methodBuilder("testJsonStream")
                ...
                .addCode(");\n")
                .addStatement("$T jsonStrings = messages.stream().map(this::mapToJson).toList()", 
                    ClassName.get("java.util", "List"))
                .addStatement("$T input = $T.join($S, jsonStrings)", 
                    ClassName.get(String.class),
                    ClassName.get(String.class),
                    "\n")
                .addStatement("$T jsonInput = new $T(input)", 
                    ClassName.get("java.io", "StringReader"), 
                    ClassName.get("java.io", "StringReader"))
                .addStatement("$T<$T> jsonStream = $T.fromJson($T.class, jsonInput)", 
                    streamClassName, 
                    ClassName.get("java.util", "Map"), 
                    streamClassName, 
                    ClassName.get("java.util", "Map"))
    ...
    private MethodSpec createSseStreamTest(ClassName streamClassName, ClassName testAnnotation) {
        return MethodSpec.methodBuilder("testSseStream")
                ...
                .addCode(");\n")
                .addStatement("$T sseStrings = events.stream().map(event -> $S + mapToJson(event)).toList()", 
                    ClassName.get("java.util", "List"),
                    "data: ")
                .addStatement("$T input = $T.join($S, sseStrings)", 
                    ClassName.get(String.class),
                    ClassName.get(String.class),
                    "\n\n")
                .addStatement("$T sseInput = new $T(input)", 
                    ClassName.get("java.io", "StringReader"), 
                    ClassName.get("java.io", "StringReader"))
                .addStatement("$T<$T> sseStream = $T.fromSse($T.class, sseInput)", 
                    streamClassName, 
                    ClassName.get("java.util", "Map"), 
                    streamClassName, 
                    ClassName.get("java.util", "Map"))
    ...
    private MethodSpec createSseStreamWithTerminatorTest(ClassName streamClassName, ClassName testAnnotation) {
        return MethodSpec.methodBuilder("testSseStreamWithTerminator")
                ...
                .addCode(");\n")
                .addStatement("$T sseStrings = events.stream().map(event -> $S + mapToJson(event)).collect($T.toList())", 
                    ClassName.get("java.util", "List"),
                    "data: ",
                ...
                .addStatement("$T sseInput = new $T(input)", 
                    ClassName.get("java.io", "StringReader"), 
                    ClassName.get("java.io", "StringReader"))
                .addStatement("$T<$T> sseStream = $T.fromSse($T.class, sseInput, $S)", 
                    streamClassName, 
                    ClassName.get("java.util", "Map"), 
                    streamClassName, 
                    ClassName.get("java.util", "Map"),
    ...
    private MethodSpec createStreamResourceTest(ClassName streamClassName, ClassName testAnnotation) {
        return MethodSpec.methodBuilder("testStreamResourceManagement")
                ...
                .addStatement("$T testInput = new $T($S)", 
                    ClassName.get("java.io", "StringReader"), 
                    ClassName.get("java.io", "StringReader"), 
                    "{\"test\":\"data\"}")
                .addStatement("$T<$T> testStream = $T.fromJson($T.class, testInput)", 
                    streamClassName, 
                    ClassName.get("java.util", "Map"), 
                    streamClassName, 
                    ClassName.get("java.util", "Map"))
    ...
    private MethodSpec createMapToJsonHelperMethod() {
        return MethodSpec.methodBuilder("mapToJson")
                ...
                .addStatement("try { return $T.JSON_MAPPER.writeValueAsString(map); } catch (Exception e) { throw new RuntimeException(e); }", 
