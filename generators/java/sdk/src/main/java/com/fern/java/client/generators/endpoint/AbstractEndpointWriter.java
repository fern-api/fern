...
public abstract class AbstractEndpointWriter {
    ...
    public final HttpEndpointMethodSpecs generate() {
        ...
        boolean emitAvailability = clientGeneratorContext.getCustomConfig().generateAvailabilityAnnotations();
        Optional<String> availabilityDocs =
                emitAvailability ? AvailabilityUtils.getAvailabilityDocs(httpEndpoint) : Optional.empty();
        boolean isDeprecated = emitAvailability && AvailabilityUtils.isDeprecated(httpEndpoint);
        ...
        HttpUrlBuilder httpUrlBuilder = new HttpUrlBuilder(
                ...
                variables
                        .sdkRequest()
                        .map(sdkRequest -> NameUtils.toName(sdkRequest.getRequestParameterName())
                                .getCamelCase()
                                .getSafeName())
                        .orElse(null),
                clientOptionsField,
