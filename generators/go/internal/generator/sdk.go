...
func (f *fileWriter) WriteRequestOptions(
	...
	for i, authScheme := range auth.Schemes {
		...
		if authScheme.Inferred != nil {
			// Track which public option functions have already been generated by
			// prior auth schemes (e.g., OAuth's WithClientID, WithClientSecret,
			// WithClientCredentials) to avoid generating duplicates.
