...
export class HttpEndpointGenerator extends AbstractEndpointGenerator {
    ...
    private generateEndpoint({
        ...
        return new go.Method({
            ...
            docs: this.getMethodDocs({ endpoint })
        });
    }

    private getMethodDocs({ endpoint }: { endpoint: FernIr.HttpEndpoint }): string | undefined {
        const parts: string[] = [];
        if (endpoint.docs != null) {
            parts.push(endpoint.docs);
        }
        const example = this.getEndpointExample({ endpoint });
        if (example != null) {
            parts.push(example);
        }
        if (parts.length === 0) {
    ...
    private getEndpointExample({ endpoint }: { endpoint: FernIr.HttpEndpoint }): string | undefined {
        ...
        if (snippet == null || snippet.endpointCall.trim().length === 0) {
            return undefined;
        }
        const indentedSnippet = snippet.endpointCall
            .split("\n")
        ...
        const fetcherBody = go.codeblock((writer) => {
            ...
            if (applyEndpointAuth) {
                ...
                writer.writeNode(
                    go.selector({
                        on: this.getReceiverCodeBlock({ subpackage }),
                        selector: go.codeblock(`options.AuthHeadersForEndpoint([][]string{${requirementsLiteral}})`)
                    })
                );
                writer.writeNewLineIfLastLineNot();
                writer.writeLine("if authErr != nil {");
                writer.indent();
                writer.writeLine("return nil, authErr");
                writer.dedent();
                writer.writeLine("}");
            }
            ...
            writer.writeNode(
                applyEndpointAuth
                    ? this.context.callMergeHeaders([baseHeaders, go.codeblock("authHeaders")])
                    : baseHeaders
            );
            writer.writeNewLineIfLastLineNot();
    ...
    private prepareRequestCall({
        ...
        return go.codeblock((writer) => {
            ...
            const buildHeaders = this.buildHeaders({ signature, endpoint, subpackage, rawClient });
            writer.writeNewLineIfLastLineNot();
    ...
    private buildHeaders({
        ...
        // In endpoint-security mode, the flat ToHeader() no longer emits any auth
