...
export class EndpointSnippetsGenerator {
    ...
    private readonly readmeEndpoints: Endpoint[] = [];

    constructor({ context }: { context: SdkGeneratorContext }) {
        this.context = context;
    }

    public getReadmeEndpoints(): Endpoint[] {
        return this.readmeEndpoints;
    }

    public async populateSnippetsCache({
        ...
        const endpointResults = await Promise.all(
            Object.entries(dynamicIr.endpoints).map(async ([endpointId, dynamicEndpoint]) => {
                ...
                const readmeEntries: Endpoint[] = [];
                const snippets = await Promise.all(
                    allExamples.map(async (example) => {
                        try {
                            const result = await this.generateSnippetWithReadmeEntry({
                                endpoint: irEndpoint,
                                endpointId,
                                dynamicEndpoint,
                                example,
                            ...
                            if (result?.readmeEntry != null) {
                                readmeEntries.push(result.readmeEntry);
                            }
                            return { snippet: result?.cacheEntry ?? null, isUserSpecified: false };
                        } catch (error) {
                ...
                const userSpecified: SingleEndpointSnippet[] = [];
                ...
                if (autogenerated.length > 0 || userSpecified.length > 0) {
                    ...
                    return { endpointId, autogenerated, userSpecified, readmeEntries };
                } else {
        ...
        for (const result of endpointResults) {
            if (result != null) {
                ...
                this.readmeEndpoints.push(...result.readmeEntries);
            }
        }
    ...
    private async generateSnippetWithReadmeEntry({
        endpoint,
        endpointId,
        dynamicEndpoint,
        example,
        ...
        dynamicSnippetsGenerator: DynamicSnippetsGenerator;
    }): Promise<{ cacheEntry: SingleEndpointSnippet; readmeEntry: Endpoint } | null> {
        try {
            this.context.logger.debug(
            ...
            const exampleId = (example as FernIr.dynamic.EndpointExample).id;

            const cacheEntry: SingleEndpointSnippet = {
                exampleIdentifier: exampleId,
                endpointCall: this.extractMethodCall(generatedSnippet.snippet)
            };

            const readmeEntry: Endpoint = {
                exampleIdentifier: exampleId,
                id: {
                    method: dynamicEndpoint.location.method,
                    path: FernGeneratorExec.EndpointPath(dynamicEndpoint.location.path),
