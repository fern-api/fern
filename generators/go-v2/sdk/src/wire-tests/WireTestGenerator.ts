...
export class WireTestGenerator {
    ...
    private callClientMethodAndAssert({
        ...
        return go.codeblock((writer) => {
            ...
            if (isSseStreaming) {
                // Iterate the SSE stream and count events
                const expectedEventCount = this.getSSEEventCount(endpoint);

                writer.writeLine("defer stream.Close()");
                writer.writeLine("eventCount := 0");
                writer.writeLine("for {");
                writer.writeLine("\t_, err := stream.Recv()");
