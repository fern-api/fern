...
export class WireTestGenerator {
    ...
    private async generateServiceTestFile(
        ...
        // Add io import if any endpoint is SSE streaming (needed for io.EOF check)
        if (endpoints.some((ep) => this.isSSEStreamingEndpoint(ep))) {
            imports.set("io", "io");
        }
    ...
    private isSSEStreamingEndpoint(endpoint: FernIr.HttpEndpoint): boolean {
        return endpoint.response?.body?.type === "streaming" && endpoint.response.body.value.type === "sse";
    }

    private getSSEEventCount(endpoint: FernIr.HttpEndpoint): number {
        const events = this.getSSEEvents(endpoint);
        return events?.length ?? 0;
    }

    private getSSEEvents(endpoint: FernIr.HttpEndpoint): FernIr.ExampleServerSideEvent[] | undefined {
        const allExamples = [...endpoint.userSpecifiedExamples, ...endpoint.autogeneratedExamples];
        for (const wrapper of allExamples) {
            const example = wrapper.example;
    ...
    private callClientMethodAndAssert({
        ...
        const isSseStreaming = this.isSSEStreamingEndpoint(endpoint);
        ...
        return go.codeblock((writer) => {
            ...
            if (isSseStreaming) {
                // For SSE streaming endpoints, capture the stream and iterate it
            ...
            if (isSseStreaming) {
                const sseEvents = this.getSSEEvents(endpoint);
                const expectedEventCount = sseEvents?.length ?? 0;

                writer.writeLine("defer stream.Close()");
                writer.writeLine("var events []interface{}");
                writer.writeLine("for {");
                writer.writeLine("\tevent, err := stream.Recv()");
                ...
                writer.writeLine();
                writer.writeLine("\t}");
                writer.writeLine("\tevents = append(events, event)");
                writer.writeLine("}");
                ...
                if (expectedEventCount > 0) {
                    writer.writeNode(
                        go.invokeFunc({
                            ...
                            arguments_: [
                                go.codeblock("t"),
                                go.codeblock(`${expectedEventCount}`),
                                go.codeblock("len(events)"),
                                go.TypeInstantiation.string(`Expected ${expectedEventCount} SSE event(s)`)
                            ],
                            multiline: false
                        })
                    );
                ...
                if (sseEvents != null && sseEvents.length > 0) {
                    for (let i = 0; i < sseEvents.length; i++) {
                        ...
                        if (dataJson != null && typeof dataJson === "object") {
                            const expectedJson = JSON.stringify(dataJson);
                            const escapedJson = expectedJson.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
                            writer.writeLine(`eventBytes${i}, marshalErr${i} := json.Marshal(events[${i}])`);
                            writer.writeLine(`require.NoError(t, marshalErr${i}, "Failed to marshal SSE event ${i}")`);
                            writer.writeLine(
                                `require.JSONEq(t, "${escapedJson}", string(eventBytes${i}), "SSE event ${i} content mismatch")`
                            );
                        }
                    }
                }
