...
export class WireTestGenerator {
    ...
    private async generateServiceTestFile(
        ...
        for (const endpoint of endpoints) {
            ...
            // Skip file download response endpoints — they return io.Reader (binary streams) and
            // autogenerated examples for these endpoints produce error responses (e.g., 404) rather
            // than meaningful binary content, causing wire tests to always fail.
            if (endpoint.response?.body?.type === "fileDownload") {
                this.context.logger.debug(`Skipping wire test for endpoint ${endpoint.id} - file download response`);
                continue;
            }
