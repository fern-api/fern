...
export abstract class AbstractEndpointGenerator {
    ...
    public getEndpointSignatureInfo({
        ...
        return {
            baseParameters: [...pathParameters, requestParameter].filter((p): p is rust.Parameter => p != null),
            pathParameters,
    ...
    private getAllPathParameters({
        ...
        const pathParameters: rust.Parameter[] = [];
            ...
            if (includePathParametersInSignature) {
                pathParameters.push(
                    rust.parameter({
                        docs: pathParam.docs,
                        name: parameterName,
                        type: this.context.rustTypeMapper.convert({ reference: pathParam.valueType })
                    })
                );
            }
        }
    ...
    private getRequestParameter({ request }: { request: EndpointRequest }): rust.Parameter {
