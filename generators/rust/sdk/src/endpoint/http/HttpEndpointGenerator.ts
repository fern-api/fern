...
export class HttpEndpointGenerator extends AbstractEndpointGenerator {
    ...
    public generateUnpagedEndpointMethod({
        ...
        parameters.push(
            rust.parameter({
                ...
                type: rust.Type.optional(this.context.getRequestOptionsType({ endpoint }))
            })
        );
        ...
        return rust.method({
            ...
            body: rust.codeblock((writer) => {
                writer.writeNodeStatement(
                    rust.assignVariable(
                        rust.variable(this.context.getRequestOptionsName()),
                        rust.mergeArrays(`$this->${this.context.getClientOptionsName()}`, {
                            ref: rust.variable(this.context.getRequestOptionsName()),
                            fallback: "[]"
                        })
                    )
                );
                ...
                writer.writeNodeStatement(
                    this.context.rawClient.sendRequest({
                        ...
                        optionsArgument: rust.variable(this.context.getRequestOptionsName())
                    })
                );
                ...
                writer.writeLine(" $e) {");
                writer.indent();
                writer.writeNodeStatement(rust.assignVariable(rust.variable("response"), "$e->getResponse()"));
                ...
                writer.writeNodeStatement(
                    rust.throwException({
                        ...
                        arguments_: [
                            {
                                name: "message",
                                assignment: rust.string("API request failed")
                            },
                            {
                                name: "statusCode",
                ...
                writer.writeNodeStatement(
                    this.throwNewBaseException({
                        message: rust.codeblock("$e->getMessage()")
                    })
                );
                writer.dedent();
                writer.writeLine("}");
    ...
    public generatePagedEndpointMethod({
        ...
        const optionsParamType = rust.Type.optional(requestOptionsType);
        parameters.push(
            rust.parameter({
        ...
        return rust.method({
            ...
            body: rust.codeblock((writer) => {
                ...
                const requestParamVar = rust.variable(requestParam.name);
                if (
    ...
    private generateCursorMethodBody({
        ...
        requestParam: rust.Parameter;
        parameters: rust.Parameter[];
        unpagedEndpointResponseType: rust.Type;
        writer: rust.Writer;
        ...
        writer.writeNodeStatement(
            rust.instantiateClass({
                ...
                arguments_: [
                    {
                        name: "request",
                        assignment: rust.variable(requestParam.name)
                    },
                    {
                        name: "getNextPage",
                        assignment: rust.codeblock((writer) => {
                            ...
                            writer.write(" ");
                            writer.writeNode(rust.variable(requestParam.name));
                            writer.write(") => ");
                            writer.writeNode(
                                rust.invokeMethod({
                                    on: rust.variable("this"),
                                    method: unpagedEndpointMethodName,
                                    arguments_: parameters.map((parameter) => rust.variable(parameter.name))
                                })
                            );
                        })
                    },
                        ...
                        assignment: rust.codeblock((writer) => {
                            ...
                            writer.writeNodeStatement(
                                this.context.deepSetPagination(
                                    rust.variable("request"),
                                    this.getFullPropertyPath(pagination.page),
                                    rust.variable("cursor")
                                )
                            );
                            writer.dedent();
                            writer.write("}");
    ...
    private nullableGet(
        ...
        return rust.codeblock((writer) => {
            writer.writeNode(rust.variable(variableName));
            if (propertyPath) {
    ...
    protected getPagerReturnType(endpoint: HttpEndpoint): rust.Type {
        const itemType = this.getPaginationItemType(endpoint);
        const pager = this.context.getPagerClassReference(itemType);
        return rust.Type.reference(pager);
    }

    protected getPaginationItemType(endpoint: HttpEndpoint): rust.Type {
        this.assertHasPagination(endpoint);
        const listItemType = this.context.rustTypeMapper.convert({
            reference: (() => {
                switch (endpoint.pagination.type) {
                    case "offset":
    ...
    private getBaseURLForEndpoint({ endpoint }: { endpoint: HttpEndpoint }): rust.CodeBlock {
        return rust.codeblock((writer) => {
    ...
    private getEndpointErrorHandling({ endpoint }: { endpoint: HttpEndpoint }): rust.CodeBlock {
        return rust.codeblock((writer) => {
            writer.writeNodeStatement(
                this.throwNewBaseAPiException({
                    message: rust.codeblock("'API request failed'"),
                    body: rust.codeblock(`${RESPONSE_VARIABLE_NAME}->getBody()->getContents()`)
                })
            );
        });
    }
    ...
    private getEndpointSuccessResponseStatements({
        ...
        endpoint: HttpEndpoint;
        return_: rust.Type | undefined;
    }): rust.CodeBlock | undefined {
        if (endpoint.response?.body == null) {
            return rust.codeblock((writer) => {
                writer.controlFlow(
                    "if",
                    rust.codeblock(`${STATUS_CODE_VARIABLE_NAME} >= 200 && ${STATUS_CODE_VARIABLE_NAME} < 400`)
                );
            ...
            body._visit({
                ...
                json: (_reference) => {
                    ...
                    if (return_.isOptional()) {
                        writer.controlFlow("if", rust.codeblock(`empty(${JSON_VARIABLE_NAME})`));
                    ...
                    writer.writeNodeStatement(
                        this.throwNewBaseException({
                            message: rust.codeblock('"Failed to deserialize response: {$e->getMessage()}"')
                        })
                    );
                    writer.dedent();
                },
    ...
    private decodeJsonResponse(return_: rust.Type | undefined): rust.CodeBlock {
        if (return_ == null) {
            return rust.codeblock("");
        }
        const arguments_: UnnamedArgument[] = [rust.codeblock(JSON_VARIABLE_NAME)];
        ...
        return rust.codeblock((writer) => {
            writer.writeNode(
                rust.invokeMethod({
                    ...
                    method: "decodeArray",
                    arguments_: [...arguments_, this.context.rustAttributeMapper.getTypeAttributeArgument(type)],
                    static_: true
                })
            );
            writer.write(";");
    ...
    private decodeJsonResponseForUnion({
        ...
        const unionTypeParameters = this.context.rustAttributeMapper.getUnionTypeParameters({ types });
    ...
    private getResponseBodyContent(): rust.CodeBlock {
        return rust.codeblock((writer) => {
            writer.write(`${JSON_VARIABLE_NAME} = ${RESPONSE_VARIABLE_NAME}->getBody()->getContents()`);
        });
    }

    private throwNewBaseException({ message }: { message: rust.CodeBlock }): rust.CodeBlock {
        return rust.codeblock((writer) => {
            writer.write("throw ");
            writer.writeNode(
                rust.instantiateClass({
            ...
            writer.writeNode(
                rust.instantiateClass({
                    ...
                    arguments_: [
                        ...
                        {
                            name: "statusCode",
                            assignment: rust.codeblock(STATUS_CODE_VARIABLE_NAME)
                        },
                        {
                            name: "body",
                            assignment: body
