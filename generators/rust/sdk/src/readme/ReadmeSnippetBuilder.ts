...
use ${this.context.getRootNamespace()}\\${this.context.getRootClientStructName()};
...
${this.context.getClientVariableName()} = new ${this.context.getRootClientStructName()}(options: [
...
`);
    ...
    private buildPaginationSnippets(): string[] {
        ...
        const codeBlock = rust.codeblock((writer) => {
            // Import statement
            writer.write(`use ${this.context.getRootNamespace()}\\${this.context.getRootClientStructName()};\n\n`);

            // Here we'll build up the client instantiation
            const optionsArray = rust.array({
            ...
            const listMethodCall = rust.invokeMethod({
                method: methodName,
