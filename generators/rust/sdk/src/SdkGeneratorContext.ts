...
export class SdkGeneratorContext extends AbstractRustGeneratorContext<SdkCustomConfigSchema> {
    ...
    private readonly endpointSnippetCache = new Map<FernIr.EndpointId, string | undefined>();
    ...
    /**
     * Returns the usage snippet for the given endpoint, generated from the same
     * dynamic snippets used for the README and reference.md. Prefers user-specified
     * examples over autogenerated ones. Returns undefined if no example is available
     * or the snippet cannot be generated.
     */
    public getEndpointSnippet(endpointId: FernIr.EndpointId): string | undefined {
        if (this.endpointSnippetCache.has(endpointId)) {
            return this.endpointSnippetCache.get(endpointId);
        }
        const snippet = this.generateEndpointSnippet(endpointId);
        this.endpointSnippetCache.set(endpointId, snippet);
        return snippet;
    }

    private generateEndpointSnippet(endpointId: FernIr.EndpointId): string | undefined {
        ...
        const example = selectExamplesForSnippets(dynamicEndpoint.examples)[0];
        if (example == null) {
            return undefined;
        }
        if (this.dynamicSnippetsGenerator == null) {
        ...
        try {
            const generated = this.dynamicSnippetsGenerator.generateSync(
