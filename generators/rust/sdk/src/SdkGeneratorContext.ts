...
export class SdkGeneratorContext extends AbstractRustGeneratorContext<SdkCustomConfigSchema> {
    ...
    public generatorAgent: RustGeneratorAgent;
    ...
    public getSubpackageField(subpackage: Subpackage): rust.Field {
        return rust.field({
    ...
    public getHttpMethod(method: HttpMethod): rust.CodeBlock {
    ...
    public getDefaultBaseUrlForEndpoint(endpoint: HttpEndpoint): rust.CodeBlock {
    ...
    public getRootClientStructName(): string {
    ...
    public getRequestOptionsType({ endpoint }: { endpoint: HttpEndpoint }): rust.Type {
        const options = [
            ...
            },
            {
                ...
                valueType: rust.Type.map(rust.Type.string(), rust.Type.mixed()),
                optional: true
            }
        ];
        ...
        return rust.Type.typeDict(options, {
            multiline: true
        });
    }
    ...
    public deepSetPagination(
        objectVarToSetOn: rust.AstNode,
        setterPath: Name[],
        valueVarToSet: rust.AstNode
    ): rust.AstNode {
        ...
        return rust.invokeMethod({
            ...
            arguments_: [
                objectVarToSetOn,
                rust.codeblock(`[${setterPath.map((path) => `"${this.getPropertyName(path)}"`).join(", ")}]`),
                valueVarToSet
            ],
            static_: true
        });
    ...
    public getTypeGetter(propertyName: Name): rust.AstNode {
        return rust.codeblock((writer) => {
    ...
    public getTypeSetter(propertyName: Name, valueVarToSet: rust.AstNode): rust.AstNode {
        return rust.codeblock((writer) => {
            if (this.shouldGenerateGetterMethods()) {
    ...
    private getDefaultBaseUrl(): rust.CodeBlock {
    ...
    private getBaseUrlForEnvironment(environmentId: string): rust.CodeBlock {
