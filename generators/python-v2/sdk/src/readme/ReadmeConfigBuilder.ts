...
export class ReadmeConfigBuilder {
    ...
    private getLanguageInfo(context: SdkGeneratorContext): FernGeneratorCli.LanguageInfo {
        return FernGeneratorCli.LanguageInfo.python({
            publishInfo: this.getPublishInfo(context)
        });
    }

    private getPublishInfo(context: SdkGeneratorContext): FernGeneratorCli.PypiPublishInfo | undefined {
    ...
    private getPackageName(context: SdkGeneratorContext): string {
        ...
        if (outputMode.type === "github" && outputMode.publishInfo?.type === "pypi") {
            return outputMode.publishInfo.packageName;
        }
        if (outputMode.type === "publish") {
            return outputMode.registriesV2.pypi.packageName;
        }
        return context.getModulePath();
    }
}
...
function getCustomSections(context: SdkGeneratorContext): FernGeneratorCli.CustomSection[] | undefined {
    ...
    for (const section of irCustomSections ?? []) {
        if (section.language === "python" && !customConfigSections?.some((s) => s.title === section.title)) {
            sections.push({
                name: section.title,
