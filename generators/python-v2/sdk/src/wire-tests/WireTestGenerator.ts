...
export class WireTestGenerator {
    ...
    /**
     * Gets the best available example from the static IR for an endpoint.
     * For SSE endpoints, prefers autogenerated examples with richer multi-variant
     * coverage. For other endpoints, prefers user-specified.
     */
    private getStaticIrExample(endpoint: FernIr.HttpEndpoint): FernIr.ExampleEndpointCall | undefined {
        return getBestEndpointExample(endpoint);
    }
    ...
    private generateEndpointTestFunction(
        ...
        try {
            ...
            if (isErrorResponse) {
                ...
                // For streaming endpoints, collect events and assert per-event content.
                ...
                if (this.isStreamingEndpoint(endpoint)) {
                    ...
                    // Extract SSE events from the IR example for per-event assertions
                    const staticExample = this.getStaticIrExample(endpoint);
                    const sseEvents = staticExample != null ? getSseExampleEvents(staticExample) : undefined;

                    if (sseEvents != null && sseEvents.length > 0) {
                        statements.push(
                            python.codeBlock(
                                `assert len(events) == ${sseEvents.length}, f"Expected ${sseEvents.length} events, got {len(events)}"`
                            )
                        );
