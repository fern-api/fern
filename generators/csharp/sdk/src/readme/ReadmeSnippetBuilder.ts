import { AbstractReadmeSnippetBuilder, getWireValue, GeneratorError } from "@fern-api/base-generator";
...
`)
    ...
    private getEndpointSnippetString(endpoint: FernGeneratorExec.Endpoint): string {
        if (endpoint.snippet.type !== "csharp") {
            throw GeneratorError.internalError(`Internal error; expected csharp snippet but got: ${endpoint.snippet.type}`);
        }
