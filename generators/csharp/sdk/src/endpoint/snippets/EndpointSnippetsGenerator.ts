    ...
    public async populateSnippetsCache(): Promise<void> {
        ...
        const allEndpoints = Object.entries(this.context.ir.services).flatMap(([serviceId, service]) =>
            service.endpoints.flatMap((endpoint) => {
                ...
                if ((endpoint.pagination != null && isPagerPagination(endpoint.pagination)) || isStreaming) {
        ...
        const endpointResults = await Promise.all(
            allEndpoints.map(async ({ id, endpoint, serviceId }) => {
                const allExamples = [
                ...
                return { id, autogenerated, userSpecified };
            })
        );

        for (const { id, autogenerated, userSpecified } of endpointResults) {
