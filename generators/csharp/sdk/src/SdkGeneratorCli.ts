    ...
    private generateRequests(context: SdkGeneratorContext, service: HttpService, serviceId: string) {
        for (const endpoint of service.endpoints) {
            if (endpoint.sdkRequest != null && endpoint.sdkRequest.shape.type === "wrapper") {
                ...
                // SNIPPET
                endpoint.autogeneratedExamples.forEach((example) => {
                    const snippet = wrappedRequestGenerator.doGenerateSnippet(example.example);
                    context.logger.info(SnippetHelper.tryFormatSnippet(snippet));
                });
                endpoint.userSpecifiedExamples.forEach((example) => {
