...
} from "../context/CsharpGeneratorContext";
 ...
 *
* /
export function precalculate(context: CsharpGeneratorContext<BaseCsharpCustomConfigSchema>) {
    ...
    for (const [typeId, typeDeclaration] of Object.entries(context.ir.types)) {
        ...
        const result = typeDeclaration.shape._visit({
            ...
            enum: (etd: EnumTypeDeclaration) => {
                const enclosingType = nameRegistry.trackType(
    ...
    Object.values(context.ir.types).forEach((typeDeclaration) => {
      const classReference = context.csharpTypeMapper.convertToClassReference(typeDeclaration.name);
    ...
    nameRegistry.trackType(context.getValueConvertReference());
    nameRegistry.trackType(context.getBaseExceptionClassReference());
    nameRegistry.trackType(context.getRootClientClassReference());
    nameRegistry.trackType(context.getRootClientClassReferenceForSnippets());
    nameRegistry.trackType(context.getBaseApiExceptionClassReference());
    nameRegistry.trackType(context.getHeadersClassReference());
    nameRegistry.trackType(context.getClientOptionsClassReference());
    nameRegistry.trackType(context.getRequestOptionsInterfaceReference());
    nameRegistry.trackType(context.getEnvironmentsClassReference());
    ...
    Object.entries(context.ir.subpackages).forEach(([_, subpackage]) => {
                ...
                endpoint.sdkRequest?.shape._visit({
                    wrapper: (wrapper) => {
                        if (wrapper.wrapperName && subpackage.service) {
                            const requestWrapperReference = context.getRequestWrapperReference(
                                subpackage.service,
                                wrapper.wrapperName
                            );
        ...
        for (const typeDeclaration of types
            ...
            .map((type) => type as FernIr.TypeDeclaration & { shape: ObjectTypeDeclaration })) {
            const examples = [...typeDeclaration.userProvidedExamples, ...typeDeclaration.autogeneratedExamples];
            if (examples.length === 0) {
                continue;
            }

            const testType = classReference({
              //   name: typeDeclaration.name.name.originalName,
                name: `${context.csharpTypeMapper.convertToClassReference(typeDeclaration.name).name}Test`,
        ...
        if (context.customConfig["generate-error-types"] ?? true) {
          for (const each of Object.values(context.ir.errors)) {
            nameRegistry.trackType(context.getExceptionClassReference(each.name));
          }
      }
      nameRegistry.trackType(classReference({
        name: "TestClient",
