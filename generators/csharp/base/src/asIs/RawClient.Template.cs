...
<% } %>
    ...
    )
    {
        ...
        for (var attempt = 0; attempt <= maxRetries; attempt++)
        {
            ...
            retryResponse = await httpClient
                ...
                .ConfigureAwait(false);
            retryResponse = await FollowRedirectsAsync(httpClient, attemptRequest, retryResponse, cancellationToken)
                .ConfigureAwait(false);
    ...
    )
    {
        ...
        while (RedirectStatusCodes.Contains((int)response.StatusCode) && redirectCount < MaxRedirects)
        {
