...
function generateAdvanced(args: { binaryName: string; envPrefix: string }): Block {
    ...
    return new Block({
        ...
        content: lines(
            ...
            `|----------|-------------|-------------|`,
            `| \`${envPrefix}_BASE_URL\` | Override the API base URL | yes |`,
            `| \`${envPrefix}_OUTPUT\` | Default output format when \`--format\` is not passed | yes |`,
            `| \`${envPrefix}_RETRIES\` | Retry attempts for failed requests (\`--retries\` wins) | yes |`,
            `| \`${envPrefix}_CA_BUNDLE\` | Path to PEM file with extra trust roots (or \`SSL_CERT_FILE\`) | no |`,
            `| \`${envPrefix}_INSECURE=1\` | Skip TLS verification (debugging only) | no |`,
            `| \`${envPrefix}_PROXY\` | HTTP(S) proxy URL | no |`,
            `| \`${envPrefix}_TIMEOUT_SECS\` | Total request timeout in seconds | no |`,
            `| \`${envPrefix}_CONNECT_TIMEOUT_SECS\` | Connection timeout in seconds | no |`,
            "",
            `Variables marked "yes" can also be stored on a profile with \`${binaryName} profiles set <profile> ${envPrefix}_RETRIES=3\`; the others apply to the whole process and are read from the environment only.`,
            "",
