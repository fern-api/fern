import type { WireTestCase } from "./manifest.js";

/**
 * Characters safe to interpolate into a Rust identifier / string literal
 * inside the generated harness. Case ids are already sanitized to
 * `[a-z0-9_]` by the manifest builder, but we re-check at the codegen
 * boundary so a bad id can never inject Rust.
 */
const SAFE_RUST_IDENT = /^[a-z][a-z0-9_]*$/;

/**
 * Render the generic Rust wire-test harness (`tests/wire_test.rs`).
 *
 * The harness is static except for two templated seams:
 *   - `CARGO_BIN_EXE_<binaryName>` — the compiled binary cargo exposes to
 *     integration tests (a compile-time `env!`, so it must be a literal).
 *   - one `#[tokio::test]` function per case — so a failure names the exact
 *     endpoint that broke.
 *
 * Everything data-driven (command chain, params, body, expected response)
 * lives in the sibling `wiremock/wire-test-cases.json` manifest, which the
 * harness reads at runtime. The harness resolves each case's CLI command
 * chain by loading the same baked OpenAPI spec the binary runs on and
 * matching `(method, path)` against the SDK's own discovery tree — so it
 * never reproduces the CLI's command-naming rules.
 */
export function renderWireTestHarness(args: { binaryName: string; cases: WireTestCase[] }): string {
    const { binaryName, cases } = args;

    const testFns = cases
        .map((c) => {
            if (!SAFE_RUST_IDENT.test(c.id)) {
                throw new Error(`Unsafe wire-test case id "${c.id}": cannot be used as a Rust identifier.`);
            }
            return [`#[tokio::test]`, `async fn wire_${c.id}() {`, `    run_case("${c.id}").await;`, `}`].join("\n");
        })
        .join("\n\n");

    // `binaryName` is validated upstream (deriveBinaryName / customConfig) to a
    // kebab identifier; cargo exposes the bin as CARGO_BIN_EXE_<name> verbatim.
    const binExeEnv = `CARGO_BIN_EXE_${binaryName}`;

    return `//! Auto-generated wire tests by @fern-api/cli-generator.
//!
...
async fn run_case(id: &str) {
    ...
    if let Some(setup) = &manifest.login_token_setup {
        ...
        let home = &auth_home;
        let mut login = tokio::process::Command::new(env!("${binExeEnv}"));
        login
    ...
    let mut cmd = tokio::process::Command::new(env!("${binExeEnv}"));
    cmd.args(&args);
