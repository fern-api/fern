...
function renderCustomRsWithSdk(sdkCrate: string): string {
    return [
        ...
        "//!",
        "//! Each handler receives typed args (via `#[derive(clap::Args)]`)",
        "//! and an `AppContext`. Use `super::sdk::client(ctx)?` to get a",
        "//! fully-wired SDK client. Use `super::sdk::block_on(future)` to",
        "//! run async SDK calls from a synchronous handler.",
        ...
        "",
        "// ── Example: top-level typed command ────────────────────────────────",
        "",
        ...
        "}",
        "",
        "fn handle_hello(args: HelloArgs, ctx: &AppContext) -> Result<(), CliError> {",
        "    if ctx.is_quiet() {",
        "        return Ok(());",
        "    }",
        '    println!("Hello, {}! (style: {})", args.name, args.style);',
        "    Ok(())",
        "}",
        "",
        "// ── Example: nested command under a custom namespace ────────────────",
        "",
        "/// Arguments for the `admin users list` subcommand.",
        "#[derive(clap::Args)]",
        ...
        "}",
        "",
        "fn handle_list_users(args: ListUsersArgs, ctx: &AppContext) -> Result<(), CliError> {",
        `    let _client = super::sdk::client(ctx)?;`,
        "    // Example: call your API via the SDK client.",
        "    // let users = super::sdk::block_on(client.users.list(args.limit))?;",
        "    let pipeline = ctx.output_pipeline();",
        '    let data = serde_json::json!([{"id": 1, "limit": args.limit}]);',
        "    pipeline.emit(&mut std::io::stdout(), &data, false, true)",
        "        .map_err(|e| CliError::Other(e.into()))?;",
        "    Ok(())",
        "}",
        "",
        "fn handle_list_users_dry_run(args: ListUsersArgs, _ctx: &AppContext) -> Result<(), CliError> {",
        '    eprintln!("[dry-run] would list up to {} users", args.limit);',
        "    Ok(())",
        "}",
        "",
        "// ── Registration ─────────────────────────────────────────────────────",
        ...
        "    app",
        "        // Top-level command (no dry-run handler → --dry-run errors).",
        '        .custom_typed::<HelloArgs>("hello")',
        '        .about("Greet someone")',
        "        .handler(handle_hello)",
        "        .register()",
        "        // Nested under a custom namespace.",
        '        .custom_typed::<ListUsersArgs>("list")',
        '        .about("List users")',
        '        .under(&["admin", "users"])',
        "        .handler(handle_list_users)",
        "        .dry_run(handle_list_users_dry_run)",
        "        .register()",
        "}",
        ""
    ].join("\n");
