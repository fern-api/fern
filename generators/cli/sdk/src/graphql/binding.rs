...
mod tests {
    ...
    // A compact GraphQL introspection schema (the graphql-fixture surface)
    // with two top-level resources: `node` and `filtered-node`.
    const FIXTURE_SCHEMA: &str = include_str!("../../cli/graphql-fixture/schema.json");

    fn prepared_binding(prefix: Option<&str>) -> GraphqlBinding {
        let mut b = GraphqlBinding::new()
            .spec(FIXTURE_SCHEMA)
            .endpoint("http://localhost/graphql");
        if let Some(p) = prefix {
            b = b.under(p);
        }
        b.set_cli_name("fixture");
        b
    }

    #[test]
    fn without_prefix_resources_are_top_level() {
        let cmd = prepared_binding(None).build_command().unwrap();
        let names: Vec<_> = cmd.get_subcommands().map(|c| c.get_name()).collect();
        assert!(names.contains(&"node"), "expected `node` at root, got {names:?}");
        assert!(
            !names.contains(&"graphql"),
            "did not expect a `graphql` wrapper without .under(), got {names:?}"
        );
    }

    #[test]
    fn under_prefix_nests_all_resources() {
        let cmd = prepared_binding(Some("graphql")).build_command().unwrap();
        let names: Vec<_> = cmd.get_subcommands().map(|c| c.get_name()).collect();
        assert!(
            names.contains(&"graphql"),
            "expected a top-level `graphql` wrapper, got {names:?}"
        );
        assert!(
            !names.contains(&"node"),
            "resources should be nested under `graphql`, not at root: {names:?}"
        );

        // The original resources live one level down, under the wrapper.
        let wrapper = cmd
            .get_subcommands()
            .find(|c| c.get_name() == "graphql")
            .expect("graphql wrapper present");
        let nested: Vec<_> = wrapper.get_subcommands().map(|c| c.get_name()).collect();
        assert!(
            nested.contains(&"node") && nested.contains(&"filtered-node"),
            "expected node/filtered-node nested under graphql, got {nested:?}"
        );
    }
}
