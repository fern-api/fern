...
impl Sandbox {
    ...
    fn run_with_env(&self, args: &[&str], env: &[(&str, &str)]) -> Output {
        ...
        command
            ...
            // The fixture's SDK variable, so a developer's shell cannot
            // satisfy `gardenId` behind a test's back.
...
/// resolution order.
fn credential_source_order(output: &Output) -> Vec<String> {
    json(output)["schemes"][0]["sources"]
        .as_array()
        .expect("sources")
        .iter()
...
#[test]
fn an_explicit_profile_flag_outranks_the_base_url_env_var() {
    let sandbox = Sandbox::new();
    sandbox.run(&["profiles", "create", "prod", "--base-url", "https://profile.example"]);

    let flagged = sandbox.run_with_env(
        &["users", "list", "--dry-run", "--format", "json", "-p", "prod"],
        &[("OPENAPI_FIXTURE_BASE_URL", "https://env.example")],
    );
    assert_eq!(
        json(&flagged)["url"], "https://profile.example/users",
        "-p should outrank the env var",
    );
}
...
#[test]
fn a_profile_can_set_an_sdk_variable() {
    // An SDK variable is the natural way to model a value on nearly every
    // path — a tenant id, a workspace — so it is exactly what a profile
    // most wants to carry. It used to be the one category profiles could
    // not set: `--set` accepted and stored the value, the bound path
    // parameter is excluded from the per-operation flag surface, and the
    // request then failed with "Missing required SDK variable".
    let sandbox = Sandbox::new();
    assert_ok(
        &sandbox.run(&["profiles", "create", "prod", "--set", "gardenId=g-profile", "--use"]),
        "profiles create",
    );
    let output = sandbox.run(&["zones", "list", "--dry-run", "--format", "json"]);
    assert_ok(&output, "zones list");
    assert!(
        json(&output)["url"].as_str().unwrap_or_default().contains("/gardens/g-profile/"),
        "{}",
        stdout(&output),
    );
}
...
#[test]
fn an_sdk_variable_follows_the_same_precedence_ladder() {
    let sandbox = Sandbox::new();
    sandbox.run(&["profiles", "create", "prod", "--set", "gardenId=g-profile", "--use"]);
    ...
    assert!(url(&explicit).contains("/gardens/g-profile/"), "{}", url(&explicit));

    // …and the flag beats everything.
    let flagged = sandbox.run_with_env(
        &["zones", "list", "--dry-run", "--format", "json", "-p", "prod", "--garden-id", "g-flag"],
        &[("GARDEN_ID", "g-env")],
    );
    assert!(url(&flagged).contains("/gardens/g-flag/"), "{}", url(&flagged));
}

#[test]
fn an_sdk_variable_still_resolves_from_env_with_no_profile() {
    let sandbox = Sandbox::new();
    let output = sandbox.run_with_env(
        &["zones", "list", "--dry-run", "--format", "json"],
        &[("GARDEN_ID", "g-env")],
    ...
    assert_ok(&output, "zones list");
    assert!(json(&output)["url"].as_str().unwrap_or_default().contains("/gardens/g-env/"));
}
...
#[test]
fn capturing_from_env_is_silent_when_nothing_shadows() {
    // The warning must not fire spuriously — `--with-token` on a shell with
    // no exported credential is the common case.
    let sandbox = Sandbox::new();
    let output = sandbox.run(&["profiles", "create", "plain"]);
    assert_ok(&output, "profiles create");
