...
Such a header is **promoted to a scheme** — gaining an `auth status` row, a documented source, and a place in precedence — only on an explicit signal: a declared `apiKey`/header security scheme naming it, or a registered auth binding for it (the per-generator `auth-schemes` path). A name is never enough to *add* a credential to the provider tree.

A header with no declaration at all is still **arbitrable by name** (`authorization`, `proxy-authorization`, `*api-key*`/`*apikey*`/`*api_key*`, `*access-token*`, `*auth-token*`, `*session-token*`). This is the ElevenLabs shape: a key that exists only as a global header, sent alongside a stored OAuth token, which the API rejects outright. The heuristic is defensible here because it is purely *subtractive* — a matching header is never sent anywhere it isn't sent today, only withheld from a request that already carries a selected credential — so a false positive drops a header from authenticated requests (visible, recoverable) rather than transmitting a secret that shouldn't go out. A CLI with no other credential never selects one and is therefore untouched. `nonCredentialHeaders` in the generator's config is the escape hatch.

The match list is deliberately narrower than the redaction patterns in `debug.rs` (no bare `key`, `secret`, or `token`): redacting a header wrongly costs one unreadable diagnostic line, withholding one changes a request.

Promotion does not change the header's **reach**. It is injected on every request as before, including explicitly-anonymous (`security: []`) ones — an API whose key goes out everywhere returns different data anonymously, so narrowing reach would silently change results. Per-operation header parameters are never withheld, and non-credential global headers (`X-API-Version`, tracing headers) are untouched.

When an undeclared global credential and a declared scheme are both present, the declared scheme wins: source rank can only arbitrate credentials the provider tree can see, and a global header's provenance (flag vs. env vs. baked-in default) isn't visible at request time. Declaring the header (either explicit signal above) puts it back under rank precedence.
