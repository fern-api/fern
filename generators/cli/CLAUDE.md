...
Known remaining gap: endpoints whose IR example omits a required body property
fail the CLI's client-side schema validation — a data-quality signal, not a
harness bug. Autogenerated examples drop required properties whose schema is a
nullable `anyOf` composition, which is the usual cause.
