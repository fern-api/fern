...
| [`src/customConfig.ts`](src/customConfig.ts) | Type + boundary validator for `generators.yml`'s `config:` block. `binaryName`, `customCommands`, `rootGroup`, `packageIdentity`, `distribution`, `profiles`, `webhookSecretEnv`. |
| [`src/webhooks.ts`](src/webhooks.ts) | `buildWebhookManifest` — lowers the IR's `webhookGroups` (plus `types` for placeholder payloads) into `cli/<binaryName>/webhooks.json`, the manifest `sdk/src/webhooks.rs` consumes for `webhook list` / `webhook invoke`. Carries the HMAC config from `x-fern-webhook-signature`; asymmetric schemes are dropped. `customConfig.webhookSecretEnv` names the secret env var (default `<BINARY>_WEBHOOK_SECRET`). |
