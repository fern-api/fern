...
export class WrappedEndpointRequest extends EndpointRequest {
    ...
    private writeQueryParameter(writer: php.Writer, query: FernIr.QueryParameter): void {
        if (!query.allowMultiple && query.explode !== false && this.isMapType(query.valueType)) {
            ...
            // wire name instead leaves the http layer to invent a format for it. array_replace
            // keeps numeric keys such as "2023", which array_merge would renumber.
            const reference = this.context.accessRequestProperty({
                requestParameterName: this.requestParameterName,
                propertyName: query.name
            });
            writer.writeTextStatement(
                `${QUERY_PARAMETER_BAG_NAME} = array_replace(${QUERY_PARAMETER_BAG_NAME}, ${reference})`
            );
            return;
        }
